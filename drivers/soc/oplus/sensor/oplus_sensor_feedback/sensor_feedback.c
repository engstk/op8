...
struct sensor_fb_conf g_fb_conf[] = {
	...
	{ PS_FIRST_REPORT_DELAY_COUNT_ID, "device_ps_rpt_delay",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ PS_ORIGIN_DATA_TO_ZERO_ID, "device_ps_to_zero",
	  SENSOR_DEBUG_DEVICE_TYPE },
	...
	{ PS_DYNAMIC_CALI_ID, "device_ps_dynamic_cali",
	  SENSOR_DEBUG_DEVICE_TYPE },
	...
	{ ALS_FIRST_REPORT_DELAY_COUNT_ID, "device_als_rpt_delay",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ ALS_ORIGIN_DATA_TO_ZERO_ID, "device_als_to_zero",
	  SENSOR_DEBUG_DEVICE_TYPE },
	...
	{ ACCEL_FIRST_REPORT_DELAY_COUNT_ID, "device_acc_rpt_delay",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ ACCEL_ORIGIN_DATA_TO_ZERO_ID, "device_acc_to_zero",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ ACCEL_CALI_DATA_ID, "device_acc_cali_data",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ ACCEL_DATA_BLOCK_ID, "device_acc_data_block", SENSOR_DEVICE_TYPE },
	{ ACCEL_SUB_DATA_BLOCK_ID, "device_sub_acc_data_block",
	  SENSOR_DEVICE_TYPE },
	{ ACCEL_DATA_FULL_RANGE_ID, "device_acc_data_full_range",
	  SENSOR_DEVICE_TYPE },
	...
	{ GYRO_FIRST_REPORT_DELAY_COUNT_ID, "device_gyro_rpt_delay",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ GYRO_ORIGIN_DATA_TO_ZERO_ID, "device_gyro_to_zero",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ GYRO_CALI_DATA_ID, "device_gyro_cali_data",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ GYRO_DATA_BLOCK_ID, "device_gyro_data_block", SENSOR_DEVICE_TYPE },
	{ GYRO_SUB_DATA_BLOCK_ID, "device_gyro_sub_data_block",
	  SENSOR_DEVICE_TYPE },
	...
	{ MAG_FIRST_REPORT_DELAY_COUNT_ID, "device_mag_rpt_delay",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ MAG_ORIGIN_DATA_TO_ZERO_ID, "device_mag_to_zero",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ MAG_CALI_DATA_ID, "device_mag_cali_data", SENSOR_DEBUG_DEVICE_TYPE },
	{ MAG_DATA_BLOCK_ID, "device_mag_data_block_data",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ MAG_DATA_FULL_RANGE_ID, "device_mag_data_full_range",
	  SENSOR_DEBUG_DEVICE_TYPE },
	...
	{ SAR_FIRST_REPORT_DELAY_COUNT_ID, "device_sar_rpt_delay",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ SAR_ORIGIN_DATA_TO_ZERO_ID, "device_sar_to_zero",
	  SENSOR_DEBUG_DEVICE_TYPE },
	...
	{ POWER_SENSOR_INFO_ID, "debug_power_sns_info",
	  SENSOR_DEBUG_POWER_TYPE },
	{ POWER_ACCEL_INFO_ID, "debug_power_acc_info",
	  SENSOR_DEBUG_POWER_TYPE },
	{ POWER_GYRO_INFO_ID, "debug_power_gyro_info",
	  SENSOR_DEBUG_POWER_TYPE },
	{ POWER_MAG_INFO_ID, "debug_power_mag_info", SENSOR_DEBUG_POWER_TYPE },
	{ POWER_PROXIMITY_INFO_ID, "debug_power_prox_info",
	  SENSOR_DEBUG_POWER_TYPE },
	{ POWER_LIGHT_INFO_ID, "debug_power_light_info",
	  SENSOR_DEBUG_POWER_TYPE },
	{ POWER_WISE_LIGHT_INFO_ID, "debug_power_wiseligt_info",
	  SENSOR_DEBUG_POWER_TYPE },
	{ POWER_WAKE_UP_RATE_ID, "debug_power_wakeup_rate",
	  SENSOR_DEBUG_POWER_TYPE },
	{ POWER_ADSP_SLEEP_RATIO_ID, "power_adsp_sleep_ratio",
	  SENSOR_POWER_TYPE },

	{ DOUBLE_TAP_REPORTED_ID, "device_double_tap_reported",
	  SENSOR_DEBUG_DEVICE_TYPE },
	{ DOUBLE_TAP_PREVENTED_BY_NEAR_ID,
	  "device_double_tap_prevented_by_near", SENSOR_DEBUG_DEVICE_TYPE },
	{ DOUBLE_TAP_PREVENTED_BY_ATTITUDE_ID,
	  "device_double_prevented_by_attitude", SENSOR_DEBUG_DEVICE_TYPE },
	{ DOUBLE_TAP_PREVENTED_BY_FREEFALL_Z_ID,
	  "device_double_prevented_by_freefall_z", SENSOR_DEBUG_DEVICE_TYPE },
	{ DOUBLE_TAP_PREVENTED_BY_FREEFALL_SLOPE_ID,
	  "device_double_prevented_by_freefall_slope",
	  SENSOR_DEBUG_DEVICE_TYPE },

	{ BAROMETER_I2C_ERR_ID, "device_bar_i2c_err", SENSOR_DEVICE_TYPE },

	{ ALAILABLE_SENSOR_LIST_ID, "available_sensor_list",
	  SENSOR_DEBUG_DEVICE_TYPE },

	{ HAL_SENSOR_NOT_FOUND, "device_hal_not_found", SENSOR_DEVICE_TYPE },
	{ HAL_QMI_ERROR, "device_hal_qmi_error", SENSOR_DEVICE_TYPE },
	{ HAL_SENSOR_TIMESTAMP_ERROR, "device_hal_ts_error",
	  SENSOR_DEBUG_DEVICE_TYPE }
...
static u64 oplus_rpmh_master_get_sleeptime(struct msm_rpmh_master_stats *record)
{
	...
	if (record->last_entered > record->last_exited) {
		...
		accumulated_duration +=
			(arch_counter_get_cntvct() - record->last_entered);
#else
		accumulated_duration +=
			(__arch_counter_get_cntvct() - record->last_entered);
#endif
	}
...
static int oplus_subsystem_sleeptime(char *name, u64 *sleeptime)
{
	...
	for (i = 0; i < ARRAY_SIZE(rpmh_masters); i++) {
		if (strncmp(rpmh_masters[i].master_name, name, strlen(name)) !=
		    0) {
			continue;
		}
		record = (struct msm_rpmh_master_stats *)qcom_smem_get(
			rpmh_masters[i].pid, rpmh_masters[i].smem_id, &size);
...
static void read_subsystem_sleep_time(struct subsystem_desc *subsystem_desc,
				      int status)
{
	int index = 0;
	int ret = 0;
	...
	if (status == 1) {
		for (index = 0; index < SUBSYS_COUNTS; index++) {
			ret = oplus_subsystem_sleeptime(
				subsystem_desc[index].subsys_name,
				&subsystem_desc[index].subsys_sleep_time_p);
			now_time = oplus_current_kernel_time();
			subsystem_desc[index].ap_sleep_time_p =
				(now_time.tv_sec * 1000 +
				 now_time.tv_nsec / 1000000);
		...
		for (index = 0; index < SUBSYS_COUNTS; index++) {
			...
			now_time = oplus_current_kernel_time();
			subsystem_desc[index].ap_sleep_time_s =
				(now_time.tv_sec * 1000 +
				 now_time.tv_nsec / 1000000);
			subsystem_desc[index].is_err = ret;
		}
	...
	for (index = 0; index < SUBSYS_COUNTS; index++) {
		if (subsystem_desc[index].is_err != 0) {
			subsys_sleep =
				subsystem_desc[index].subsys_sleep_time_p -
				subsystem_desc[index].subsys_sleep_time_s;
			ap_sleep = subsystem_desc[index].ap_sleep_time_p -
				   subsystem_desc[index].ap_sleep_time_s;
			subsys_sleep_ratio = subsys_sleep * 100 / ap_sleep;
			subsystem_desc[index].subsys_sleep_ratio =
				subsys_sleep_ratio;
			...
			pr_info("subsys_sleep_ratio =%d, subsys_name=%s\n",
				subsys_sleep_ratio,
				subsystem_desc[index].subsys_name);
			if ((ap_sleep > (2 * 3600 * 1000)) &&
			    (subsys_sleep_ratio < 10)) {
				flag = 1;
...
static ssize_t hal_info_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	...
	scnprintf(payload, sizeof(payload),
		  ...
		  g_fb_conf[index].fb_field, event_ct, strbuf);
	pr_info("payload =%s\n", payload);

	return count;
}

static ssize_t test_id_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	struct sensor_fb_cxt *sensor_fb_cxt = g_sensor_fb_cxt;
	uint16_t adsp_event_counts = 0;
	...
	err = sscanf(buf, "%hu %hu %hu %hu", &node_type, &adsp_event_counts,
		     &event_id, &event_data);
...
static struct attribute *sensor_feedback_attributes[] = {
	&dev_attr_adsp_notify.attr, &dev_attr_adsp_recv.attr,
	&dev_attr_hal_info.attr,    &dev_attr_test_id.attr,
	&dev_attr_sensor_list.attr, NULL
};
...
static int read_data_from_share_mem(struct sensor_fb_cxt *sensor_fb_cxt)
{
	...
	smem_addr = qcom_smem_get(QCOM_SMEM_HOST_ANY, SMEM_SENSOR_FEEDBACK,
				  &smem_size);
...
static int reserve_fb_share_mem()
{
	...
	rc = qcom_smem_alloc(QCOM_SMEM_HOST_ANY, SMEM_SENSOR_FEEDBACK,
			     smem_size);
	if (rc < 0 && rc != -EEXIST) {
			    ...
			    struct sensor_fb_cxt *sensor_fb_cxt)
{
	...
	if (event_id == ALAILABLE_SENSOR_LIST_ID) {
		sensor_fb_cxt->sensor_list[0] =
			(uint32_t)sensor_fb_cxt->fb_smem.event[count].buff[0];
		sensor_fb_cxt->sensor_list[1] =
			(uint32_t)sensor_fb_cxt->fb_smem.event[count].buff[1];
		pr_info("sensor_list virt_sns = 0x%x, phy_sns = 0x%x\n",
			sensor_fb_cxt->sensor_list[0],
			sensor_fb_cxt->sensor_list[1]);
		ret = 1;
	} else if (event_id >= POWER_ACCEL_INFO_ID &&
		   event_id <= POWER_WISE_LIGHT_INFO_ID) {
		index = find_event_id(event_id);
		if (index >= 0) {
			...
			pr_info("sensor_power_monitor %s : proc_type: %s, %s, %s, %s, %s, %s, %s, %s\n",
				g_fb_conf[index].fb_field,
				proc_t[(sensor_fb_cxt->fb_smem.event[count]
						.buff[0] >>
					0) &
				       0x07]
					.name,
			...
			pr_info("sensor_power_monitor %s : delivery_type: %s, %s, %s, %s, %s, %s, %s, %s\n",
				g_fb_conf[index].fb_field,
				delivery_t[(sensor_fb_cxt->fb_smem.event[count]
						    .buff[1] >>
					    0) &
					   0x07]
					.name,
		...
		pr_info("sensor_power_monitor: proximity:%d, wise_light:%d, ambient_light:%d, accel:%d, gyro:%d, mag:%d\n",
			(sensor_fb_cxt->fb_smem.event[count].buff[1] >> 0) &
				0x1,
			(sensor_fb_cxt->fb_smem.event[count].buff[1] >> 1) &
				0x1,
			(sensor_fb_cxt->fb_smem.event[count].buff[1] >> 2) &
				0x1,
			(sensor_fb_cxt->fb_smem.event[count].buff[1] >> 3) &
				0x1,
			(sensor_fb_cxt->fb_smem.event[count].buff[1] >> 4) &
				0x1,
			(sensor_fb_cxt->fb_smem.event[count].buff[1] >> 5) &
				0x1);
		ret = 1;
...
static int parse_shr_info(struct sensor_fb_cxt *sensor_fb_cxt)
{
	...
	for (count = 0; count < sensor_fb_cxt->adsp_event_counts; count++) {
		...
		fb_len += scnprintf(
			...
			g_fb_conf[index].fb_field,
			sensor_fb_cxt->fb_smem.event[count].count, detail_buff,
...
static int sensor_report_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		clear_bit(THREAD_WAKEUP,
			  (unsigned long *)&sensor_fb_cxt->wakeup_flag);
		set_bit(THREAD_SLEEP,
			(unsigned long *)&sensor_fb_cxt->wakeup_flag);
		spin_lock(&sensor_fb_cxt->rw_lock);
		...
		if (node_type == 0) {
			...
			read_subsystem_sleep_time(sensor_fb_cxt->subsystem_desc,
						  1);
			cal_subsystem_sleep_ratio(
				sensor_fb_cxt->subsystem_desc);
		...
		spin_lock(&sensor_fb_cxt->rw_lock);
		memset((void *)&sensor_fb_cxt->fb_smem, 0,
		       sizeof(struct fb_event_smem));
				     ...
				     size_t count, loff_t *off)
{
	...
	int len = 0;
	struct sensor_fb_cxt *sensor_fb_cxt =
		(struct sensor_fb_cxt *)PDE_DATA(file_inode(file));
			      ...
			      void *data)
{
	...
	struct sensor_fb_cxt *sns_cxt =
		container_of(nb, struct sensor_fb_cxt, fb_notif);
	...
	if (event == MSM_DRM_EARLY_EVENT_BLANK) {
		...
		if (blank == MSM_DRM_BLANK_UNBLANK) {
			...
			set_bit(THREAD_WAKEUP,
				(unsigned long *)&sns_cxt->wakeup_flag);
