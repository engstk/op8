	...
	if ((panel->oplus_priv.is_oplus_project) &&
		...
		case DSI_CMD_AOD_HBM_ON:
			if (oplus_hbm_status.hbm_pvt_status == 1) {
				DSI_DEBUG("%s skip cmd=%s", __func__,
					  prop_map[type]);
				ret = 1;
			}
			break;
				     ...
				     enum dsi_cmd_set_type type)
{
	...
	DSI_DEBUG("%s [hbm_pvt,hbm,hbm_aod] = [%d %d %d]", __func__,
		  oplus_hbm_status.hbm_pvt_status, oplus_hbm_status.hbm_status,
		  oplus_hbm_status.hbm_aod_status);

	return;
}

static struct oplus_brightness_alpha brightness_seed_alpha_lut_dc[] = {
	{ 0, 0xff },   { 1, 0xfc },   { 2, 0xfb },   { 3, 0xfa },
	{ 4, 0xf9 },   { 5, 0xf8 },   { 6, 0xf7 },   { 8, 0xf6 },
	{ 10, 0xf4 },  { 15, 0xf0 },  { 20, 0xea },  { 30, 0xe0 },
	{ 45, 0xd0 },  { 70, 0xbc },  { 100, 0x98 }, { 120, 0x80 },
	{ 140, 0x70 }, { 160, 0x58 }, { 180, 0x48 }, { 200, 0x30 },
	{ 220, 0x20 }, { 240, 0x10 }, { 260, 0x00 },
};
...
int dsi_panel_parse_oplus_dc_config(struct dsi_panel *panel)
{
	...
	arr = utils->get_property(utils->data, "oplus,dsi-dc-brightness",
				  &length);
	if (!arr) {
...
int sde_connector_update_backlight(struct drm_connector *connector, bool post)
{
			...
			if (!post) {
				...
				ret = wait_event_timeout(
					*drm_crtc_vblank_waitqueue(crtc),
					...
					msecs_to_jiffies(34));
				current_vblank =
					drm_crtc_vblank_count(crtc) + vblank;
				if (delay > 0)
					usleep_range(delay, delay + 100);
				_sde_connector_update_bl_scale_(c_conn);
				if (delay_after)
					usleep_range(delay_after,
						     delay_after + 100);
				if (vblank > 0) {
					ret = wait_event_timeout(
	...
	if (oplus_dimlayer_bl_enable_v3 != oplus_dimlayer_bl_enable_v3_real) {
		...
		if (oplus_datadimming_v3_skip_frame > 0) {
			...
			oplus_dimlayer_bl_enable_v3_real =
				oplus_dimlayer_bl_enable_v3;
			_sde_connector_update_bl_scale_(c_conn);
...
static int sde_connector_update_hbm_enter(struct drm_connector *connector,
					  struct dsi_display *dsi_display,
					  unsigned int fps_period_us)
{
	...
	if (!dsi_display->panel->oplus_priv.is_aod_ramless ||
		...
		dsi_display_clk_ctrl(dsi_display->dsi_clk_handle, DSI_CORE_CLK,
				     DSI_CLK_ON);
		...
		if (oplus_seed_backlight) {
			...
			oplus_panel_process_dimming_v2(
				panel, panel->bl_config.bl_level, true);
			if (is_skip_panel_dc_set_brightness(
		...
		if (!strcmp(dsi_display->panel->oplus_priv.vendor_name,
			    "ANA6706")) {
			rc = dsi_panel_tx_cmd_set(dsi_display->panel,
						  DSI_CMD_MCA_OFF);
			if (rc) {
			...
			ktime_t delta =
				ktime_sub(ktime_get(), oplus_backlight_time);
			s64 delta_us = ktime_to_us(delta);
			if (delta_us < 34000 && delta_us >= 0)
				usleep_range(34000 - delta_us,
					     34000 - delta_us + 100);
		}
#endif /* OPLUS_FEATURE_AOD_RAMLESS */
		...
		if ((dsi_display->panel->oplus_priv.is_oplus_project) &&
		    (!strcmp(dsi_display->panel->oplus_priv.vendor_name,
			     "AMB655X"))) {
			dsi_panel_seed_mode(dsi_display->panel,
					    PANEL_LOADING_EFFECT_OFF);
		}

		if (OPLUS_DISPLAY_AOD_SCENE != get_oplus_display_scene() &&
		    dsi_display->panel->bl_config
			    .bl_level) { /* NO OPLUS_DISPLAY_AOD_SCENE */
			if (strcmp(dsi_display->panel->oplus_priv.vendor_name,
				   "AMB655X")) {
				if (dsi_display->config.panel_mode !=
				    DSI_OP_VIDEO_MODE) {
					...
					if (!strcmp(dsi_display->panel
							    ->oplus_priv
							    .vendor_name,
						    "S6E3HC3")) {
						SDE_ATRACE_BEGIN("wait_vblank");
						if (dsi_display->panel->cur_mode
							    ->timing
							    .refresh_rate ==
						    60) {
							ret = fingerprint_wait_vsync(
								c_conn->encoder,
								dsi_display
									->panel);
						} else {
			...
			if (!strcmp(panel->oplus_priv.vendor_name,
				    "AMS643YE01") ||
			...
			if (is_support_panel_hbm_enter_send_hbm_on_cmd(
				    dsi_display->panel->oplus_priv.vendor_name)) {
				if (!strcmp(dsi_display->panel->oplus_priv
						    .vendor_name,
					    "AMB655X")) {
					ret = fingerprint_wait_vsync(
						c_conn->encoder,
			...
			if (!strcmp(dsi_display->panel->oplus_priv.vendor_name,
				...
				rc = dsi_panel_tx_cmd_set(dsi_display->panel,
							  DSI_CMD_AOD_HBM_ON);
				if ((dsi_display->panel->oplus_priv
					     .is_oplus_project) &&
				    (!strcmp(dsi_display->panel->oplus_priv
						     .vendor_name,
					     "AMB655X"))) {
					set_oplus_display_scene(
...
static int sde_connector_update_hbm_exit(struct drm_connector *connector,
					 struct dsi_display *dsi_display,
					 unsigned int fps_period_us)
{
	struct sde_connector *c_conn = to_sde_connector(connector);
	...
	if (oplus_panel_hbm_exit_check_wait_vblank(
		    dsi_display->panel->oplus_priv.vendor_name)) {
		if (!strcmp(dsi_display->panel->oplus_priv.vendor_name,
			    "S6E3HC3")) {
			ret = wait_event_timeout(
	...
	if (OPLUS_DISPLAY_AOD_HBM_SCENE ==
	    get_oplus_display_scene()) { /* OPLUS_DISPLAY_AOD_HBM_SCENE */
		if (OPLUS_DISPLAY_POWER_DOZE_SUSPEND ==
			...
			if (is_support_panel_hbm_enter_send_hbm_off_cmd(
				    dsi_display)) {
				rc = dsi_panel_tx_cmd_set(dsi_display->panel,
							  DSI_CMD_HBM_OFF);
			} else {
	...
	} else if (OPLUS_DISPLAY_AOD_SCENE ==
		   get_oplus_display_scene()) { /* OPLUS_DISPLAY_AOD_SCENE */
		if (!strcmp(dsi_display->panel->oplus_priv.vendor_name,
			    "S6E3HC3") &&
		    (panel->panel_id2 >= 5)) {
			rc = dsi_panel_tx_cmd_set(dsi_display->panel,
		...
		if ((!strcmp(panel->oplus_priv.vendor_name, "S6E3HC3")) ||
		    ...
		    (!strcmp(panel->oplus_priv.vendor_name, "AMB655UV01") &&
		     (dsi_display->panel->oplus_priv.is_oplus_project)) ||
		    (!strcmp(panel->oplus_priv.vendor_name, "ANA6706") &&
		     (dsi_display->panel->oplus_priv.is_oplus_project)) ||
		    (!strcmp(panel->oplus_priv.vendor_name, "AMB655X") &&
		     (dsi_display->panel->oplus_priv.is_oplus_project))) {
			if (!strcmp(panel->oplus_priv.vendor_name, "S6E3HC3")) {
				fingerprint_wait_vsync(c_conn->encoder,
						       dsi_display->panel);
			}
			dsi_panel_seed_mode(dsi_display->panel, seed_mode);
			dsi_panel_tx_cmd_set(dsi_display->panel,
					     DSI_CMD_HBM_AOR_RESTORE);
			rc = dsi_panel_tx_cmd_set(dsi_display->panel,
						  DSI_CMD_HBM_OFF);
			oplus_panel_update_backlight_unlock(panel);
...
int sde_connector_update_hbm(struct drm_connector *connector)
{
	...
	fingerprint_mode =
		sde_crtc_get_fingerprint_mode(c_conn->encoder->crtc->state);

	if (OPLUS_DISPLAY_AOD_SCENE == get_oplus_display_scene()) {
		if (sde_crtc_get_fingerprint_pressed(
			    c_conn->encoder->crtc->state)) {
			sde_crtc_set_onscreenfinger_defer_sync(
				c_conn->encoder->crtc->state, true);
		} else {
			sde_crtc_set_onscreenfinger_defer_sync(
				c_conn->encoder->crtc->state, false);
	...
	if (fingerprint_mode != dsi_display->panel->is_hbm_enabled) {
		...
		if (oplus_fod_on_vblank >= 0)
			panel->cur_mode->priv_info->fod_on_vblank =
				oplus_fod_on_vblank;
		if (oplus_fod_off_vblank >= 0)
			panel->cur_mode->priv_info->fod_off_vblank =
				oplus_fod_off_vblank;
		...
		if (dsi_display->panel->oplus_priv.dc_apollo_sync_enable &&
		    dsi_display->panel->is_hbm_enabled) {
			dc_apollo.pcc_last =
				dsi_display->panel->oplus_priv
					.dc_apollo_sync_brightness_level_pcc;
		}

		SDE_ATRACE_BEGIN(fingerprint_name);
		if (fingerprint_mode) {
			rc = sde_connector_update_hbm_enter(
				connector, dsi_display, fps_period_us);
			if (rc) {
				pr_err("failed to sde_connector_update_hbm_enter, rc=%d\n",
				       rc);
			}
		} else {
			rc = sde_connector_update_hbm_exit(
				connector, dsi_display, fps_period_us);
			if (rc) {
				...
				enum dsi_cmd_set_type type)
{
	...
	if (type != DSI_CMD_SEED_MODE0 && type != DSI_CMD_SEED_MODE1 &&
	    ...
	    type != DSI_CMD_SEED_MODE4 && type != DSI_CMD_SEED_MODE0_SWITCH &&
	    ...
	    type != DSI_CMD_SEED_MODE0_DC && type != DSI_CMD_SEED_MODE1_DC &&
	    type != DSI_CMD_SEED_MODE2_DC && type != DSI_CMD_SEED_OFF) {
		return NULL;
	}
	...
	for (i = 0; i < count; i++) {
		...
		oplus_cmd[i].msg.tx_buf =
			kmemdup(cmds[i].msg.tx_buf, size, GFP_KERNEL);
