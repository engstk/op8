...
int __init midas_dev_init(void)
{
	...
	rc = register_trace_android_vh_cpufreq_acct_update_power(midas_record_task_times, NULL);
	if (rc < 0) {
...
err_device_register:
	unregister_trace_android_vh_cpufreq_acct_update_power(midas_record_task_times, NULL);
err_trace_register:
	return rc;
}
