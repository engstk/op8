...
static void __iris_update_vfr_work(struct work_struct *work)
{
	struct iris_cfg *pcfg =
		container_of(work, struct iris_cfg, vfr_update_work);
...
void iris_frc_parameter_init(void)
{
	...
	if ((refresh_rate % 10) != 0) {
		...
		IRIS_LOGW("change refresh_rate from %d to %d!",
...
int32_t iris_set_second_channel_power(bool pwr)
{
	...
	if (pwr) { //on
		...
		if (pcfg2->iris_osd_autorefresh_enabled)
			IRIS_LOGW(
				"osd_autorefresh is not disable before enable osd!");
		else
			IRIS_LOGI(
				"osd_autorefresh is disable before enable osd!");

		if (pcfg2->panel->power_info.refcount == 0) {
			IRIS_LOGW("%s(), AP mipi2 tx hasn't been power on.",
				  __func__);
...
static void _iris_osd_blending_off(void)
{
	...
	iris_set_pwil_mode(pcfg->panel, pcfg->pwil_mode, false,
...
static void _iris_osd_blending_on(void)
{
	...
	iris_set_pwil_mode(pcfg->panel, pcfg->pwil_mode, true,
			   DSI_CMD_SET_STATE_HS);
	iris_set_pwil_disp_ctrl();
}
...
int32_t iris_switch_osd_blending(u32 val)
{
	...
	if (val) {
		if (pcfg->dual_test & 0x100) {
			...
			payload = iris_get_ipopt_payload_data(IRIS_IP_RX_2,
							      0xF0, 2);
			while (count < 20) {
				cmd_2b_page = iris_ocp_read(
					0xf1840304, DSI_CMD_SET_STATE_HS);
				if (cmd_2b_page != payload[2]) {
					count++;
					IRIS_LOGW(
						"Warning: cmd_2b_page: %x not right, %d!",
						cmd_2b_page, count);
...
void iris_register_osd_irq(void *disp)
{
	...
	osd_gpio = pcfg->iris_osd_gpio;
	IRIS_LOGI("%s(), for display %s, osd status gpio is %d", __func__,
		  display->name, osd_gpio);
	if (!gpio_is_valid(osd_gpio)) {
		IRIS_LOGE("%s(%d), osd status gpio not specified", __func__,
			  __LINE__);
		return;
	}

	pdev = display->pdev;
	IRIS_LOGI("%s, display: %s, irq: %d", __func__, display->name,
		  gpio_to_irq(osd_gpio));
	rc = devm_request_irq(&pdev->dev, gpio_to_irq(osd_gpio),
			      iris_osd_handler, IRQF_TRIGGER_RISING, "OSD_GPIO",
			      display);
	if (rc) {
...
void iris_frc_prepare(struct iris_cfg *pcfg)
{
	if (pcfg->osd_switch_on_pending) {
		...
		IRIS_LOGI("%s(), set secondary channel power for osd pending",
			  __func__);
		mutex_lock(&pcfg0->panel->panel_lock);
		iris_set_second_channel_power(true);
		mutex_unlock(&pcfg0->panel->panel_lock);
		IRIS_LOGI("%s(), finish setting secondary channel power",
			  __func__);
	}
}
...
static void iris_frc_setting_switch(bool dual)
{
	...
	for (i = 0; i < 2; i++) {
		payload =
			iris_get_ipopt_payload_data(IRIS_IP_PWIL, opt_id[i], 2);
	...
	payload = iris_get_ipopt_payload_data(IRIS_IP_DSC_ENC_2,
					      dual ? 0xf2 : 0xf1, 2);
...
void iris_frc_dsc_setting(bool dual)
{
	struct iris_ctrl_opt arr_single[] = {
		{ 0x25, 0xf1,
		  0x01 }, // IRIS_IP_DSC_DEN_2, dsc_encoder_frc: init
		{ 0x24, 0xf1,
		  0x01 }, // IRIS_IP_DSC_ENC_2, dsc_encoder_frc: init
