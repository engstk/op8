...
static bool is_vooc30_cfg = false;
static bool is_vooc30_project(void)
{
	return is_vooc30_cfg;
}

static bool usb_ntc_support = false;
static int oplus_usb_ntc_parse_dt(struct oplus_chg_chip *chip)
{
	...
	usb_ntc_support = of_property_read_bool(node, "qcom,usb_ntc_support");
	if (usb_ntc_support == false) {
		chg_err("not support usb ntc!\n");
	}

	return 0;
}

static int oplus_vooc30_project_parse_dt(struct oplus_chg_chip *chip)
{
	struct device_node *node = NULL;
	...
	is_vooc30_cfg = of_property_read_bool(node, "qcom,is_vooc30_project");
	if (is_vooc30_project == false) {
		chg_err("not vooc30 project!\n");
	}
...
#define CHARGE_NTC_MUTIPLY	1500
#define CHARGE_NTC_DIVIDE	4096
#define MUTIPLY_THOUSAND	1000
int mt_get_charger_ntc_volt(void)
{
	...
	if (!pinfo->chargeric_temp_chan) {
		...
		rc = iio_read_channel_processed(pinfo->chargeric_temp_chan, &charger_ntc_volt);
		if (rc < 0) {
			chg_err("read chargeric_temp_chan fail, rc=%d\n", rc);
			return 0;
		}

		charger_ntc_volt = charger_ntc_volt * CHARGE_NTC_MUTIPLY / CHARGE_NTC_DIVIDE;
		chg_debug("charger_ntc_volt=%d\n", charger_ntc_volt);
	}

	return charger_ntc_volt;
}

static int get_charger_ntc_temp(struct thermal_zone_device *tz,
					int *temp)
{
	int i = 0;
	int charger_ntc_volt = 0;
	charger_ntc_volt = mt_get_charger_ntc_volt();
	...
	*temp = *temp * MUTIPLY_THOUSAND;
	chr_err("[%s] charger_ntc temp:%d\n ", __func__, *temp);
	return 0;
}

struct thermal_zone_device_ops charger_ntc_thermal_zone_ops = {
	.get_temp = get_charger_ntc_temp,
};
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	rc = oplus_usb_ntc_parse_dt(chip);
	if (rc) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: oplus_usb_ntc_parse_dt fail!\n", __func__);
		return -EINVAL;
	}

	rc = oplus_vooc30_project_parse_dt(chip);
	if (rc) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: oplus_vooc30_project_parse_dt fail!\n", __func__);
		return -EINVAL;
	}
...
int oplus_mt6360_pd_setup_forsvooc(void)
{
	...
	pr_err("%s get_project()=%x\n", __func__, get_project());
	if (get_project() == 0X2169E || get_project() == 0X2069F ||
		get_project() == 21711 || get_project() == 21712 || get_project() == 0X2162D) {
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if(pinfo->chargeric_temp_chan){
		tz_dev = thermal_zone_device_register("mp2762_ntc",
				0, 0, oplus_chip, &charger_ntc_thermal_zone_ops, NULL, 0, 0);
