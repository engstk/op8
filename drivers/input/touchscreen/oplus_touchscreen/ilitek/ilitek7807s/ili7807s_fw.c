...
static int calc_hw_dma_crc(u32 start_addr, u32 block_size)
{
    ...
    /* Dma1 stop */
    if (ili_ice_mode_write(0x072100, 0x02000000,
                           4) < 0) {
    ...
    /* Dma1 start */
    if (ili_ice_mode_write(0x072100, 0x01000000,
                           4) < 0) {
    ...
    while (count > 0) {
        ...
        if (ili_ice_mode_read(0x048006, &busy,
                              sizeof(u8)) < 0) {
    ...
    if (ili_ice_mode_read(0x04101C, &busy,
                          sizeof(u32)) < 0) {
...
static int ilitek_tddi_fw_iram_read(u8 *buf,
                                    u32 start, int len)
{
    ...
    for (cnt = 0, addr = start; cnt < loop;
        ...
        if (ilits->wrapper(cmd, 4, NULL, 0, OFF,
                           OFF) < 0) {
            ILI_ERR("Failed to write iram data\n");
            return -ENODEV;
        }

        if (ilits->wrapper(NULL, 0, buf + cnt * limit,
                           tmp_len, OFF, OFF) < 0) {
...
int ili_fw_dump_iram_data(u32 start, u32 end,
                          bool save, bool mcu)
{
    struct file *f = NULL;
    mm_segment_t old_fs;
    ...
    ret = ilitek_tddi_fw_iram_read(ilits->update_buf,
                                   start, len);
    ...
    if (save) {
        ...
        ili_dump_data(ilits->update_buf, 8, len, 0,
                      "IRAM");
        ili_debug_en = tmp;
    }
...
static int ilitek_tddi_fw_iram_program(u32 start,
                                       u8 *w_buf, u32 w_len, u32 split_len)
{
    int i = 0, j = 0, addr = 0;
    u32 end = start + w_len;
    ...
    if (split_len != 0) {
        for (addr = start, i = 0; addr < end;
            ...
            ilits->update_buf[2] = (char)((addr &
                                           0x000000FF));
            ilits->update_buf[3] = (char)((addr & 0x0000FF00)
                                          >> 8);
            ilits->update_buf[4] = (char)((addr & 0x00FF0000)
                                          >> 16);
            ...
            if (fix_4_alignment) {
                ...
                ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                         split_len - 4, ilits->update_buf[5 + split_len -
                                                            4]);
                ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                         split_len - 3, ilits->update_buf[5 + split_len -
                                                            3]);
                ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                         split_len - 2, ilits->update_buf[5 + split_len -
                                                            2]);
                ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                         split_len - 1, ilits->update_buf[5 + split_len -
                                                            1]);

                for (j = 0; j < (4 - (split_len % 4)); j++) {
                    ilits->update_buf[5 + j + split_len] = 0xFF;
                    ILI_INFO("idev->update_buf[5 + 0x%X] = 0x%X\n",
                             j + split_len, ilits->update_buf[5 + j +
                                                                split_len]);
                }
            ...
            if (ilits->spi_write_then_read(ilits->spi,
                                           ilits->update_buf, split_len + 5, NULL, 0)) {
        ...
        ilits->update_buf[0] = SPI_WRITE;
        ilits->update_buf[1] = 0x25;
        ilits->update_buf[2] = (char)((start &
                                       0x000000FF));
        ilits->update_buf[3] = (char)((start & 0x0000FF00)
                                      >> 8);
        ilits->update_buf[4] = (char)((start & 0x00FF0000)
                                      >> 16);
        ...
        if (ilits->spi_write_then_read(ilits->spi,
                                       ilits->update_buf, w_len + 5, NULL, 0) < 0) {
            ILI_ERR("Failed to write data via SPI in host download (%x)\n",
                    w_len + 5);
            return -EIO;
        }
                                       ...
                                       bool mcu)
{
    ...
    for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
        if (fbi[i].mode == mode && fbi[i].len != 0) {
            ...
            if (ilitek_tddi_fw_iram_program(fbi[i].mem_start,
                                            (fw_ptr + fbi[i].start), fbi[i].len, 0) < 0) {
                ILI_ERR("IRAM program failed\n");
            }

#endif
            crc = CalculateCRC32(fbi[i].start,
                                 fbi[i].len - crc_len, fw_ptr);
            dma = calc_hw_dma_crc(fbi[i].mem_start,
                                  fbi[i].len - crc_len);

            if (mode != GESTURE) {
                ilitek_tddi_fw_iram_read(crc_temp,
                                         (fbi[i].mem_start + fbi[i].len - crc_len),
                                         sizeof(crc_temp));
                iram_crc = crc_temp[0] << 24 | crc_temp[1] << 16 |
                           crc_temp[2] << 8 | crc_temp[3];
            ...
            ILI_DBG("%s CRC is %s hex(%x) : dma(%x) : iram(%x), calculation len is 0x%x\n",
                     fbi[i].name, ((crc != dma)
                                   || (iram_crc_err)) ? "Invalid !" : "Correct !",
    ...
    if (!ilits->info_from_hex
        || (ilits->chip->core_ver < CORE_VER_1410)) {
        mdelay(100);
    }
...
static int ilitek_fw_calc_file_crc(u8 *pfw)
{
    ...
    for (i = 0; i < ARRAY_SIZE(fbi); i++) {
        ...
        ex_addr = fbi[i].end;
        data_crc = CalculateCRC32(fbi[i].start,
                                  fbi[i].len - 4, pfw);
        file_crc = pfw[ex_addr - 3] << 24 | pfw[ex_addr -
                                                2] << 16 | pfw[ex_addr - 1] << 8 | pfw[ex_addr];
...
static int ilitek_tddi_fw_update_block_info(
    u8 *pfw)
{
    u32 ges_area_section = 0, ges_info_addr = 0,
        ges_fw_start = 0, ges_fw_end = 0;
    ...
    fbi[AP].mem_start = (fbi[AP].fix_mem_start !=
                         INT_MAX) ? fbi[AP].fix_mem_start : 0;
    fbi[DATA].mem_start = (fbi[DATA].fix_mem_start !=
                           INT_MAX) ? fbi[DATA].fix_mem_start :
                          DLM_START_ADDRESS;
    fbi[TUNING].mem_start = (fbi[TUNING].fix_mem_start
                             != INT_MAX) ? fbi[TUNING].fix_mem_start :
                            fbi[DATA].mem_start + fbi[DATA].len;
    fbi[MP].mem_start = (fbi[MP].fix_mem_start !=
                         INT_MAX) ? fbi[MP].fix_mem_start : 0;
    fbi[GESTURE].mem_start =
        (fbi[GESTURE].fix_mem_start != INT_MAX) ?
        fbi[GESTURE].fix_mem_start : 0;
    fbi[TAG].mem_start = (fbi[TAG].fix_mem_start !=
                          INT_MAX) ? fbi[TAG].fix_mem_start : 0;
    fbi[PARA_BACKUP].mem_start =
        (fbi[PARA_BACKUP].fix_mem_start != INT_MAX) ?
        fbi[PARA_BACKUP].fix_mem_start : 0;
    fbi[DDI].mem_start = (fbi[DDI].fix_mem_start !=
                          INT_MAX) ? fbi[DDI].fix_mem_start : 0;
    /* Parsing gesture info form AP code */
    ges_info_addr = (fbi[AP].end + 1 - 60);
    ges_area_section = (pfw[ges_info_addr + 3] << 24)
                       + (pfw[ges_info_addr + 2] << 16) +
                       (pfw[ges_info_addr + 1] << 8) +
                       pfw[ges_info_addr];
    fbi[GESTURE].mem_start = (pfw[ges_info_addr + 7]
                              << 24) + (pfw[ges_info_addr + 6] << 16) +
                             (pfw[ges_info_addr + 5] << 8) + pfw[ges_info_addr
                                                                 + 4];
    ap_end = (pfw[ges_info_addr + 11] << 24) +
             (pfw[ges_info_addr + 10] << 16) +
             (pfw[ges_info_addr + 9] << 8) + pfw[ges_info_addr
                                                 + 8];
    ...
    ges_fw_start = (pfw[ges_info_addr + 15] << 24) +
                   (pfw[ges_info_addr + 14] << 16) +
                   (pfw[ges_info_addr + 13] << 8) + pfw[ges_info_addr
                                                        + 12];
    ges_fw_end = (pfw[ges_info_addr + 19] << 24) +
                 (pfw[ges_info_addr + 18] << 16) +
                 (pfw[ges_info_addr + 17] << 8) + pfw[ges_info_addr
                                                      + 16];
    ...
    fbi[DDI].name = "DDI";
    /* upgrade mode define */
    fbi[DATA].mode = fbi[AP].mode = fbi[TUNING].mode =
                                        AP;
    fbi[MP].mode = MP;
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw)
{
    ...
    if (!ilits->p_firmware_headfile->firmware_data
    ...
    CTPM_FW = ilits->p_firmware_headfile->firmware_data;
    size = ilits->p_firmware_headfile->firmware_size;

    if (size < ILI_FILE_HEADER
        || size > MAX_HEX_FILE_SIZE) {
    ...
    for (i = 0; i < FW_BLOCK_INFO_NUM - 1; i++) {
        /* B0 tag */
        b0_addr = (CTPM_FW[4 + i * 4] << 16) |
                  (CTPM_FW[5 + i * 4] << 8) | (CTPM_FW[6 + i * 4]);
        ...
        if (((blk_map >> i) & 0x01) == 0x01) {
            fbi[num].start = (CTPM_FW[132 + i * 6] << 16) |
                             (CTPM_FW[133 + i * 6] << 8) | CTPM_FW[134 + i *
                                                                   6];
            fbi[num].end = (CTPM_FW[135 + i * 6] << 16) |
                           (CTPM_FW[136 + i * 6] << 8) |  CTPM_FW[137 + i *
                                                                  6];
    ...
    memcpy(pfw, CTPM_FW + ILI_FILE_HEADER,
           size - ILI_FILE_HEADER);
...
static int ilitek_tddi_fw_hex_convert(u8 *phex,
                                      int size, u8 *pfw)
{
    int block = 0;
    ...
    for (; i < size;) {
        ...
        if (type == 0x04) {
            ...
            fbi[num].fix_mem_start = HexToDec(&phex[i + 9],
                                              6);
...
static int ilitek_tdd_fw_hex_open(u8 op, u8 *pfw)
{
    ...
    if (ilitek_tddi_fw_hex_convert((u8 *)
                                   ilits->tp_fw.data, ilits->tp_fw.size, pfw) < 0) {
...
int ili_fw_upgrade(int op)
{
    ...
    if (ilits->oplus_fw_update
        || ilits->force_fw_update || ERR_ALLOC_MEM(pfw)) {
