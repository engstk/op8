...
#define INFO_MSG(msg, args...) \
	pr_info("%s:%d info: " msg "\n", __func__, __LINE__, ##args)
#define ERR_MSG(msg, args...) \
	pr_err("%s:%d err: " msg "\n", __func__, __LINE__, ##args)
#define WARN_MSG(msg, args...) \
	pr_warn("%s:%d warn: " msg "\n", __func__, __LINE__, ##args)

#define seq_scan_lu(lun) \
	for (lun = 0; lun < UFS_UPIU_MAX_GENERAL_LUN; lun = lun + 1)
...
#define TMSG(ufsf, lun, msg, args...)                                       \
	do {                                                                \
		if (ufsf->sdev_ufs_lu[lun] &&                               \
			...
			blk_add_trace_msg(                                  \
				ufsf->sdev_ufs_lu[lun]->request_queue, msg, \
				##args);                                    \
	} while (0)
