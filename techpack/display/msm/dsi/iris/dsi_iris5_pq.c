...
static u32 iris_color_x_buf[] = {
	4637, 4626, 4615, 4603, 4591, 4578, 4565, 4552, 4538, 4524, 4510, 4496,
	4481, 4467, 4452, 4437, 4422, 4407, 4392, 4377, 4362, 4347, 4332, 4317,
	4302, 4287, 4272, 4257, 4243, 4228, 4213, 4199, 4184, 4170, 4156, 4141,
	4127, 4113, 4099, 4086, 4072, 4058, 4045, 4032, 4018, 4005, 3992, 3980,
	3967, 3954, 3942, 3929, 3917, 3905, 3893, 3881, 3869, 3858, 3846, 3835,
	3823, 3812, 3801, 3790, 3779, 3769, 3758, 3748, 3737, 3727, 3717, 3707,
	3697, 3687, 3677, 3668, 3658, 3649, 3639, 3630, 3621, 3612, 3603, 3594,
	3585, 3577, 3568, 3560, 3551, 3543, 3535, 3527, 3519, 3511, 3503, 3495,
	3487, 3480, 3472, 3465, 3457, 3450, 3443, 3436, 3429, 3422, 3415, 3408,
	3401, 3394, 3388, 3381, 3375, 3368, 3362, 3356, 3349, 3343, 3337, 3331,
	3325, 3319, 3313, 3307, 3302, 3296, 3290, 3285, 3279, 3274, 3268, 3263,
	3258, 3252, 3247, 3242, 3237, 3232, 3227, 3222, 3217, 3212, 3207, 3202,
	3198, 3193, 3188, 3184, 3179, 3175, 3170, 3166, 3161, 3157, 3153, 3149,
	3144, 3140, 3136, 3132, 3128, 3124, 3120, 3116, 3112, 3108, 3104, 3100,
	3097, 3093, 3089, 3085, 3082, 3078, 3074, 3071, 3067, 3064, 3060, 3057,
	3054, 3050, 3047, 3043, 3040, 3037, 3034, 3030, 3027, 3024, 3021, 3018,
	3015, 3012, 3009, 3006, 3003, 3000, 2997, 2994, 2991, 2988, 2985, 2982,
	2980, 2977, 2974, 2971, 2969, 2966, 2963, 2961, 2958, 2955, 2953, 2950,
	2948, 2945, 2943, 2940, 2938, 2935, 2933, 2930, 2928, 2926, 2923, 2921,
	2919, 2916, 2914, 2912, 2910, 2907, 2905, 2903, 2901, 2899, 2896, 2894,
	2892, 2890, 2888, 2886, 2884, 2882, 2880, 2878, 2876, 2874, 2872, 2870,
	2868, 2866, 2864, 2862, 2860, 2858, 2856, 2854, 2853, 2851, 2849, 2847,
	2845, 2844, 2842, 2840, 2838, 2837, 2835, 2833, 2831, 2830, 2828, 2826,
	2825, 2823, 2821, 2820, 2818, 2817, 2815, 2813, 2812, 2810, 2809, 2807,
	2806, 2804, 2803, 2801, 2800, 2798, 2797, 2795, 2794, 2792, 2791, 2789,
	2788,
};
...
static int iris_capture_disable_pq(struct iris_update_ipopt *popt,
				   bool *skiplast)
{
	int len = 0;
	...
	*skiplast = 0;
	if ((!iris_capture_ctrl_en) && (!iris_debug_cap)) {
		if (!iris_dynamic_power_get())
			len = iris_init_update_ipopt_t(popt, IP_OPT_MAX,
						       IRIS_IP_PWIL, 0x50, 0x50,
						       IRIS_LAST_BIT_CTRL);
		else
			len = iris_init_update_ipopt_t(popt, IP_OPT_MAX,
						       IRIS_IP_PWIL, 0x52, 0x52,
						       IRIS_LAST_BIT_CTRL);
...
static int iris_capture_enable_pq(struct iris_update_ipopt *popt, int oldlen)
{
	...
	if ((!iris_capture_ctrl_en) && (!iris_debug_cap))
		len = iris_init_update_ipopt_t(
			popt, IP_OPT_MAX, IRIS_IP_PWIL, 0x51, 0x51,
...
static int iris_capture_disable_lce(struct iris_update_ipopt *popt,
				    bool *skiplast)
{
	int len = 0;

	*skiplast = 0;
	if ((!iris_capture_ctrl_en) && (iris_lce_power_status_get()) &&
	    (!iris_debug_cap)) {
		if (!iris_dynamic_power_get())
...
void iris_scaler_filter_ratio_get(void)
{
	...
	pcfg = iris_get_cfg();
	dwRatioDiff =
		pcfg->frc_setting.disp_hres * 10 / pcfg->frc_setting.memc_hres;
...
void iris_pq_parameter_init(void)
{
	...
	IRIS_LOGI("%s, iris_min_x_value=%d, iris_max_x_value = %d", __func__,
		  iris_min_x_value, iris_max_x_value);
}
...
void iris_peaking_level_set(u32 level)
{
	...
	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_PEAKING, level, 0x01);
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_PEAKING, csc,
				 skiplast);
...
static int iris_cm_csc_para_set(struct iris_update_ipopt *popt,
				uint8_t skip_last, uint32_t csc_ip,
				uint32_t *csc_value)
{
	int32_t ip = 0;
	int32_t opt_id = 0;
	...
	if (psopt == NULL) {
		IRIS_LOGE("can not find i_iris_p = %02x opt_id = %02x", ip,
			  opt_id);
		return 1;
	}
	...
	IRIS_LOGD("csc: csc0=0x%x, csc1=0x%x, csc2=0x%x, csc3=0x%x, csc4=0x%x",
		  csc_value[0], csc_value[1], csc_value[2], csc_value[3],
		  csc_value[4]);
	val = csc_value[0];
	val &= 0x7fff7fff;
	data[3] = val;
...
void iris_cm_ftc_enable_set(u32 level)
{
	...
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_CM, locallevel,
				 skiplast);
...
void iris_scurve_enable_set(u32 level)
{
	...
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DPP, locallevel,
				 skiplast);
...
int iris_cm_ratio_set(struct iris_update_ipopt *popt, uint8_t skip_last)
{
	...
	else if (pqlt_cur_setting->pq_setting.cmcolortempmode ==
	...
	if (xvalue == iris_min_x_value) {
		...
		tablesel = 0;
		ratio = ((xvalue - IRIS_X_6500K) * 16383) /
			(iris_min_x_value - IRIS_X_6500K);
		regvalue = tablesel | (ratio << 16);
	} else if ((xvalue >= iris_max_x_value) && (xvalue < IRIS_X_6500K)) {
		tablesel = 1;
		ratio = ((xvalue - iris_max_x_value) * 16383) /
			(IRIS_X_6500K - iris_max_x_value);
		regvalue = tablesel | (ratio << 16);
	}
	...
	len = iris_init_update_ipopt_t(popt, IP_OPT_MAX, IRIS_IP_DPP, 0xfd,
				       0xfd, skip_last);
...
u32 iris_cm_ratio_set_for_iic(void)
{
	...
	if (xvalue == iris_min_x_value) {
		...
		tablesel = 0;
		ratio = ((xvalue - IRIS_X_7700K) * 16383) /
			(iris_min_x_value - IRIS_X_7700K);
		regvalue = tablesel | (ratio << 16);
	} else if ((xvalue >= iris_max_x_value) && (xvalue < IRIS_X_7700K)) {
		tablesel = 1;
		ratio = ((xvalue - iris_max_x_value) * 16383) /
			(IRIS_X_7700K - iris_max_x_value);
		regvalue = tablesel | (ratio << 16);
	}
...
void iris_cm_colortemp_mode_set(u32 mode)
{
	...
	if (pqlt_cur_setting->source_switch != 1) {
		...
		if (mode == IRIS_COLOR_TEMP_OFF)
			iris_update_ip_opt(popt, IP_OPT_MAX, GAMMA_LUT, 0x00,
					   0x01);
		else
			iris_update_ip_opt(
				popt, IP_OPT_MAX, GAMMA_LUT,
...
void iris_cm_color_gamut_pre_set(u32 source_switch)
{
	...
	iris_update_bitmask_regval_nonread(&regval, false);
	len = iris_init_update_ipopt_t(popt, IP_OPT_MAX, IRIS_IP_DPP, 0xfc,
				       0xfc, skiplast);
	len = iris_capture_enable_pq(popt, len);
	is_ulps_enable = iris_disable_ulps(path);
	iris_update_pq_opt(popt, len, path);
...
void iris_cm_color_gamut_set(u32 level)
{
	...
	len = iris_update_ip_opt(
		popt, IP_OPT_MAX, CM_LUT, level * 3 + 2,
		(pqlt_cur_setting->source_switch == 0) ? 0x01 : skiplast);
...
void iris_dpp_gamma_set(void)
{
	...
	len = iris_update_ip_opt(popt, IP_OPT_MAX, GAMMA_LUT, gammalevel,
				 skiplast);
...
static int iris_lce_gamm1k_set(struct iris_update_ipopt *popt,
			       uint8_t skip_last)
{
	...
	if (pqlt_cur_setting->pq_setting.alenable == true) {
		...
		if (pqlt_cur_setting->luxvalue >= (m_dwTH_LuxAdj * 8))
			dwLux_i = MIN(8191, (8 * m_dwTH_LuxAdj +
					     pqlt_cur_setting->luxvalue / 8 -
					     m_dwTH_LuxAdj));
		else
		...
		iris_update_bitmask_regval_nonread(&regval, false);
		len = iris_init_update_ipopt_t(popt, IP_OPT_MAX, IRIS_IP_LCE,
					       0xfd, 0xfd, skip_last);
	}
...
static int iris_lce_gamm1k_restore(struct iris_update_ipopt *popt,
				   uint8_t skip_last)
{
	u32 level;
	struct iris_update_regval regval;
	...
	iris_update_bitmask_regval_nonread(&regval, false);
	len = iris_init_update_ipopt_t(popt, IP_OPT_MAX, IRIS_IP_LCE, 0xfd,
				       0xfd, skip_last);
...
void iris_lce_mode_set(u32 mode)
{
	...
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_LCE, locallevel,
				 0x01);
...
void iris_lce_level_set(u32 level)
{
	...
	if (pqlt_cur_setting->pq_setting.lcemode == IRIS_LCE_GRAPHIC)
		locallevel = level;
	else
		locallevel = level + 6;
...
void iris_lce_demo_window_set(u32 vsize, u32 hsize, u8 inwnd)
{
	...
	data = (payload[0] & (~0x0FFF0FFF)) | ((vsize & 0x0FFF) << 16) |
	       (hsize & 0x0FFF);
	...
	data = (payload[12] & (~0x0002C000)) | ((hsize != 0 ? 1 : 0) << 14) |
	       ((vsize != 0 ? 1 : 0) << 15) | ((inwnd & 0x01) << 17);
	iris_set_ipopt_payload_data(IRIS_IP_LCE, level, 14, data);
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_LCE, level,
				 skiplast);
...
void iris_dbc_level_set(u32 level)
{
	...
	iris_update_ip_opt(popt, IP_OPT_MAX, DBC_LUT, DBC_OFF + level, 0x01);
	iris_init_update_ipopt_t(popt, IP_OPT_MAX, DBC_LUT,
				 CABC_DLV_OFF + level, CABC_DLV_OFF + level,
				 0x01);

	iris_init_update_ipopt_t(popt, IP_OPT_MAX, IRIS_IP_DBC, localindex,
				 localindex, 0x01);
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DBC, dbcenable,
				 skiplast);
...
void iris_reading_mode_set(u32 level)
{
	...
	if (pqlt_cur_setting->pq_setting.sdr2hdr == SDR2HDR_Bypass) {
		...
		iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_CM, locallevel_cm,
				   0x01);
		len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DPP,
					 locallevel, skiplast);
...
#define AL_SDR2HDR_INDEX_OFFSET 7
	...
	if (pqlt_cur_setting->pq_setting.alenable == true) {
		if (pqlt_cur_setting->pq_setting.sdr2hdr >= HDR10In_ICtCp &&
			...
			if (!(iris_sdr2hdr_lut2ctl & 0xFFE00000)) {
				payload = iris_get_ipopt_payload_data(
					IRIS_IP_SDR2HDR,
					(AL_SDR2HDR_INDEX_OFFSET + 2), 83);
				iris_sdr2hdr_lut2ctl = payload[0];
				iris_sdr2hdr_lut2ctl =
					((lut_offset << 19) |
					 (0x17FFFF & iris_sdr2hdr_lut2ctl));
				iris_set_ipopt_payload_data(
					IRIS_IP_SDR2HDR,
					(AL_SDR2HDR_INDEX_OFFSET + 2), 83,
					iris_sdr2hdr_lut2ctl);
			}
			...
			len = iris_update_ip_opt(popt, IP_OPT_MAX,
						 AMBINET_SDR2HDR_LUT, 0x0,
						 skiplast);
			payload = iris_get_ipopt_payload_data(
				IRIS_IP_SDR2HDR, (AL_SDR2HDR_INDEX_OFFSET + 2),
				83);
		} else if (pqlt_cur_setting->pq_setting.sdr2hdr >=
				   SDR709_2_709 &&
			...
			len = iris_update_ip_opt(popt, IP_OPT_MAX,
						 ...
						 skiplast);
			payload = iris_get_ipopt_payload_data(
				IRIS_IP_SDR2HDR, (AL_SDR2HDR_INDEX_OFFSET + 5),
				83);
		}
	} else {
		u32 hdr_lut_idx;
		...
		if (!(iris_sdr2hdr_lut2ctl & 0xFFE00000)) {
			payload = iris_get_ipopt_payload_data(
				IRIS_IP_SDR2HDR,
				pqlt_cur_setting->pq_setting.sdr2hdr, 83);
			iris_sdr2hdr_lut2ctl = payload[0];
		}
		if (!(iris_sdr2hdr_lutyctl & 0xFFFF0000)) {
			payload = iris_get_ipopt_payload_data(
				IRIS_IP_SDR2HDR,
				pqlt_cur_setting->pq_setting.sdr2hdr, 23);
			iris_sdr2hdr_lutyctl = payload[0];
		}
		...
		if (pqlt_cur_setting->pq_setting.sdr2hdr >= HDR10In_ICtCp &&
		    pqlt_cur_setting->pq_setting.sdr2hdr <= ICtCpIn_YCbCr) {
			hdr_lut_idx =
				((pqlt_cur_setting->pq_setting.sdr2hdr - 1) &
				 0xf) |
				((SDR2HDR_LUT0 << 4) & 0xf0);
			iris_update_ip_opt(popt, IP_OPT_MAX, SDR2HDR_LUT,
					   hdr_lut_idx, 0x01);
			hdr_lut_idx =
				((pqlt_cur_setting->pq_setting.sdr2hdr - 1) &
				 0xf) |
			...
			hdr_lut_idx =
				((pqlt_cur_setting->pq_setting.sdr2hdr - 1) &
				 0xf) |
				((SDR2HDR_UVY2 << 4) & 0xf0);
			len = iris_update_ip_opt(popt, IP_OPT_MAX, SDR2HDR_LUT,
						 hdr_lut_idx, skiplast);
		} else if (pqlt_cur_setting->pq_setting.sdr2hdr >=
				   SDR709_2_709 &&
			   pqlt_cur_setting->pq_setting.sdr2hdr <=
				   SDR709_2_2020) {
			...
			hdr_lut_idx =
				((pqlt_cur_setting->pq_setting.sdr2hdr - 1) &
				 0xf) |
				((SDR2HDR_INV_UV0 << 4) & 0xf0);
			iris_update_ip_opt(popt, IP_OPT_MAX, SDR2HDR_LUT,
					   hdr_lut_idx, 0x01);
			hdr_lut_idx =
				((pqlt_cur_setting->pq_setting.sdr2hdr - 1) &
				 0xf) |
				((SDR2HDR_INV_UV1 << 4) & 0xf0);
			len = iris_update_ip_opt(popt, IP_OPT_MAX, SDR2HDR_LUT,
						 hdr_lut_idx, skiplast);
		} else {
			hdr_lut_idx =
	...
	iris_update_pq_opt(popt, len, path);
	iris_enable_ulps(path, is_ulps_enable);
	IRIS_LOGD("al=%d hdr level=%d", pqlt_cur_setting->pq_setting.alenable,
		  pqlt_cur_setting->pq_setting.sdr2hdr);
}
...
void iris_maxcll_lut_set(u32 lutpos)
{
	...
	if (!(iris_sdr2hdr_lutyctl & 0xFFFF0000)) {
		len = iris_update_ip_opt(popt, IP_OPT_MAX, AMBINET_HDR_GAIN,
					 0x0, 1);

		regval.ip = IRIS_IP_SDR2HDR;
		regval.opt_id = 0x92;
		regval.mask = 0x0000FFFF;
		regval.value = ((lutpos << 6) & 0xc0) |
			       (iris_sdr2hdr_lutyctl & 0xFF3F);
		iris_update_bitmask_regval_nonread(&regval, false);

		iris_sdr2hdr_lutyctl_set(lutpos);
		len += iris_init_update_ipopt_t(popt, IP_OPT_MAX,
						IRIS_IP_SDR2HDR, 0x92, 0x92,
						skiplast);
	} else
		len = iris_update_ip_opt(popt, IP_OPT_MAX, AMBINET_HDR_GAIN,
					 0x0, skiplast);
...
void iris_dbclce_datapath_set(bool bEn)
{
	...
	pwil_datapath = (payload[2] & (~0x1)) |
			((bEn == true) ? 0x00000001 : 0x00000000);
	iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xF1, 4, pwil_datapath);
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_PWIL, 0xF1,
				 skiplast);

	if ((!iris_capture_ctrl_en) && (!iris_dynamic_power_get()) &&
	    (!iris_debug_cap))
...
void iris_sdr2hdr_level_set(u32 level)
{
	...
	if (level >= HDR10In_ICtCp && level <= SDR709_2_2020) {
		...
		//		0x60 + level - 1, 0x01);
		hdr_lut_idx = ((level - 1) & 0xf) |
			      ((SDR2HDR_LUT0 << 4) & 0xf0);
		iris_update_ip_opt(popt, IP_OPT_MAX, SDR2HDR_LUT, hdr_lut_idx,
				   0x01);
	...
	iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_PWIL, pwil_channel_order,
			   0x01);
	...
	pwil_datapath =
		(payload[2] & (~0x4018000)) |
		pwil_datapath; //CSC_MODE 18000  POST_PQ_PROCESS_EN 4000000
	pwil_datapath1 = (payload[3] & (~0x8020)) |
			 pwil_datapath1; //UNPACK_BYPASS_EN
	...
	if (!(iris_sdr2hdr_lut2ctl & 0xFFE00000)) {
		payload =
			iris_get_ipopt_payload_data(IRIS_IP_SDR2HDR, level, 83);
		iris_sdr2hdr_lut2ctl = payload[0];
	}
	if (!(iris_sdr2hdr_lutyctl & 0xFFFF0000)) {
		payload =
			iris_get_ipopt_payload_data(IRIS_IP_SDR2HDR, level, 23);
		iris_sdr2hdr_lutyctl = payload[0];
	}
	...
	if (pqlt_cur_setting->source_switch == 1) {
		...
		if (pqlt_cur_setting->pq_setting.cmcolortempmode ==
			...
			gammalevel =
				pqlt_cur_setting->pq_setting.cmcolorgamut + 1;
	...
	//len = iris_cm_csc_set(popt, 0x01, cm_csc_enable);
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_PEAKING, peaking_csc,
				 skiplast);
...
void iris_sdr2hdr_lut2ctl_set(u32 value)
{
	...
	iris_sdr2hdr_lut2ctl =
		((value << 19) | (0x17FFFF & iris_sdr2hdr_lut2ctl));
}
...
void iris_cm_6axis_seperate_gain(u8 gain_type, u32 value)
{
	...
	IRIS_LOGI("cm gain type = %d, value = %d, len = %d", gain_type, value,
		  len);
}
...
void iris_pwm_freq_set(u32 value)
{
	...
	iris_update_bitmask_regval_nonread(&regval, false);
	len = iris_init_update_ipopt_t(popt, IP_OPT_MAX, IRIS_IP_PWM, 0xfd,
				       0xfd, 0);
...
void iris_dbc_bl_user_set(u32 value)
{
	...
	iris_update_bitmask_regval_nonread(&regval, false);
	len = iris_init_update_ipopt_t(popt, IP_OPT_MAX, IRIS_IP_DBC, 0xfd,
				       0xfd, skiplast);
...
void iris_scaler_filter_update(u8 scaler_type, u32 level)
{
	...
	len = iris_init_update_ipopt_t(popt, IP_OPT_MAX, ip, level, level, 0);
...
void iris_scaler_gamma_enable(bool lightup_en, u32 level)
{
	...
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DPP, level,
				 skiplast);
...
static void iris_dynamic_vfr(struct iris_cfg *pcfg, struct dsi_display *display)
{
	if (iris_virtual_display(display)) {
		int video_update_wo_osd =
			atomic_read(&pcfg->video_update_wo_osd);
		...
		IRIS_LOGV("video_update_wo_osd: %d",
			  atomic_read(&pcfg->video_update_wo_osd));
...
void iris_hdr_csc_complete(int step)
{
	...
	if (iris_capture_ctrl_en == true) {
		if (!iris_dynamic_power_get()) {
			skiplast = 1;
			len = iris_init_update_ipopt_t(
				popt, IP_OPT_MAX, IRIS_IP_PWIL, 0x30, 0x30, 1);
		}
			   ...
			   uint8_t opt_id, uint8_t skip_last)
{
		...
		if ((opt_id & 0xe0) == 0x40) { /*CM LUT table*/
			for (i = 0; i < pq_ipopt_val->opt_cnt; i++) {
				if (((opt_id & 0x1f) % CM_LUT_GROUP) ==
					...
					cnt = iris_init_update_ipopt_t(
						popt, len, ip, old_opt, opt_id,
						skip_last);
					return cnt;
				}
			}
	...
	if (ip == CM_LUT) {
		for (i = 0; i < pq_ipopt_val->opt_cnt; i++) {
			if ((opt_id % CM_LUT_GROUP) ==
			    (pq_ipopt_val->popt[i] % CM_LUT_GROUP)) {
				old_opt = pq_ipopt_val->popt[i];
				pq_ipopt_val->popt[i] = opt_id;
				cnt = iris_init_update_ipopt_t(popt, len, ip,
							       old_opt, opt_id,
	...
	for (i = 0; i < pq_ipopt_val->opt_cnt; i++) {
		if ((opt_id & 0xf0) == ((pq_ipopt_val->popt[i]) & 0xf0)) {
			old_opt = pq_ipopt_val->popt[i];
			pq_ipopt_val->popt[i] = opt_id;
			cnt = iris_init_update_ipopt_t(popt, len, ip, old_opt,
						       opt_id, skip_last);
			return cnt;
		}
	}
...
void iris_psf_mif_efifo_set(u8 mode, bool osd_enable)
{
	...
	iris_update_pq_opt(popt, len, path);
	iris_enable_ulps(path, is_ulps_enable);
	IRIS_LOGI("mode = %d, regval.value = 0x%x, len = %d", mode,
		  regval.value, len);
}
...
void iris_ms_pwil_dma_update(struct iris_mspwil_parameter *par)
{
	...
	if (par->frc_var_disp != -1) {
		...
		IRIS_LOGI("%s, pwil_ad_frc_info=%x", __func__,
			  pwil_ad_frc_info);
	}
	...
	if (par->frc_pt_switch_on != -1) {
		...
		iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xF1, 4,
					    pwil_datapath);
	...
	if (par->ratio_update) {
		payload = iris_get_ipopt_payload_data(IRIS_IP_PWIL, 0xF4, 2);
		temp = (payload[9] & (~0xff000000)) |
		       (par->out_fps_ratio << 24);
		iris_set_ipopt_payload_data(IRIS_IP_PWIL, 0xF4, 11, temp);
		iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_PWIL, 0xF4, 0x01);
	...
	if (par->mvc_01phase_update) {
		...
		if (pcfg->dual_setting)
			pwil_video_opt = 0xB1;
		payload = iris_get_ipopt_payload_data(IRIS_IP_PWIL,
						      pwil_video_opt, 2);
		temp = (payload[2] & (~0x80000000)) | (par->mvc_01phase << 31);
		iris_set_ipopt_payload_data(IRIS_IP_PWIL, pwil_video_opt, 4,
					    temp);
...
void iris_dtg_frame_rate_set(u32 framerate)
{
	...
	if (framerate == HIGH_FREQ)
		len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DTG, 0x1,
					 0x0);
	else
		len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DTG, 0x0,
					 0x0);
...
int iris_update_backlight(u8 pkg_mode, u32 bl_lvl)
{
	...
	char led_pwm1[3] = { MIPI_DCS_SET_DISPLAY_BRIGHTNESS, 0x0, 0x0 };
	struct dsi_cmd_desc backlight_cmd = { { 0, MIPI_DSI_DCS_LONG_WRITE, 0,
						0, 0, sizeof(led_pwm1),
						led_pwm1, 0, NULL },
					      1,
					      1 };
	...
	if (pcfg->abypss_ctrl.abypass_mode == PASS_THROUGH_MODE)
		...
		rc = iris_dsi_send_cmds(panel, cmdset.cmds, cmdset.count,
					cmdset.state);
...
void iris_pwil_sdr2hdr_resolution_set(bool enter_frc_mode)
{
	...
	if (pqlt_cur_setting->pq_setting.sdr2hdr >= SDR709_2_p3 &&
	    pqlt_cur_setting->pq_setting.sdr2hdr <= SDR709_2_2020) {
		pcfg = iris_get_cfg();
		iris_init_ipopt_ip(popt, IP_OPT_MAX);
		if (enter_frc_mode) {
			num_ratio = (1 << 28) / (pcfg->frc_setting.memc_hres *
						 pcfg->frc_setting.memc_vres);
			resolution = (pcfg->frc_setting.memc_vres & 0xFFF) |
				     (pcfg->frc_setting.memc_hres & 0xFFF)
					     << 16;
		} else {
			num_ratio = (1 << 28) / (pcfg->frc_setting.disp_vres *
						 pcfg->frc_setting.disp_hres);
			resolution = (pcfg->frc_setting.disp_vres & 0xFFF) |
				     (pcfg->frc_setting.disp_hres & 0xFFF)
					     << 16;
		}
		...
		iris_set_ipopt_payload_data(IRIS_IP_SDR2HDR, 0x93, 2,
					    resolution);
		iris_set_ipopt_payload_data(IRIS_IP_SDR2HDR, 0x93, 3,
					    num_ratio);
		len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_SDR2HDR,
					 0x93, skiplast);
...
void iris_dom_set(int mode)
{
	...
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DPORT, 0xF0,
				 skiplast);
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DPORT, 0x80,
				 skiplast);
...
static int iris_brightness_para_set(struct iris_update_ipopt *popt,
				    uint8_t skip_last, uint32_t *value)
{
	int32_t ip = 0;
	int32_t opt_id = 0;
	uint32_t *data = NULL;
				    ...
				    struct iris_cfg *pcfg)
{
	...
	rc = of_property_read_u32(np, "pxlw,min-color-temp",
				  &(pcfg->min_color_temp));
	if (rc) {
	...
	rc = of_property_read_u32(np, "pxlw,max-color-temp",
				  &(pcfg->max_color_temp));
	if (rc) {
					 ...
					 const uint8_t *pdata, int32_t item_cnt)
{
	...
	if (pdata == NULL || item_cnt == 0) {
		IRIS_LOGE("%s(), invalide input, data: %p, size: %d", pdata,
			  item_cnt);
		return -EINVAL;
	}
...
void iris_cm_setting_switch(bool dual)
{
	...
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_CM, cm_csc,
				 skiplast);
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_PWIL, pwil_csc,
				 skiplast);
