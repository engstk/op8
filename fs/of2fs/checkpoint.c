...
retry:
	...
	if (IS_ERR(page)) {
		...
		f2fs_err(sbi, "f2fs readed meta page is not uptodate!");
...
#ifdef CONFIG_F2FS_CHECK_FS
	mir_exist = f2fs_test_bit(offset,
					se->cur_valid_map_mir);
	if (unlikely(exist != mir_exist)) {
		f2fs_err(sbi, "Inconsistent with mirror "
			"when validating bitmap, blk:%u, exist:%d, mirror:%d",
			blkaddr, exist, mir_exist);
	}
#endif
	if (!exist && type == DATA_GENERIC_ENHANCE) {
		f2fs_err(sbi, "Inconsistent error blkaddr:%u, sit bitmap:%d",
			 blkaddr, exist);
		f2fs_err(sbi, "dump segment info:"
			"type:%u, vblks:%u, cp_vblks:%u",
			se->type, se->valid_blocks, se->ckpt_valid_blocks);
