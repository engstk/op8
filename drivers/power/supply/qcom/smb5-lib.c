...
/* @bsp, 2019/10/22 add new panel node for PD charging */
...
/* avoid abnormal icl set after switch fast charge to normal charge begin*/
static void op_typec_state_change_irq_handler(void);
/* @bsp, add to check enumeration status after boot */
				  ...
				  bool enable)
{
	...
	/* @bsp, 2019/10/14 Fixed ITN-975 end*/
			...
			union power_supply_propval *val)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_UNKNOWN &&
		!chg->pd_active)
		val->intval = 0;
	// gerry.ding@BSP 2019/10/22 add, wireless charge not report usb online.
	if (chg->wireless_present)
		val->intval = 0;
				    ...
				    union power_supply_propval *val)
{
	 ...
	 * voltages.
	 */
	/* @bsp 2019/11/20 use the usbin_v_chan with low voltage charger */
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	case SNK_RP_STD_BIT:
	/* @bsp add to fix huawei cable compatible issue */
...
static int get_rp_based_dcp_current(struct smb_charger *chg, int typec_mode)
{
	...
	case POWER_SUPPLY_TYPEC_SOURCE_HIGH:
		/* @bsp add to set pd charging current 2.0A when panel on */
...
/* @bsp, 2019/07/05 Battery & Charging porting */
	...
	vbus_rising = (bool)(stat & USBIN_PLUGIN_RT_STS_BIT);
	/* @bsp, 2019/12/07 Support wireless charging high voltage mode */
	if (chg->wireless_present) {
		if (chg->wireless_high_vol_mode)
...
/* @bsp, 2019/07/05 Battery & Charging porting */
	...
	if (vbus_rising) {
		...
		/* @bsp 2019/08/31 wireless charge support */
		if (!chg->wireless_present) {
			rc = smblib_request_dpdm(chg, true);
			if (rc < 0)
				smblib_err(chg, "Couldn't to enable DPDM rc=%d\n", rc);
...
/* @bsp, 2018/07/16 Battery & Charging porting */
...
/* @bsp, avoid abnormal icl set after switch to normal charge */
static void op_typec_state_change_irq_handler(void)
{
	int typec_mode;
...
/* @bsp, 2019/1/28 Solve the problem of too small current caused by FCC_STEPPER_VOTER voting */
	if (chg->wireless_present) {
				...
				const union power_supply_propval *val)
{
	...
	if (chg->pr_swap_in_progress) {
		...
		/* not to enable DRP after OTG detached */
		if (!chg->hw_detect)
			return 0;
