...
static int iris_frc_var_disp =
	0; //0: disable, 1: set before FRC, 2: set after FRC
...
static int iris_mcu_enable =
	1; // 0: disable, 1: dynamic enable , 2: always enable
...
static void iris_frc_cmd_payload_init(void)
{
	...
	if (iris_frc_cmd_payload == NULL)
		iris_frc_cmd_payload =
			vmalloc(IRIS_FRC_CMD_PAYLOAD_LENGTH * sizeof(u32));
	if (iris_frc_cmd_payload == NULL)
		IRIS_LOGE("can not vmalloc size = %d in %s",
			  IRIS_FRC_CMD_PAYLOAD_LENGTH, __func__);
}

static void iris_frc_cmd_payload_release(void)
{
	if (iris_frc_cmd_payload != NULL && iris_frc_cmd_payload_count != 0) {
		IRIS_LOGI("iris_frc_cmd_payload_count: %d",
			  iris_frc_cmd_payload_count);
		iris_ocp_write_mult_vals(iris_frc_cmd_payload_count,
					 iris_frc_cmd_payload);
...
void iris_rfb_mode_enter(void)
{
	...
	iris_set_pwil_mode(pcfg->panel, pcfg->pwil_mode, pcfg->osd_enable,
...
#define IRIS_FRC_V2_LUT_TABLE_NUM 19
	...
	struct frc_lut_config
		video_config_table[IRIS_FRC_V2_LUT_TABLE_NUM + 8] = {
			{ 1, 12 | 60 << 8, 1 | 5 << 11, 0 },
			{ 2, 12 | 90 << 8, 2 | 15 << 11, 1 },
			{ 3, 12 | 120 << 8, 1 | 10 << 11, 0 },
			{ 4, 15 | 60 << 8, 1 | 4 << 11, 1 },
	...
	if (pcfg->rx_mode == DSI_OP_CMD_MODE) {
		for (i = 0; i < IRIS_FRC_V2_LUT_TABLE_NUM + 8; i++) {
			if (inout_ratio == config_table[i].in_out_ratio) {
				frc_setting->v2_lut_index =
					config_table[i].index;
				frc_setting->v2_period_phasenum =
					config_table[i].period_phasenum;
				frc_setting->v2_phaselux_idx_max =
					config_table[i].idx_max;
				IRIS_LOGI("inout_ratio: %d", inout_ratio);
				IRIS_LOGI("select frc lut index: %d",
					  frc_setting->v2_lut_index);
				IRIS_LOGI("select frc inout ratio: %d",
					  frc_setting->v2_period_phasenum);
				return true;
			}
		}
		...
		for (i = 0; i < IRIS_FRC_V2_LUT_TABLE_NUM + 8; i++) {
			if (inout_ratio == video_config_table[i].in_out_ratio) {
				frc_setting->v2_lut_index =
					video_config_table[i].index;
				frc_setting->v2_period_phasenum =
					video_config_table[i].period_phasenum;
				frc_setting->v2_phaselux_idx_max =
					video_config_table[i].idx_max;
				IRIS_LOGI("inout_ratio: %d", inout_ratio);
				IRIS_LOGI("select frc lut index: %d",
					  frc_setting->v2_lut_index);
				IRIS_LOGI("select frc inout ratio: %d",
...
{
	struct iris_cfg *pcfg = iris_get_cfg();
	u32 val_frcc_reg5 = 0x3c000000, val_frcc_reg8 = 0x10000000,
	    val_frcc_reg16 = 0x413120c8, val_frcc_reg17 = 0x8000;
	u32 val_frcc_reg18 = 0x80000000, val_frcc_cmd_th = 0x8000,
	    val_frcc_dtg_sync = 0;
	...
	u32 mvc_01phase_vcnt, dedicate_01phase_en = 0, mvc_01phase_metagen2,
			      mvc_01phase_metagen, mvc_01phase_metarec;
	u32 mvc_metagen_thr3, mvc_metarec_thr3, three_metagen_en = 1,
						three_metagen_fi_en = 0;
	...
	fmif_vd_hstride = iris_frc_video_hstride_calc(
		frc_setting->memc_hres, frc_setting->memc_dsc_bpp, 1);
	fmif_vd_offset = fmif_vd_hstride * frc_setting->memc_vres * 8;
	fmif_dsy_hstride = (((frc_setting->memc_hres + 1) / 2) * 8 + 63) / 64;
	fmif_dsy_frm_offset =
		fmif_dsy_hstride * (frc_setting->memc_vres * 5 / 10) * 8;
	fmif_mv_hstride = ((frc_setting->memc_hres + 15) / 16 * 32 + 63) / 64;
	fmif_mv_frm_offset =
		fmif_mv_hstride * CEILING(frc_setting->memc_vres, 16) * 8;
	mv_hres = CEILING(frc_setting->memc_hres, 16);
	mv_vres = CEILING(frc_setting->memc_vres, 16);

	if (!(pcfg->dual_test & 0x2))
		frc_setting->mv_baseaddr =
			0x300000 -
			(4 + frc_setting->mv_buf_num) * fmif_mv_frm_offset;
	IRIS_LOGI("mv_baseaddr: %x, offset: %x", frc_setting->mv_baseaddr,
		  fmif_mv_frm_offset);
	if ((frc_setting->video_baseaddr + 3 * fmif_vd_offset) >
	    frc_setting->mv_baseaddr) {
		IRIS_LOGE("buffer check failed!");
		IRIS_LOGE("video_baseaddr: %x, offset: %x",
			  frc_setting->video_baseaddr, fmif_vd_offset);
		IRIS_LOGE("mv_baseaddr: %x, offset: %x",
			  frc_setting->mv_baseaddr, fmif_mv_frm_offset);
	}
	...
	IRIS_LOGI(" print video enter fps = %d, output fps = %d.",
		  frc_setting->in_fps, frc_setting->out_fps);
	...
	repeatcf_th = 252;
	fi_repeatcf_th = 252;
	input_record_thr = (input_vtotal / 8 + start_line) * hsync_freq_out /
			   hsync_freq_in;
	mvc_metarec_thr1 = (input_vtotal * 5 / 8 + start_line) *
			   hsync_freq_out / hsync_freq_in;
	mvc_metarec_thr2 = (input_vtotal * 7 / 8 + start_line) *
			   hsync_freq_out / hsync_freq_in;
	vfr_mvc_metagen_th1 = (input_vtotal / 8 + start_line) * hsync_freq_out /
			      hsync_freq_in;
	vfr_mvc_metagen_th2 = (input_vtotal * 6 / 8 + start_line) *
			      hsync_freq_out / hsync_freq_in;
	...
	if ((m / n) == 4) {
		...
		IRIS_LOGI(" print m = %d, n = %d, vtotal_frcc = %d.", m, n,
			  vtotal_frcc);
	} else if ((m / n) == 5 || (m / n) == 6) {
	...
	if (frc_setting->memc_osd && (memc_level > 2))
		memc_level = 2;
	val_frcc_reg5 += carry_th + (keep_th << 8) + (phase_map_en << 16) +
			 (memc_level << 17);
	...
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG1,
			 0x80280014 + (memc_level << 25), 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG2,
			 (0x96100043 | (iris_dynamic_sadgain_mid << 10) |
			  (iris_dynamic_sadgain_low << 16)),
			 0);
	if (pcfg->rx_mode == DSI_OP_CMD_MODE)
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG3,
				 (0x18002100 | iris_dynamic_sadcore_mid |
				  (iris_dynamic_sadcore_low << 6) |
				  (iris_dynamic_outgain_mid << 18) |
				  (iris_dynamic_outgain_low << 24)) +
					 (1 << 30),
				 0);
	else
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG3, 0x18602104,
				 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG4,
			 0x000C2020 | (iris_dynamic_outcore_mid << 6) |
				 (iris_dynamic_outcore_low << 12),
			 0);
	...
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG7,
			 0x20000000 | frc_ratio, 0);
	...
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG10, fmif_vd_offset,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG11, 0x380000, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG12,
			 fmif_dsy_frm_offset, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG13,
			 fmif_dsy_hstride + (fmif_mv_hstride << 16), 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG14,
			 frc_setting->mv_baseaddr, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG15,
			 fmif_mv_frm_offset, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG16, val_frcc_reg16,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG17, val_frcc_reg17,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG18, val_frcc_reg18,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CMD_MOD_TH, val_frcc_cmd_th,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_DTG_SYNC, val_frcc_dtg_sync,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_SWITCH_CTRL, 0x8000000a, 0);
	if (pcfg->frc_setting.short_video == 2)
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_SWFBK_CTRL,
				 0x00000080, 0);
...
#else
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_01PHASE_CTRL0,
			 mvc_01phase_vcnt | dedicate_01phase_en << 15 |
				 mvc_01phase_metagen2 << 16,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_01PHASE_CTRL1,
			 mvc_01phase_metagen | mvc_01phase_metarec << 16, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_TEOFFSET_CTRL,
			 0x00000060 | phase_teadj_en, 0);
	...
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_PERF_CTRL,
			 frc_setting->frcc_pref_ctrl, 0);
	...
	if (iris_frc_pt_switch_on) {
		u32 mntswitch_th = (iris_mtnsw_low_th & 0xf) |
				   ((iris_mtnsw_high_th & 0xf) << 4);

		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_MNTSWITCH_TH,
				 mntswitch_th, 0);
	} else
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_MNTSWITCH_TH,
				 0x00000000, 0);
	if (iris_frc_fallback_disable)
		...
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_EXT_MOTION,
				 iris_frc_mnt_level, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_EXT_TE_OFFSET, 0x00000000, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_PHASELUT_CLIP,
			 0x0000fc05 | frc_setting->v2_phaselux_idx_max << 16,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_RW_PROT_TH, 0x00000080, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_REC_META_CTRL,
			 max_fi_meta_fifo, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_METAGEN3,
			 mvc_metagen_thr3 | mvc_metarec_thr3 << 16 |
				 three_metagen_fi_en << 30 |
				 three_metagen_en << 31,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_FRCPT_SWITCH_CTRL,
			 iris_frcpt_switch_th, 0);
#endif
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_REG_SHDW, 0x00000002, 0);

	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + IMIF_MODE_CTRL,
			 0x4 + ((frc_setting->memc_vres * 3 / 4) << 16), 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + IMIF_DW_PER_LINE, fmif_vd_hstride,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + IMIF_VSIZE,
			 (frc_setting->memc_vres), 0);
	...
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + MMIF_CTRL2,
			 0x8 + (disp_vtotal << 16), 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + MMIF_PHASE1_BA,
			 frc_setting->mv_baseaddr +
				 fmif_mv_frm_offset *
					 (2 + frc_setting->mv_buf_num),
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + MMIF_PHASE0_BA,
			 frc_setting->mv_baseaddr +
				 fmif_mv_frm_offset * frc_setting->mv_buf_num,
			 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + MMIF_UPDATE, 0x00000001, 0);

	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FMIF_CTRL, 0xFF004034, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FMIF_VD_FRM_ATTRIBUTE0,
			 (0x10 << 16) + fmif_vd_hstride, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FMIF_VD_FRM_ATTRIBUTE1,
			 (0x40 << 16) + frc_setting->memc_vres, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FMIF_MV_FRM_ATTRIBUTE0,
			 (mv_hres << 16) + mv_vres, 0);
	iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FMIF_MV_FRM_ATTRIBUTE1,
			 (0x4 << 16) + fmif_mv_hstride, 0);
...
void iris_gmd_reg_set(void)
{
	...
	iris_frc_reg_add(
		IRIS_GMD_ADDR + GMD_STOP,
		frc_setting->memc_hres + (frc_setting->memc_vres << 16), 0);
...
void iris_fbd_reg_set(void)
{
	iris_frc_reg_add(IRIS_FBD_ADDR + FILMBD_RESOLUTION,
			 (frc_setting->memc_hres / 2) +
				 ((frc_setting->memc_vres / 2) << 16),
			 0);
	iris_frc_reg_add(
		IRIS_FBD_ADDR + FILMBD_WIN_STOP_SET,
		frc_setting->memc_hres + (frc_setting->memc_vres << 16), 0);
...
void iris_mvc_reg_set(void)
{
	...
	if (frc_setting->memc_hres <
	    300) { // FRC resolution 270x480 in FPGA verfication
...
void iris_fi_reg_set(void)
{
	...
	val_range_ctrl =
		vrange_top + (vrange_bot << 9) + (max_search_range << 18);
	iris_frc_reg_add(IRIS_FI_ADDR + FI_RANGE_CTRL, val_range_ctrl, 0);
	iris_frc_reg_add(IRIS_FI_ADDR + FI_CLOCK_GATING, 0xff2c0, 0);
	iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_MODE_CTRL,
			 0x00004022 | iris_frc_label << 16 |
				 (iris_frc_demo_window ? 1 : 0),
			 0); // set flim label
	if (pcfg->osd_on) {
	...
	if (iris_frc_demo_window == 1) {
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_COL_SIZE,
				 frc_setting->memc_hres << 16, 0);
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_ROW_SIZE,
				 frc_setting->memc_vres << 15, 0);
	} else if (iris_frc_demo_window == 2) {
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_COL_SIZE,
				 frc_setting->memc_hres << 16, 0);
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_ROW_SIZE,
				 frc_setting->memc_vres >> 1 |
					 frc_setting->memc_vres << 16,
				 0);
	} else if (iris_frc_demo_window == 3) {
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_COL_SIZE,
				 frc_setting->memc_hres << 15, 0);
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_ROW_SIZE,
				 frc_setting->memc_vres << 16, 0);
	} else if (iris_frc_demo_window == 4) {
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_COL_SIZE,
				 frc_setting->memc_hres >> 1 |
					 frc_setting->memc_hres << 16,
				 0);
		iris_frc_reg_add(IRIS_FI_ADDR + FI_DEMO_ROW_SIZE,
				 frc_setting->memc_vres << 16, 0);
	...
	iris_frc_reg_add(IRIS_FI_ADDR + FI_SHDW_CTRL, 0x00000001,
			 1); // set last flag
}
...
void iris_pwil_frc_ratio_and_01phase_ocp_set(void)
{
	...
	if (pcfg->rx_mode == DSI_OP_CMD_MODE)
		iris_frc_reg_add(IRIS_PWIL_ADDR + PWIL_VIDEO_CTRL2,
				 0x12061b09 | (mvc_01phase << 31), 0);
	else
		iris_frc_reg_add(IRIS_PWIL_ADDR + PWIL_VIDEO_CTRL2,
				 0x12061b0d | (mvc_01phase << 31), 0);
...
void iris_frc_mode_prepare(void)
{
	...
	if (!iris_frc_dynamic_off) {
		if (frc_setting->in_fps_configured == frc_setting->in_fps) {
			IRIS_LOGI("FRC setting has been configured: %d",
				  frc_setting->in_fps);
			return;
		}
	}
...
static uint32_t iris_pwil_disp_ctrl0(void)
{
	...
	if (pcfg->panel && pcfg->panel->cur_mode) {
		...
		if (display_mode->priv_info &&
		    display_mode->priv_info->dsc_enabled)
			disp_ctrl0 |= 0x100;
	}
	return disp_ctrl0;
}
...
static bool iris_get_tx_reserve_0(uint32_t *tx_reserve_0)
{
	...
	if (popt != NULL) {
		if (popt->cmd->msg.tx_len >= 19) { // length
			payload = iris_get_ipopt_payload_data(IRIS_IP_TX, 0x00,
							      2);
			if (payload) {
				...
				IRIS_LOGE(
					"cannot find IRIS_TX_RESERVE_0, [15]: %x, [16]: %x",
					payload[15], payload[16]);
			} else {
				IRIS_LOGE(
					"cannot find IRIS_TX_RESERVE_0, payload NULL");
			}
		} else {
			IRIS_LOGE("TX cmd length: %zu is less than 19.",
				  popt->cmd->msg.tx_len);
		}
	}
	return false;
...
static void iris_ms_pwil_ocp_update(bool force_repeat,
				    bool var_disp_in_frc_post)
{
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	IRIS_LOGI("%s, pwil_ctrl=%x, pwil_datapath: %x", __func__, pwil_ctrl,
		  pwil_datapath);
	...
	iris_frc_reg_add(IRIS_PWIL_ADDR + PWIL_PIAD_FRC_INFO, pwil_ad_frc_info,
			 0);
	if (var_disp_in_frc_post && !pcfg->osd_enable) {
		...
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_PERF_CTRL,
				 frc_setting->frcc_pref_ctrl, 0);
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_REG_SHDW, 0x00000002,
				 0);
	}
...
static void iris_dtg_te_n2m_mode(int frc)
{
	...
	if (iris_te_dly_frc != -1)
		iris_frc_reg_add(IRIS_DTG_ADDR + DTG_REG_26, iris_te_dly_frc,
				 0);
	iris_frc_reg_add(IRIS_DTG_ADDR + DTG_REG_41, m | n << 8 | 0x80000000,
			 0);
	...
	if (frc == 0) {
		if (pcfg->panel_te == 90)
			iris_set_ipopt_payload_data(IRIS_IP_DTG, 0x00, 2 + 41,
						    m | n << 8 | 0x80000000);
		else
			iris_set_ipopt_payload_data(IRIS_IP_DTG, 0x01, 2 + 41,
						    m | n << 8 | 0x80000000);
	}
}
...
static void iris_rfb_mode_prepare(void)
{
	...
	if (iris_frc_var_disp) {
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_PERF_CTRL, 0x00019f10,
				 0);
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_REG_SHDW, 0x00000002,
				 1);
		...
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_MNTSWITCH_TH,
				 0x00000000, 0);
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_FRCPT_SWITCH_CTRL,
				 iris_frcpt_switch_th & ~0x20, 0);
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_REG_SHDW, 0x00000003,
				 1);
...
static bool iris_mcu_is_stop(void)
{
	if (iris_mcu_stop_check) {
		...
		u32 mcu_info_2 =
			iris_ocp_read(IRIS_MCU_INFO_2, DSI_CMD_SET_STATE_HS);
...
static void iris_blending_timeout_set(int frc)
{
	...
	if ((framerate == HIGH_FREQ) &&
		...
		high = false;
	if (frc) {
		iris_send_ipopt_cmds(IRIS_IP_BLEND,
				     high ? 0x31 : 0x11); // blending: frc ocp
		iris_send_ipopt_cmds(IRIS_IP_BLEND,
				     high ? 0x30 : 0x10); // blending: frc
	} else {
...
void iris_mode_switch_proc(u32 mode)
{
	...
	if (mode == IRIS_MODE_FRC_PREPARE) {
		if (pcfg->dual_setting) {
			iris_frc_var_disp =
				iris_frc_var_disp_dual; // enable vfr in dual case by default
			iris_frc_pt_switch_on =
				0; // disable frc+pt in dual case
		} else {
		...
		if (pcfg->osd_enable)
			iris_psf_mif_efifo_set(pcfg->pwil_mode,
					       pcfg->osd_enable);
		if (iris_mcu_enable)
			iris_mcu_sw_reset(0);
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_CTRL_REG8,
				 iris_val_frcc_reg8, 0);
		iris_frc_reg_add(IRIS_FRC_MIF_ADDR + FRCC_REG_SHDW, 0x00000002,
				 0);
		...
		if (iris_mcu_enable == 1) {
			if (pcfg->rx_mode == 1) { // command mode
				if (iris_mcu_is_stop())
					...
					IRIS_LOGI(
						"iris mcu not in stop, can't reset mcu");
			} else {
				iris_mcu_sw_reset(1);
			}
		}
...
static int iris_get_mode(void)
{
	...
	int switch_mode;
	int pwil_mode = -1;
	u32 pwil_status = iris_ocp_read(IRIS_PWIL_ADDR + PWIL_STATUS,
					DSI_CMD_SET_STATE_HS);
...
int32_t iris_fi_osd_protect_window(u32 Top_left_position,
				   u32 bottom_right_position,
				   u32 osd_window_ctrl, u32 Enable,
				   u32 DynCompensate)
{
	int32_t rc = 0;
	struct iris_cfg *pcfg = iris_get_cfg();
	...
	if ((temp0 > (frc_setting->disp_hres - 1)) ||
	    (temp1 > (frc_setting->disp_vres - 1)) ||
	    (temp2 > (frc_setting->disp_hres - 1)) ||
	    (temp3 > (frc_setting->disp_vres - 1))) {
	...
	if ((temp0 > temp2) || (temp1 > temp3)) {
		IRIS_LOGE(
			"OSD protect window begin point position is bigger than end point position.");
		return -EINVAL;
	}

	if (!DynCompensate)
		pcfg->frc_setting.iris_osd_win_dynCompensate &=
			(~(1 << osd_window_ctrl));
	else
		pcfg->frc_setting.iris_osd_win_dynCompensate |=
			(1 << osd_window_ctrl);
	...
	if (!Enable) {
		pcfg->frc_setting.iris_osd_window_ctrl &=
			(~(7 << (osd_window_ctrl * 3)));
	} else {
		pcfg->frc_setting.iris_osd_window_ctrl |=
			(1 << (osd_window_ctrl * 3));
...
void iris_fi_demo_window_cal(void)
{
	...
	IRIS_LOGD(
		"%s, input osd protect area: osd0_tl = 0x%x, osd0_br = 0x%x, osd1_tl = 0x%x, osd1_br = 0x%x, osd2_tl = 0x%x, osd2_br = 0x%x",
		__func__, pcfg->frc_setting.iris_osd0_tl,
		pcfg->frc_setting.iris_osd0_br, pcfg->frc_setting.iris_osd1_tl,
		pcfg->frc_setting.iris_osd1_br, pcfg->frc_setting.iris_osd2_tl,
		pcfg->frc_setting.iris_osd2_br);
	IRIS_LOGD(
		"%s, input osd protect area: osd3_tl = 0x%x, osd3_br = 0x%x, osd4_tl = 0x%x, osd4_br = 0x%x",
		__func__, pcfg->frc_setting.iris_osd3_tl,
		pcfg->frc_setting.iris_osd3_br, pcfg->frc_setting.iris_osd4_tl,
		pcfg->frc_setting.iris_osd4_br);
	IRIS_LOGD(
		"%s, real osd protect area: osd0_tl = 0x%x, osd0_br = 0x%x, osd1_tl = 0x%x, osd1_br = 0x%x, osd2_tl = 0x%x, osd2_br = 0x%x",
		__func__, osd0_tl, osd0_br, osd1_tl, osd1_br, osd2_tl, osd2_br);
	IRIS_LOGD(
		"%s, real osd protect area: osd3_tl = 0x%x, osd3_br = 0x%x, osd4_tl = 0x%x, osd4_br = 0x%x",
		__func__, osd3_tl, osd3_br, osd4_tl, osd4_br);
	IRIS_LOGD(
		"%s, osd window ctrl:  = 0x%x, iris_osd_win_dynCompensate = 0x%x",
		__func__, pcfg->frc_setting.iris_osd_window_ctrl,
		pcfg->frc_setting.iris_osd_win_dynCompensate);
	...
	if (pcfg->frc_setting.iris_osd_window_ctrl & 0x1249) { //osd protection
		...
		iris_frc_reg_add(IRIS_FI_ADDR + FI_OSD_WINDOW_CTRL,
				 pcfg->frc_setting.iris_osd_window_ctrl, 0);
		//iris_frc_reg_add(IRIS_PROXY_MB0, pcfg->frc_setting.iris_osd_win_dynCompensate, 0);
	} else {
		iris_frc_reg_add(IRIS_FI_ADDR + FI_OSD_WINDOW_CTRL, 0x00000000,
				 0);
	}
}
int iris_mode_switch_update(void)
{
	...
	else if (mode == IRIS_MODE_PT_PREPARE ||
		 mode == IRIS_MODE_PTLOW_PREPARE)
...
void iris_set_out_frame_rate(u32 framerate)
{
	...
	IRIS_LOGI("%s, default out framerate: %u, set framerate: %u", __func__,
...
void iris_set_ap_te(u8 ap_te)
{
	...
	if (ap_te > pcfg->panel_te) {
		IRIS_LOGI("%s, ap_te[%d] > panel_te[%d]", __func__, ap_te,
			  pcfg->panel_te);
		ap_te = pcfg->panel_te;
	}
