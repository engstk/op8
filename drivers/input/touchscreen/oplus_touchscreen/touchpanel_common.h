...
struct panel_info {
	...
	int project_num;
	struct firmware_headfile
		firmware_headfile; /*firmware headfile for noflash ic*/
...
struct spurious_fp_touch {
	...
	fp_touch_state
		fp_touch_st; /*provide result to fingerprint of touch status data*/
	struct task_struct
		*thread; /*tread use for fingerprint susprious touch check*/
};
...
struct touchpanel_data {
	...
	struct wakeup_source *
		ws; /* Qualcomm KBA-211220012446, To make power manager stay awake*/
	...
	struct completion
		pm_complete; /*completion for control suspend and resume flow*/
	...
	struct resolution_info
		resolution_info; /*resolution of touchpanel && LCD*/
	struct gesture_info gesture; /*gesture related info*/
	struct touch_major_limit
		touch_major_limit; /*used for control touch major reporting area*/
	struct fp_underscreen_info
		fp_info; /*tp info used for underscreen fingerprint*/
	struct pen_config pen_config;

	struct work_struct speed_up_work; /*using for speedup resume*/
	struct workqueue_struct
		*speedup_resume_wq; /*using for touchpanel speedup resume wq*/
	struct work_struct
		lcd_trigger_load_tp_fw_work; /*trigger load tp fw by lcd driver after lcd reset*/
	struct workqueue_struct *
		lcd_trigger_load_tp_fw_wq; /*trigger laod tp fw by lcd driver after lcd reset*/
	...
	struct proc_dir_entry
		*prEntry_tp; /*struct proc_dir_entry of "/proc/touchpanel"*/
	struct proc_dir_entry *
		prEntry_debug_tp; /*struct proc_dir_entry of "/proc/touchpanel/debug_info"*/
	struct debug_info_proc_operations *debug_info_ops; /*debug info data*/
...
struct oplus_touchpanel_operations {
	...
	fw_check_state (*fw_check)(
		void *chip_data, struct resolution_info *resolution_info,
		struct panel_info *panel_data); /*return < 0 :failed; 0 sucess*/
	fw_update_state (*fw_update)(
	...
	u8 (*trigger_reason)(
		...
		int is_suspended); /*clear innterrupt reg && detect irq trigger reason*/
	...
	void (*resume_prepare)(
		void *chip_data); /*using for operation before resume flow,
                                                                                                eg:incell 3320 need to disable gesture to release inter pins for lcd resume*/
	fp_touch_state (*spurious_fp_check)(
	...
	void (*write_ps_status)(
		void *chip_data,
		int ps_status); /*when detect iron plate, if ps is near ,enter iron plate mode;if ps is far, can not enter; exit esd mode when ps is far*/
	...
	void (*health_report_v2)(
		void *chip_data,
		struct monitor_data_v2 *mon_data_v2); /*data logger get*/
	...
	void (*screenon_fingerprint_info)(
		void *chip_data,
		struct fp_underscreen_info *
			fp_tpinfo); /*get gesture info of fingerprint underscreen when screen on*/
	...
	int (*get_touch_points_auto)(
		void *chip_data, struct point_info *points, int max_num,
		struct resolution_info
			*resolution_info); /*return point bit-map auto*/

	int (*get_gesture_info_auto)(
		void *chip_data, struct gesture_info *gesture,
		struct resolution_info
			*resolution_info); /*return 0:success;other:failed*/

	void (*screenon_fingerprint_info_auto)(
		void *chip_data, struct fp_underscreen_info *fp_tpinfo,
		struct resolution_info *
...
struct debug_info_proc_operations {
	...
	void (*delta_snr_read)(struct seq_file *s, void *chip_data,
			       uint32_t count);
};
...
void input_report_key_oplus(struct input_dev *dev, unsigned int code,
			    int value);
...
__attribute__((weak)) int
oplus_ofp_touchpanel_event_irq_call(struct fp_underscreen_info *fp_tpinfo)
{
	return 0;
}
__attribute__((weak)) int
opticalfp_irq_handler_uff(struct fp_underscreen_info *fp_tpinfo)
{
	return 0;
}
