...
*/
enum tfa_error tfa_load_cnt_v6(void *cnt, int length)
{
	...
	/* check CRC */
	if (tfaContCrcCheckContainer_v6(cntbuf)) {
		pr_err("CRC error\n");
		return tfa_error_container;
	}
 ...
 */
void tfaContShowHeader_v6(nxpTfaHeader_t *hdr)
{
	char _id[2];
 ...
 */
nxpTfaDeviceList_t *tfaContGetDevList_v6(nxpTfaContainer_t *cont, int dev_idx)
{
 ...
 */
nxpTfaProfileList_t *tfaContGetDevProfList_v6(nxpTfaContainer_t *cont,
...
int tfa_cnt_get_dev_nprof(struct tfa_device *tfa)
{
	...
	dev = tfaContGetDevList_v6(tfa->cnt, tfa->dev_idx);
	if (dev) {
 ...
 */
nxpTfaLiveDataList_t *tfaContGetDevLiveDataList_v6(nxpTfaContainer_t *cont,
 ...
 */
int tfacont_get_max_vstep_v6(struct tfa_device *tfa, int prof_idx)
{
	nxpTfaVolumeStep2File_t *vp;
	struct nxpTfaVolumeStepMax2File *vp3;
	int vstep_count = 0;
	vp = (nxpTfaVolumeStep2File_t *)tfacont_getfiledata_v6(tfa, prof_idx,
							       volstepHdr);
	if (vp == NULL)
		return 0;
	...
	if (tfa->tfa_family == 2) {
		...
		vp3 = (struct nxpTfaVolumeStepMax2File *)tfacont_getfiledata_v6(
			tfa, prof_idx, volstepHdr);
		if (vp3) {
 ...
 * tree. There can only be one type of file within profile or device.
  */
nxpTfaFileDsc_t *tfacont_getfiledata_v6(struct tfa_device *tfa, int prof_idx,
					enum nxpTfaHeaderType type)
{
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscFile) {
			file = (nxpTfaFileDsc_t *)(dev->list[i].offset +
						   (uint8_t *)tfa->cnt);
			if (file != NULL) {
	 ...
	 */
	prof = tfaContGetDevProfList_v6(tfa->cnt, tfa->dev_idx, prof_idx);
	if (prof == NULL) {
	...
	for (i = 0; i < prof->length; i++) {
		if (prof->list[i].type == dscFile) {
			file = (nxpTfaFileDsc_t *)(prof->list[i].offset +
						   (uint8_t *)tfa->cnt);
			if (file != NULL) {
				hdr = (nxpTfaHeader_t *)file->data;
				if (hdr != NULL) {
					    ...
					    int vstep)
{
	...
	if (vstep < vp->vsteps) {
		...
		vol = (unsigned short)(-2 *
				       float_to_int(
					       *((uint32_t *)&vp->vstep[vstep]
							 .attenuation)));
		if (vol > 255) /* restricted to 8 bits */
			vol = 255;

		err = tfa98xx_set_volume_level_v6(tfa, vol);
		if (err != Tfa98xx_Error_Ok)
			return err;

		err = tfa98xx_dsp_write_preset_v6_v6(
			tfa, sizeof(vp->vstep[0].preset),
			vp->vstep[vstep].preset);
		if (err != Tfa98xx_Error_Ok)
			return err;
		err = tfa_cont_write_filterbank_v6(tfa,
...
static enum Tfa98xx_Error tfaContWriteVstepMax2_One(
	struct tfa_device *tfa, struct nxpTfaVolumeStepMessageInfo *new_msg,
	struct nxpTfaVolumeStepMessageInfo *old_msg, int enable_partial_update)
{
	...
	if (use_partial_coeff) {
		err = dsp_partial_coefficients(tfa, old_msg->ParameterData,
						...
						int vstep_msg_idx)
{
	...
	for (i = 0; i < regInfo->NrOfRegisters * 2; i++) {
		...
		err = tfaRunWriteBitfield_v6(tfa, bitF);
		if (err != Tfa98xx_Error_Ok)
			return err;
	}
...
enum Tfa98xx_Error tfaContWriteDrcFile_v6(struct tfa_device *tfa, int size,
					  uint8_t data[])
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	uint8_t *msg = NULL;
 ...
 */
enum Tfa98xx_Error tfaContWriteFile_v6(struct tfa_device *tfa,
				       ...
				       int vstep_msg_idx)
{
	...
	if (tfa->verbose) {
		tfaContShowHeader_v6(hdr);
	}

	type = (nxpTfaHeaderType_t)hdr->id;

	if ((type == msgHdr) ||
	    ((type == volstepHdr) && (tfa->tfa_family == 2))) {
		...
		if ((subversion > 0) && (((hdr->customer[0]) == 'A') &&
			...
			if (tfa->is_probus_device) {
				...
				for (i = 0; i < 3; i++) {
					if (tfa->fw_itf_ver[i] !=
					    hdr->customer[i +
							  4]) //+4 to skip "?PIV" string part in the .msg file.
					{
						ERRORMSG(
							"Error: tfaContWriteFile: Expected FW API version = %d.%d.%d, Msg File version: %d.%d.%d \n",
							tfa->fw_itf_ver[0],
							tfa->fw_itf_ver[1],
							tfa->fw_itf_ver[2],
							hdr->customer[4],
	...
	case msgHdr: /* generic DSP message */
		...
		err = dsp_msg(tfa, size,
			      (const char *)((nxpTfaMsgFile_t *)hdr)->data);
		break;
	case volstepHdr:
		if (tfa->tfa_family == 2) {
			err = tfaContWriteVstepMax2(
				tfa, (nxpTfaVolumeStepMax2File_t *)hdr,
				vstep_idx, vstep_msg_idx);
		} else {
			err = tfaContWriteVstep(
				tfa, (nxpTfaVolumeStep2File_t *)hdr, vstep_idx);
		}
		break;
	case speakerHdr:
		if (tfa->tfa_family == 2) {
			...
			err = dsp_msg(
				tfa, size,
				(const char *)(((nxpTfaSpeakerFile_t *)hdr)
						       ->data +
					       (sizeof(struct nxpTfaFWVer))));
		} else {
			size = hdr->size - sizeof(nxpTfaSpeakerFile_t);
			err = tfa98xx_dsp_write_speaker_parameters_v6(
				tfa, size,
				(const unsigned char *)((nxpTfaSpeakerFile_t *)
								hdr)
					->data);
		}
		break;
	case presetHdr:
		...
		break;
	case equalizerHdr:
		err = tfa_cont_write_filterbank_v6(
			tfa, ((nxpTfaEqualizerFile_t *)hdr)->filter);
		break;
	case patchHdr:
		size = hdr->size -
		       sizeof(nxpTfaPatch_t); // size is total length
		err = tfa_dsp_patch_v6(
			tfa, size,
			(const unsigned char *)((nxpTfaPatch_t *)hdr)->data);
		break;
	case configHdr:
		size = hdr->size - sizeof(nxpTfaConfig_t);
		err = tfa98xx_dsp_write_config_v6(
			tfa, size,
	...
	case drcHdr:
		if (hdr->version[0] == NXPTFA_DR3_VERSION) {
			...
			err = tfaContWriteDrcFile_v6(
				tfa, size, ((nxpTfaDrc2_t *)hdr)->data);
		} else {
			/*
			...
			err = tfa98xx_dsp_write_drc_v6(
				tfa, size,
				((const unsigned char *)((nxpTfaDrc_t *)hdr)
					 ->data +
				 381));
		}
		break;
 ...
 */
static nxpTfaDescPtr_t *tfa_cnt_get_dsc(nxpTfaContainer_t *cnt,
					nxpTfaDescriptorType_t type,
					int dev_idx)
{
	nxpTfaDeviceList_t *dev = tfaContDevice_v6(cnt, dev_idx);
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == (uint32_t)type) {
			_this = (nxpTfaDescPtr_t *)(dev->list[i].offset +
						    (uint8_t *)cnt);
			return _this;
		}
	}
 ...
 */
int tfa_cnt_get_devid_v6(nxpTfaContainer_t *cnt, int dev_idx)
{
 ...
 */
enum Tfa98xx_Error tfaContGetSlave_v6(struct tfa_device *tfa,
				      uint8_t *slave_addr)
{
	...
	if (tfa->cnt != NULL) {
		dev = tfaContDevice_v6(tfa->cnt, tfa->dev_idx);
	}
...
int tfa_cont_get_idx(struct tfa_device *tfa)
{
	...
	for (i = 0; i < tfa->cnt->ndev; i++) {
		...
		if (dev && (dev->dev == tfa->slave_address))
			break;
	}
	if (i == tfa->cnt->ndev)
		return -1;
 ...
 * write a bit field
 */
enum Tfa98xx_Error tfaRunWriteBitfield_v6(struct tfa_device *tfa,
					  nxpTfaBitfield_t bf)
{
	...
	if (tfa->verbose)
		pr_debug("bitfield: %s=0x%x (0x%x[%d..%d]=0x%x)\n",
			 ...
			 bfUni.Enum.address, bfUni.Enum.pos,
			 bfUni.Enum.pos + bfUni.Enum.len, value);
#endif
	error = tfa_set_bf_v6(tfa, bfUni.field, value);
 ...
 * read a bit field
 */
enum Tfa98xx_Error tfaRunReadBitfield_v6(struct tfa_device *tfa,
					 nxpTfaBitfield_t *bf)
{
	enum Tfa98xx_Error error;
	union {
 ...
 */
static enum Tfa98xx_Error tfaRunWriteDspMem(struct tfa_device *tfa,
					    nxpTfaDspMem_t *cfmem)
{
	...
	for (i = 0; i < cfmem->size; i++) {
		...
		error = mem_write(tfa, cfmem->address++, cfmem->words[i],
				  cfmem->type);
		if (error)
			return error;
	}
 ...
 */
static enum Tfa98xx_Error tfaRunWriteFilter(struct tfa_device *tfa,
					    nxpTfaContBiquad_t *bq)
{
	...
	for (i = 0; i < runs; i++) {
		...
		dmem = tfa98xx_filter_mem_v6(tfa, bq->aa.index, &address,
					     channel);
		...
		if (tfa->tfa_family == 2) {
			error = tfa_dsp_cmd_id_write_v6(tfa, MODULE_FRAMEWORK,
							FW_PAR_ID_SET_MEMORY,
							sizeof(data), data);
		} else {
			error = tfa_dsp_cmd_id_write_v6(tfa, MODULE_FRAMEWORK,
							4 /* param */,
	...
	if (tfa->verbose) {
		if (bq->aa.index == 13) {
			...
			pr_debug("=%d,%.0f,%.1f,%.1f \n", bq->aa.type,
				 bq->aa.cutOffFreq, bq->aa.rippleDb,
				 bq->aa.rolloff);
		} else {
 ...
 */
static enum Tfa98xx_Error tfaRunWriteRegister(struct tfa_device *tfa,
...
enum Tfa98xx_Error tfaContWriteRegsDev_v6_v6(struct tfa_device *tfa)
{
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dscBitfield) {
			bitF = (nxpTfaBitfield_t *)(dev->list[i].offset +
						    (uint8_t *)tfa->cnt);
			err = tfaRunWriteBitfield_v6(tfa, *bitF);
		}
		if (dev->list[i].type == dscRegister) {
			err = tfaRunWriteRegister(
				tfa, (nxpTfaRegpatch_t *)(dev->list[i].offset +
							  (char *)tfa->cnt));
		}
...
enum Tfa98xx_Error tfaContWriteRegsProf_v6(struct tfa_device *tfa, int prof_idx)
{
	nxpTfaProfileList_t *prof =
		tfaContGetDevProfList_v6(tfa->cnt, tfa->dev_idx, prof_idx);
	nxpTfaBitfield_t *bitf;
	unsigned int i;
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	...
	if (tfa->verbose)
		pr_info("----- profile: %s (%d) -----\n",
			tfaContGetString_v6(tfa->cnt, &prof->name), prof_idx);
	...
	for (i = 0; i < prof->length; i++) {
		...
		if (prof->list[i].type == dscBitfield) {
			bitf = (nxpTfaBitfield_t *)(prof->list[i].offset +
						    (uint8_t *)tfa->cnt);
			err = tfaRunWriteBitfield_v6(tfa, *bitf);
		}
		if (prof->list[i].type == dscRegister) {
			err = tfaRunWriteRegister(
				tfa, (nxpTfaRegpatch_t *)(prof->list[i].offset +
							  (char *)tfa->cnt));
		}
		if (err)
			break;
...
enum Tfa98xx_Error tfaContWritePatch_v6(struct tfa_device *tfa)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	nxpTfaDeviceList_t *dev = tfaContDevice_v6(tfa->cnt, tfa->dev_idx);
	nxpTfaFileDsc_t *file;
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscPatch) {
			...
			if (tfa->verbose)
				tfaContShowHeader_v6(&patchfile->hdr);
 ...
 */
static void create_dsp_buffer_msg(struct tfa_device *tfa, nxpTfaMsg_t *msg,
...
enum Tfa98xx_Error tfaContWriteFile_v6s(struct tfa_device *tfa)
{
	nxpTfaDeviceList_t *dev = tfaContDevice_v6(tfa->cnt, tfa->dev_idx);
	nxpTfaFileDsc_t *file;
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	...
	for (i = 0; i < dev->length; i++) {
		if (dev->list[i].type == dscFile) {
			file = (nxpTfaFileDsc_t *)(dev->list[i].offset +
						   (uint8_t *)tfa->cnt);
			if (tfaContWriteFile_v6(tfa, file, 0,
		...
		if (dev->list[i].type == dscSetInputSelect ||
			...
			create_dsp_buffer_msg(
				tfa,
				(nxpTfaMsg_t *)(dev->list[i].offset +
						(char *)tfa->cnt),
				buffer, &size);
			if (tfa->verbose) {
				pr_debug("command: %s=0x%02x%02x%02x \n",
					 tfaContGetCommandString_v6(
						 dev->list[i].type),
		...
		if (dev->list[i].type == dscCfMem) {
			err = tfaRunWriteDspMem(
				tfa, (nxpTfaDspMem_t *)(dev->list[i].offset +
							(uint8_t *)tfa->cnt));
		}
 ...
 */
enum Tfa98xx_Error tfaContWriteFile_v6sProf_v6(struct tfa_device *tfa,
					       int prof_idx, int vstep_idx)
{
	...
	for (i = 0; i < prof->length; i++) {
		...
		case dscFile:
			file = (nxpTfaFileDsc_t *)(prof->list[i].offset +
						   (uint8_t *)tfa->cnt);
			err = tfaContWriteFile_v6(tfa, file, vstep_idx,
						  TFA_MAX_VSTEP_MSG_MARKER);
			break;
		case dscPatch:
			file = (nxpTfaFileDsc_t *)(prof->list[i].offset +
		...
		case dscCfMem:
			err = tfaRunWriteDspMem(
				tfa, (nxpTfaDspMem_t *)(prof->list[i].offset +
							(uint8_t *)tfa->cnt));
			break;
		...
		case dscSetVddpConfig:
			create_dsp_buffer_msg(
				tfa,
				(nxpTfaMsg_t *)(prof->list[i].offset +
						(uint8_t *)tfa->cnt),
				buffer, &size);
			if (tfa->verbose) {
				pr_debug("command: %s=0x%02x%02x%02x \n",
...
static enum Tfa98xx_Error tfaContWriteItem(struct tfa_device *tfa,
					   nxpTfaDescPtr_t *dsc)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	nxpTfaRegpatch_t *reg;
	...
	case dscString: // ascii: zero terminated string
		pr_debug(";string: %s\n", tfaContGetString_v6(tfa->cnt, dsc));
		break;
	...
	case dscMode:
		...
		if (cas->value == Tfa98xx_Mode_RCV)
			tfa98xx_select_mode_v6(tfa, Tfa98xx_Mode_RCV);
		else
			tfa98xx_select_mode_v6(tfa, Tfa98xx_Mode_Normal);
		break;
	case dscCfMem:
		err = tfaRunWriteDspMem(
			tfa,
			(nxpTfaDspMem_t *)(dsc->offset + (uint8_t *)tfa->cnt));
		break;
	case dscBitfield:
		bitf = (nxpTfaBitfield_t *)(dsc->offset + (uint8_t *)tfa->cnt);
		return tfaRunWriteBitfield_v6(tfa, *bitf);
		break;
	case dscFilter:
		return tfaRunWriteFilter(
			tfa, (nxpTfaContBiquad_t *)(dsc->offset +
						    (uint8_t *)tfa->cnt));
		break;
	}
...
unsigned int tfa98xx_get_profile_sr_v6(struct tfa_device *tfa,
				       unsigned int prof_idx)
{
	...
	if (tfa->verbose)
		pr_info("%s - profile fs: 0x%x = %dHz (%d - %d)\n",
			...
			tfa98xx_sr_from_field(fs_profile), tfa->dev_idx,
			prof_idx);
...
get_sample_rate_info(struct tfa_device *tfa, nxpTfaProfileList_t *prof,
		     nxpTfaProfileList_t *previous_prof,
		     int fs_previous_profile)
{
	...
	for (i = 0; i < previous_prof->length; i++) {
		...
		if (i < previous_prof->length) {
			if (previous_prof->list[i].type == dscBitfield) {
				bitf = (nxpTfaBitfield_t *)(previous_prof
								    ->list[i]
								    .offset +
							    (uint8_t *)tfa->cnt);
				if (bitf->field == TFA_FAM(tfa, AUDFS)) {
	...
	*/
	...
	if (fs_next_profile != fs_default_profile) {
		 ...
		 */
		err = tfa98xx_dsp_write_tables_v6(tfa, fs_next_profile);
		 ...
		 */
		err = tfa98xx_dsp_write_tables_v6(tfa, fs_default_profile);
	}
 ...
 *
 */
enum Tfa98xx_Error tfaContWriteProfile_v6(struct tfa_device *tfa, int prof_idx,
					  int vstep_idx)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	nxpTfaProfileList_t *prof =
		tfaContGetDevProfList_v6(tfa->cnt, tfa->dev_idx, prof_idx);
	nxpTfaProfileList_t *previous_prof = tfaContGetDevProfList_v6(
	...
	if (tfa->verbose) {
		tfa98xx_trace_printk(
			"device:%s profile:%s vstep:%d\n",
			tfaContDevice_v6Name_v6(tfa->cnt, tfa->dev_idx),
			tfaContProfileName_v6(tfa->cnt, tfa->dev_idx, prof_idx),
			vstep_idx);
	}
	...
	if (prof->group == previous_prof->group && prof->group != 0) {
		if (tfa->verbose) {
			pr_debug(
				...
				tfaContGetString_v6(tfa->cnt, &prof->name),
				tfaContGetString_v6(tfa->cnt,
						    &previous_prof->name));
		}
	} else {
		/* mute */
		err = tfaRunMute_v6(tfa);
		if (err)
			return err;
	...
	if (tfa->verbose) {
		pr_info("---------- default settings profile: %s (%d) ---------- \n",
			tfaContGetString_v6(tfa->cnt, &previous_prof->name),
	...
	if (tfa->ext_dsp != 0) {
		if (tfa->tfa_family == 2) {
			for (i = 0; i < previous_prof->length; i++) {
				...
				if (i < previous_prof->length) {
					if (previous_prof->list[i].type ==
						    dscFile ||
					    previous_prof->list[i].type ==
						    dscPatch) {
						...
						file = (nxpTfaFileDsc_t
								*)(previous_prof
									   ->list[i]
									   .offset +
								   (uint8_t *)tfa
									   ->cnt);
						err = tfaContWriteFile_v6(
							tfa, file, vstep_idx,
							TFA_MAX_VSTEP_MSG_MARKER);
					}
				}
 ...
 *
 */
enum Tfa98xx_Error tfaContWriteFilesVstep_v6(struct tfa_device *tfa,
					     int prof_idx, int vstep_idx)
{
	nxpTfaProfileList_t *prof =
		tfaContGetDevProfList_v6(tfa->cnt, tfa->dev_idx, prof_idx);
...
char *tfaContGetString_v6(nxpTfaContainer_t *cnt, nxpTfaDescPtr_t *dsc)
{
...
char *tfaContGetCommandString_v6(uint32_t type)
{
 ...
 *  return device name
 */
char *tfaContDevice_v6Name_v6(nxpTfaContainer_t *cnt, int dev_idx)
{
	nxpTfaDeviceList_t *dev;

	dev = tfaContDevice_v6(cnt, dev_idx);
	if (dev == NULL)
		return "!ERROR!";

	return tfaContGetString_v6(cnt, &dev->name);
}
 ...
 *
 */
int tfa_cnt_get_app_name_v6(struct tfa_device *tfa, char *name)
{
	unsigned int i;
	int len = 0;
 ...
 */
int tfaContGetCalProfile_v6(struct tfa_device *tfa)
{
	int prof, cal_idx = -2;
 ...
 */
int tfaContIsTapProfile_v6(struct tfa_device *tfa, int prof_idx)
{
	if ((tfa->dev_idx < 0) || (tfa->dev_idx >= tfa->cnt->ndev))
		return -1;

	/* Check if next profile is tap profile */
	if (strstr(tfaContProfileName_v6(tfa->cnt, tfa->dev_idx, prof_idx),
		   ".tap") != NULL) {
 ...
 *  return profile name
 */
char *tfaContProfileName_v6(nxpTfaContainer_t *cnt, int dev_idx, int prof_idx)
{
	nxpTfaProfileList_t *prof = NULL;

	/* the Nth profiles for this device */
	prof = tfaContGetDevProfList_v6(cnt, dev_idx, prof_idx);

	/* If the index is out of bound */
	if (prof == NULL)
		return "NONE";

	return tfaContGetString_v6(cnt, &prof->name);
}

/*
 * return 1st profile list
 */
nxpTfaProfileList_t *tfaContGet1stProfList_v6(nxpTfaContainer_t *cont)
{
	nxpTfaProfileList_t *prof;
 ...
 */
nxpTfaLiveDataList_t *tfaContGet1stLiveDataList_v6(nxpTfaContainer_t *cont)
{
 ...
 */
nxpTfaDeviceList_t *tfaContDevice_v6(nxpTfaContainer_t *cnt, int dev_idx)
{
	return tfaContGetDevList_v6(cnt, dev_idx);
}
 ...
 */
nxpTfaProfileList_t *tfaContNextProfile_v6(nxpTfaProfileList_t *prof)
{
 ...
 */
nxpTfaLiveDataList_t *tfaContNextLiveData_v6(nxpTfaLiveDataList_t *livedata)
{
	nxpTfaLiveDataList_t *nextlivedata =
		(nxpTfaLiveDataList_t *)((char *)livedata +
					 (livedata->length * 4) +
 ...
 */
int tfaContCrcCheckContainer_v6(nxpTfaContainer_t *cont)
{
	uint8_t *base;
	size_t size;
	uint32_t crc;
...
/* wrapper function */
void get_hw_features_from_cnt_v6(struct tfa_device *tfa,
				 int *hw_feature_register)
{
	int sw_feature_register[2];
...
/* wrapper function */
void get_sw_features_from_cnt_v6(struct tfa_device *tfa,
				 int sw_feature_register[2])
{
	int hw_feature_register;
	get_all_features_from_cnt(tfa, &hw_feature_register,
...
enum Tfa98xx_Error tfa_set_filters_v6(struct tfa_device *tfa, int prof_idx)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	nxpTfaProfileList_t *prof =
		tfaContGetDevProfList_v6(tfa->cnt, tfa->dev_idx, prof_idx);
