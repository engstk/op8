...
static int __init oplus_project_init(void)
{
	...
	p_entry = proc_create_data("prjName", S_IRUGO, oplus_info,
				   &project_info_fops,
				   UINT2Ptr(PROJECT_VERSION));
	if (!p_entry)
		goto error_init;
	...
	p_entry = proc_create_data("oplusBootmode", S_IRUGO, oplus_info,
				   &project_info_fops,
				   UINT2Ptr(OPLUS_BOOTMODE));
	if (!p_entry)
		goto error_init;
