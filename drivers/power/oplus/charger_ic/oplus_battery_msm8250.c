...
module_param_named(debug_mask, __debug_mask, int, 0600);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
void __attribute__((weak)) switch_usb_state(int usb_state)
{
	return;
}
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
bool __attribute__((weak)) boot_with_console(void)
{
	return false;
}
int __attribute__((weak)) oplus_pdo_select(int vbus_mv, int ibus_ma)
{
	return 0;
}
#endif

#define smblib_err(chg, fmt, ...) pr_err("%s: %s: " fmt, chg->name, __func__, ##__VA_ARGS__)

#ifdef OPLUS_FEATURE_CHG_BASIC
#define smblib_dbg(chg, reason, fmt, ...)                                                                              \
	do {                                                                                                           \
		if (*chg->debug_mask & (reason))                                                                       \
			pr_debug("%s: %s: " fmt, chg->name, __func__, ##__VA_ARGS__);                                  \
		else                                                                                                   \
			pr_debug("%s: %s: " fmt, chg->name, __func__, ##__VA_ARGS__);                                  \
	} while (0)
#else
#define smblib_dbg(chg, reason, fmt, ...)                                                                              \
	do {                                                                                                           \
		if (*chg->debug_mask & (reason))                                                                       \
			pr_info("%s: %s: " fmt, chg->name, __func__, ##__VA_ARGS__);                                   \
		else                                                                                                   \
			pr_debug("%s: %s: " fmt, chg->name, __func__, ##__VA_ARGS__);                                  \
	} while (0)
#endif
...
static void oplus_wired_conn_irq_register(struct oplus_chg_chip *chip)
{
	...
	ret = devm_request_threaded_irq(chip->dev, chg->wired_conn_irq, NULL, oplus_wired_conn_change_handler,
					IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT, "wired_conn-change",
					chip);
	if (ret < 0) {
...
void oplus_wireless_set_otg_en_val(int value)
{
	...
	if (IS_ERR_OR_NULL(chg->otg_en_pinctrl) || IS_ERR_OR_NULL(chg->otg_en_active) ||
	    IS_ERR_OR_NULL(chg->otg_en_sleep) || IS_ERR_OR_NULL(chg->otg_en_default)) {
	...
	if (value) {
		...
		pinctrl_select_state(chg->otg_en_pinctrl, chg->otg_en_active);
	} else {
		gpio_direction_output(chg->otg_en_gpio, 0);
		pinctrl_select_state(chg->otg_en_pinctrl, chg->otg_en_default);
	}
...
void oplus_set_idt_en_val(int value) // 0  active, 1 inactive
{
	...
	if (IS_ERR_OR_NULL(chg->idt_en_pinctrl) || IS_ERR_OR_NULL(chg->idt_en_active) ||
	    IS_ERR_OR_NULL(chg->idt_en_sleep) || IS_ERR_OR_NULL(chg->idt_en_default)) {
		chg_err("pinctrl null, return\n");
		return;
	}
	...
	if (value) {
		...
		pinctrl_select_state(chg->idt_en_pinctrl, chg->idt_en_active);
	} else {
		gpio_direction_output(chg->idt_en_gpio, 0);
		pinctrl_select_state(chg->idt_en_pinctrl, chg->idt_en_default);
	}
...
bool oplus_get_wired_otg_online(void)
{
	...
	if (val.intval >= POWER_SUPPLY_TYPEC_SINK && val.intval <= POWER_SUPPLY_TYPEC_POWERED_CABLE_ONLY &&
...
#define typec_rp_med_high(chg, typec_mode)                                                                             \
	((typec_mode == POWER_SUPPLY_TYPEC_SOURCE_MEDIUM || typec_mode == POWER_SUPPLY_TYPEC_SOURCE_HIGH) &&           \
...
int smblib_get_iio_channel(struct smb_charger *chg, const char *propname, struct iio_channel **chan)
{
	int rc = 0;

	rc = of_property_match_string(chg->dev->of_node, "io-channel-names", propname);
	if (rc < 0)
		return 0;
...
#define DIV_FACTOR_MILI_V_I 1000
#define DIV_FACTOR_DECIDEGC 100
int smblib_read_iio_channel(struct smb_charger *chg, struct iio_channel *chan, int div, int *data)
{
	int rc = 0;
	*data = -ENODATA;
...
int smblib_get_jeita_cc_delta(struct smb_charger *chg, int *cc_delta_ua)
{
	...
	if (stat & BAT_TEMP_STATUS_HOT_SOFT_BIT) {
		rc = smblib_get_charge_param(chg, &chg->param.jeita_cc_comp_hot, &cc_minus_ua);
		if (rc < 0) {
			smblib_err(chg, "Couldn't get jeita cc minus rc=%d\n", rc);
			return rc;
		}
	} else if (stat & BAT_TEMP_STATUS_COLD_SOFT_BIT) {
		rc = smblib_get_charge_param(chg, &chg->param.jeita_cc_comp_cold, &cc_minus_ua);
		if (rc < 0) {
			smblib_err(chg, "Couldn't get jeita cc minus rc=%d\n", rc);
			return rc;
		}
...
int smblib_icl_override(struct smb_charger *chg, enum icl_override_mode mode)
{
	...
	rc = smblib_masked_write(chg, USBIN_ICL_OPTIONS_REG, USBIN_MODE_CHG_BIT, usb51_mode);
	if (rc < 0) {
		smblib_err(chg, "Couldn't set USBIN_ICL_OPTIONS rc=%d\n", rc);
		return rc;
	}

	rc = smblib_masked_write(chg, CMD_ICL_OVERRIDE_REG, ICL_OVERRIDE_BIT, icl_override);
	if (rc < 0) {
		smblib_err(chg, "Couldn't override ICL rc=%d\n", rc);
		return rc;
	}

	rc = smblib_masked_write(chg, USBIN_LOAD_CFG_REG, ICL_OVERRIDE_AFTER_APSD_BIT, apsd_override);
	if (rc < 0) {
 ...
 */
static int smblib_select_sec_charger_locked(struct smb_charger *chg, int sec_chg)
{
	int rc = 0;
	...
	case POWER_SUPPLY_CHARGER_SEC_CP:
		...
		rc = smblib_masked_write(chg, MISC_SMB_CFG_REG, SMB_EN_SEL_BIT, SMB_EN_SEL_BIT);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't select SMB charger rc=%d\n", rc);
			return rc;
		}
		/* Enable Charge Pump, under HW control */
		rc = smblib_masked_write(chg, MISC_SMB_EN_CMD_REG, EN_CP_CMD_BIT, EN_CP_CMD_BIT);
		if (rc < 0) {
	...
	case POWER_SUPPLY_CHARGER_SEC_PL:
		...
		rc = smblib_masked_write(chg, MISC_SMB_CFG_REG, SMB_EN_SEL_BIT, 0);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't select SMB charger rc=%d\n", rc);
			return rc;
		}
		/* Enable slave charger, under HW control */
		rc = smblib_masked_write(chg, MISC_SMB_EN_CMD_REG, EN_STAT_CMD_BIT, EN_STAT_CMD_BIT);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't enable SMB charger rc=%d\n", rc);
			return rc;
		}
...
static int smblib_select_sec_charger(struct smb_charger *chg, int sec_chg, int reason, bool toggle)
{
	int rc;
	...
	if (toggle && sec_chg == POWER_SUPPLY_CHARGER_SEC_CP) {
		rc = smblib_select_sec_charger_locked(chg, POWER_SUPPLY_CHARGER_SEC_NONE);
		if (rc < 0) {
...
static void smblib_notify_usb_host(struct smb_charger *chg, bool enable)
{
	...
	if (enable) {
		...
		if (rc < 0) {
			smblib_err(chg, "Couldn't enable VBUS in OTG mode rc=%d\n", rc);
			return;
		}
		...
		if (rc < 0) {
			smblib_err(chg, "Couldn't disable VBUS in OTG mode rc=%d\n", rc);
			return;
		}
	}
...
int smblib_get_charge_param(struct smb_charger *chg, struct smb_chg_param *param, int *val_u)
{
	int rc = 0;
	u8 val_raw;
...
static const s16 therm_lookup_table[] = {
	...
	0x6C92, 0x6C43, 0x6BF0, 0x6B98, 0x6B3A, 0x6AD8, 0x6A70, 0x6A03, 0x6990, 0x6916, 0x6897, 0x6811, 0x6785,
	0x66F2, 0x6658, 0x65B7, 0x650F, 0x6460, 0x63AA, 0x62EC, 0x6226, 0x6159, 0x6084, 0x5FA8, 0x5EC3, 0x5DD8,
	0x5CE4, 0x5BE9, 0x5AE7, 0x59DD, 0x58CD, 0x57B5, 0x5696, 0x5571, 0x5446, 0x5314, 0x51DD, 0x50A0, 0x4F5E,
	0x4E17, 0x4CCC, 0x4B7D, 0x4A2A, 0x48D4, 0x477C, 0x4621, 0x44C4, 0x4365, 0x4206, 0x40A6, 0x3F45, 0x3DE6,
	0x3C86, 0x3B28, 0x39CC, 0x3872, 0x3719, 0x35C4, 0x3471, 0x3322, 0x31D7, 0x308F, 0x2F4C, 0x2E0D, 0x2CD3,
	0x2B9E, 0x2A6E, 0x2943, 0x281D, 0x26FE, 0x25E3, 0x24CF, 0x23C0, 0x22B8, 0x21B5, 0x20B8, 0x1FC2, 0x1ED1,
	0x1DE6, 0x1D01, 0x1C22, 0x1B49, 0x1A75, 0x19A8, 0x18E0, 0x181D, 0x1761, 0x16A9, 0x15F7, 0x154A, 0x14A2,
	0x13FF, 0x1361, 0x12C8, 0x1234, 0x11A4, 0x1119, 0x1091, 0x100F, 0x0F90, 0x0F15, 0x0E9E, 0x0E2B, 0x0DBC,
	0x0D50, 0x0CE8, 0x0C83, 0x0C21, 0x0BC3, 0x0B67, 0x0B0F, 0x0AB9, 0x0A66, 0x0A16, 0x09C9, 0x097E,
};
...
enum { UNKNOWN, SDP, CDP, DCP, OCP, FLOAT, HVDCP2, HVDCP3, MAX_TYPES };
...
static int smblib_is_input_present(struct smb_charger *chg, int *present)
{
	int rc;
...
int smblib_set_chg_freq(struct smb_chg_param *param, int val_u, u8 *val_raw)
{
	u8 i;
...
int smblib_set_opt_switcher_freq(struct smb_charger *chg, int fsw_khz)
{
	...
	if (chg->mode == PARALLEL_MASTER && chg->pl.psy) {
		 ...
		 */
		power_supply_set_property(chg->pl.psy, POWER_SUPPLY_PROP_BUCK_FREQ, &pval);
	}

	return rc;
}

int smblib_set_charge_param(struct smb_charger *chg, struct smb_chg_param *param, int val_u)
{
	int rc = 0;
	u8 val_raw;
	...
	if (param->set_proc) {
		...
		if (val_u > param->max_u || val_u < param->min_u)
			smblib_dbg(chg, PR_MISC, "%s: %d is out of range [%d, %d]\n", param->name, val_u, param->min_u,
				   param->max_u);
...
#endif
	if (suspend)
		vote(chg->icl_irq_disable_votable, USB_SUSPEND_VOTER, true, 0);

	suspend_usb = suspend;

	rc = smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, suspend ? USBIN_SUSPEND_BIT : 0);
	if (rc < 0)
		smblib_err(chg, "Couldn't write %s to USBIN_SUSPEND_BIT rc=%d\n", suspend ? "suspend" : "resume", rc);

	if (!suspend)
		vote(chg->icl_irq_disable_votable, USB_SUSPEND_VOTER, false, 0);
...
int smblib_set_dc_suspend(struct smb_charger *chg, bool suspend)
{
	...
	rc = smblib_masked_write(chg, DCIN_CMD_IL_REG, DCIN_SUSPEND_BIT, suspend ? DCIN_SUSPEND_BIT : 0);
	if (rc < 0)
...
int op_wireless_high_vol_en(bool enable)
{
	...
	rc = smblib_write(g_chg, USBIN_ADAPTER_ALLOW_CFG_REG,
			  enable ? USBIN_ADAPTER_ALLOW_5V_TO_12V : USBIN_ADAPTER_ALLOW_5V);
	if (rc < 0) {
		smblib_err(g_chg, "Couldn't write USBIN_ADAPTER_ALLOW_CFG_REG, rc=%d\n", rc);
		return rc;
	}
	...
	if (g_chg->wireless_present) {
		...
		rc = smblib_masked_write(g_chg, USBIN_LOAD_CFG_REG,
					 USBIN_AICL_STEP_TIMING_SEL_MASK | USBIN_IN_COLLAPSE_GF_SEL_MASK, 0);
		if (rc < 0) {
			smblib_err(g_chg, "Couldn't turn on USBIN_LOAD_CFG_REG rc=%d\n", rc);
			return rc;
		}
...
static int smblib_usb_pd_adapter_allowance_override(struct smb_charger *chg, u8 allowed_voltage)
{
	int rc = 0;
	...
	rc = smblib_write(chg, USBIN_ADAPTER_ALLOW_OVERRIDE_REG, allowed_voltage);
	if (rc < 0)
...
static int smblib_set_usb_pd_allowed_voltage(struct smb_charger *chg, int min_allowed_uv, int max_allowed_uv)
{
	...
	rc = smblib_set_charge_param(chg, &chg->param.aicl_cont_threshold, aicl_threshold);
	if (rc < 0) {
...
int smblib_set_aicl_cont_threshold(struct smb_chg_param *param, int val_u, u8 *val_raw)
{
	int base = param->min_u;
	int offset = 0;
	int step = param->step_u;
...
int smblib_get_qc3_main_icl_offset(struct smb_charger *chg, int *offset_ua)
{
	...
	if ((chg->real_charger_type != POWER_SUPPLY_TYPE_USB_HVDCP_3) || chg->hvdcp3_standalone_config ||
	    !is_cp_available(chg)) {
		*offset_ua = 0;
		return 0;
	}

	rc = power_supply_get_property(chg->cp_psy, POWER_SUPPLY_PROP_CP_ILIM, &pval);
	if (rc < 0) {
...
int smblib_get_prop_from_bms(struct smb_charger *chg, enum power_supply_property psp, union power_supply_propval *val)
{
	int rc;
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (!chg->bms_psy) {
...
void smblib_apsd_enable(struct smb_charger *chg, bool enable)
{
	...
	rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, BC1P2_SRC_DETECT_BIT, enable ? BC1P2_SRC_DETECT_BIT : 0);
	if (rc < 0)
...
void smblib_bypass_vsafe0_control(struct smb_charger *chg, bool enable)
{
	...
	rc = smblib_masked_write(g_chg, TYPE_C_EXIT_STATE_CFG_REG, BYPASS_VSAFE0V_DURING_ROLE_SWAP_BIT,
...
#endif
	rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, mask, enable ? mask : 0);
	if (rc < 0)
		smblib_err(chg, "failed to write USBIN_OPTIONS_1_CFG rc=%d\n", rc);
...
void smblib_hvdcp_hw_inov_enable(struct smb_charger *chg, bool enable)
{
	...
	rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, HVDCP_AUTONOMOUS_MODE_EN_CFG_BIT,
...
void smblib_hvdcp_exit_config(struct smb_charger *chg)
{
	...
	if (stat & (QC_3P0_BIT | QC_2P0_BIT)) {
		/* force HVDCP to 5V */
		smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG, HVDCP_AUTONOMOUS_MODE_EN_CFG_BIT, 0);
...
int opchg_get_charger_type(void)
{
	...
	if (type == POWER_SUPPLY_TYPE_UNKNOWN && oplus_chg_get_voocphy_support() == NO_VOOCPHY &&
...
static int smblib_notifier_call(struct notifier_block *nb, unsigned long ev, void *v)
{
	...
	if (chg->sec_pl_present && !chg->pl.psy && !strcmp(psy->desc->name, "parallel")) {
		chg->pl.psy = psy;
...
int smblib_mapping_soc_from_field_value(struct smb_chg_param *param, int val_u, u8 *val_raw)
{
	if (val_u > param->max_u || val_u < param->min_u)
		return -EINVAL;
...
int smblib_mapping_cc_delta_to_field_value(struct smb_chg_param *param, u8 val_raw)
{
...
int smblib_mapping_cc_delta_from_field_value(struct smb_chg_param *param, int val_u, u8 *val_raw)
{
...
static void smblib_uusb_removal(struct smb_charger *chg)
{
	...
	sec_charger = chg->sec_pl_present ? POWER_SUPPLY_CHARGER_SEC_PL : POWER_SUPPLY_CHARGER_SEC_NONE;
	smblib_select_sec_charger(chg, sec_charger, POWER_SUPPLY_CP_NONE, false);
	...
	vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, is_flash_active(chg) ? SDP_CURRENT_UA : SDP_100_MA);
	...
	if (chg->cp_disable_votable)
		vote(chg->cp_disable_votable, SW_THERM_REGULATION_VOTER, false, 0);
	...
	rc = smblib_masked_write(chg, USBIN_OPTIONS_2_CFG_REG, (u8)FLOAT_OPTIONS_MASK, chg->float_cfg);
	if (rc < 0)
	...
	if (chg->qc2_unsupported_voltage) {
		rc = smblib_masked_write(chg, HVDCP_PULSE_COUNT_MAX_REG, HVDCP_PULSE_COUNT_MAX_QC2_MASK,
					 chg->qc2_max_pulses);
		if (rc < 0)
			smblib_err(chg, "Couldn't restore max pulses rc=%d\n", rc);

		rc = smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG, SUSPEND_ON_COLLAPSE_USBIN_BIT,
...
void smblib_suspend_on_debug_battery(struct smb_charger *chg)
{
	...
	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_DEBUG_BATTERY, &val);
	if (rc < 0) {
	...
	if (chg->suspend_input_on_debug_batt) {
		...
		vote(chg->chg_disable_votable, DEBUG_BOARD_VOTER, val.intval, 0);
	}
}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	apsd_result = smblib_get_apsd_result(chg);
	if ((apsd_result->pst != POWER_SUPPLY_TYPE_UNKNOWN) && (apsd_result->pst != POWER_SUPPLY_TYPE_USB) &&
...
static int set_sdp_current(struct smb_charger *chg, int icl_ua)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB && apsd_result->pst == POWER_SUPPLY_TYPE_USB_FLOAT) {
		/*
		 * change the float charger configuration to SDP, if this
		 * is the case of SDP being detected as FLOAT
		 */
		rc = smblib_masked_write(chg, USBIN_OPTIONS_2_CFG_REG, FORCE_FLOAT_SDP_CFG_BIT,
	...
	rc = smblib_masked_write(chg, USBIN_ICL_OPTIONS_REG, CFG_USB3P0_SEL_BIT | USB51_MODE_BIT, icl_options);
	if (rc < 0) {
...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	smblib_err(chg, "icl_ua=%d, connector_type=%d, real_charger_type=%d,wireles_present[%d]\n", icl_ua,
		   chg->connector_type, chg->real_charger_type, chg->wireless_present);
#endif // OPLUS_CUSTOM_OP_DEF
	...
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC) {
#endif // OPLUS_CUSTOM_OP_DEF
		rc = smblib_masked_write(chg, USB_CMD_PULLDOWN_REG, EN_PULLDOWN_USB_IN_BIT,
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB &&
	    (chg->typec_legacy || chg->typec_mode == POWER_SUPPLY_TYPEC_SOURCE_DEFAULT ||
...
int smblib_toggle_smb_en(struct smb_charger *chg, int toggle)
{
	...
	rc = smblib_select_sec_charger(chg, chg->sec_chg_selected, chg->cp_reason, true);

	return rc;
}

int smblib_get_irq_status(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	u8 reg;
...
static int smblib_set_moisture_protection(struct smb_charger *chg, bool enable)
{
	int rc = 0;
	...
	if (enable) {
		...
		rc = smblib_masked_write(chg, TYPEC_U_USB_CFG_REG, EN_MICRO_USB_FACTORY_MODE_BIT, 0);
		if (rc < 0) {
		...
		rc = smblib_masked_write(chg, TYPE_C_INTERRUPT_EN_CFG_2_REG,
					 TYPEC_WATER_DETECTION_INT_EN_BIT | MICRO_USB_STATE_CHANGE_INT_EN_BIT, 0);
		if (rc < 0) {
		...
		rc = smblib_masked_write(chg,
					 ...
					 EN_MICRO_USB_WATER_PROTECTION_BIT | MICRO_USB_DETECTION_ON_TIME_CFG_MASK |
						 MICRO_USB_DETECTION_PERIOD_CFG_MASK,
					 EN_MICRO_USB_WATER_PROTECTION_BIT | MICRO_USB_DETECTION_ON_TIME_20_MS |
		...
		rc = smblib_masked_write(chg, TYPEC_U_USB_CFG_REG, EN_MICRO_USB_FACTORY_MODE_BIT,
 ...
 * VOTABLE CALLBACKS *
 *********************/
static int smblib_smb_disable_override_vote_callback(struct votable *votable, void *data, int disable_smb,
						     const char *client)
{
	...
	rc = smblib_masked_write(chg, MISC_SMB_EN_CMD_REG, SMB_EN_OVERRIDE_BIT | SMB_EN_OVERRIDE_VALUE_BIT,
...
static int smblib_dc_suspend_vote_callback(struct votable *votable, void *data, int suspend, const char *client)
{
	struct smb_charger *chg = data;
...
static int smblib_awake_vote_callback(struct votable *votable, void *data, int awake, const char *client)
{
	struct smb_charger *chg = data;
...
static int smblib_chg_disable_vote_callback(struct votable *votable, void *data, int chg_disable, const char *client)
{
	struct smb_charger *chg = data;
	int rc;

	rc = smblib_masked_write(chg, CHARGING_ENABLE_CMD_REG, CHARGING_ENABLE_CMD_BIT,
...
static int smblib_hdc_irq_disable_vote_callback(struct votable *votable, void *data, int disable, const char *client)
{
	struct smb_charger *chg = data;
...
static int smblib_limited_irq_disable_vote_callback(struct votable *votable, void *data, int disable,
						    const char *client)
{
	struct smb_charger *chg = data;
...
static int smblib_icl_irq_disable_vote_callback(struct votable *votable, void *data, int disable, const char *client)
{
	struct smb_charger *chg = data;
...
static int smblib_temp_change_irq_disable_vote_callback(struct votable *votable, void *data, int disable,
							const char *client)
{
	struct smb_charger *chg = data;
...
int smblib_vconn_regulator_enable(struct regulator_dev *rdev)
{
	...
	orientation = stat & TYPEC_CCOUT_VALUE_BIT ? 0 : VCONN_EN_ORIENTATION_BIT;
	rc = smblib_masked_write(chg, TYPE_C_VCONN_CONTROL_REG, VCONN_EN_VALUE_BIT | VCONN_EN_ORIENTATION_BIT,
...
int smblib_vconn_regulator_disable(struct regulator_dev *rdev)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_VCONN_CONTROL_REG, VCONN_EN_VALUE_BIT, 0);
	if (rc < 0)
...
int smblib_vbus_regulator_disable(struct regulator_dev *rdev)
{
	...
	if (chip->vbatt_num == 2) {
		...
		if (chip->wireless_support) {
			...
			oplus_wpc_set_ext2_wireless_otg_en_val(1);
			if (oplus_wpc_get_normal_charging() != true && oplus_wpc_get_fast_charging() != true &&
...
int smblib_get_prop_batt_present(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	u8 stat;
	...
	val->intval = !(stat & (BAT_THERM_OR_ID_MISSING_RT_STS_BIT | BAT_TERMINAL_MISSING_RT_STS_BIT));

	return rc;
}

int smblib_get_prop_batt_capacity(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc = -EINVAL;
...
int smblib_get_prop_batt_status(struct smb_charger *chg, union power_supply_propval *val)
{
	union power_supply_propval pval = {
		0,
	};
	...
	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_DEBUG_BATTERY, &pval);
	if (rc < 0) {
	 ...
	 */
	if (is_client_vote_enabled_locked(chg->usb_icl_votable, CHG_TERMINATION_VOTER)) {
		val->intval = POWER_SUPPLY_STATUS_FULL;
		return 0;
	}
	...
	stat &= ENABLE_TRICKLE_BIT | ENABLE_PRE_CHARGING_BIT | ENABLE_FULLON_MODE_BIT;
...
int smblib_get_prop_batt_charge_type(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	u8 stat;
...
int smblib_get_prop_batt_health(struct smb_charger *chg, union power_supply_propval *val)
{
	union power_supply_propval pval;
	int rc;
	int effective_fv_uv;
...
int smblib_get_prop_system_temp_level(struct smb_charger *chg, union power_supply_propval *val)
{
	val->intval = chg->system_temp_level;
	return 0;
}

int smblib_get_prop_system_temp_level_max(struct smb_charger *chg, union power_supply_propval *val)
{
	val->intval = chg->thermal_levels;
	return 0;
}

int smblib_get_prop_input_current_limited(struct smb_charger *chg, union power_supply_propval *val)
{
	u8 stat;
	int rc;
...
int smblib_get_prop_batt_iterm(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc, temp;
	u8 stat, buf[2];
	...
	if (chg->chg_param.smb_version == PMI632_SUBTYPE)
		temp = DIV_ROUND_CLOSEST(temp * ITERM_LIMITS_PMI632_MA, ADC_CHG_ITERM_MASK);
	else
		temp = DIV_ROUND_CLOSEST(temp * ITERM_LIMITS_PM8150B_MA, ADC_CHG_ITERM_MASK);
...
int smblib_get_prop_batt_charge_done(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	u8 stat;
...
int smblib_get_batt_current_now(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;

	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_CURRENT_NOW, val);
	if (!rc)
		val->intval *= (-1);
...
int smblib_set_prop_batt_capacity(struct smb_charger *chg, const union power_supply_propval *val)
{
...
int smblib_set_prop_system_temp_level(struct smb_charger *chg, const union power_supply_propval *val)
{
	...
	vote(chg->fcc_votable, THERMAL_DAEMON_VOTER, true, chg->thermal_mitigation[chg->system_temp_level]);
	return 0;
}

int smblib_set_prop_input_current_limited(struct smb_charger *chg, const union power_supply_propval *val)
{
	chg->fake_input_current_limited = val->intval;
	return 0;
}

int smblib_set_prop_rechg_soc_thresh(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc;
	u8 new_thr = DIV_ROUND_CLOSEST(val->intval * 255, 100);

	rc = smblib_write(chg, CHARGE_RCHG_SOC_THRESHOLD_CFG_REG, new_thr);
	if (rc < 0) {
...
int smblib_run_aicl(struct smb_charger *chg, int type)
{
	...
	rc = smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG, USBIN_AICL_PERIODIC_RERUN_EN_BIT,
...
static int smblib_dp_pulse(struct smb_charger *chg)
{
	...
	/* QC 3.0 increment */
	rc = smblib_masked_write(chg, CMD_HVDCP_2_REG, SINGLE_INCREMENT_BIT, SINGLE_INCREMENT_BIT);
	if (rc < 0)
...
static int smblib_dm_pulse(struct smb_charger *chg)
{
	...
	/* QC 3.0 decrement */
	rc = smblib_masked_write(chg, CMD_HVDCP_2_REG, SINGLE_DECREMENT_BIT, SINGLE_DECREMENT_BIT);
	if (rc < 0)
		smblib_err(chg, "Couldn't write to CMD_HVDCP_2_REG rc=%d\n", rc);
...
static void smblib_hvdcp_set_fsw(struct smb_charger *chg, int bit)
{
	...
	case QC_12V_BIT:
		...
		break;
	default:
		smblib_set_opt_switcher_freq(chg, chg->chg_freq.freq_removal);
		break;
	}
}
...
static void smblib_hvdcp_adaptive_voltage_change(struct smb_charger *chg)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP) {
		...
		if (rc < 0) {
			smblib_err(chg, "Couldn't read QC_CHANGE_STATUS rc=%d\n", rc);
			return;
		}
#ifdef OPLUS_CUSTOM_OP_DEF
...
int smblib_dp_dm(struct smb_charger *chg, int val)
{
	...
	case POWER_SUPPLY_DP_DM_ICL_DOWN:
		 ...
		 * again.
		 */
		if (target_icl_ua != get_client_vote(chg->usb_icl_votable, SW_QC3_VOTER))
			chg->usb_icl_delta_ua = 0;

		chg->usb_icl_delta_ua += 100000;
		vote(chg->usb_icl_votable, SW_QC3_VOTER, true, target_icl_ua - 100000);
...
#endif
	       JEITA_EN_HOT_SL_FCV_BIT | JEITA_EN_HOT_SL_CCC_BIT | JEITA_EN_COLD_SL_CCC_BIT;
	rc = smblib_masked_write(chg, JEITA_EN_CFG_REG, mask, disable ? 0 : mask);
	if (rc < 0) {
...
static int smblib_set_sw_thermal_regulation(struct smb_charger *chg, bool enable)
{
	int rc = 0;
	...
	if (enable) {
		 ...
		 * condition.
		 */
		rc = smblib_masked_write(chg, SNARL_BARK_BITE_WD_CFG_REG, SNARL_WDOG_TIMEOUT_MASK,
					 SNARL_WDOG_TMOUT_62P5MS);
		if (rc < 0) {
		 ...
		 */
		if (is_client_vote_enabled(chg->usb_icl_votable, SW_THERM_REGULATION_VOTER)) {
...
static int smblib_update_thermal_readings(struct smb_charger *chg)
{
	...
	rc = smblib_read_iio_channel(chg, chg->iio.die_temp_chan, DIV_FACTOR_DECIDEGC, &chg->die_temp);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read DIE TEMP channel, rc=%d\n", rc);
		return rc;
	}

	rc = smblib_read_iio_channel(chg, chg->iio.connector_temp_chan, DIV_FACTOR_DECIDEGC, &chg->connector_temp);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read CONN TEMP channel, rc=%d\n", rc);
		return rc;
	}

	rc = smblib_read_iio_channel(chg, chg->iio.skin_temp_chan, DIV_FACTOR_DECIDEGC, &chg->skin_temp);
	if (rc < 0) {
	...
	if (chg->sec_chg_selected == POWER_SUPPLY_CHARGER_SEC_CP) {
		if (is_cp_available(chg)) {
			rc = power_supply_get_property(chg->cp_psy, POWER_SUPPLY_PROP_CP_DIE_TEMP, &pval);
			if (rc < 0) {
	...
	} else if (chg->pl.psy && chg->sec_chg_selected == POWER_SUPPLY_CHARGER_SEC_PL) {
...
static int smblib_process_thermal_readings(struct smb_charger *chg)
{
	 ...
	 */
	if (chg->connector_temp > CONNECTOR_TEMP_SHDN_THRESH || chg->skin_temp > SKIN_TEMP_SHDN_THRESH) {
	...
	if (chg->connector_temp > CONNECTOR_TEMP_RST_THRESH || chg->skin_temp > SKIN_TEMP_RST_THRESH ||
	    chg->smb_temp > SMB_TEMP_RST_THRESH || chg->die_temp > DIE_TEMP_RST_THRESH) {
		thermal_status = TEMP_ALERT_LEVEL;
		wdog_timeout = SNARL_WDOG_TMOUT_1S;
		goto out;
	}

	if (chg->connector_temp > CONNECTOR_TEMP_REG_H_THRESH || chg->skin_temp > SKIN_TEMP_REG_H_THRESH ||
	    chg->smb_temp > SMB_TEMP_REG_H_THRESH || chg->die_temp > DIE_TEMP_REG_H_THRESH) {
		thermal_status = TEMP_ABOVE_RANGE;
		wdog_timeout = SNARL_WDOG_TMOUT_1S;
		goto out;
	}

	if (chg->connector_temp > CONNECTOR_TEMP_REG_L_THRESH || chg->skin_temp > SKIN_TEMP_REG_L_THRESH ||
	    chg->smb_temp > SMB_TEMP_REG_L_THRESH || chg->die_temp > DIE_TEMP_REG_L_THRESH) {
...
out:
	smblib_dbg(
		chg, PR_MISC,
		"Current temperatures: \tDIE_TEMP: %d,\tCONN_TEMP: %d,\tSMB_TEMP: %d,\tSKIN_TEMP: %d\nTHERMAL_STATUS: %d\n",
		chg->die_temp, chg->connector_temp, chg->smb_temp, chg->skin_temp, thermal_status);
	...
	if (thermal_status != chg->thermal_status) {
		...
		vote(chg->smb_override_votable, SW_THERM_REGULATION_VOTER, disable_smb, 0);
		...
		if (chg->cp_disable_votable)
			vote(chg->cp_disable_votable, SW_THERM_REGULATION_VOTER, disable_smb, 0);

		vote(chg->pl_disable_votable, SW_THERM_REGULATION_VOTER, disable_smb, 0);
		...
		vote(chg->dc_suspend_votable, SW_THERM_REGULATION_VOTER, suspend_input, 0);
		vote(chg->usb_icl_votable, SW_THERM_REGULATION_VOTER, suspend_input, 0);
...
exit:
	 ...
	 */
	if (is_client_vote_enabled(chg->usb_icl_votable, SW_THERM_REGULATION_VOTER)) {
		schedule_delayed_work(&chg->thermal_regulation_work, msecs_to_jiffies(THERM_REG_RECHECK_DELAY_1S));
		return 0;
	}

	rc = smblib_masked_write(chg, SNARL_BARK_BITE_WD_CFG_REG, SNARL_WDOG_TIMEOUT_MASK, wdog_timeout);
	if (rc < 0)
...
int smblib_get_prop_voltage_wls_output(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	...
	rc = power_supply_get_property(chg->wls_psy, POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, val);
	if (rc < 0)
...
int smblib_get_prop_dc_present(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	u8 stat;
...
int smblib_get_prop_dc_online(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc = 0;
	u8 stat;
	...
	if (is_client_vote_enabled(chg->dc_suspend_votable, CHG_TERMINATION_VOTER)) {
	...
	val->intval = (stat & USE_DCIN_BIT) && (stat & VALID_INPUT_POWER_SOURCE_STS_BIT);

	return rc;
}

int smblib_get_prop_dc_current_max(struct smb_charger *chg, union power_supply_propval *val)
{
	return smblib_get_charge_param(chg, &chg->param.dc_icl, &val->intval);
}

int smblib_get_prop_dc_voltage_max(struct smb_charger *chg, union power_supply_propval *val)
{
	val->intval = MICRO_12V;
	return 0;
}

int smblib_get_prop_dc_voltage_now(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
...
int smblib_set_prop_dc_current_max(struct smb_charger *chg, const union power_supply_propval *val)
{
	return smblib_set_charge_param(chg, &chg->param.dc_icl, val->intval);
}

#define DCIN_AICL_RERUN_DELAY_MS 5000
int smblib_set_prop_voltage_wls_output(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc;
	...
	rc = power_supply_set_property(chg->wls_psy, POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, val);
	if (rc < 0)
	...
	if ((val->intval > 0) && (val->intval < chg->last_wls_vout)) {
		alarm_start_relative(&chg->dcin_aicl_alarm, ms_to_ktime(DCIN_AICL_RERUN_DELAY_MS));
	}
...
int smblib_set_prop_dc_reset(struct smb_charger *chg)
{
	...
	rc = smblib_masked_write(chg, DCIN_CMD_IL_REG, DCIN_EN_MASK, DCIN_EN_OVERRIDE_BIT);
	if (rc < 0) {
...
int smblib_get_prop_usb_present(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	u8 stat;
...
#endif
int smblib_get_prop_usb_online(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc = 0;
	u8 stat;
	...
	val->intval = (stat & USE_USBIN_BIT) && (stat & VALID_INPUT_POWER_SOURCE_STS_BIT);
	return rc;
}

int smblib_get_usb_online(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
...
int smblib_get_prop_usb_voltage_max_design(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_TYPE_USB_HVDCP:
		if (chg->qc2_unsupported_voltage == QC2_NON_COMPLIANT_9V) {
			val->intval = MICRO_5V;
			break;
		} else if (chg->qc2_unsupported_voltage == QC2_NON_COMPLIANT_12V) {
			val->intval = MICRO_9V;
			break;
		}
		/* else, fallthrough */
...
int smblib_get_prop_usb_voltage_max(struct smb_charger *chg, union power_supply_propval *val)
{
	switch (chg->real_charger_type) {
	case POWER_SUPPLY_TYPE_USB_HVDCP:
		if (chg->qc2_unsupported_voltage == QC2_NON_COMPLIANT_9V) {
...
static int smblib_estimate_adaptor_voltage(struct smb_charger *chg, union power_supply_propval *val)
{
	switch (chg->real_charger_type) {
	case POWER_SUPPLY_TYPE_USB_HVDCP:
		val->intval = MICRO_12V;
	...
	case POWER_SUPPLY_TYPE_USB_PD:
		...
		val->intval = chg->voltage_min_uv + ((chg->voltage_max_uv - chg->voltage_min_uv) / 2);
		break;
	default:
		val->intval = MICRO_5V;
		break;
...
static int smblib_read_mid_voltage_chan(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
...
static int smblib_read_usbin_voltage_chan(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
...
int smblib_get_prop_usb_voltage_now(struct smb_charger *chg, union power_supply_propval *val)
{
	union power_supply_propval pval = {
		0,
	};
	...
	if (chg->wa_flags & USBIN_ADC_WA) {
		...
		rc = smblib_write(chg, BATIF_ADC_CHANNEL_EN_REG, IBATT_CHANNEL_EN_BIT);
		if (rc < 0) {
...
int smblib_get_prop_vph_voltage_now(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
...
bool smblib_rsbux_low(struct smb_charger *chg, int r_thr)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_CRUDE_SENSOR_CFG_REG, EN_SRC_CRUDE_SENSOR_BIT | EN_SNK_CRUDE_SENSOR_BIT,
				 0);
	if (rc < 0) {
...
int smblib_get_prop_charger_temp(struct smb_charger *chg, union power_supply_propval *val)
{
	int temp, rc;
	int input_present;
	...
	if (chg->iio.temp_chan) {
		rc = iio_read_channel_processed(chg->iio.temp_chan, &temp);
		if (rc < 0) {
...
int smblib_get_prop_typec_cc_orientation(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc = 0;
	u8 stat;
...
inline int smblib_get_usb_prop_typec_mode(struct smb_charger *chg, union power_supply_propval *val)
{
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB)
...
int smblib_get_prop_typec_power_role(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc = 0;
	u8 ctrl;
...
{
	return (chg->typec_mode > POWER_SUPPLY_TYPEC_NONE && chg->typec_mode < POWER_SUPPLY_TYPEC_SOURCE_DEFAULT);
}

int smblib_get_prop_typec_select_rp(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc, rp;
	u8 stat;
...
int smblib_get_prop_usb_current_now(struct smb_charger *chg, union power_supply_propval *val)
{
	union power_supply_propval pval = {
		0,
	};
	...
	if (chg->iio.usbin_i_chan) {
		...
		if (chg->otg_present || smblib_get_prop_dfp_mode(chg) != POWER_SUPPLY_TYPEC_NONE) {
			val->intval = DIV_ROUND_CLOSEST(val->intval * 100, boost_scale);
			return rc;
		}
		...
		if (!pval.intval)
			...
			val->intval = DIV_ROUND_CLOSEST(val->intval * 100, buck_scale);
	} else {
		val->intval = 0;
		rc = -ENODATA;
	}
...
int smblib_get_prop_low_power(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	u8 stat;
...
int smblib_get_prop_input_current_settled(struct smb_charger *chg, union power_supply_propval *val)
{
	return smblib_get_charge_param(chg, &chg->param.icl_stat, &val->intval);
}

int smblib_get_prop_input_voltage_settled(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc, pulses;
...
int smblib_get_prop_pd_in_hard_reset(struct smb_charger *chg, union power_supply_propval *val)
{
	val->intval = chg->pd_hard_reset;
	return 0;
}

int smblib_get_pe_start(struct smb_charger *chg, union power_supply_propval *val)
{
...
int smblib_get_die_health(struct smb_charger *chg, union power_supply_propval *val)
{
...
int smblib_get_prop_scope(struct smb_charger *chg, union power_supply_propval *val)
{
	int rc;
	union power_supply_propval pval;
...
int smblib_set_prop_pd_current_max(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc, icl;
...
static int smblib_handle_usb_current(struct smb_charger *chg, int usb_current)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (usb_current == -ETIMEDOUT) {
			if ((chg->float_cfg & FLOAT_OPTIONS_MASK) == FORCE_FLOAT_SDP_CFG_BIT) {
				/*
				 * Confiugure USB500 mode if Float charger is
				 * configured for SDP mode.
				 */
				rc = vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, USBIN_500MA);
				if (rc < 0)
					smblib_err(chg, "Couldn't set SDP ICL rc=%d\n", rc);
				return rc;
			}
			...
			if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC) {
				...
				rc = vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, rp_ua);
				if (rc < 0)
					return rc;
			} else {
...
int smblib_set_prop_sdp_current_max(struct smb_charger *chg, const union power_supply_propval *val)
{
	union power_supply_propval pval;
	int rc = 0;
	...
	if (!chg->pd_active) {
		...
		if (val->intval <= USBIN_25MA)
			rc = vote(chg->usb_icl_votable, PD_SUSPEND_SUPPORTED_VOTER, true, val->intval);
		else
			rc = vote(chg->usb_icl_votable, PD_SUSPEND_SUPPORTED_VOTER, false, 0);
	}
	return rc;
}

int smblib_set_prop_boost_current(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc = 0;

	rc = smblib_set_charge_param(chg, &chg->param.freq_switcher,
				     val->intval <= chg->boost_threshold_ua ? chg->chg_freq.freq_below_otg_threshold :
...
int smblib_set_prop_usb_voltage_max_limit(struct smb_charger *chg, const union power_supply_propval *val)
{
	union power_supply_propval pval = {
		0,
	};
...
void smblib_typec_irq_config(struct smb_charger *chg, bool en)
{
	...
	if (en) {
		enable_irq(chg->irq_info[TYPEC_ATTACH_DETACH_IRQ].irq);
		enable_irq(chg->irq_info[TYPEC_CC_STATE_CHANGE_IRQ].irq);
		enable_irq(chg->irq_info[TYPEC_OR_RID_DETECTION_CHANGE_IRQ].irq);
	} else {
		disable_irq_nosync(chg->irq_info[TYPEC_ATTACH_DETACH_IRQ].irq);
		disable_irq_nosync(chg->irq_info[TYPEC_CC_STATE_CHANGE_IRQ].irq);
...
#ifdef OPLUS_CUSTOM_OP_DEF
int smblib_set_prop_reset_rd(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc;
	u8 stat;
	...
	rc = smblib_masked_write(chg, TYPE_C_CC_CURRSRC_CONTROL_REG, TYPE_C_CC_CURRSRC_CONTROL_MASK, val->intval);
	if (rc < 0)
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, reset_rd_work.work);
	int rc;
	u8 stat;

	rc = smblib_masked_write(chg, TYPE_C_CC_CURRSRC_CONTROL_REG, TYPE_C_CC_CURRSRC_CONTROL_MASK, 0);
	if (rc < 0)
		smblib_err(chg, "Couldn't write to TYPE_C_CC_CURRSRC_CONTROL_REG rc=%d\n", rc);
...
int smblib_set_prop_typec_power_role(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc = 0;
	u8 power_role;
	...
	smblib_dbg(chg, PR_MISC, "power role change: %d --> %d!", chg->power_role, val->intval);

	if (chg->power_role == val->intval) {
		smblib_dbg(chg, PR_MISC, "power role already in %d, ignore!", chg->power_role);
		return 0;
	}

	typec_mode = smblib_get_prop_typec_mode(chg);
	if (typec_mode >= POWER_SUPPLY_TYPEC_SINK && typec_mode <= POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER)
		snk_attached = true;
	else if (typec_mode >= POWER_SUPPLY_TYPEC_SOURCE_DEFAULT && typec_mode <= POWER_SUPPLY_TYPEC_SOURCE_HIGH)
		src_attached = true;
	...
	if (!chg->pr_lock_in_progress && is_pr_lock) {
		...
		schedule_delayed_work(&chg->pr_lock_clear_work, msecs_to_jiffies(PR_LOCK_TIMEOUT_MS));
	...
	smblib_err(chg, " power_role=%d stat =0x%x level =%d chip->otg_switch=%d\n", power_role, stat, level,
		   chip->otg_switch);
#endif

	rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_POWER_ROLE_CMD_MASK | TYPEC_TRY_MODE_MASK, power_role);
	if (rc < 0) {
...
int smblib_set_prop_typec_select_rp(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc;
	...
	if (val->intval < TYPEC_SRC_RP_MAX_ELEMENTS) {
		rc = smblib_masked_write(chg, TYPE_C_CURRSRC_CFG_REG, TYPEC_SRC_RP_SEL_MASK, val->intval);
		if (rc < 0)
...
int smblib_set_prop_pd_voltage_min(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc, min_uv;
	...
	rc = smblib_set_usb_pd_allowed_voltage(chg, min_uv, chg->voltage_max_uv);
	if (rc < 0) {
...
int smblib_set_prop_pd_voltage_max(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc, max_uv;
	...
	rc = smblib_set_usb_pd_allowed_voltage(chg, chg->voltage_min_uv, max_uv);
	if (rc < 0) {
...
int smblib_set_prop_pd_active(struct smb_charger *chg, const union power_supply_propval *val)
{
	...
	if (chg->pd_active) {
		vote(chg->limited_irq_disable_votable, CHARGER_TYPE_VOTER, false, 0);
		vote(chg->hdc_irq_disable_votable, CHARGER_TYPE_VOTER, false, 0);
		 ...
		 * present.
		 */
		if (chg->pd_active == POWER_SUPPLY_PD_PPS_ACTIVE && chg->sec_cp_present) {
			rc = smblib_select_sec_charger(chg, POWER_SUPPLY_CHARGER_SEC_CP, POWER_SUPPLY_CP_PPS, false);
			if (rc < 0)
		...
		sec_charger = chg->sec_pl_present ? POWER_SUPPLY_CHARGER_SEC_PL : POWER_SUPPLY_CHARGER_SEC_NONE;
		rc = smblib_select_sec_charger(chg, sec_charger, POWER_SUPPLY_CP_NONE, false);
		if (rc < 0)
			dev_err(chg->dev, "Couldn't enable secondary charger rc=%d\n", rc);
	...
	smblib_usb_pd_adapter_allowance_override(chg, !!chg->pd_active ? FORCE_5V : FORCE_NULL);
	smblib_update_usb_type(chg);
...
int smblib_set_prop_ship_mode(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc;

	smblib_dbg(chg, PR_MISC, "Set ship mode: %d!!\n", !!val->intval);

	rc = smblib_masked_write(chg, SHIP_MODE_REG, SHIP_MODE_EN_BIT, !!val->intval ? SHIP_MODE_EN_BIT : 0);
	if (rc < 0)
...
int smblib_set_prop_pd_in_hard_reset(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc = 0;
	...
	rc = smblib_masked_write(chg, TYPE_C_EXIT_STATE_CFG_REG, EXIT_SNK_BASED_ON_CC_BIT,
...
static int smblib_update_jeita(struct smb_charger *chg, u32 *thresholds, int type)
{
	...
	if (rc < 0) {
		smblib_err(chg, "Couldn't configure Jeita %s hot threshold rc=%d\n",
	...
	if (rc < 0) {
		smblib_err(chg, "Couldn't configure Jeita %s cold threshold rc=%d\n",
			   (type == JEITA_SOFT) ? "Soft" : "Hard", rc);
		return rc;
	}
...
static int smblib_charge_inhibit_en(struct smb_charger *chg, bool enable)
{
	...
	rc = smblib_masked_write(chg, CHGR_CFG2_REG, CHARGER_INHIBIT_BIT, enable ? CHARGER_INHIBIT_BIT : 0);
	return rc;
}
...
static int smblib_soft_jeita_arb_wa(struct smb_charger *chg)
{
	...
	if (chg->jeita_soft_fcc[0] < 0 || chg->jeita_soft_fcc[1] < 0 || chg->jeita_soft_fv[0] < 0 ||
	    chg->jeita_soft_fv[1] < 0)
		return 0;
	...
	if (!chg->jeita_arb_flag && soft_jeita) {
		...
		rc = smblib_update_jeita(chg, chg->jeita_soft_hys_thlds, JEITA_SOFT);
		if (rc < 0)
			smblib_err(chg, "Couldn't configure Jeita soft threshold rc=%d\n", rc);

		if (pval.intval == POWER_SUPPLY_HEALTH_COOL) {
			vote(chg->fcc_votable, JEITA_ARB_VOTER, true, chg->jeita_soft_fcc[0]);
			vote(chg->fv_votable, JEITA_ARB_VOTER, true, chg->jeita_soft_fv[0]);
		} else {
			vote(chg->fcc_votable, JEITA_ARB_VOTER, true, chg->jeita_soft_fcc[1]);
			vote(chg->fv_votable, JEITA_ARB_VOTER, true, chg->jeita_soft_fv[1]);
		}
		...
		rc = smblib_update_jeita(chg, chg->jeita_soft_thlds, JEITA_SOFT);
		if (rc < 0)
			smblib_err(chg, "Couldn't configure Jeita soft threshold rc=%d\n", rc);
 ...
 ************************/
int smblib_get_prop_fcc_delta(struct smb_charger *chg, union power_supply_propval *val)
{
...
int smblib_get_charge_current(struct smb_charger *chg, int *total_current_ua)
{
	...
	if (chg->pd_active) {
		*total_current_ua = get_client_vote_locked(chg->usb_icl_votable, PD_VOTER);
		return rc;
	}
...
static void smblib_eval_chg_termination(struct smb_charger *chg, u8 batt_status)
{
	...
	if ((batt_status == TERMINATE_CHARGE) && (pval.intval == 100)) {
		chg->cc_soc_ref = 0;
		chg->last_cc_soc = 0;
		alarm_start_relative(&chg->chg_termination_alarm, ms_to_ktime(CHG_TERM_WA_ENTRY_DELAY_MS));
...
irqreturn_t usbin_uv_irq_handler(int irq, void *data)
{
	...
	if ((chg->wa_flags & WEAK_ADAPTER_WA) && is_storming(&irq_data->storm_data)) {
		if (chg->aicl_max_reached) {
			smblib_dbg(chg, PR_MISC, "USBIN_UV storm at max AICL threshold\n");
			return IRQ_HANDLED;
		}
		...
		if (rc < 0)
			dev_err(chg->dev, "Error in setting AICL threshold rc=%d\n", rc);
		else
			chg->aicl_5v_threshold_mv += AICL_STEP_MV;
	...
	unsuspend_input:
		...
		if (chg->aicl_max_reached) {
			smblib_dbg(chg, PR_MISC, "Reached max AICL threshold resctricting ICL to 100mA\n");
			vote(chg->usb_icl_votable, AICL_THRESHOLD_VOTER, true, USBIN_100MA);
			smblib_run_aicl(chg, RESTART_AICL);
		} else {
			smblib_run_aicl(chg, RESTART_AICL);
			vote(chg->usb_icl_votable, AICL_THRESHOLD_VOTER, false, 0);
		}
	...
	if (!chg->qc2_unsupported_voltage && apsd->pst == POWER_SUPPLY_TYPE_USB_HVDCP) {
		rc = smblib_read(chg, QC_CHANGE_STATUS_REG, &stat);
		if (rc < 0)
			smblib_err(chg, "Couldn't read CHANGE_STATUS_REG rc=%d\n", rc);
		...
		if (rc < 0)
			smblib_err(chg, "Couldn't read QC2 max pulses rc=%d\n", rc);

		chg->qc2_max_pulses = (max_pulses & HVDCP_PULSE_COUNT_MAX_QC2_MASK);
...
irqreturn_t icl_change_irq_handler(int irq, void *data)
{
	...
	if (chg->mode == PARALLEL_MASTER) {
		...
		if (chg->hw_die_temp_mitigation) {
			rc = smblib_read(chg, DIE_TEMP_STATUS_REG, &stat);
			if (rc < 0) {
				smblib_err(chg, "Couldn't read DIE_TEMP rc=%d\n", rc);
				return IRQ_HANDLED;
			}
			if (stat & (DIE_TEMP_UB_BIT | DIE_TEMP_LB_BIT)) {
				smblib_dbg(chg, PR_PARALLEL, "skip ICL change DIE_TEMP %x\n", stat);
				return IRQ_HANDLED;
			}
		}
		...
		schedule_delayed_work(&chg->icl_change_work, msecs_to_jiffies(delay));
	}
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (!(chg->hvdcp_detect_ok &&
		      (chg->hvdcp_detach_time - chg->hvdcp_detect_time <= OPLUS_HVDCP_DETECT_TO_DETACH_TIME))) {
		...
		if (chg->fcc_stepper_enable)
			vote(chg->fcc_votable, FCC_STEPPER_VOTER, true, 1500000);
	}
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (vbus_rising) {
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (chg->wireless_present) {
		if (chg->wireless_high_vol_mode)
			smblib_set_opt_switcher_freq(chg,
						     vbus_rising ? chg->chg_freq.freq_12V : chg->chg_freq.freq_removal);
		else
			smblib_set_opt_switcher_freq(chg, vbus_rising ? chg->chg_freq.freq_6V_8V :
									chg->chg_freq.freq_removal);
	} else {
		smblib_set_opt_switcher_freq(chg, vbus_rising ? chg->chg_freq.freq_5V : chg->chg_freq.freq_removal);
	...
	if (vbus_rising) {
		...
		if (chg->wa_flags & WEAK_ADAPTER_WA) {
			...
			smblib_set_charge_param(chg, &chg->param.aicl_cont_threshold, chg->aicl_cont_threshold_mv);
...
static void smblib_handle_slow_plugin_timeout(struct smb_charger *chg, bool rising)
{
	smblib_dbg(chg, PR_INTERRUPT, "IRQ: slow-plugin-timeout %s\n", rising ? "rising" : "falling");
}

static void smblib_handle_sdp_enumeration_done(struct smb_charger *chg, bool rising)
{
	smblib_dbg(chg, PR_INTERRUPT, "IRQ: sdp-enumeration-done %s\n", rising ? "rising" : "falling");
}

/* triggers when HVDCP 3.0 authentication has finished */
static void smblib_handle_hvdcp_3p0_auth_done(struct smb_charger *chg, bool rising)
{
	const struct apsd_result *apsd_result;
	int rc;
	...
	if ((apsd_result->bit & QC_3P0_BIT) && chg->sec_cp_present) {
		rc = smblib_select_sec_charger(chg, POWER_SUPPLY_CHARGER_SEC_CP, POWER_SUPPLY_CP_HVDCP3, false);
		if (rc < 0)
...
static void smblib_handle_hvdcp_check_timeout(struct smb_charger *chg, bool rising, bool qc_charger)
{
	if (rising) {
		if (qc_charger) {
...
static void smblib_handle_hvdcp_detect_done(struct smb_charger *chg, bool rising)
{
...
static void update_sw_icl_max(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB_CDP:
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true, CDP_CURRENT_UA);
		break;
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	printk(KERN_ERR "%s: !!!fg_oplus_set_input_current[%d] wireless_present[%d]\n", __FUNCTION__,
	       fg_oplus_set_input_current, chg->wireless_present);
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data)
{
...
#ifndef OPLUS_FEATURE_CHG_BASIC
	if ((chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB) && (stat & APSD_DTC_STATUS_DONE_BIT) &&
	...
	smblib_handle_hvdcp_3p0_auth_done(chg, (bool)(stat & QC_AUTH_DONE_STATUS_BIT));

	smblib_handle_sdp_enumeration_done(chg, (bool)(stat & ENUMERATION_DONE_BIT));

	smblib_handle_slow_plugin_timeout(chg, (bool)(stat & SLOW_PLUGIN_TIMEOUT_BIT));
...
enum alarmtimer_restart smblib_lpd_recheck_timer(struct alarm *alarm, ktime_t time)
{
	union power_supply_propval pval;
	struct smb_charger *chg = container_of(alarm, struct smb_charger, lpd_recheck_timer);
	int rc;
	...
	if (chg->lpd_reason == LPD_MOISTURE_DETECTED) {
		...
		rc = smblib_masked_write(chg, TYPE_C_INTERRUPT_EN_CFG_2_REG, TYPEC_WATER_DETECTION_INT_EN_BIT,
...
static void typec_src_fault_condition_cfg(struct smb_charger *chg, bool src)
{
	...
	rc = smblib_masked_write(chg, OTG_FAULT_CONDITION_CFG_REG, mask, src ? 0 : mask);
	if (rc < 0)
...
static void typec_sink_insertion(struct smb_charger *chg)
{
	...
	rc = smblib_set_charge_param(chg, &chg->param.freq_switcher, chg->chg_freq.freq_above_otg_threshold);
	if (rc < 0)
		dev_err(chg->dev, "Error in setting freq_boost rc=%d\n", rc);
	...
	if (!chg->pr_swap_in_progress)
		chg->ok_to_pd = (!(chg->pd_disabled) || chg->early_usb_attach) && !chg->pd_not_supported;
}
...
static void typec_src_insertion(struct smb_charger *chg)
{
	...
	chg->ok_to_pd = (!(chg->typec_legacy || chg->pd_disabled) || chg->early_usb_attach) && !chg->pd_not_supported;
...
static void typec_sink_removal(struct smb_charger *chg)
{
	...
	rc = smblib_set_charge_param(chg, &chg->param.freq_switcher, chg->chg_freq.freq_removal);
	if (rc < 0)
...
static void typec_src_removal(struct smb_charger *chg)
{
	...
	if (!chg->pr_swap_in_progress) {
		...
		rc = smblib_set_charge_param(chg, &chg->param.aicl_cont_threshold, chg->default_aicl_cont_threshold_mv);
		if (rc < 0)
			smblib_err(chg, "Couldn't restore aicl_cont_threshold, rc=%d", rc);
	}
	/*
	 * if non-compliant charger caused UV, restore original max pulses
	 * and turn SUSPEND_ON_COLLAPSE_USBIN_BIT back on.
...
static void smblib_handle_rp_change(struct smb_charger *chg, int typec_mode)
{
	...
	if (apsd->pst == POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (get_client_vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER) <= USBIN_100MA ||
		    (chg->float_cfg & FLOAT_OPTIONS_MASK) == FORCE_FLOAT_SDP_CFG_BIT)
			return;
	}
...
static void smblib_lpd_launch_ra_open_work(struct smb_charger *chg)
{
	...
	if (!(stat & TYPEC_TCCDEBOUNCE_DONE_STATUS_BIT) && chg->lpd_stage == LPD_STAGE_NONE) {
		chg->lpd_stage = LPD_STAGE_FLOAT;
		cancel_delayed_work_sync(&chg->lpd_ra_open_work);
		vote(chg->awake_votable, LPD_VOTER, true, 0);
		schedule_delayed_work(&chg->lpd_ra_open_work, msecs_to_jiffies(300));
	}
}
...
irqreturn_t typec_state_change_irq_handler(int irq, void *data)
{
	...
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB) {
		smblib_dbg(chg, PR_INTERRUPT, "Ignoring for micro USB\n");
		return IRQ_HANDLED;
	}

	if (oplus_chg_get_voocphy_support() == NO_VOOCPHY && oplus_vooc_get_fastchg_to_normal() == true) {
	...
	if (!chg->wls_psy)
		chg->wls_psy = power_supply_get_by_name("wireless");
	if (chg->wls_psy) {
		rc = power_supply_get_property(chg->wls_psy, POWER_SUPPLY_PROP_PRESENT, &pval);
		if (rc < 0)
...
irqreturn_t typec_attach_detach_irq_handler(int irq, void *data)
{
	...
	if (attached) {
		...
		if (smblib_get_prop_dfp_mode(chg) == POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER) {
		...
		if (chg->lpd_stage == LPD_STAGE_FLOAT_CANCEL)
			schedule_delayed_work(&chg->lpd_detach_work, msecs_to_jiffies(1000));
	}
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, dcin_aicl_work);
	dcin_aicl(chg);
}

static enum alarmtimer_restart dcin_aicl_alarm_cb(struct alarm *alarm, ktime_t now)
{
	struct smb_charger *chg = container_of(alarm, struct smb_charger, dcin_aicl_alarm);
...
static bool is_cp_topo_vbatt(struct smb_charger *chg)
{
	...
	rc = power_supply_get_property(chg->cp_psy, POWER_SUPPLY_PROP_PARALLEL_OUTPUT_MODE, &pval);
	if (rc < 0)
		return false;
	...
	if (dcin_present && !vbus_present) {
		...
		if (chg->sec_cp_present) {
			...
			if (is_cp_topo_vbatt(chg))
				vote(chg->fcc_main_votable, WLS_PL_CHARGING_VOTER, true, 800000);
			...
			rc = smblib_select_sec_charger(chg, POWER_SUPPLY_CHARGER_SEC_CP, POWER_SUPPLY_CP_WIRELESS,
						       false);
			if (rc < 0)
				dev_err(chg->dev, "Couldn't enable secondary chargers  rc=%d\n", rc);
		} else {
		...
		if (chg->fcc_stepper_enable)
			vote(chg->fcc_votable, FCC_STEPPER_VOTER, true, 3000000);
		chg->last_wls_vout = 0;
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, bb_removal_work.work);
...
irqreturn_t switcher_power_ok_irq_handler(int irq, void *data)
{
	...
	if (is_storming(&irq_data->storm_data)) {
		...
		if (!is_client_vote_enabled(chg->usb_icl_votable, WEAK_CHARGER_VOTER)) {
			smblib_err(chg, "Weak charger detected: voting %dmA ICL\n", chg->weak_chg_icl_ua / 1000);
			vote(chg->usb_icl_votable, WEAK_CHARGER_VOTER, true, chg->weak_chg_icl_ua);
			/*
			 ...
			 */
			schedule_delayed_work(&chg->bb_removal_work, msecs_to_jiffies(BOOST_BACK_UNVOTE_DELAY_MS));
		}
#else
		if (printk_ratelimit())
			smblib_err(chg, "Reverse boost detected: voting 0mA to suspend input\n");
...
static void smblib_die_rst_icl_regulate(struct smb_charger *chg)
{
	...
	vote(chg->usb_icl_votable, DIE_TEMP_VOTER, temp & DIE_TEMP_RST_BIT, 500000);
}
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, usbov_dbc_work.work);
...
irqreturn_t usbin_ov_irq_handler(int irq, void *data)
{
	...
	if (stat & USBIN_OV_RT_STS_BIT) {
		...
		schedule_delayed_work(&chg->usbov_dbc_work, msecs_to_jiffies(USB_OV_DBC_PERIOD_MS));
	} else {
		cancel_delayed_work_sync(&chg->usbov_dbc_work);
		chg->dbc_usbov = false;
		vote(chg->awake_votable, USBOV_DBC_VOTER, false, 0);
...
int smblib_get_prop_pr_swap_in_progress(struct smb_charger *chg, union power_supply_propval *val)
{
...
int smblib_set_prop_pr_swap_in_progress(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc;
	...
	if (!chg->pr_swap_in_progress) {
		...
		schedule_delayed_work(&chg->pr_swap_detach_work, msecs_to_jiffies(DETACH_DETECT_DELAY_MS));
	}

	rc = smblib_masked_write(chg, TYPE_C_DEBOUNCE_OPTION_REG, REDUCE_TCCDEBOUNCE_TO_2MS_BIT,
	...
	if (!chg->wireless_present) {
#endif
		rc = smblib_masked_write(chg, TYPE_C_EXIT_STATE_CFG_REG, BYPASS_VSAFE0V_DURING_ROLE_SWAP_BIT,
	...
	if (chg->pr_swap_in_progress) {
		...
		orientation = stat & CC_ORIENTATION_BIT ? TYPEC_CCOUT_VALUE_BIT : 0;
		rc = smblib_masked_write(chg, TYPE_C_CCOUT_CONTROL_REG,
					 TYPEC_CCOUT_SRC_BIT | TYPEC_CCOUT_BUFFER_EN_BIT | TYPEC_CCOUT_VALUE_BIT,
					 TYPEC_CCOUT_SRC_BIT | TYPEC_CCOUT_BUFFER_EN_BIT | orientation);
		if (rc < 0) {
			smblib_err(chg, "Couldn't read TYPE_C_CCOUT_CONTROL_REG rc=%d\n", rc);
		}
	} else {
		rc = smblib_masked_write(chg, TYPE_C_CCOUT_CONTROL_REG, TYPEC_CCOUT_SRC_BIT, 0);
		if (rc < 0) {
			smblib_err(chg, "Couldn't read TYPE_C_CCOUT_CONTROL_REG rc=%d\n", rc);
		}
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, pr_lock_clear_work.work);
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, pr_swap_detach_work.work);
	int rc;
	u8 stat;
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, bms_update_work);
...
{
	union power_supply_propval prop_val;
	struct smb_charger *chg = container_of(work, struct smb_charger, pl_update_work);
	int rc;

	if (chg->smb_temp_max == -EINVAL) {
		rc = smblib_get_thermal_threshold(chg, SMB_REG_H_THRESHOLD_MSB_REG, &chg->smb_temp_max);
		if (rc < 0) {
	...
	rc = power_supply_set_property(chg->pl.psy, POWER_SUPPLY_PROP_CHARGER_TEMP_MAX, &prop_val);
	if (rc < 0) {
	...
	smblib_select_sec_charger(chg, POWER_SUPPLY_CHARGER_SEC_PL, POWER_SUPPLY_CP_NONE, false);
}

static void clear_hdc_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger, clear_hdc_work.work);
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, icl_change_work.work);
	int rc, settled_ua;
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, pl_enable_work.work);
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, thermal_regulation_work.work);
	int rc;
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, moisture_protection_work);
	int rc;
	bool usb_plugged_in;
	u8 stat;
	 ...
	 * moisture.
	 */
	if (stat == U_USB_STATUS_WATER_PRESENT || stat == U_USB_FMB1_BIT || stat == U_USB_FMB2_BIT ||
	    (usb_plugged_in && stat == U_USB_FLOAT1_BIT)) {
...
static enum alarmtimer_restart moisture_protection_alarm_cb(struct alarm *alarm, ktime_t now)
{
	struct smb_charger *chg = container_of(alarm, struct smb_charger, moisture_protection_alarm);
...
{
	union power_supply_propval pval;
	struct smb_charger *chg = container_of(work, struct smb_charger, chg_termination_work);
	...
	rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_CHARGE_FULL, &pval);
	if (rc < 0)
		goto out;
	...
	if (pval.intval != chg->charge_full_cc || !chg->cc_soc_ref) {
		...
		rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_CC_SOC, &pval);
		if (rc < 0)
			goto out;
	...
	smblib_dbg(chg, PR_MISC, "Chg Term WA readings: cc_soc: %d, cc_soc_ref: %d, delay: %d\n", pval.intval,
		   chg->cc_soc_ref, delay);
	alarm_start_relative(&chg->chg_termination_alarm, ms_to_ktime(delay));
out:
	vote(chg->awake_votable, CHG_TERMINATION_VOTER, false, 0);
}

static enum alarmtimer_restart chg_termination_alarm_cb(struct alarm *alarm, ktime_t now)
{
	struct smb_charger *chg = container_of(alarm, struct smb_charger, chg_termination_alarm);
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, jeita_update_work);
	...
	pnode = of_batterydata_get_best_profile(batt_node, val.intval / 1000, NULL);
	if (IS_ERR(pnode)) {
		rc = PTR_ERR(pnode);
	...
	rc = of_property_read_u32_array(pnode, "qcom,jeita-hard-thresholds", jeita_hard_thresholds, 2);
	if (!rc) {
		rc = smblib_update_jeita(chg, jeita_hard_thresholds, JEITA_HARD);
		if (rc < 0) {
	...
	rc = of_property_read_u32_array(pnode, "qcom,jeita-soft-thresholds", chg->jeita_soft_thlds, 2);
	if (!rc) {
		rc = smblib_update_jeita(chg, chg->jeita_soft_thlds, JEITA_SOFT);
		if (rc < 0) {
		...
		rc = of_property_read_u32_array(pnode, "qcom,jeita-soft-hys-thresholds", chg->jeita_soft_hys_thlds, 2);
		if (rc < 0) {
	...
	rc = of_property_read_u32_array(pnode, "qcom,jeita-soft-fcc-ua", tmp, 2);
	if (rc < 0) {
	...
	rc = of_property_read_u32_array(pnode, "qcom,jeita-soft-fv-uv", tmp, 2);
	if (rc < 0) {
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, lpd_ra_open_work.work);
	union power_supply_propval pval;
	u8 stat;
	int rc;
	...
	if (!(stat & TYPEC_WATER_DETECTION_STATUS_BIT) || (stat & TYPEC_TCCDEBOUNCE_DONE_STATUS_BIT)) {
		chg->lpd_stage = LPD_STAGE_NONE;
		goto out;
	}
	...
	if (smblib_rsbux_low(chg, RSBU_K_300K_UV)) {
		...
		rc = smblib_masked_write(chg, TYPE_C_INTERRUPT_EN_CFG_2_REG, TYPEC_WATER_DETECTION_INT_EN_BIT, 0);
		if (rc < 0) {
			smblib_err(chg, "Couldn't set TYPE_C_INTERRUPT_EN_CFG_2_REG rc=%d\n", rc);
			goto out;
		}
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, lpd_detach_work.work);
...
static void smblib_cp_status_change_work(struct work_struct *work)
{
	int rc;
	union power_supply_propval pval;
	struct smb_charger *chg = container_of(work, struct smb_charger, cp_status_change_work);
	...
	if (chg->cp_topo == -EINVAL) {
		...
		if (chg->cp_topo == POWER_SUPPLY_PL_OUTPUT_VBAT && chg->cp_reason == POWER_SUPPLY_CP_WIRELESS)
			vote(chg->fcc_main_votable, WLS_PL_CHARGING_VOTER, true, 800000);
	}
relax:
	pm_relax(chg->dev);
...
static void oplus_hvdcp_disable_work(struct work_struct *work)
{
	...
	u8 stat;
	bool vbus_rising;
	struct smb_charger *chg = container_of(work, struct smb_charger, hvdcp_disable_work.work);
	if (chg == NULL)
		return;
...
{
	struct smb_charger *chg = container_of(work, struct smb_charger, chg_monitor_work.work);
	struct oplus_chg_chip *chip = g_oplus_chip;
	int boot_mode = get_boot_mode();
	static int counts = 0;
	int rechg_vol;
	...
	if (counts >= (chip->batt_full ? 8 : 3)) { //because rechg counts=6
		...
		if (rc < 0) {
			printk(KERN_ERR
			       "oplus_chg_monitor_work: Couldn't get BATTERY_CHARGER_STATUS_8_REG status rc=%d\n",
			       rc);
			goto rerun_work;
		}
...
static int smblib_create_votables(struct smb_charger *chg)
{
	...
	if (chg->pl_enable_votable_indirect == NULL) {
		rc = -EINVAL;
		smblib_err(chg, "Couldn't find votable PL_ENABLE_INDIRECT rc=%d\n", rc);
		return rc;
	}
	...
	chg->smb_override_votable =
		create_votable("SMB_EN_OVERRIDE", VOTE_SET_ANY, smblib_smb_disable_override_vote_callback, chg);
	...
	chg->dc_suspend_votable = create_votable("DC_SUSPEND", VOTE_SET_ANY, smblib_dc_suspend_vote_callback, chg);
	...
	chg->awake_votable = create_votable("AWAKE", VOTE_SET_ANY, smblib_awake_vote_callback, chg);
	...
	chg->chg_disable_votable = create_votable("CHG_DISABLE", VOTE_SET_ANY, smblib_chg_disable_vote_callback, chg);
	...
	chg->limited_irq_disable_votable =
		create_votable("USB_LIMITED_IRQ_DISABLE", VOTE_SET_ANY, smblib_limited_irq_disable_vote_callback, chg);
	...
	chg->hdc_irq_disable_votable =
		create_votable("USB_HDC_IRQ_DISABLE", VOTE_SET_ANY, smblib_hdc_irq_disable_vote_callback, chg);
	...
	chg->icl_irq_disable_votable =
		create_votable("USB_ICL_IRQ_DISABLE", VOTE_SET_ANY, smblib_icl_irq_disable_vote_callback, chg);
	...
	chg->temp_change_irq_disable_votable = create_votable("TEMP_CHANGE_IRQ_DISABLE", VOTE_SET_ANY,
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->thermal_regulation_work, smblib_thermal_regulation_work);
	INIT_DELAYED_WORK(&chg->usbov_dbc_work, smblib_usbov_dbc_work);
	INIT_DELAYED_WORK(&chg->pr_swap_detach_work, smblib_pr_swap_detach_work);
	...
	if (chg->wa_flags & CHG_TERMINATION_WA) {
		INIT_WORK(&chg->chg_termination_work, smblib_chg_termination_work);

		if (alarmtimer_get_rtcdev()) {
			alarm_init(&chg->chg_termination_alarm, ALARM_BOOTTIME, chg_termination_alarm_cb);
		} else {
	...
	if (chg->uusb_moisture_protection_enabled) {
		INIT_WORK(&chg->moisture_protection_work, smblib_moisture_protection_work);

		if (alarmtimer_get_rtcdev()) {
			alarm_init(&chg->moisture_protection_alarm, ALARM_BOOTTIME, moisture_protection_alarm_cb);
		} else {
	...
	if (alarmtimer_get_rtcdev()) {
		alarm_init(&chg->dcin_aicl_alarm, ALARM_REALTIME, dcin_aicl_alarm_cb);
	} else {
	...
	case PARALLEL_MASTER:
		...
		rc = qcom_step_chg_init(chg->dev, chg->step_chg_enabled, chg->sw_jeita_enabled, false);
		if (rc < 0) {
		...
		if (chg->sec_pl_present) {
			...
			if (chg->pl.psy) {
				if (chg->sec_chg_selected != POWER_SUPPLY_CHARGER_SEC_CP) {
					rc = smblib_select_sec_charger(chg, POWER_SUPPLY_CHARGER_SEC_PL,
				...
				prop_val.intval = chg->smb_temp_max;
				rc = power_supply_set_property(chg->pl.psy, POWER_SUPPLY_PROP_CHARGER_TEMP_MAX,
							       &prop_val);
				if (rc < 0) {
					dev_err(chg->dev, "Couldn't set POWER_SUPPLY_PROP_CHARGER_TEMP_MAX rc=%d\n",
						rc);
		...
		if (rc < 0) {
			smblib_err(chg, "Couldn't register notifier rc=%d\n", rc);
			return rc;
		}
		break;
	case PARALLEL_SLAVE:
...
static int oplus_shortc_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chip->normalchg_gpio.shortc_active = pinctrl_lookup_state(chip->normalchg_gpio.pinctrl, "shortc_active");
...
static int oplus_shipmode_id_gpio_init(struct oplus_chg_chip *chip)
{
	...
	chg->shipmode_id_active = pinctrl_lookup_state(chg->shipmode_id_pinctrl, "shipmode_id_active");
...
static void oplus_ccdetect_irq_register(struct oplus_chg_chip *chip)
{
	...
	ret = devm_request_threaded_irq(chip->dev, chg->ccdetect_irq, NULL, oplus_ccdetect_change_handler,
					IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT, "ccdetect-change",
					chip);
	if (ret < 0) {
...
static void oplus_usbtemp_thread_init(void)
{
	...
	oplus_usbtemp_kthread = kthread_run(oplus_usbtemp_monitor_common, g_oplus_chip, "usbtemp_kthread");
...
void oplus_wake_up_usbtemp_thread(void)
{
	...
	if (chg && chg->adc_check_pulse) {
		...
		if (val != 0) {
			rc = smblib_masked_write(chg, ADC_CHECK_HIGH_LEVEL_3, BIT(7), 0);
			if (rc < 0) {
				dev_err(chg->dev, "Couldn't configure 0x3546 rc=%d\n", rc);
			}
		}
	...
	if (g_oplus_chip) {
		...
		if (g_oplus_chip->normalchg_gpio.chargerid_switch_gpio_pvt <= 0) {
			...
			if (gpio_is_valid(g_oplus_chip->normalchg_gpio.chargerid_switch_gpio_pvt)) {
				rc = gpio_request(g_oplus_chip->normalchg_gpio.chargerid_switch_gpio_pvt,
						  "chargerid_switch-gpio-pvt");
				if (rc) {
		...
		if (g_oplus_chip->normalchg_gpio.chargerid_switch_gpio <= 0) {
			...
			if (gpio_is_valid(g_oplus_chip->normalchg_gpio.chargerid_switch_gpio)) {
				rc = gpio_request(g_oplus_chip->normalchg_gpio.chargerid_switch_gpio,
						  "chargerid-switch-gpio");
				if (rc) {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (g_oplus_chip) {
		g_oplus_chip->normalchg_gpio.ship_gpio = of_get_named_gpio(node, "qcom,ship-gpio", 0);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (g_oplus_chip) {
		g_oplus_chip->normalchg_gpio.shortc_gpio = of_get_named_gpio(node, "qcom,shortc-gpio", 0);
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (g_oplus_chip) {
		chg->shipmode_id_gpio = of_get_named_gpio(node, "qcom,shipmode-id-gpio", 0);
...
#endif
static ssize_t pd_disabled_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t pd_disabled_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int val;
	struct smb5 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;
...
static ssize_t weak_chg_icl_ua_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct smb5 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;
...
static ssize_t weak_chg_icl_ua_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int val;
	struct smb5 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;
...
static int smb5_chg_config_init(struct smb5 *chip)
{
	...
	case PMI632_SUBTYPE:
		...
		chg->wa_flags |=
			WEAK_ADAPTER_WA | USBIN_OV_WA | CHG_TERMINATION_WA | USBIN_ADC_WA | SKIP_MISC_PBS_IRQ_WA;
		...
		chg->hw_max_icl_ua = (chip->dt.usb_icl_ua > 0) ? chip->dt.usb_icl_ua : PMI632_MAX_ICL_UA;
		break;
	default:
...
static int smb5_configure_internal_pull(struct smb_charger *chg, int type, int pull)
{
	int rc;
	int shift = type * 2;
	...
	rc = smblib_masked_write(chg, BATIF_ADC_INTERNAL_PULL_UP_REG, mask, val);
	if (rc < 0)
		dev_err(chg->dev, "Couldn't configure ADC pull-up reg rc=%d\n", rc);
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	of_property_read_u32(node, "qcom,sec-charger-config", &chip->dt.sec_charger_config);
	chg->sec_cp_present = chip->dt.sec_charger_config == POWER_SUPPLY_CHARGER_SEC_CP ||
			      chip->dt.sec_charger_config == POWER_SUPPLY_CHARGER_SEC_CP_PL;

	chg->sec_pl_present = chip->dt.sec_charger_config == POWER_SUPPLY_CHARGER_SEC_PL ||
			      chip->dt.sec_charger_config == POWER_SUPPLY_CHARGER_SEC_CP_PL;

	chg->soc_ready_check = of_property_read_bool(node, "qcom,soc-ready-check");

	chg->step_chg_enabled = of_property_read_bool(node, "qcom,step-charging-enable");

	chg->typec_legacy_use_rp_icl = of_property_read_bool(node, "qcom,typec-legacy-rp-icl");

	chg->sw_jeita_enabled = of_property_read_bool(node, "qcom,sw-jeita-enable");

	chg->pd_not_supported = chg->pd_not_supported || of_property_read_bool(node, "qcom,usb-pd-disable");

	chg->lpd_disabled = of_property_read_bool(node, "qcom,lpd-disable");

	rc = of_property_read_u32(node, "qcom,wd-bark-time-secs", &chip->dt.wd_bark_time);
	if (rc < 0 || chip->dt.wd_bark_time < MIN_WD_BARK_TIME)
		chip->dt.wd_bark_time = DEFAULT_WD_BARK_TIME;

	rc = of_property_read_u32(node, "qcom,wd-snarl-time-config", &chip->dt.wd_snarl_time_cfg);
	if (rc < 0)
		chip->dt.wd_snarl_time_cfg = DEFAULT_WD_SNARL_TIME_8S;

	chip->dt.no_battery = of_property_read_bool(node, "qcom,batteryless-platform");

	if (of_find_property(node, "qcom,thermal-mitigation", &byte_len)) {
		chg->thermal_mitigation = devm_kzalloc(chg->dev, byte_len, GFP_KERNEL);

		if (chg->thermal_mitigation == NULL)
			return -ENOMEM;

		chg->thermal_levels = byte_len / sizeof(u32);
		rc = of_property_read_u32_array(node, "qcom,thermal-mitigation", chg->thermal_mitigation,
						chg->thermal_levels);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't read threm limits rc = %d\n", rc);
			return rc;
		}
	}

	rc = of_property_read_u32(node, "qcom,charger-temp-max", &chg->charger_temp_max);
	if (rc < 0)
		chg->charger_temp_max = -EINVAL;

	rc = of_property_read_u32(node, "qcom,smb-temp-max", &chg->smb_temp_max);
	if (rc < 0)
	...
	chip->dt.hvdcp_disable = of_property_read_bool(node, "qcom,hvdcp-disable");
	chg->hvdcp_disable = chip->dt.hvdcp_disable;

	chip->dt.hvdcp_autonomous = of_property_read_bool(node, "qcom,hvdcp-autonomous-enable");

	chip->dt.auto_recharge_soc = -EINVAL;
	rc = of_property_read_u32(node, "qcom,auto-recharge-soc", &chip->dt.auto_recharge_soc);
	if (!rc && (chip->dt.auto_recharge_soc < 0 || chip->dt.auto_recharge_soc > 100)) {
		pr_err("qcom,auto-recharge-soc is incorrect\n");
		return -EINVAL;
	}
	chg->auto_recharge_soc = chip->dt.auto_recharge_soc;

	chg->suspend_input_on_debug_batt = of_property_read_bool(node, "qcom,suspend-input-on-debug-batt");

	rc = of_property_read_u32(node, "qcom,otg-deglitch-time-ms", &chg->otg_delay_ms);
	if (rc < 0)
		chg->otg_delay_ms = OTG_DEFAULT_DEGLITCH_TIME_MS;

	chg->fcc_stepper_enable = of_property_read_bool(node, "qcom,fcc-stepping-enable");
	chg->adc_check_pulse = of_property_read_bool(node, "qcom,adc-check-pulse");
	chg->low_voltage_charger = of_property_read_bool(node, "op,low-voltage-charger");

	if (chg->uusb_moisture_protection_capable)
		chg->uusb_moisture_protection_enabled =
			of_property_read_bool(node, "qcom,uusb-moisture-protection-enable");

	chg->hw_die_temp_mitigation = of_property_read_bool(node, "qcom,hw-die-temp-mitigation");

	chg->hw_connector_mitigation = of_property_read_bool(node, "qcom,hw-connector-mitigation");

	chg->hw_skin_temp_mitigation = of_property_read_bool(node, "qcom,hw-skin-temp-mitigation");

	chg->en_skin_therm_mitigation = of_property_read_bool(node, "qcom,en-skin-therm-mitigation");

	chg->connector_pull_up = -EINVAL;
	of_property_read_u32(node, "qcom,connector-internal-pull-kohm", &chg->connector_pull_up);

	chip->dt.disable_suspend_on_collapse = of_property_read_bool(node, "qcom,disable-suspend-on-collapse");
	chg->smb_pull_up = -EINVAL;
	of_property_read_u32(node, "qcom,smb-internal-pull-kohm", &chg->smb_pull_up);

	chip->dt.adc_based_aicl = of_property_read_bool(node, "qcom,adc-based-aicl");

	of_property_read_u32(node, "qcom,fcc-step-delay-ms", &chg->chg_param.fcc_step_delay_ms);
	if (chg->chg_param.fcc_step_delay_ms <= 0)
		chg->chg_param.fcc_step_delay_ms = DEFAULT_FCC_STEP_UPDATE_DELAY_MS;

	of_property_read_u32(node, "qcom,fcc-step-size-ua", &chg->chg_param.fcc_step_size_ua);
	 ...
	 * with HVDCP3 adapter.
	 */
	chg->hvdcp3_standalone_config = of_property_read_bool(node, "qcom,hvdcp3-standalone-config");

	of_property_read_u32(node, "qcom,hvdcp3-max-icl-ua", &chg->chg_param.hvdcp3_max_icl_ua);
...
static int smb5_parse_dt_adc_channels(struct smb_charger *chg)
{
	...
	rc = smblib_get_iio_channel(chg, "usb_in_voltage", &chg->iio.usbin_v_chan);
	if (rc < 0)
		return rc;
	...
	rc = smblib_get_iio_channel(chg, "usb_in_current", &chg->iio.usbin_i_chan);
	if (rc < 0)
		return rc;
	...
	rc = smblib_get_iio_channel(chg, "conn_temp", &chg->iio.connector_temp_chan);
	if (rc < 0)
		return rc;
...
static int smb5_parse_dt_currents(struct smb5 *chip, struct device_node *node)
{
	...
	rc = of_property_read_u32(node, "qcom,fcc-max-ua", &chip->dt.batt_profile_fcc_ua);
	if (rc < 0)
...
#endif
	rc = of_property_read_u32(node, "qcom,otg-cl-ua", &chg->otg_cl_ua);
	if (rc < 0)
		chg->otg_cl_ua = (chip->chg.chg_param.smb_version == PMI632_SUBTYPE) ? MICRO_1PA : MICRO_3PA;
	...
	if (chip->dt.term_current_src == ITERM_SRC_ADC)
		rc = of_property_read_u32(node, "qcom,chg-term-base-current-ma", &chip->dt.term_current_thresh_lo_ma);

	rc = of_property_read_u32(node, "qcom,chg-term-current-ma", &chip->dt.term_current_thresh_hi_ma);

	chg->wls_icl_ua = DCIN_ICL_MAX_UA;
	rc = of_property_read_u32(node, "qcom,wls-current-max-ua", &tmp);
...
static int smb5_parse_dt_voltages(struct smb5 *chip, struct device_node *node)
{
	...
	rc = of_property_read_u32(node, "qcom,fv-max-uv", &chip->dt.batt_profile_fv_uv);
	if (rc < 0)
		chip->dt.batt_profile_fv_uv = -EINVAL;

	rc = of_property_read_u32(node, "qcom,chg-inhibit-threshold-mv", &chip->dt.chg_inhibit_thr_mv);
	if (!rc && (chip->dt.chg_inhibit_thr_mv < 0 || chip->dt.chg_inhibit_thr_mv > 300)) {
		pr_err("qcom,chg-inhibit-threshold-mv is incorrect\n");
		return -EINVAL;
	}

	chip->dt.auto_recharge_vbat_mv = -EINVAL;
	rc = of_property_read_u32(node, "qcom,auto-recharge-vbat-mv", &chip->dt.auto_recharge_vbat_mv);
...
static int smb5_set_prop_comp_clamp_level(struct smb_charger *chg, const union power_supply_propval *val)
{
	int rc = 0, i;
	...
	for (i = 0; i < ARRAY_SIZE(clamp_config.reg); i++) {
		rc = smblib_write(chg, clamp_levels[level].reg[i], clamp_levels[level].val[i]);
		if (rc < 0)
			dev_err(chg->dev, "Failed to configure comp clamp settings for reg=0x%04x rc=%d\n",
...
static int smb5_usb_get_prop(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if ((chg->real_charger_type == POWER_SUPPLY_TYPE_USB_PD) && (opluschg_pd_sdp == true)) {
			val->intval = 1;
			break;
		}
#endif
	...
	case POWER_SUPPLY_PROP_SDP_CURRENT_MAX:
		val->intval = get_client_vote(chg->usb_icl_votable, USB_PSY_VOTER);
		break;
	...
	case POWER_SUPPLY_PROP_QC_OPTI_DISABLE:
		if (chg->hw_die_temp_mitigation)
			val->intval = POWER_SUPPLY_QC_THERMAL_BALANCE_DISABLE | POWER_SUPPLY_QC_INOV_THERMAL_DISABLE;
	...
	case POWER_SUPPLY_PROP_THERM_ICL_LIMIT:
		val->intval = get_client_vote(chg->usb_icl_votable, THERMAL_THROTTLE_VOTER);
		break;
...
static int smb5_usb_set_prop(struct power_supply *psy, enum power_supply_property psp,
			     const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BOOST_CURRENT:
		...
		break;
	case POWER_SUPPLY_PROP_CTM_CURRENT_MAX:
		rc = vote(chg->usb_icl_votable, CTM_VOTER, val->intval >= 0, val->intval);
		break;
	case POWER_SUPPLY_PROP_PR_SWAP:
	...
	case POWER_SUPPLY_PROP_CONNECTOR_HEALTH:
		...
		break;
	case POWER_SUPPLY_PROP_THERM_ICL_LIMIT:
		if (!is_client_vote_enabled(chg->usb_icl_votable, THERMAL_THROTTLE_VOTER)) {
			chg->init_thermal_ua = get_effective_result(chg->usb_icl_votable);
			icl = chg->init_thermal_ua + val->intval;
		} else {
			icl = get_client_vote(chg->usb_icl_votable, THERMAL_THROTTLE_VOTER) + val->intval;
		}

		if (icl >= MIN_THERMAL_VOTE_UA)
			rc = vote(chg->usb_icl_votable, THERMAL_THROTTLE_VOTER,
				  (icl != chg->init_thermal_ua) ? true : false, icl);
		else
			rc = -EINVAL;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_LIMIT:
...
static int smb5_usb_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_CTM_CURRENT_MAX:
#ifdef OPLUS_CUSTOM_OP_DEF
...
static int smb5_usb_port_get_prop(struct power_supply *psy, enum power_supply_property psp,
				  union power_supply_propval *val)
{
	struct smb5 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
...
static int smb5_usb_port_set_prop(struct power_supply *psy, enum power_supply_property psp,
				  const union power_supply_propval *val)
{
	int rc = 0;
...
static int smb5_init_usb_port_psy(struct smb5 *chip)
{
	...
	chg->usb_port_psy = devm_power_supply_register(chg->dev, &usb_port_psy_desc, &usb_port_cfg);
...
static int smb5_usb_main_get_prop(struct power_supply *psy, enum power_supply_property psp,
				  union power_supply_propval *val)
{
	struct smb5 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
...
static int smb5_usb_main_set_prop(struct power_supply *psy, enum power_supply_property psp,
				  const union power_supply_propval *val)
{
	struct smb5 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_FLASH_ACTIVE:
		if ((chg->chg_param.smb_version == PMI632_SUBTYPE) && (chg->flash_active != val->intval)) {
			...
			if (pval.intval) {
				rc = smblib_force_vbus_voltage(chg, chg->flash_active ? FORCE_5V_BIT : IDLE_BIT);
				if (rc < 0)
					pr_err("Failed to force 5V\n");
				else
					chg->pulse_cnt = 0;
	...
	case POWER_SUPPLY_PROP_MAIN_FCC_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_FORCE_MAIN_FCC:
		vote_override(chg->fcc_main_votable, CC_MODE_VOTER, (val->intval < 0) ? false : true, val->intval);
		/* Main FCC updated re-calculate FCC */
		rerun_election(chg->fcc_votable);
		break;
	case POWER_SUPPLY_PROP_FORCE_MAIN_ICL:
		vote_override(chg->usb_icl_votable, CC_MODE_VOTER, (val->intval < 0) ? false : true, val->intval);
...
static int smb5_usb_main_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	int rc;
...
static int smb5_init_usb_main_psy(struct smb5 *chip)
{
	...
	chg->usb_main_psy = devm_power_supply_register(chg->dev, &usb_main_psy_desc, &usb_main_cfg);
...
static int smb5_dc_get_prop(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	struct smb5 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
	int rc = 0;
...
static int smb5_dc_set_prop(struct power_supply *psy, enum power_supply_property psp,
			    const union power_supply_propval *val)
{
	struct smb5 *chip = power_supply_get_drvdata(psy);
	struct smb_charger *chg = &chip->chg;
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
		rc = vote(chg->dc_suspend_votable, WBC_VOTER, (bool)val->intval, 0);
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
...
static int smb5_dc_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION:
	case POWER_SUPPLY_PROP_CURRENT_MAX:
...
static int smb5_init_dc_psy(struct smb5 *chip)
{
	...
	chg->dc_psy = devm_power_supply_register(chg->dev, &dc_psy_desc, &dc_cfg);
...
static int smb5_init_ac_psy(struct smb5 *chip)
{
	...
	chg->ac_psy = devm_power_supply_register(chg->dev, &ac_psy_desc, &ac_cfg);
...
static int smb5_batt_get_prop(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_QNOVO:
		val->intval = get_client_vote_locked(chg->fv_votable, QNOVO_VOTER);
		break;
#ifndef OPLUS_FEATURE_CHG_BASIC
	...
	case POWER_SUPPLY_PROP_CURRENT_QNOVO:
		val->intval = get_client_vote_locked(chg->fcc_votable, QNOVO_VOTER);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
	...
	case POWER_SUPPLY_PROP_PARALLEL_DISABLE:
		val->intval = get_client_vote(chg->pl_disable_votable, USER_VOTER);
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_CHARGE_COUNTER, val);
#ifdef OPLUS_FEATURE_CHG_BASIC
		if (g_oplus_chip) {
...
static int smb5_batt_set_prop(struct power_supply *psy, enum power_supply_property prop,
			      const union power_supply_propval *val)
{
	int rc = 0;
	struct smb_charger *chg = power_supply_get_drvdata(psy);
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_QNOVO:
		vote(chg->fv_votable, QNOVO_VOTER, (val->intval >= 0), val->intval);
		break;
	case POWER_SUPPLY_PROP_STEP_CHARGING_ENABLED:
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_CURRENT_QNOVO:
		vote(chg->pl_disable_votable, PL_QNOVO_VOTER, val->intval != -EINVAL && val->intval < 2000000, 0);
	...
	case POWER_SUPPLY_PROP_SET_SHIP_MODE:
		...
		if (!val->intval)
			break;
		if (chg->pl.psy)
			power_supply_set_property(chg->pl.psy, POWER_SUPPLY_PROP_SET_SHIP_MODE, val);
...
static int smb5_batt_prop_is_writeable(struct power_supply *psy, enum power_supply_property psp)
{
	/*oplus own battery props*/
	int rc = 0;
...
static int smb5_init_vbus_regulator(struct smb5 *chip)
{
	...
	chg->vbus_vreg = devm_kzalloc(chg->dev, sizeof(*chg->vbus_vreg), GFP_KERNEL);
	if (!chg->vbus_vreg)
		return -ENOMEM;
	...
	chg->vbus_vreg->rdev = devm_regulator_register(chg->dev, &chg->vbus_vreg->rdesc, &cfg);
...
static int smb5_init_vconn_regulator(struct smb5 *chip)
{
	...
	chg->vconn_vreg = devm_kzalloc(chg->dev, sizeof(*chg->vconn_vreg), GFP_KERNEL);
	if (!chg->vconn_vreg)
		return -ENOMEM;
	...
	chg->vconn_vreg->rdev = devm_regulator_register(chg->dev, &chg->vconn_vreg->rdesc, &cfg);
...
static int smb5_configure_typec(struct smb_charger *chg)
{
	...
	rc = smblib_write(chg, TYPE_C_INTERRUPT_EN_CFG_2_REG, TYPEC_SRC_BATT_HPWR_INT_EN_BIT | val);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure Type-C interrupts rc=%d\n", rc);
		return rc;
	}
	...
	rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, EN_TRY_SNK_BIT | EN_SNK_ONLY_BIT, EN_TRY_SNK_BIT);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure TYPE_C_MODE_CFG_REG rc=%d\n", rc);
		return rc;
	}
	...
	if (!chg->pd_not_supported) {
		rc = smblib_masked_write(chg, TYPE_C_VCONN_CONTROL_REG, VCONN_EN_SRC_BIT | VCONN_EN_VALUE_BIT,
					 VCONN_EN_SRC_BIT);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't configure VCONN for SW control rc=%d\n", rc);
			return rc;
		}
	}

	/* Enable detection of unoriented debug accessory in source mode */
	rc = smblib_masked_write(chg, DEBUG_ACCESS_SRC_CFG_REG, EN_UNORIENTED_DEBUG_ACCESS_SRC_BIT,
				 EN_UNORIENTED_DEBUG_ACCESS_SRC_BIT);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure TYPE_C_DEBUG_ACCESS_SRC_CFG_REG rc=%d\n", rc);
		return rc;
	}

	if (chg->chg_param.smb_version != PMI632_SUBTYPE) {
		rc = smblib_masked_write(chg, USBIN_LOAD_CFG_REG,
					 USBIN_IN_COLLAPSE_GF_SEL_MASK | USBIN_AICL_STEP_TIMING_SEL_MASK, 0);
		if (rc < 0) {
	...
	rc = smblib_masked_write(chg, TYPE_C_EXIT_STATE_CFG_REG, SEL_SRC_UPPER_REF_BIT, SEL_SRC_UPPER_REF_BIT);
	if (rc < 0)
		dev_err(chg->dev, "Couldn't configure CC threshold voltage rc=%d\n", rc);
#ifdef OPLUS_FEATURE_CHG_BASIC
	//smblib_write(chg, 0x1152, 0x02);
	rc = smblib_masked_write(chg, DCDC_OTG_CURRENT_LIMIT_CFG_REG, DCDC_OTG_CURRENT_LIMIT_1000MA_BIT,
				 DCDC_OTG_CURRENT_LIMIT_1000MA_BIT);
	if (rc < 0)
		dev_err(chg->dev, "Couldn't DCDC_OTG_CURRENT_LIMIT_CFG_REG rc=%d\n", rc);
#endif /* OPLUS_FEATURE_CHG_BASIC */
#ifdef OPLUS_FEATURE_CHG_BASIC
...
static int smb5_configure_micro_usb(struct smb_charger *chg)
{
	...
	rc = smblib_masked_write(chg, TYPE_C_INTERRUPT_EN_CFG_2_REG, MICRO_USB_STATE_CHANGE_INT_EN_BIT,
...
#define RAW_ITERM(iterm_ma, max_range) div_s64((int64_t)iterm_ma *ADC_CHG_ITERM_MASK, max_range)
static int smb5_configure_iterm_thresholds_adc(struct smb5 *chip)
{
	...
	if (chip->dt.term_current_thresh_hi_ma) {
		raw_hi_thresh = RAW_ITERM(chip->dt.term_current_thresh_hi_ma, max_limit_ma);
		raw_hi_thresh = sign_extend32(raw_hi_thresh, 15);
		buf = (u8 *)&raw_hi_thresh;
		raw_hi_thresh = buf[1] | (buf[0] << 8);

		rc = smblib_batch_write(chg, CHGR_ADC_ITERM_UP_THD_MSB_REG, (u8 *)&raw_hi_thresh, 2);
		if (rc < 0) {
	...
	if (chip->dt.term_current_thresh_lo_ma) {
		raw_lo_thresh = RAW_ITERM(chip->dt.term_current_thresh_lo_ma, max_limit_ma);
		raw_lo_thresh = sign_extend32(raw_lo_thresh, 15);
		buf = (u8 *)&raw_lo_thresh;
		raw_lo_thresh = buf[1] | (buf[0] << 8);

		rc = smblib_batch_write(chg, CHGR_ADC_ITERM_LO_THD_MSB_REG, (u8 *)&raw_lo_thresh, 2);
		if (rc < 0) {
...
static int smb5_configure_iterm_thresholds(struct smb5 *chip)
{
	...
	case ITERM_SRC_ADC:
		if (chip->chg.chg_param.smb_version == PM8150B_SUBTYPE) {
			rc = smblib_masked_write(chg, CHGR_ADC_TERM_CFG_REG, TERM_BASED_ON_SYNC_CONV_OR_SAMPLE_CNT,
...
static int smb5_configure_mitigation(struct smb_charger *chg)
{
	...
	if (!chg->hw_die_temp_mitigation && !chg->hw_connector_mitigation && !chg->hw_skin_temp_mitigation) {
		src_cfg = THERMREG_SW_ICL_ADJUST_BIT;
	} else {
		if (chg->hw_die_temp_mitigation) {
			chan = DIE_TEMP_CHANNEL_EN_BIT;
			src_cfg = THERMREG_DIE_ADC_SRC_EN_BIT | THERMREG_DIE_CMP_SRC_EN_BIT;
		}
		...
		rc = smblib_masked_write(chg, BATIF_ADC_CHANNEL_EN_REG,
					 CONN_THM_CHANNEL_EN_BIT | DIE_TEMP_CHANNEL_EN_BIT | MISC_THM_CHANNEL_EN_BIT,
					 chan);
		if (rc < 0) {
	...
	rc = smblib_masked_write(chg, MISC_THERMREG_SRC_CFG_REG,
				 THERMREG_SW_ICL_ADJUST_BIT | THERMREG_DIE_ADC_SRC_EN_BIT |
					 THERMREG_DIE_CMP_SRC_EN_BIT | THERMREG_SKIN_ADC_SRC_EN_BIT | SKIN_ADC_CFG_BIT |
					 THERMREG_CONNECTOR_ADC_SRC_EN_BIT,
				 src_cfg);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure THERM_SRC reg rc=%d\n", rc);
		return rc;
	}
...
static int smb5_init_dc_peripheral(struct smb_charger *chg)
{
	...
	rc = smblib_masked_write(chg, DCIN_LOAD_CFG_REG, INPUT_MISS_POLL_EN_BIT, 0);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't disable DC Input missing poller rc=%d\n", rc);
		return rc;
	}
...
static int smb5_configure_recharging(struct smb5 *chip)
{
	...
	rc = smblib_masked_write(chg, CHGR_CFG2_REG, RECHG_MASK,
				 (chip->dt.auto_recharge_vbat_mv != -EINVAL) ? VBAT_BASED_RECHG_BIT : 0);
	if (rc < 0) {
	...
	if (chip->dt.auto_recharge_vbat_mv != -EINVAL) {
		...
		temp = ((temp & 0xFF00) >> 8) | ((temp & 0xFF) << 8);
		rc = smblib_batch_write(chg, CHGR_ADC_RECHARGE_THRESHOLD_MSB_REG, (u8 *)&temp, 2);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't configure ADC_RECHARGE_THRESHOLD REG rc=%d\n", rc);
			return rc;
		}
		/* Program the sample count for VBAT based recharge to 3 */
		rc = smblib_masked_write(chg, CHGR_NO_SAMPLE_TERM_RCHG_CFG_REG, NO_OF_SAMPLE_FOR_RCHG,
	...
	rc = smblib_masked_write(chg, CHGR_CFG2_REG, RECHG_MASK,
				 (chip->dt.auto_recharge_soc != -EINVAL) ? SOC_BASED_RECHG_BIT : VBAT_BASED_RECHG_BIT);
	if (rc < 0) {
	...
	if (chip->dt.auto_recharge_soc != -EINVAL) {
		...
		rc = smblib_masked_write(chg, CHGR_NO_SAMPLE_TERM_RCHG_CFG_REG, NO_OF_SAMPLE_FOR_RCHG, 0);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't configure CHGR_NO_SAMPLE_FOR_TERM_RCHG_CFG rc=%d\n", rc);
			return rc;
		}
...
static int smb5_configure_float_charger(struct smb5 *chip)
{
	...
	rc = smblib_masked_write(chg, USBIN_OPTIONS_2_CFG_REG, FLOAT_OPTIONS_MASK | DCD_TIMEOUT_SEL_BIT, val);
	if (rc < 0) {
...
static int smb5_init_connector_type(struct smb_charger *chg)
{
	...
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure TypeC/micro-USB mode rc=%d\n", rc);
		return rc;
	}
...
static void otg_enable_pmic_id_value(void)
{
	...
	/* set DRP mode */
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, TYPEC_POWER_ROLE_CMD_MASK, 0x0); //bit[2:0]=0
	if (rc < 0) {
...
static void otg_disable_pmic_id_value(void)
{
	...
	/* set sink mode only */
	rc = smblib_masked_write(chg, TYPE_C_INTRPT_ENB_SOFTWARE_CTRL_REG, TYPEC_POWER_ROLE_CMD_MASK,
				 UFP_EN_CMD_BIT); //bit[2:0]=0x4
	if (rc < 0) {
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (chip->dt.batt_profile_fcc_ua < 0)
		smblib_get_charge_param(chg, &chg->param.fcc, &chg->batt_profile_fcc_ua);

	if (chip->dt.batt_profile_fv_uv < 0)
		smblib_get_charge_param(chg, &chg->param.fv, &chg->batt_profile_fv_uv);

	smblib_get_charge_param(chg, &chg->param.usb_icl, &chg->default_icl_ua);
	smblib_get_charge_param(chg, &chg->param.aicl_5v_threshold, &chg->default_aicl_5v_threshold_mv);
	chg->aicl_5v_threshold_mv = chg->default_aicl_5v_threshold_mv;
	smblib_get_charge_param(chg, &chg->param.aicl_cont_threshold, &chg->default_aicl_cont_threshold_mv);
	chg->aicl_cont_threshold_mv = chg->default_aicl_cont_threshold_mv;

	if (chg->charger_temp_max == -EINVAL) {
		rc = smblib_get_thermal_threshold(chg, DIE_REG_H_THRESHOLD_MSB_REG, &chg->charger_temp_max);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't get charger_temp_max rc=%d\n", rc);
			return rc;
		}
	...
	if (chg->wa_flags & SW_THERM_REGULATION_WA) {
		rc = smblib_write(chg, MISC_THERMREG_SRC_CFG_REG,
				  THERMREG_SW_ICL_ADJUST_BIT | THERMREG_DIE_CMP_SRC_EN_BIT);
		if (rc < 0) {
	...
	vote(chg->usb_icl_votable, DEFAULT_VOTER, chip->dt.no_battery, 0);
	vote(chg->dc_suspend_votable, DEFAULT_VOTER, chip->dt.no_battery, 0);
	vote(chg->fcc_votable, HW_LIMIT_VOTER, chip->dt.batt_profile_fcc_ua > 0, chip->dt.batt_profile_fcc_ua);
	vote(chg->fv_votable, HW_LIMIT_VOTER, chip->dt.batt_profile_fv_uv > 0, chip->dt.batt_profile_fv_uv);
	vote(chg->fcc_votable, BATT_PROFILE_VOTER, chg->batt_profile_fcc_ua > 0, chg->batt_profile_fcc_ua);
	vote(chg->fv_votable, BATT_PROFILE_VOTER, chg->batt_profile_fv_uv > 0, chg->batt_profile_fv_uv);

	/* Some h/w limit maximum supported ICL */
	vote(chg->usb_icl_votable, HW_LIMIT_VOTER, chg->hw_max_icl_ua > 0, chg->hw_max_icl_ua);
	 ...
	 */
#ifndef OPLUS_FEATURE_CHG_BASIC
	mask = USBIN_AICL_PERIODIC_RERUN_EN_BIT | USBIN_AICL_ADC_EN_BIT | USBIN_AICL_EN_BIT |
	       SUSPEND_ON_COLLAPSE_USBIN_BIT;
	...
	rc = smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG, mask, val);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't config AICL rc=%d\n", rc);
		return rc;
	}

	rc = smblib_write(chg, AICL_RERUN_TIME_CFG_REG, AICL_RERUN_TIME_12S_VAL);
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure AICL rerun interval rc=%d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure VBUS for SW control rc=%d\n", rc);
		return rc;
	}
#ifndef OPLUS_FEATURE_CHG_BASIC
	val = (ilog2(chip->dt.wd_bark_time / 16) << BARK_WDOG_TIMEOUT_SHIFT) & BARK_WDOG_TIMEOUT_MASK;
	val |= (BITE_WDOG_TIMEOUT_8S | BITE_WDOG_DISABLE_CHARGING_CFG_BIT);
	val |= (chip->dt.wd_snarl_time_cfg << SNARL_WDOG_TIMEOUT_SHIFT) & SNARL_WDOG_TIMEOUT_MASK;

	rc = smblib_masked_write(chg, SNARL_BARK_BITE_WD_CFG_REG,
				 BITE_WDOG_DISABLE_CHARGING_CFG_BIT | SNARL_WDOG_TIMEOUT_MASK | BARK_WDOG_TIMEOUT_MASK |
	...
	rc = smblib_masked_write(chg, WD_CFG_REG,
				 WATCHDOG_TRIGGER_AFP_EN_BIT | WDOG_TIMER_EN_ON_PLUGIN_BIT | BARK_WDOG_INT_EN_BIT, val);
	if (rc < 0) {
		pr_err("Couldn't configue WD config rc=%d\n", rc);
		return rc;
	}
	...
	rc = smblib_masked_write(chg, FG_UPDATE_CFG_2_SEL_REG, IBT_LT_CHG_TERM_THRESH_SEL_BIT, 1);
	if (rc < 0) {
	...
	case 50:
		rc = smblib_masked_write(chg, CHARGE_INHIBIT_THRESHOLD_CFG_REG, CHARGE_INHIBIT_THRESHOLD_MASK,
	...
	rc = smblib_write(chg, CHGR_FAST_CHARGE_SAFETY_TIMER_CFG_REG, FAST_CHARGE_SAFETY_TIMER_1536_MIN);
	if (rc < 0) {
	...
	rc = smblib_masked_write(chg, DCDC_ENG_SDCDC_CFG5_REG, ENG_SDCDC_BAT_HPWR_MASK, BOOST_MODE_THRESH_3P6_V);
	if (rc < 0) {
	...
	if (chg->connector_pull_up != -EINVAL) {
		rc = smb5_configure_internal_pull(chg, CONN_THERM, get_valid_pullup(chg->connector_pull_up));
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't configure CONN_THERM pull-up rc=%d\n", rc);
			return rc;
		}
	}

	if (chg->smb_pull_up != -EINVAL) {
		rc = smb5_configure_internal_pull(chg, SMB_THERM, get_valid_pullup(chg->smb_pull_up));
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't configure SMB pull-up rc=%d\n", rc);
			return rc;
		}
	}
...
static int smb5_request_interrupt(struct smb5 *chip, struct device_node *node, const char *irq_name)
{
	struct smb_charger *chg = &chip->chg;
	...
	rc = devm_request_threaded_irq(chg->dev, irq, NULL, smb5_irqs[irq_index].handler, IRQF_ONESHOT, irq_name,
				       irq_data);
	if (rc < 0) {
...
static void smb5_free_interrupts(struct smb_charger *chg)
{
	...
	for (i = 0; i < ARRAY_SIZE(smb5_irqs); i++) {
		if (smb5_irqs[i].irq > 0) {
			...
			devm_free_irq(chg->dev, smb5_irqs[i].irq, smb5_irqs[i].irq_data);
		}
	}
}
...
DEFINE_DEBUGFS_ATTRIBUTE(force_usb_psy_update_ops, NULL, force_usb_psy_update_write, "0x%02llx\n");
...
DEFINE_DEBUGFS_ATTRIBUTE(force_dc_psy_update_ops, NULL, force_dc_psy_update_write, "0x%02llx\n");
...
static void smb5_create_debugfs(struct smb5 *chip)
{
	...
	file = debugfs_create_file("force_usb_psy_update", 0600, chip->dfs_root, chip, &force_usb_psy_update_ops);
	if (IS_ERR_OR_NULL(file))
		pr_err("Couldn't create force_usb_psy_update file rc=%ld\n", (long)file);

	file = debugfs_create_file("force_dc_psy_update", 0600, chip->dfs_root, chip, &force_dc_psy_update_ops);
	if (IS_ERR_OR_NULL(file))
		pr_err("Couldn't create force_dc_psy_update file rc=%ld\n", (long)file);

	file = debugfs_create_u32("debug_mask", 0600, chip->dfs_root, &__debug_mask);
	if (IS_ERR_OR_NULL(file))
...
#endif
	rc = vote(chip->pmic_spmi.smb5_chip->chg.fcc_votable, DEFAULT_VOTER, true, current_ma * 1000);
	if (rc < 0)
...
static void smbchg_aicl_enable(bool enable)
{
	...
	rc = smblib_masked_write(&chip->pmic_spmi.smb5_chip->chg, USBIN_AICL_OPTIONS_CFG_REG, USBIN_AICL_EN_BIT,
...
static int oplus_chg_set_input_current(int current_ma)
{
	...
	if (pre_current != -1) {
		...
		if (usb_icl[n] > 1200) {
			rc = vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, USB_PSY_VOTER, true,
				  usb_icl[n] * 1000);
			msleep(90);
			n--;
			...
			if (usb_icl[n] >= 1200) {
				...
				if (usb_icl[n] >= 1200) {
					...
					if (usb_icl[n] >= 1200) {
						rc = vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, USB_PSY_VOTER,
							  true, usb_icl[n] * 1000);
						msleep(90);
						n--;
						if (usb_icl[n] >= 1200) {
							rc = vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable,
								  USB_PSY_VOTER, true, usb_icl[n] * 1000);
							msleep(90);
							n--;
						}
					}
...
static int smbchg_float_voltage_set(int vfloat_mv)
{
	...
	rc = vote(chip->pmic_spmi.smb5_chip->chg.fv_votable, BATT_PROFILE_VOTER /*DEFAULT_VOTER*/, true,
		  vfloat_mv * 1000);
	if (rc < 0)
...
static int smbchg_charging_enble(void)
{
	...
	rc = vote(chip->pmic_spmi.smb5_chip->chg.chg_disable_votable, DEFAULT_VOTER, false, 0);
	if (rc < 0)
...
static int smbchg_charging_disble(void)
{
	...
	rc = vote(chip->pmic_spmi.smb5_chip->chg.chg_disable_votable, DEFAULT_VOTER, true, 0);
	if (rc < 0)
...
bool oplus_chg_is_usb_present(void)
{
	...
	if ((chip->pmic_spmi.smb5_chip->chg.typec_mode == POWER_SUPPLY_TYPEC_SINK ||
	    ...
	    chip->vbatt_num == 2 && oplus_wpc_get_wireless_charge_start() == false &&
	    oplus_vooc_get_fastchg_started() == false && oplus_vooc_get_fastchg_to_normal() == false) {
...
static int oplus_chg_get_dyna_aicl_result(void)
{
	...
	if (usb_psy) {
		power_supply_get_property(usb_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED, &pval);
...
int oplus_chg_get_charger_subtype(void)
{
	...
	apsd_result = smblib_get_apsd_result(chg);
	if (apsd_result->pst == POWER_SUPPLY_TYPE_USB_HVDCP || apsd_result->pst == POWER_SUPPLY_TYPE_USB_HVDCP_3)
...
int oplus_chg_set_qc_config(void)
{
	...
	if (chip->limits.vbatt_pdqc_to_5v_thr > 0 && chip->charger_volt > 7500 &&
		...
		if (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY)
			oplus_voocphy_set_pdqc_config();
		smblib_masked_write(chg, CMD_HVDCP_2_REG, FORCE_5V_BIT,
				    FORCE_5V_BIT); //Before request 9V, need to force 5V first.
		msleep(300);
		oplus_chg_suspend_charger();
		oplus_chg_config_charger_vsys_threshold(0x02); //set Vsys Skip threshold 104%
		oplus_chg_enable_burst_mode(false);
...
static int smb5_show_charger_status(struct smb5 *chip)
{
	...
	pr_info("SMB5 status - usb:present=%d type=%d batt:present = %d health = %d charge = %d\n", usb_present,
		chg->real_charger_type, batt_present, batt_health, batt_charge_type);
	return rc;
}
...
static void oplus_usbpd_response_cb(struct usbpd_svid_handler *hdlr, u8 cmd, enum usbpd_svdm_cmd_type cmd_type,
...
static void oplus_usbpd_connect_cb(struct usbpd_svid_handler *hdlr, bool peer_usb_comm)
{
	if (!g_oplus_chip) {
		return;
	} else {
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	if (rc >= 0) {
		chg->iio.chgid_v_chan = iio_channel_get(chg->dev, "chgID_voltage_adc");
	...
	if (rc >= 0) {
		chg->iio.usbtemp_v_chan = iio_channel_get(chg->dev, "usb_temp_adc");
	...
	if (rc >= 0) {
		chg->iio.btbtemp_v_chan = iio_channel_get(chg->dev, "btb_temp_therm");
	...
	if (rc >= 0) {
		chg->iio.usbtemp_sup_v_chan = iio_channel_get(chg->dev, "usb_supplementary_temp_adc");
	...
	if (rc >= 0) {
		chg->iio.op_skin_therm_chan = iio_channel_get(chg->dev, "skin_therm");
	...
	if (chg->soc_ready_check == true) {
		...
		if (bms_psy == NULL)
			return -EPROBE_DEFER;
		power_supply_get_property(bms_psy, POWER_SUPPLY_PROP_SOC_REPORTING_READY, &pval);
	...
	if (alarmtimer_get_rtcdev())
		alarm_init(&chg->lpd_recheck_timer, ALARM_REALTIME, smblib_lpd_recheck_timer);
	else
		return -EPROBE_DEFER;
	...
	if (rc < 0) {
		dev_err(chg->dev, "failed to configure extcon capabilities\n");
		goto cleanup;
	}
	...
	if (main_psy) {
		...
		power_supply_set_property(main_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
		pval.intval = 1000 * oplus_chg_get_charging_current(oplus_chip);
		power_supply_set_property(main_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX, &pval);
	}
#endif
#ifndef OPLUS_FEATURE_CHG_BASIC
	rc = smb5_init_usb_psy(chip);
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC)
		smblib_masked_write(chg, TYPE_C_MODE_CFG_REG, TYPEC_POWER_ROLE_CMD_MASK, EN_SNK_ONLY_BIT);
