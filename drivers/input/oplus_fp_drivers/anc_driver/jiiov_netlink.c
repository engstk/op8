...
static int netlink_send_message(const char *p_buffer, uint16_t length)
{
	...
	ret = netlink_unicast(gp_netlink_sock, p_sk_buff, USER_PORT,
			      MSG_DONTWAIT);
...
static void netlink_receive_message(struct sk_buff *p_sk_buffer)
{
	...
	if (p_sk_buffer->len >= nlmsg_total_size(0)) {
		...
		if (p_user_message) {
			...
			netlink_send_message_to_user(p_user_message,
						     strlen(p_user_message));
		}
	}
}
...
int anc_netlink_init(void)
{
	...
	gp_netlink_sock = (struct sock *)netlink_kernel_create(
		&init_net, NETLINK_ANC, &g_netlink_kernel_config);
