			   ...
			   struct fingerprint_message_t **msg)
{
	...
	if ((ret = wait_event_interruptible(fp_queue,
					    g_reporte_condition == 1)) != 0) {
...
static ssize_t fp_event_node_read(struct file *file, char __user *buf,
			     ...
			     unsigned int size)
{
	...
	case E_FP_TP:
		...
		g_fingerprint_msg.event = event == 1 ? E_FP_EVENT_TP_TOUCHDOWN :
						       E_FP_EVENT_TP_TOUCHUP;
		g_fingerprint_msg.out_size =
			size <= MAX_MESSAGE_SIZE ? size : MAX_MESSAGE_SIZE;
		memcpy(g_fingerprint_msg.out_buf, data,
		       g_fingerprint_msg.out_size);
		need_report = 1;
		break;
	case E_FP_LCD:
		g_fingerprint_msg.module = E_FP_LCD;
		g_fingerprint_msg.event = event == 1 ? E_FP_EVENT_UI_READY :
						       E_FP_EVENT_UI_DISAPPEAR;
		need_report = 1;
		break;
	case E_FP_HAL:
