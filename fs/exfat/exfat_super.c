...
#endif
{
	qstr->hash = full_name_hash(NULL, qstr->name, exfat_striptail_len(qstr));
	return 0;
}
...
static int exfat_getattr(const struct path *path, struct kstat *stat, u32 request_mask, unsigned int query_flags)
{
