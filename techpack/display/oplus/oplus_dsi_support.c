...
int set_oplus_display_vendor(const char *display_name)
{
	...
	if (!strcmp(display_name,
		...
		oplus_display_vendor =
			OPLUS_SAMSUNG_ANA6706_DISPLAY_FHD_DSC_CMD_PANEL;
		register_device_proc("lcd", "ANA6706", "samsung1024");
	} else if (!strcmp(display_name,
			   "qcom,mdss_dsi_samsung_oplus_dsc_cmd")) {
		oplus_display_vendor =
	...
	enum oplus_display_power_status power_status)
{
	...
	case OPLUS_DISPLAY_POWER_ON:
		...
		oplus_display_notifier_call_chain(
			OPLUS_DISPLAY_EARLY_EVENT_BLANK, &oplus_notifier_data);
		break;
