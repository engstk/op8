...
static void xa_alloc_index(struct xarray *xa, unsigned long index, gfp_t gfp)
{
	...
	XA_BUG_ON(xa, xa_alloc(xa, &id, xa_mk_value(index & LONG_MAX), xa_limit_32b,
				gfp) != 0);
...
static noinline void check_xa_alloc_1(struct xarray *xa, unsigned int base)
{
	...
	XA_BUG_ON(xa, xa_alloc(xa, &id, xa_mk_value(UINT_MAX - 1),
				XA_LIMIT(UINT_MAX - 1, UINT_MAX),
				GFP_KERNEL) != 0);
	XA_BUG_ON(xa, id != 0xfffffffeU);
	XA_BUG_ON(xa, xa_alloc(xa, &id, xa_mk_value(UINT_MAX),
				XA_LIMIT(UINT_MAX - 1, UINT_MAX),
				GFP_KERNEL) != 0);
	XA_BUG_ON(xa, id != 0xffffffffU);
	id = 3;
	XA_BUG_ON(xa, xa_alloc(xa, &id, xa_mk_value(0),
				XA_LIMIT(UINT_MAX - 1, UINT_MAX),
				GFP_KERNEL) != -EBUSY);
	XA_BUG_ON(xa, id != 3);
	xa_destroy(xa);

	XA_BUG_ON(xa, xa_alloc(xa, &id, xa_mk_value(10), XA_LIMIT(10, 5),
				GFP_KERNEL) != -EBUSY);
