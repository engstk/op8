	...
	for_each_available_child_of_node(np, child) {
		...
		case FP_POWER_MODE_LDO:
			...
			ret = of_property_read_string(
				child, FP_POWER_NAME_NODE,
				&(pwr_list[child_node_index].vreg_config.name));
			if (ret) {
			...
			pr_debug("get ldo node name %s",
				 pwr_list[child_node_index].vreg_config.name);

			/* read ldo config name */
			ret = of_property_read_string(child, FP_POWER_CONFIG,
						      &node_name);
			if (ret) {
				pr_err("the param %s is not found !\n",
				       FP_POWER_CONFIG);
			...
			ldo_param_amount = of_property_count_elems_of_size(
				np, node_name, sizeof(u32));
			...
			ret = of_property_read_u32_index(
				np, node_name, LDO_UA_INDEX,
				&(pwr_list[child_node_index]
					  .vreg_config.ua_load));
			if (ret) {
				pr_err("failed to request %s(%d), rc = %u\n",
		...
		case FP_POWER_MODE_GPIO:
			/* read GPIO name */
			ret = of_property_read_string(child, FP_POWER_NAME_NODE,
						      &node_name);
			if (ret) {
				pr_err("the param %s is not found !\n",
				       FP_POWER_NAME_NODE);
			...
			/* get gpio by name */
			gf_dev->pwr_list[child_node_index].pwr_gpio =
				of_get_named_gpio(np, node_name, 0);
			...
			if (ret) {
				...
				if (pwr_list[child_node_index].poweron_level !=
				    0) {
					pwr_list[child_node_index]
						.poweron_level = 1;
				}
			...
			ret = devm_gpio_request(
				dev, pwr_list[child_node_index].pwr_gpio,
				node_name);
			if (ret) {
			...
			gpio_direction_output(
				pwr_list[child_node_index].pwr_gpio,
				(pwr_list[child_node_index].poweron_level == 0 ?
					 1 :
					 0));
		...
		ret = of_property_read_u32(child, FP_POWER_DELAY_TIME,
					   &pwr_list[child_node_index].delay);
		if (ret) {
			pr_err("failed to request %s, ret = %d\n",
...
int gf_parse_ftm_poweroff_flag(struct gf_dev *gf_dev)
{
	...
	ret = of_property_read_u32(np, FP_FTM_POWEROFF_FLAG,
				   &ftm_poweroff_flag);
	if (ret) {
		pr_err("failed to request %s, ret = %d\n", FP_FTM_POWEROFF_FLAG,
		       ret);
		goto exit;
	}
...
int gf_parse_notify_tpinfo_flag(struct gf_dev *gf_dev)
{
	...
	ret = of_property_read_u32(np, FP_NOTIFY_TPINFO_FLAG,
				   &notify_tpinfo_flag);
	if (ret) {
		pr_err("failed to request %s, ret = %d\n",
...
int gf_power_on(struct gf_dev *gf_dev)
{
	...
	for (index = 0; index < gf_dev->power_num; index++) {
		...
		case FP_POWER_MODE_LDO:
			rc = vreg_setup(gf_dev, &(gf_dev->pwr_list[index]),
					true);
			pr_info("---- power on ldo ----\n");
			break;
		case FP_POWER_MODE_GPIO:
			gpio_set_value(gf_dev->pwr_list[index].pwr_gpio,
		...
		case FP_POWER_MODE_GPIO:
			gpio_set_value(gf_dev->pwr_list[index].pwr_gpio,
				       (gf_dev->pwr_list[index].poweron_level ==
						...
						0));
			pr_info("set pwr_gpio %d\n",
				(gf_dev->pwr_list[index].poweron_level == 0 ?
					 1 :
					 0));
			break;
		case FP_POWER_MODE_AUTO:
