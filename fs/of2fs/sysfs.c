...
/* add f2fs frag_score and undiscard_score */
...
static int __maybe_unused free_frag_seq_show(struct seq_file *seq, void *offset)
{
	struct super_block *sb = seq->private;
	struct f2fs_sb_info *sbi = F2FS_SB(sb);
	unsigned int i, total_segs =
...
int f2fs_register_sysfs(struct f2fs_sb_info *sbi)
{
	...
	if (sbi->s_proc) {
		...
		/* add f2fs frag_score and undiscard_score */
		proc_create_single_data("frag_score", S_IRUGO, sbi->s_proc,
				frag_score_seq_show, sb);
		proc_create_single_data("free_frag", S_IRUGO, sbi->s_proc,
				free_frag_seq_show, sb);
		proc_create_single_data("undiscard_score", S_IRUGO, sbi->s_proc,
