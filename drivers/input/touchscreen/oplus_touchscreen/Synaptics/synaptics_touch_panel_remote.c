...
static int remote_rmi4_i2c_read(unsigned short addr, unsigned char *data,
				unsigned short length);
static int remote_rmi4_i2c_write(unsigned short addr, unsigned char *data,
				 unsigned short length);
...
static struct remotepanel_data *
remote_free_panel_data(struct remotepanel_data *pdata);
...
static struct device_attribute attrs[] = {
	__ATTR(open, S_IRUSR | S_IWUSR, NULL, rmidev_sysfs_open_store),
	__ATTR(release, S_IRUSR | S_IWUSR, NULL, rmidev_sysfs_release_store),
	__ATTR(address, S_IRUSR | S_IWUSR, NULL, rmidev_sysfs_address_store),
	__ATTR(length, S_IRUSR | S_IWUSR, NULL, rmidev_sysfs_length_store),
	__ATTR(attn_state, S_IRUSR | S_IWUSR, rmidev_sysfs_attn_state_show,
	       NULL),
};
				      ...
				      char *buf, loff_t pos, size_t count)
{
	...
	if (data_length) {
		retval = remote_rmi4_i2c_read(
			rmidev->address, (unsigned char *)buf, data_length);
		if (retval < 0) {
			dev_err(device_ptr, "%s: Failed to read data\n",
				__func__);
			return retval;
		}
	} else {
				       ...
				       char *buf, loff_t pos, size_t count)
{
	...
	if (data_length) {
		retval = remote_rmi4_i2c_write(
			rmidev->address, (unsigned char *)buf, data_length);
		if (retval < 0) {
			dev_err(device_ptr, "%s: Failed to write data\n",
				__func__);
			return retval;
		}
	} else {
...
int remote_rmi4_i2c_read(unsigned short addr, unsigned char *data,
			 unsigned short length)
{
	int retval;
	unsigned char retry;
...
int remote_rmi4_i2c_write(unsigned short addr, unsigned char *data,
			  unsigned short length)
{
	int retval;
	unsigned char retry;
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		dev_err(device_ptr, "%s: New position 0x%04x is invalid\n",
...
static int rmidev_create_attr(bool create)
{
     ...
     */
	rmidev->sysfs_dir =
		kobject_create_and_add("rmidev", &input_dev->dev.kobj);
	if (!rmidev->sysfs_dir) {
		dev_err(device_ptr, "%s: Failed to create sysfs directory\n",
			__func__);
		return -1;
	}
	...
	if (retval < 0) {
		dev_err(device_ptr, "%s: Failed to create sysfs bin file\n",
			__func__);
		goto err_sysfs_bin;
	}
...
static void rmidev_device_cleanup(struct rmidev_data *dev_data)
{
	...
	if (dev_data) {
		...
		dev_dbg(device_ptr, "%s: rmidev device removed\n", __func__);
	}
...
int register_remote_device(struct remotepanel_data *pdata)
{
	...
	if (IS_ERR(device_ptr)) {
		dev_err(device_ptr, "%s: Failed to create rmi char device\n",
			__func__);
		retval = -ENODEV;
		goto err_char_device;
	}
