...
static struct short_c_batt_item short_c_batt_items[] = {
	/*batt chging cycle*/
	{ .name = "batt_chging_cycle_threshold", .value = 0 },
	{ .name = "batt_chging_cycles", .value = 0 },
	/*timer*/
	{ .name = "cv_timer1", .value = 0 },
	{ .name = "full_timer2", .value = 0 },
	{ .name = "full_timer3", .value = 0 },
	{ .name = "full_timer4", .value = 0 },
	{ .name = "full_timer5", .value = 0 },
	/*rbatt*/
	{ .name = "cool_temp_rbatt", .value = 0 },
	{ .name = "little_cool_temp_rbatt", .value = 0 },
	{ .name = "normal_temp_rbatt", .value = 0 },
	/*delta vbatt*/
	{ .name = "full_delta_vbatt1_mv", .value = 0 },
	{ .name = "full_delta_vbatt2_mv", .value = 0 },
	/*exception2*/
	{ .name = "ex2_lower_ibatt_ma", .value = 0 },
	{ .name = "ex2_low_ibatt_ma", .value = 0 },
	{ .name = "ex2_high_ibatt_ma", .value = 0 },
	{ .name = "ex2_lower_ibatt_count", .value = 0 },
	{ .name = "ex2_low_ibatt_count", .value = 0 },
	{ .name = "ex2_high_ibatt_count", .value = 0 },
	/*dynamic delta vbatt*/
	{ .name = "dyna1_low_avg_dv_mv", .value = 0 },
	{ .name = "dyna1_high_avg_dv_mv", .value = 0 },
	{ .name = "dyna1_delta_dv_mv", .value = 0 },
	{ .name = "dyna2_low_avg_dv_mv", .value = 0 },
	{ .name = "dyna2_high_avg_dv_mv", .value = 0 },
	{ .name = "dyna2_delta_dv_mv", .value = 0 },
	/*control switch*/
	{ .name = "is_recheck_on", .value = 0 },
	{ .name = "is_switch_on", .value = 0 },
	{ .name = "is_feature_on", .value = 0 },
};
...
bool oplus_short_c_batt_is_prohibit_chg(struct oplus_chg_chip *chip)
{
	...
	if (chip->short_c_batt.is_feature_hw_on == SHORT_C_BATT_FEATURE_HW_STATUS__ON) {
...
static int oplus_short_c_batt_read_file(struct oplus_chg_chip *chip)
{
	...
	chip->short_c_batt.cv_timer1 = short_c_batt_items[2].value / OPLUS_CHG_UPDATE_INTERVAL_SEC;
	chip->short_c_batt.full_timer2 = short_c_batt_items[3].value / OPLUS_CHG_UPDATE_INTERVAL_SEC;
	chip->short_c_batt.full_timer3 = short_c_batt_items[4].value / OPLUS_CHG_UPDATE_INTERVAL_SEC;
	chip->short_c_batt.full_timer4 = short_c_batt_items[5].value / OPLUS_CHG_UPDATE_INTERVAL_SEC;
	chip->short_c_batt.full_timer5 = short_c_batt_items[6].value / OPLUS_CHG_UPDATE_INTERVAL_SEC;
...
{
	if (chip->short_c_batt.batt_chging_cycle_threshold < 1 || chip->short_c_batt.batt_chging_cycles < 0 ||
	    chip->short_c_batt.cv_timer1 < 1 || chip->short_c_batt.cv_timer1 > chip->limits.max_chg_time_sec ||
	    chip->short_c_batt.full_timer2 < 1 || chip->short_c_batt.full_timer2 > chip->limits.max_chg_time_sec ||
	    chip->short_c_batt.full_timer3 < 1 || chip->short_c_batt.full_timer3 > chip->limits.max_chg_time_sec ||
	    chip->short_c_batt.full_timer4 < 1 || chip->short_c_batt.full_timer4 > chip->limits.max_chg_time_sec ||
	    chip->short_c_batt.full_timer5 < 1 || chip->short_c_batt.full_timer5 > chip->limits.max_chg_time_sec ||
	    chip->short_c_batt.cool_temp_rbatt < 1 || chip->short_c_batt.little_cool_temp_rbatt < 1 ||
	    chip->short_c_batt.normal_temp_rbatt < 1 || chip->short_c_batt.full_delta_vbatt1_mv < 1 ||
	    chip->short_c_batt.full_delta_vbatt2_mv < 1 || chip->short_c_batt.ex2_lower_ibatt_ma < 1 ||
	    chip->short_c_batt.ex2_lower_ibatt_ma > 2000 || chip->short_c_batt.ex2_low_ibatt_ma < 1 ||
	    chip->short_c_batt.ex2_low_ibatt_ma > 2000 || chip->short_c_batt.ex2_high_ibatt_ma < 1 ||
	    chip->short_c_batt.ex2_high_ibatt_ma > 2000 || chip->short_c_batt.ex2_lower_ibatt_count < 1 ||
	    chip->short_c_batt.ex2_low_ibatt_count < 1 || chip->short_c_batt.ex2_high_ibatt_count < 1 ||
	    chip->short_c_batt.dyna1_delta_dv_mv < 1 || chip->short_c_batt.dyna2_delta_dv_mv < 1) {
		return false;
	}
 ...
 */
static int oplus_short_c_batt_write_err_code(int err_code, int temp1, int temp2, int vbatt1_mv, int vbatt2_mv,
					     int timer_counts)
{
	struct file *fp;
	mm_segment_t old_fs;
	...
	sprintf(buf, "err_code%d:%d,%d,%d,%d,%d,#", err_code, temp1, temp2, vbatt1_mv, vbatt2_mv, timer_counts);
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	pos = 0;
...
static int oplus_short_c_batt_write_exit_code(int exit_code, int temp1, int temp2, int vbatt1, int vbatt2)
{
	struct file *fp;
	mm_segment_t old_fs;
	loff_t pos;
...
static int oplus_short_c_batt_write_chg_data(int current_ma, int volt_mv, int lower_count, int low_count,
					     int high_count)
{
	struct file *fp;
	mm_segment_t old_fs;
	...
	/*add ",#" and the end of the string for userspace*/
	sprintf(buf, "chg_data:%d,%d,%d,%d,%d,#", current_ma, volt_mv, lower_count, low_count, high_count);
	...
	pos = 0;
	vfs_read(fp, buf_log, sizeof(buf), &pos);
	if (pre_lower_count != lower_count || pre_low_count != low_count || pre_high_count != high_count) {
		short_debug("%s\n", buf_log);
...
void oplus_chg_short_c_hw_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->short_c_batt.is_feature_hw_on == SHORT_C_BATT_FEATURE_HW_STATUS__OFF) {
		return;
	}
	if (chip->charger_exist && (shortc_gpio_status != chip->short_c_batt.shortc_gpio_status)) {
...
void oplus_chg_short_c_ic_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_exist && (short_ic_otp_st != chip->short_c_batt.ic_short_otp_st)) {
		if (oplus_vooc_get_fastchg_started() == true) {
			oplus_chg_set_chargerid_switch_val(0);
			oplus_vooc_switch_mode(NORMAL_CHARGER_MODE);
		}
...
void oplus_chg_short_c_battery_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->short_c_batt.short_c_bat_cv_mv == -EINVAL || chip->limits.short_c_bat_vfloat_mv == -EINVAL ||
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		if (oplus_is_power_off_chg(chip) == true) {
			data_mounted_count++;
			if (data_mounted_count < 30 && oplus_short_c_batt_data_is_mounted() == true) { //3min
				oplus_short_c_batt_read_file(chip);
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN && chip->short_c_batt.is_recheck_on != 0) {
	...
	if (chip->temperature <= LITTLE_COLD_TEMP_THRESHOLD || chip->temperature > WARM_TEMP_THRESHOLD) { //0~45
		...
		if (oplus_short_c_batt_get_cv_status(chip) == true) {
			oplus_short_c_batt_write_exit_code(EXIT_CODE__EX5_OUT_OF_TEMP_0_45, INVALID_CODE,
							   chip->temperature, INVALID_CODE, INVALID_CODE);
	...
	if (chip->batt_full == true && chip->chging_on == false && chip->charging_state == CHARGING_STATUS_FULL) {
		...
		oplus_short_c_batt_write_chg_data(chip->icharging, chip->batt_volt, full_over_10ma_count,
						  full_over_20ma_count, full_over_50ma_count);

		if (full_over_10ma_count >= chip->short_c_batt.ex2_lower_ibatt_count) {
			oplus_short_c_batt_write_exit_code(EXIT_CODE__EX2_OVER_LOWER_IBATT, INVALID_CODE, INVALID_CODE,
							   INVALID_CODE, INVALID_CODE);
			chg_err("don't exit_check, over [%d]mA [%d]times\n", chip->short_c_batt.ex2_lower_ibatt_ma,
				chip->short_c_batt.ex2_lower_ibatt_count);
			short_debug("don't exit_check, full_over_10ma_count[%d]\n", full_over_10ma_count);
			goto cv_reset;
		}
		if (full_over_20ma_count >= chip->short_c_batt.ex2_low_ibatt_count) {
			oplus_short_c_batt_write_exit_code(EXIT_CODE__EX2_OVER_LOW_IBATT, INVALID_CODE, INVALID_CODE,
							   INVALID_CODE, INVALID_CODE);
			chg_err("don't exit_check, over [%d]mA [%d]times\n", chip->short_c_batt.ex2_low_ibatt_ma,
				chip->short_c_batt.ex2_low_ibatt_count);
			short_debug("don't exit_check, full_over_20ma_count[%d]\n", full_over_20ma_count);
			goto cv_reset;
		}
		if (full_over_50ma_count >= chip->short_c_batt.ex2_high_ibatt_count) {
			oplus_short_c_batt_write_exit_code(EXIT_CODE__EX2_OVER_HIGH_IBATT, INVALID_CODE, INVALID_CODE,
							   INVALID_CODE, INVALID_CODE);
			chg_err("don't exit_check, over [%d]mA [%d]times\n", chip->short_c_batt.ex2_high_ibatt_ma,
				chip->short_c_batt.ex2_high_ibatt_count);
		...
		if (full_timer5_count > chip->short_c_batt.full_timer5 &&
			...
			if (full_vbatt_2 > 0 && chip->limits.iterm_ma != 0) {
				...
				short_debug("cv_avg_vbatt[%d], full_vbatt_2[%d], rbatt[%d]\n", cv_avg_vbatt,
					    full_vbatt_2, rbatt);
				if (exit_check_status == true) {
					oplus_short_c_batt_write_exit_code(EXIT_CODE__OVER_RBATT, chip->temperature,
					...
					oplus_short_c_batt_write_exit_code(EXIT_CODE__NOT_OVER_RBATT, chip->temperature,
									   chip->temperature, full_vbatt_2, rbatt);
		...
		if (full_timer3_count <= chip->short_c_batt.full_timer3) {
			...
			if (full_temp1 != INVALID_TEMP && abs(full_temp1 - chip->temperature) > DELTA_TEMP_THRESHOLD) {
				oplus_short_c_batt_write_exit_code(EXIT_CODE__OVER_DELTA_TEMP_IN_TIMER3, full_temp1,
								   chip->temperature, INVALID_CODE, INVALID_CODE);
				chg_err("don't exit_check, temperature[%d]\n", chip->temperature);
		...
		if (full_timer3_count > chip->short_c_batt.full_timer3 &&
			...
			if (full_vbatt_n > 0 && full_vbatt1 > 0) {
				if (full_timer3_count == chip->short_c_batt.full_timer3 + 1) {
					if ((chip->short_c_batt.batt_chging_cycles <
					     chip->short_c_batt.batt_chging_cycle_threshold) &&
						...
						chip->short_c_batt.err_code = SHORT_C_BATT_STATUS__FULL_ERR_CODE2;
						oplus_short_c_batt_write_err_code(chip->short_c_batt.err_code,
										  full_temp1, chip->temperature,
										  full_vbatt1, full_vbatt_n,
										  full_timer3_count * 5);
					}
				...
				if (full_timer3_count == chip->short_c_batt.full_timer3 + 2) {
					...
					if (chip->temperature <= COOL_TEMP_THRESHOLD) { //2~12
						if (chip->short_c_batt.dyna1_low_avg_dv_mv != INVALID_DATA &&
							...
							if ((full_vbatt1 - full_vbatt_n -
								     chip->short_c_batt.dyna1_low_avg_dv_mv >
							     chip->short_c_batt.dyna1_delta_dv_mv) &&
							    (full_vbatt1 - full_vbatt_n >
							     chip->short_c_batt.full_delta_vbatt1_mv)) {
								//set err_code4;
								short_debug("set err_code[%d]\n",
									    SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4);
								chip->short_c_batt.err_code =
									SHORT_C_BATT_STATUS__DYNAMIC_ERR_CODE4;
								oplus_short_c_batt_write_err_code(
									chip->short_c_batt.err_code, full_temp1,
									chip->temperature, full_vbatt1, full_vbatt_n,
									INVALID_CODE);
							}
						}
					} else { //12~43
						if (chip->short_c_batt.dyna1_high_avg_dv_mv != INVALID_DATA &&
							...
							if ((full_vbatt1 - full_vbatt_n -
								     chip->short_c_batt.dyna1_high_avg_dv_mv >
							     chip->short_c_batt.dyna1_delta_dv_mv) &&
							    (full_vbatt1 - full_vbatt_n >
							     chip->short_c_batt.full_delta_vbatt1_mv)) {
								//set err_code4;
		...
		if (full_timer4_count <= chip->short_c_batt.full_timer4) {
			...
			oplus_short_c_batt_get_average_vbatt_mv(chip, true); //reset
			if (full_temp1 != INVALID_TEMP && abs(full_temp1 - chip->temperature) > DELTA_TEMP_THRESHOLD) {
				oplus_short_c_batt_write_exit_code(EXIT_CODE__OVER_DELTA_TEMP_IN_TIMER4, full_temp1,
								   chip->temperature, INVALID_CODE, INVALID_CODE);
				chg_err("don't exit_check, temperature[%d]\n", chip->temperature);
				short_debug("don't exit_check, T1[%d], T2[%d]\n", full_temp1, chip->temperature);
				goto cv_reset;
				...
				if (full_timer4_count == chip->short_c_batt.full_timer4 + 2) {
					...
					if (chip->temperature <= COOL_TEMP_THRESHOLD) { //2~12
						if (chip->short_c_batt.dyna2_low_avg_dv_mv != INVALID_DATA &&
							...
							if ((full_vbatt1 - full_vbatt_n -
								     chip->short_c_batt.dyna2_low_avg_dv_mv >
							     chip->short_c_batt.dyna2_delta_dv_mv) &&
							    (full_vbatt1 - full_vbatt_n >
							     chip->short_c_batt.full_delta_vbatt2_mv)) {
								//set err_code5;
								short_debug("set err_code[%d]\n",
						...
						if (chip->short_c_batt.dyna2_high_avg_dv_mv != INVALID_DATA &&
							...
							if ((full_vbatt1 - full_vbatt_n -
								     chip->short_c_batt.dyna2_high_avg_dv_mv >
							     chip->short_c_batt.dyna2_delta_dv_mv) &&
							    (full_vbatt1 - full_vbatt_n >
							     chip->short_c_batt.full_delta_vbatt2_mv)) {
								//set err_code5;
	...
	if (chip->short_c_batt.cv_satus == true) {
		...
		if (cv_timer1_count == chip->short_c_batt.cv_timer1) {
			...
			oplus_short_c_batt_write_err_code(SHORT_C_BATT_STATUS__CV_ERR_CODE1, cv_temp, chip->temperature,
