...
int get_all_gpio_val(void)
{
	...
	for (i = 0; i < rf_cable_data->gpio_num; i++) {
		gpiostate = gpiostate +
			    (gpio_get_value(rf_cable_data->cable_gpio[i]) *
			     local_pow(10, rf_cable_data->gpio_num - i - 1));
	}
	return gpiostate;
}
...
static void cable_connect_state(int enable)
{
	...
	if (rf_cable_data->uevent_feature) {
		if (enable) {
			kobject_uevent_env(&rf_uevent_device->kobj, KOBJ_CHANGE,
					   connected);
...
static void rf_cable_work(struct work_struct *work)
{
	...
	pr_err("%s rf_state_pre=%d, rf_state=%d gpio=%2d,\n", __func__,
	       rf_cable_data->rf_state_pre, rf_cable_data->rf_state,
	       current_gpio_state);
...
int create_rf_cable_procfs(void)
{
	...
	proc_create_data("rf_cable", S_IRUGO, oplus_rf, &cable_proc_fops_cable,
			 rf_cable_data);
...
static int rf_cable_gpio_pinctrl_init(struct platform_device *pdev)
{
	...
	rf_cable_data->gpio_pinctrl_active = pinctrl_lookup_state(
		rf_cable_data->gpio_pinctrl, RF_CABLE_STATE_ACTIVE);
...
static int op_rf_cable_probe(struct platform_device *pdev)
{
	...
	rf_cable_data->uevent_feature = of_property_read_bool(
		pdev->dev.of_node, "oem,rf_uevent_feature_enable");
	...
	rc = of_property_read_u32(pdev->dev.of_node, "rf,cable-gpio-num",
				  &rf_cable_data->gpio_num);
	if (rc) {
	...
	rf_cable_data->wqueue =
		create_singlethread_workqueue("op_rf_cable_wqueue");
	...
	for (i = 0; i < rf_cable_data->gpio_num; i++) {
		...
		rc = op_rf_request_named_gpio(cable,
					      &rf_cable_data->cable_gpio[i]);
		if (rc) {
		...
		rf_cable_data->irq[i] =
			gpio_to_irq(rf_cable_data->cable_gpio[i]);
