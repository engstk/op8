...
struct spi_device *g_mtk_spi_device = NULL;
...
static ssize_t hw_reset_set(struct device *dev, struct device_attribute *attr, const char *buf,
    ...
    if (!strncmp(buf, "reset", strlen("reset"))) {
        ...
        rc = anc_reset(data);
        /* Active spi pins */
        select_pin_ctl(data, "anc_spi_active");
    } else {
        rc = -EINVAL;
    }
...
static void anc_power_onoff(struct anc_data *data, int power_onoff) {
    ...
    if (power_onoff == 1) {
        ...
        if (g_anc_data->is_powered_on == 1) {
            /* Pull down reset/spi pins */
            gpio_direction_output(g_anc_data->rst_gpio, 0);
            select_pin_ctl(data, "anc_spi_output_low");
            if (anc_gpio_pwr_flag == 1) {
...
static ssize_t hw_spi_enable(struct device *dev, struct device_attribute *attr, const char *buf,
                             size_t count) {
    ssize_t rc = count;
    struct anc_data *data = dev_get_drvdata(dev);
...
static int anc_gpio_init(struct device *dev, struct anc_data *data) {
    ...
    rc = select_pin_ctl(data, "anc_reset_default");
    if (rc) {
        goto exit;
    }
...
static long anc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
        case ANC_IOC_RESET:
            ...
            /* Active spi pins */
            select_pin_ctl(dev_data, "anc_spi_active");
            break;
...
struct spi_driver spi_driver = {.driver =
                                    {
                                        .name = "anc_mtk",
                                        .owner = THIS_MODULE,
