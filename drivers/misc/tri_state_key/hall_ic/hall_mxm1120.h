...
#define M1120_IOCTL_SET_THRESHOLD_HIGH \
	_IOW(M1120_IOCTL_BASE, 0x08, unsigned int)
#define M1120_IOCTL_GET_THRESHOLD_HIGH \
	_IOR(M1120_IOCTL_BASE, 0x09, unsigned int *)
#define M1120_IOCTL_SET_THRESHOLD_LOW _IOW(M1120_IOCTL_BASE, 0x0A, unsigned int)
#define M1120_IOCTL_GET_THRESHOLD_LOW \
	_IOR(M1120_IOCTL_BASE, 0x0B, unsigned int *)
...
struct m1120_mutex_t {
	struct mutex enable;
	struct mutex data;
};

struct m1120_atomic_t {
	atomic_t enable;
	atomic_t delay;
	atomic_t debug;
};

struct m1120_platform_data_t {
	int power_vi2c;
	int power_vdd;
	int interrupt_gpio;
	int interrupt_irq;
...
struct m1120_data_t {
	...
	struct device *dev;
	struct m1120_mutex_t mtx;
	struct m1120_atomic_t atm;
