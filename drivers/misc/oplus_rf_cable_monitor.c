...
//=====================================
static ssize_t cable_read_proc(struct file *file, char __user *buf,
			       size_t count, loff_t *off)
{
	...
	for (i = 0; i < MAX_CABLE_STS; i++) {
		if (gpio_is_valid(rf_data->cable_gpio[i])) {
			rf_data->rf_cable_gpio_sts[i] =
				gpio_get_value(rf_data->cable_gpio[i]);
		} else {
...
static ssize_t pds_read_proc(struct file *file, char __user *buf, size_t count,
			     loff_t *off)
{
	struct rf_cable_data *rf_data = PDE_DATA(file_inode(file));
	char page[128] = { 0 };
	...
	for (i = 0; i < MAX_CABLE_STS; i++) {
		if (gpio_is_valid(rf_data->pds_gpio[i])) {
			rf_data->rf_pds_gpio_sts[i] =
				gpio_get_value(rf_data->pds_gpio[i]);
		} else {
...
static int rf_cable_initial_request_irq(struct rf_cable_data *rf_data)
{
	...
	for (i = 0; i < rf_data->rf_cable_support_num; i++) {
		if (gpio_is_valid(rf_data->cable_gpio[i])) {
			rc = devm_request_threaded_irq(
				rf_data->dev, rf_data->cable_irq[i], NULL,
				cable_interrupt,
				IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING |
					IRQF_ONESHOT | IRQF_NO_SUSPEND,
				"rf_cable_irq", rf_data);
			if (rc) {
	...
	for (i = 0; i < rf_data->rf_pds_support_num; i++) {
		if (gpio_is_valid(rf_data->pds_gpio[i])) {
			rc = devm_request_threaded_irq(
				rf_data->dev, rf_data->pds_irq[i], NULL,
				cable_interrupt,
				IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING |
					IRQF_ONESHOT | IRQF_NO_SUSPEND,
				"rf_pds_irq", rf_data);
				      ...
				      struct rf_cable_data *rf_data)
{
	...
	for (i = 0; i < MAX_CABLE_STS; i++) {
		...
		if (i < rf_data->rf_cable_support_num) {
			rf_data->cable_gpio[i] =
				of_get_named_gpio(np, gpio_str, 0);
		} else {
	...
	rf_data->rf_pds_active =
		pinctrl_lookup_state(rf_data->gpio_pinctrl, "rf_pds_active");
	...
	for (i = 0; i < MAX_CABLE_STS; i++) {
		if (gpio_is_valid(rf_data->cable_gpio[i])) {
			...
			rf_data->cable_irq[i] =
				gpio_to_irq(rf_data->cable_gpio[i]);
...
static int op_rf_cable_probe(struct platform_device *pdev)
{
	...
	rc = of_property_read_u32(np, "rf_cable_support_num",
				  &rf_data->rf_cable_support_num);
	if (rc < 0) {
	...
	rc = of_property_read_u32(np, "rf_pds__support_num",
				  &rf_data->rf_pds_support_num);
	if (rc < 0) {
	...
	rf_data->the_rf_format = (struct rf_info_type *)qcom_smem_get(
		SMEM_APPS, SMEM_DRAM_TYPE, &smem_size);
	...
	if (get_boot_mode() == MSM_BOOT_MODE__RF ||
		...
		for (i = 0; i < rf_data->rf_cable_support_num; i++) {
			rf_data->the_rf_format->rf_cable_gpio_sts[i] =
				RF_CABLE_IN;
		}
		return 0;
	}
	...
	if (rf_data->rf_cable_support_num) {
		proc_create_data("rf_cable", S_IRUGO, oplus_rf,
				 &cable_proc_fops_cable, rf_data);
	}

	if (rf_data->rf_pds_support_num) {
		proc_create_data("rf_pds", S_IRUGO, oplus_rf,
				 &cable_proc_fops_pds, rf_data);
	}
	...
	pr_err("%s: probe ok, rf_cable, SMEM_RF_INFO:%d, sts:%d, "
	       ...
	       rf_data->cable_gpio[0],
	       rf_data->the_rf_format->rf_cable_gpio_sts[0],
	       rf_data->cable_gpio[1],
	       rf_data->the_rf_format->rf_cable_gpio_sts[1],
	       rf_data->cable_gpio[2],
	       rf_data->the_rf_format->rf_cable_gpio_sts[2],
	       rf_data->cable_gpio[3],
	       rf_data->the_rf_format->rf_cable_gpio_sts[3],
	...
	pr_err("%s: probe ok, rf_pds, SMEM_RF_INFO:%d, sts:%d, "
	       ...
	       rf_data->pds_irq[0], rf_data->pds_irq[1], rf_data->pds_irq[2],
	       rf_data->pds_irq[3]);
