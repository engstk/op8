				     ...
				     size_t count, loff_t *ppos)
{
	...
	len += snprintf(kbuf + len, size - len,
			...
			"%-20s:\t%d\n"
			"%-20s:\t%d\n",
			"mode", pqlt_cur_setting->pq_setting.lcemode, "level",
	...
	len += snprintf(kbuf + len, size - len,
			...
			"cmftc", pqlt_cur_setting->cmftc, "cmcolortempmode",
			...
			"max_colortempvalue",
			pqlt_cur_setting->max_colortempvalue, "cmcolorgamut",
			pqlt_cur_setting->pq_setting.cmcolorgamut, "demomode",
			pqlt_cur_setting->pq_setting.demomode, "source_switch",
			pqlt_cur_setting->source_switch);
	...
	len += snprintf(kbuf + len, size - len,
			...
			"%-20s:\t%d\n",
			"readingmode",
			pqlt_cur_setting->pq_setting.readingmode);
	...
	len += snprintf(kbuf + len, size - len,
			...
			"%-20s:\t%d\n"
			"%-20s:\t%d\n",
			"memc_level", pcfg1->frc_setting.memc_level, "memc_osd",
			pcfg1->frc_setting.memc_osd, "in_fps",
			pcfg->frc_setting.in_fps, "out_fps",
			pcfg->frc_setting.out_fps, "in_fps_configured",
			pcfg->frc_setting.in_fps_configured, "low_latency",
			pcfg->frc_low_latency);
	...
	len += snprintf(kbuf + len, size - len,
			...
			"%-20s:\t0x%x\n"
			"%-20s:\t0x%x\n",
			"osd0_tl", pcfg1->frc_setting.iris_osd0_tl, "osd0_br",
			pcfg1->frc_setting.iris_osd0_br, "osd1_tl",
			pcfg1->frc_setting.iris_osd1_tl, "osd1_br",
			pcfg1->frc_setting.iris_osd1_br, "osd2_tl",
			pcfg1->frc_setting.iris_osd2_tl, "osd2_br",
			pcfg1->frc_setting.iris_osd2_br, "osd3_tl",
			pcfg1->frc_setting.iris_osd3_tl, "osd3_br",
			pcfg1->frc_setting.iris_osd3_br, "osd4_tl",
			pcfg1->frc_setting.iris_osd4_tl, "osd4_br",
			pcfg1->frc_setting.iris_osd4_br, "osd_win_ctrl",
			pcfg1->frc_setting.iris_osd_window_ctrl, "osd_win_ctrl",
			pcfg1->frc_setting.iris_osd_win_dynCompensate);

	len += snprintf(kbuf + len, size - len,
			"***firmware_version***\n"
			"%-20s:\t%d\n" //CID101064
			"%-20s:\t%d%d/%d/%d\n",
			"version", pcfg1->app_version, "date",
			pcfg1->app_date[3], pcfg1->app_date[2],
			pcfg1->app_date[1], pcfg1->app_date[0]);
...
void iris_display_mode_name_update(void)
{
	...
	strlcpy(pcfg->display_mode_name, "Not Set",
		sizeof(pcfg->display_mode_name));

	if (pcfg->abypss_ctrl.abypass_mode == ANALOG_BYPASS_MODE) {
		strlcpy(pcfg->display_mode_name, "ABYPASS",
			sizeof(pcfg->display_mode_name));
	} else {
		if (pcfg->osd_enable) {
			if (pcfg->pwil_mode == FRC_MODE)
				strncpy(pcfg->display_mode_name, "DUAL-MEMC",
					sizeof(pcfg->display_mode_name));
			else
				strncpy(pcfg->display_mode_name, "DUAL-PT",
					sizeof(pcfg->display_mode_name));
			return;
		}
		strncpy(pcfg->display_mode_name, "PT",
			sizeof(pcfg->display_mode_name));
		if (pcfg->pwil_mode == FRC_MODE) {
			strlcpy(pcfg->display_mode_name, "MEMC",
				sizeof(pcfg->display_mode_name));
		} else if (pcfg->pwil_mode == RFB_MODE) {
			strlcpy(pcfg->display_mode_name, "RFB",
				sizeof(pcfg->display_mode_name));
		}
	}
}
...
int iris_dbgfs_status_init(struct dsi_display *display)
{
	...
	if (debugfs_create_file(IRIS_DBG_FUNCSTATUS_FILE, 0644, pcfg->dbg_root,
				display, &iris_dbg_fstatus_fops) == NULL)
		IRIS_LOGE("create file func_status failed\n");

	if (debugfs_create_file("display_mode", 0644, pcfg->dbg_root, display,
