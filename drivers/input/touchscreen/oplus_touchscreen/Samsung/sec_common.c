				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	do {
		...
		if (offset == 0)
			ret = touch_i2c_read_block(ts->client, lv1cmd, length,
						   &read_lv1_buff[offset]);
		else
			ret = touch_i2c_read(ts->client, NULL, 0,
					...
					const char *buf, size_t size)
{
	...
	lv1cmd = buf[0];
	lv1_readsize =
		((unsigned int)buf[4] << 24) | ((unsigned int)buf[3] << 16) |
...
void sec_limit_read(struct seq_file *s, struct touchpanel_data *ts)
{
	...
	for (m = 0; m < item_cnt; m++) {
		item_head = (struct sec_test_item_header *)(fw->data +
							    p_item_offset[m]);
		...
		p_data32 = (int32_t *)(fw->data + p_item_offset[m] +
				       sizeof(struct sec_test_item_header));
...
static int tp_auto_test_read_func(struct seq_file *s, void *v)
{
	...
	snprintf(
		data_buf, 128,
		"/sdcard/TpTestReport/screenOn/tp_testlimit_%02d%02d%02d-%02d%02d%02d-utc.csv",
	...
	if ((test_head->magic1 != 0x494D494C) ||
	    (test_head->magic2 != 0x474D4954)) {
		TPD_INFO("limit image is not generated by oplus\n");
		seq_printf(s, "limit image is not generated by oplus\n");
		goto OUT;
	}
...
static ssize_t proc_kernel_grip_para_write(struct file *file,
					   const char __user *buffer,
					   size_t count, loff_t *ppos)
{
	int para = 0;
		    ...
		    struct sec_proc_operations *sec_ops)
{
	...
	prEntry_tmp = proc_create_data("kernel_grip_para", 0666, ts->prEntry_tp,
				       &proc_kernel_grip_para_ops, ts);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__,
			 __LINE__);
