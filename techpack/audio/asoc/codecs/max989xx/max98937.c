...
#define MAX989XX_DRV_NAME "max98937_codec"
...
int max98927_reg_common_map[][2] = {
	...
	//Chunyu.xie add for changing 98927's IV order
	{MAX98927_PCM_Tx_Channel_Sources_B, 0x00},
	#else/*OPLUS_ARCH_EXTENDS*/
	{MAX98927_PCM_Tx_Channel_Sources_B, 0x20},
	#endif/*OPLUS_ARCH_EXTENDS*/
...
int max98937_reg_common_map[][2] = {
	{MAX98937_Clock_monitor_enable, 0x02}, //disable bclk monitor
	{MAX98937_Brownout_level_infinite_hold,  0x00},
	{MAX98937_Brownout_level_hold,  0x00},
	{MAX98937_Brownout__level_1_amp_1_control_2,  0x00},
	{MAX98937_Brownout__level_1_amp_1_control_3,  0x00},
	{MAX98937_Brownout__level_2_current_limit,  0x00},
	{MAX98937_Brownout__level_2_amp_1_control_1,  0x00},
	{MAX98937_Brownout__level_2_amp_1_control_2,  0x00},
	{MAX98937_Brownout__level_2_amp_1_control_3,  0x00},
	{MAX98937_Brownout__level_3_current_limit,  0x00},
	{MAX98937_Brownout__level_3_amp_1_control_1,  0x00},
	{MAX98937_Brownout__level_3_amp_1_control_2,  0x00},
	{MAX98937_Brownout__level_3_amp_1_control_3,  0x00},
	{MAX98937_Brownout__level_4_current_limit,  0x00},
	{MAX98937_Brownout__level_4_amp_1_control_1,  0x00},
	{MAX98937_Brownout__level_4_amp_1_control_2,  0x00},
	{MAX98937_Brownout__level_4_amp_1_control_3,  0x00},
	{MAX98937_Brownout_AMP_gain_attack_release,  0x00},
	{MAX98937_Brownout_AMP1_clip_mode,  0x00},
	{MAX98937_Brownout__level_1_current_limit,  0x3a},
	{MAX98937_Brownout__level_1_threshold,  0x70},
	{MAX98937_Brownout__level_1_amp_1_control_1,  0x02},
	{MAX98937_Brownout_threshold_hysterysis,  0x14},
...
//
//Speaker DC resistance is 7 +/-15% (5.5 --> 8.5)
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
#define SPEAKER_RDC_MIN  (221680932)  // 5.5 / 3.33 * (1<<27)
#define SPEAKER_RDC_MAX  (342597804)  // 8.5 / 3.33 * (1<<27)
#define SPEAKER_RDC_DEFAULT (282139368)  // 7 / 3.33 * 134217728

//Receiver DC resistance is 6.3 +/-15% (4.8 --> 7.8)
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
// #define EAR_RDC_MIN  (445377145)  // 11.05 / 3.33 * (1<<27)
// #define EAR_RDC_MAX  (602569079)  // 14.95 / 3.33 * (1<<27)
// #define EAR_RDC_DEFAULT (523973112)  // 13 / 3.33 * 134217728

#define EAR_RDC_MIN  (193466995)  // 4.8 / 3.33 * (1<<27)
#define EAR_RDC_MAX  (314383867)  // 7.8 / 3.33 * (1<<27)
#define EAR_RDC_DEFAULT (253925431)  // 6.3 / 3.33 * 134217728
...
{
	if (!max98927) {
		pr_err("%s: max98927 is NULL, can not use dsm\n", __func__);
		return false;
	}else if (max98927->rcv_mix_mode || max98927->factory_test || !max98927->dsm_enable) {
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	// struct snd_soc_codec *codec = container_of(snd_soc_kcontrol_component(kcontrol), struct snd_soc_codec, snd_soc_kcontrol_component(kcontrol));
#endif
	//struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	struct max989xx_priv *max98927 = snd_soc_component_get_drvdata(snd_soc_kcontrol_component(kcontrol));

	if (max98927->bIsMax98937[MAX98927L] != UNKNOWN_ID) {
...
#define CALIBRATE_FILE_L   "/mnt/vendor/persist/media/spkr_calib_l.bin"
#define CALIBRATE_FILE_R   "/mnt/vendor/persist/media/spkr_calib_r.bin"
#define SPK_MUTE_VALUE   (0xCACACACA)
// #define SPK_MUTE_VALUE   274078243
...
//mOhms = (calibrate_value *  DIV_COEF * 3.33) >> 27
#define CALIBRATE_VALUE_TO_MOHMS(x) ((uint32_t)((((uint64_t)(x)) * 3330) >> 27))
#define TEMPERATURE_VALUE_TO_MILLI_CELSIUS(x) ((uint32_t)((((uint64_t)(x)) * DIV_COEF) >> 19))

static inline bool rdc_check_valid(struct max989xx_priv *max98927, uint32_t rdc, int ch)
{
	uint32_t mohms_min, mohms_max;
	uint32_t mohms = 0;

	if ((ch == MAX98927L) || (ch == MAX98927R)) {
		mohms_min = max98927->range_mohms[ch][RANGE_MIN];
		mohms_max = max98927->range_mohms[ch][RANGE_MAX];
	} else {
		pr_err("%s: invalid param ch: %d\n", __func__, ch);
		return false;
	}

	mohms = CALIBRATE_VALUE_TO_MOHMS(rdc);
	pr_info("%s: ch:%d, rdc=%u coversion to %u mOhms\n", __func__, ch, rdc, mohms);

	if (mohms > mohms_min && mohms < mohms_max)
		return true;

	pr_info("%s: mohms=%u error, range[%u, %u] \n",
...
static ssize_t max989xx_dbg_impedance_read(struct file *file,
					     char __user *user_buf, size_t count,
					     loff_t *ppos)
{
#ifdef CONFIG_DEBUG_FS
...
static ssize_t max989xx_dbg_f0_read(struct file *file,
				      char __user *user_buf, size_t count,
				      loff_t *ppos)
{
#ifdef CONFIG_DEBUG_FS
...
static ssize_t max989xx_dbg_temperature_read(struct file *file,
					       char __user *user_buf, size_t count,
					       loff_t *ppos)
{
#ifdef CONFIG_DEBUG_FS
	...
	uint32_t *payload = (uint32_t *)&gParam[PKG_HEADER];
	int ret = 0;
	uint32_t coiltempl, coiltempr = 0;
	char *str;
...
static const struct file_operations max989xx_dbg_calibrate_fops = {
	.open = simple_open,
	.read = max989xx_dbg_calibrate_read,
	.llseek = default_llseek,
};

static ssize_t max989xx_dbg_status_read(struct file *file,
					  char __user *user_buf, size_t count,
					  loff_t *ppos)
{
	loff_t pos = *ppos;
	...
	max989xx_dbg_calibrate_read(file, user_buf, count, ppos);
	*ppos = pos;
						...
						loff_t *ppos)
{
	...
	n = snprintf(str, PAGE_SIZE, "left[%d, %d]",
		max98927->range_mohms[MAX98927L][RANGE_MIN],
		max98927->range_mohms[MAX98927L][RANGE_MAX]);

	if (max98927->mono_stereo == 3) {
		n += snprintf(str + n, PAGE_SIZE - n, ", right[%d, %d]\n",
		max98927->range_mohms[MAX98927R][RANGE_MIN],
...
static ssize_t max989xx_dbg_calibration_read(struct file *file,
						char __user *user_buf, size_t count,
						loff_t *ppos)
{
#ifdef CONFIG_DEBUG_FS
	...
	max989xx_calib_get(&calibration_value, MAX98927L);
	n = snprintf(str, PAGE_SIZE, "left=%d",
		CALIBRATE_VALUE_TO_MOHMS(calibration_value));
	if (max98927->mono_stereo == 3) {
		max989xx_calib_get(&calibration_value, MAX98927R);
		n += snprintf(str+n, PAGE_SIZE-n, ", right=%d\n",
			CALIBRATE_VALUE_TO_MOHMS(calibration_value));
	}
	pr_info("%s: read str = %s \n", __func__, str);
...
static const struct file_operations max989xx_dbg_impedance_fops = {
	.open = simple_open,
	.read = max989xx_dbg_impedance_read,
	.llseek = default_llseek,
};

static const struct file_operations max989xx_dbg_f0_fops = {
	.open = simple_open,
	.read = max989xx_dbg_f0_read,
	.llseek = default_llseek,
};

static const struct file_operations max989xx_dbg_temperature_fops = {
	.open = simple_open,
	.read = max989xx_dbg_temperature_read,
	.llseek = default_llseek,
};

static const struct file_operations max989xx_dbg_status_fops = {
	.open = simple_open,
...
static const struct file_operations max989xx_dbg_calibartion_fops = {
	.open = simple_open,
	.read = max989xx_dbg_calibration_read,
	.llseek = default_llseek,
};
#endif /* OPLUS_ARCH_EXTENDS */
...
static void max989xx_debug_init(struct max989xx_priv *max98927, struct i2c_client *i2c)
{
	...
	debugfs_create_file("calibration", S_IRUGO|S_IWUGO, max98927->dbg_dir,
		i2c, &max989xx_dbg_calibartion_fops);
	#endif /* OPLUS_ARCH_EXTENDS */
#else
	...
	proc_create_data("impedance", S_IRUGO|S_IWUGO, max98927->dbg_dir,
		&max989xx_dbg_impedance_fops, i2c);
	proc_create_data("f0detect", S_IRUGO|S_IWUGO, max98927->dbg_dir,
		&max989xx_dbg_f0_fops, i2c);
	proc_create_data("temperature", S_IRUGO|S_IWUGO, max98927->dbg_dir,
	...
	proc_create_data("calibration", S_IRUGO|S_IWUGO, max98927->dbg_dir,
		&max989xx_dbg_calibartion_fops, i2c);
	#endif /* OPLUS_ARCH_EXTENDS */
#endif
}
...
#else
		remove_proc_entry("max98927L", NULL);
#endif
    }
}

#define MAX98927_MAX_RETRY_TIMES   6
#define MAX98927_DELAY_MS          5

static int max98927_regmap_update_bits(struct regmap *map, unsigned int reg,
						unsigned int mask, unsigned int val)
{
	int i;
	int rc;

	for (i = 0; i < MAX98927_MAX_RETRY_TIMES; i++) {
		rc = regmap_update_bits(map, reg, mask, val);
		if (!rc) {
			break;
		}
		pr_err("%s: reg:0x%x, val: 0x%x, retry(%d), rc = %d\n", __func__, reg, val, i+1, rc);
		msleep(MAX98927_DELAY_MS);
	}
...
static int max98927_stream_mute(struct snd_soc_dai *codec_dai, int mute, int stream)
{
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (mute) {
			...
			unsigned int pa0_reg4, pa0_reg5, pa0_reg6, amp_reg;
			#ifdef OPLUS_FEATURE_SPEAKER_MUTE
			pa_state_mark = 0;
			pr_info("%s: dsm_enable=%d\n", __func__, max98927->dsm_enable);
			if (max98927->dsm_enable == 1) {
			...
			/* if (max98927->mono_stereo != 0x3){ */
				...
				max98927_wrapper_read(max98927, 0, 0x04, &pa0_reg4);
				max98927_wrapper_read(max98927, 0, 0x05, &pa0_reg5);
				max98927_wrapper_read(max98927, 0, 0x06, &pa0_reg6);
				max98927_wrapper_read(max98927, 0, amp_enable, &amp_reg);

				pr_info("%s: PA0:0x04=0x%x,0x05=0x%x,0x06=0x%x, 0x%x=0x%x\n",
					__func__, pa0_reg4, pa0_reg5, pa0_reg6, amp_enable, amp_reg);
	...
	if (!max98927->bIsMax98937[MAX98927L])
		return max98927_reg_get(kcontrol, ucontrol, MAX98927_Boost_Control_1,
			MAX98927_Boost_Control_1_BST_ILIM_Mask, MAX98927_BST_ILIM_SHIFT);
                ...
                int data;
                max98927_wrapper_read(max98927, 0, MAX98937_Boost_Control_1, &data);
                ucontrol->value.integer.value[0] =
                    ((data & MAX98937_Boost_Control_1_BST_ILIM_Mask) - 2 ) >> MAX98927_BST_ILIM_SHIFT;
                return 0;
	}
}
		...
		if(i2c_states & (1 << i)){
			if (max98927->bIsMax98937[i])
				regmap_update_bits(max98927->regmap[i], MAX98937_Boost_Control_1,
						MAX98937_Boost_Control_1_BST_ILIM_Mask, (sel << MAX98927_BST_ILIM_SHIFT) + 2);
			else
...
static int max98927_boost_voltage_get_l(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
//	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
//	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	...
	if(i2c_states & MAX98927_CH0){
		if (!max98927->bIsMax98937[MAX98927L]){
			regmap_read(max98927->regmap[MAX98927L], MAX98927_Boost_Control_0, &data);
			ucontrol->value.integer.value[0] =
				(data & MAX98927_Boost_Control_0_BST_VOUT_Mask);
		}else{
			regmap_read(max98927->regmap[MAX98927L], MAX98937_Boost_Control_0, &data);
			ucontrol->value.integer.value[0] =
				(data & MAX98937_Boost_Control_0_BST_VOUT_Mask);
		}
...
static int max98927_boost_voltage_put_l(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
//	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
//	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	...
	if(i2c_states & MAX98927_CH0){
		if (max98927->bIsMax98937[MAX98927L])
			max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98937_Boost_Control_0,
				MAX98937_Boost_Control_0_BST_VOUT_Mask, sel);
		else
			max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98927_Boost_Control_0,
				MAX98927_Boost_Control_0_BST_VOUT_Mask, sel);
	}
	return 0;
}

static int max98927_boost_voltage_get_r(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
//	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
//	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
    ...
    if(i2c_states & MAX98927_CH1){
		if (!max98927->bIsMax98937[MAX98927R]){
			regmap_read(max98927->regmap[MAX98927R], MAX98927_Boost_Control_0, &data);
			ucontrol->value.integer.value[0] =
				(data & MAX98927_Boost_Control_0_BST_VOUT_Mask);
		}else{
			regmap_read(max98927->regmap[MAX98927R], MAX98937_Boost_Control_0, &data);
			ucontrol->value.integer.value[0] =
				(data & MAX98937_Boost_Control_0_BST_VOUT_Mask);
		}
		pr_info("%s: value:%d", __func__, data);
...
static int max98927_boost_voltage_put_r(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
//	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
//	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
    ...
    if(i2c_states & MAX98927_CH1){
        if (max98927->bIsMax98937[MAX98927R])
            max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98937_Boost_Control_0,
                    MAX98937_Boost_Control_0_BST_VOUT_Mask, sel);
        else
            max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98927_Boost_Control_0,
                    MAX98927_Boost_Control_0_BST_VOUT_Mask, sel);
    }
	return 0;
}

static int max98927_boost_input_limit_get_l(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
//	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
//	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
    ...
    if(i2c_states & MAX98927_CH0){
        if (!max98927->bIsMax98937[MAX98927L]){
            regmap_read(max98927->regmap[MAX98927L], MAX98927_Boost_Control_1, &data);
            ucontrol->value.integer.value[0] =
                (data & MAX98927_Boost_Control_1_BST_ILIM_Mask) >>  MAX98927_BST_ILIM_SHIFT;
        }else {
            regmap_read(max98927->regmap[MAX98927L], MAX98937_Boost_Control_1, &data);
            ucontrol->value.integer.value[0] =
                ((data & MAX98937_Boost_Control_1_BST_ILIM_Mask) - 2 ) >> MAX98927_BST_ILIM_SHIFT;
        }
    }
...
static int max98927_boost_input_limit_put_l(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
//	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
//	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	...
	if(i2c_states & MAX98927_CH0){
		if (max98927->bIsMax98937[MAX98927L])
			max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98937_Boost_Control_1,
					MAX98937_Boost_Control_1_BST_ILIM_Mask, (sel << MAX98927_BST_ILIM_SHIFT) + 2);
		else
			max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98927_Boost_Control_1,
					MAX98927_Boost_Control_1_BST_ILIM_Mask, sel << MAX98927_BST_ILIM_SHIFT);
	}
	return 0;
}

static int max98927_boost_input_limit_get_r(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
//	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
//	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
    ...
    if(i2c_states & MAX98927_CH1){
        if (!max98927->bIsMax98937[MAX98927R]){
            regmap_read(max98927->regmap[MAX98927R], MAX98927_Boost_Control_1, &data);
            ucontrol->value.integer.value[0] =
                (data & MAX98927_Boost_Control_1_BST_ILIM_Mask) >>  MAX98927_BST_ILIM_SHIFT;
        }else {
            regmap_read(max98927->regmap[MAX98927R], MAX98937_Boost_Control_1, &data);
            ucontrol->value.integer.value[0] =
                ((data & MAX98937_Boost_Control_1_BST_ILIM_Mask) - 2 ) >> MAX98927_BST_ILIM_SHIFT;
        }
    }
...
static int max98927_boost_input_limit_put_r(struct snd_kcontrol *kcontrol,
						struct snd_ctl_elem_value *ucontrol)
{
//	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
//	struct max989xx_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	...
	if(i2c_states & MAX98927_CH1){
		if (max98927->bIsMax98937[MAX98927R])
			max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98937_Boost_Control_1,
					MAX98937_Boost_Control_1_BST_ILIM_Mask, (sel << MAX98927_BST_ILIM_SHIFT) + 2);
		else
			max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98927_Boost_Control_1,
					MAX98927_Boost_Control_1_BST_ILIM_Mask, sel << MAX98927_BST_ILIM_SHIFT);
	}
	return 0;
}
					    ...
					    struct snd_ctl_elem_value *ucontrol)
{
	...
	if(i2c_states & MAX98927_CH0){
	    if (!max98927->bIsMax98937[MAX98927L]){
			...
			ucontrol->value.integer.value[0] = 	(data_amp & MAX98927_AMP_enables_SPK_EN);
		}else{
			regmap_read(max98927->regmap[MAX98927L], MAX98937_AMP_enables, &data_amp);
			ucontrol->value.integer.value[0] = (data_amp & MAX98937_AMP_enables_SPK_EN);
		}
	}
...
#ifdef OPLUS_FEATURE_SPEAKER_MUTE
static int max98927_spk_mute_ctrl_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
...
static int max98927_spk_mute_ctrl_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct max989xx_priv *max98927 = snd_soc_component_get_drvdata(component);
	...
	pr_info("%s: dsm_enable=%d\n", __func__, max98927->dsm_enable);
	/* Do not control in handset mode, just return */
	if (max98927->dsm_enable == 0) {
		pr_info("%s: handset or normal mode, no need to mute speaker\n", __func__);
		return 0;
	}
	...
	if (val) {
		...
		if (pa_state_mark == 0) {
			...
			pr_warn("%s: speaker muted by normally conduct!\n");
		}
	}
	return 0;
}
...
static const struct snd_kcontrol_new max98927_snd_controls[] = {
	...
	SOC_SINGLE_EXT("BDE Enable", MAX98937_Brownout_enables,
		0, 1, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0:disable 1:enable
	SOC_SINGLE_EXT("Amp DSP Enable", MAX98937_Brownout_enables,
		MAX98927_BDE_DSP_SHIFT, 1, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0:disable 1:enable
	SOC_SINGLE_EXT("BDE AMP Enable", MAX98937_Brownout_enables,
	...
	SOC_SINGLE_EXT("Level1 Threshold", MAX98937_Brownout__level_1_threshold,
		0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 255
	SOC_SINGLE_EXT("Level2 Threshold", MAX98937_Brownout__level_2_threshold,
		0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 255
	SOC_SINGLE_EXT("Level3 Threshold", MAX98937_Brownout__level_3_threshold,
		0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 255
	SOC_SINGLE_EXT("Level4 Threshold", MAX98937_Brownout__level_4_threshold,
		0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 255
	SOC_SINGLE_EXT("Level1 Current Limit", MAX98937_Brownout__level_1_current_limit,
		0, 63, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level2 Current Limit", MAX98937_Brownout__level_2_current_limit,
		0, 63, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level3 Current Limit", MAX98937_Brownout__level_3_current_limit,
		0, 63, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level4 Current Limit", MAX98937_Brownout__level_4_current_limit,
		0, 63, 0, max98927_reg_get_w, max98927_reg_put_w),
	//0 ~ 63
	SOC_SINGLE_EXT("Level1 Amp1 Control1", MAX98937_Brownout__level_1_amp_1_control_1,
		0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),

	SOC_SINGLE_EXT("Level1 Amp1 Control2", MAX98937_Brownout__level_1_amp_1_control_2,
		0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),

	SOC_SINGLE_EXT("Level1 Amp1 Control3", MAX98937_Brownout__level_1_amp_1_control_3,
		0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),

	SOC_SINGLE_EXT("Threshold Hysteresis",  MAX98937_Brownout_threshold_hysterysis,
		0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),

	SOC_SINGLE_EXT("Amp Limiter Attack Release",  MAX98937_Brownout_AMP_limiter_attack_release,
		0, 255, 0, max98927_reg_get_w, max98927_reg_put_w),
	...
	SOC_ENUM_EXT("Left Boost Output Voltage", max98927_enum[2],
		max98927_boost_voltage_get_l, max98927_boost_voltage_put_l),

	SOC_ENUM_EXT("Right Boost Output Voltage", max98927_enum[2],
		max98927_boost_voltage_get_r, max98927_boost_voltage_put_r),

	//boost voltage
	SOC_ENUM_EXT("Left Boost Current Limit", max98927_enum[4],
		max98927_boost_input_limit_get_l, max98927_boost_input_limit_put_l),

	//boost voltage
	SOC_ENUM_EXT("Right Boost Current Limit", max98927_enum[4],
		max98927_boost_input_limit_get_r, max98927_boost_input_limit_put_r),
	...
	SOC_ENUM_EXT("Speaker_Mute_Switch", spk_mute_ctrl_enum,
			max98927_spk_mute_ctrl_get, max98927_spk_mute_ctrl_put),
	#endif /* OPLUS_FEATURE_SPEAKER_MUTE */
};
...
static const struct snd_soc_component_driver soc_codec_dev_max98927 = {
	.name = MAX989XX_DRV_NAME,
	.probe = max98927_probe,
	.controls = max98927_snd_controls,
	.num_controls = ARRAY_SIZE(max98927_snd_controls),
	.dapm_widgets = max98927_dapm_widgets,
	...
	struct device *dev, int id)
{
	...
	if ((value == RANGE_COUNT) &&
	    !of_property_read_u32_array(dNode, "range_mohms", max98927->range_mohms[id], value)) {
		pr_err("%s: get range_mohms min: %d, max: %d\n ", __func__,
			max98927->range_mohms[id][RANGE_MIN],
			max98927->range_mohms[id][RANGE_MAX]);
	} else {
		pr_err("%s: get range_mohms error, count: %d, set default\n ", __func__, value);
    ...
    if (id == MAX98927L) {
        ...
        gpio_free(max98927->reset_gpio_r);
			      ...
			      const struct i2c_device_id *id)
{
	...
	if(presence){
		...
		if(max98927->dev == NULL){
			...
			ret = snd_soc_register_component(&i2c->dev, &soc_codec_dev_max98927,
					&max98927_dai[0], ARRAY_SIZE(max98927_dai));
			if (ret < 0) {
