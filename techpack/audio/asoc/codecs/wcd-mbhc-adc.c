...
static void wcd_mbhc_adc_detect_plug_type(struct wcd_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cb->mbhc_micbias_control) {
		...
		if (mbhc->headset_bias_alwayon) {
			mbhc->micbias_enable = true;
			pr_info("%s: Mic Bias is set for alwayon\n", __func__);
		}
#endif /* OPLUS_ARCH_EXTENDS */
	} else {
...
#ifdef OPLUS_ARCH_EXTENDS
static void wcd_mech_irq_dwork(struct work_struct *work)
{
	struct delayed_work *dwork;
	struct wcd_mbhc *mbhc;

	dwork = to_delayed_work(work);
	mbhc = container_of(dwork, struct wcd_mbhc, mech_irq_trigger_dwork);
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	if (((plug_type != MBHC_PLUG_TYPE_HEADSET) ||
		...
		if (mbhc->mbhc_cfg->swap_gnd_mic &&
			...
			plug_type_second = wcd_mbhc_get_plug_from_adc(
				mbhc, output_mv_second);
			pr_debug(
				"%s: second check output_mv = %d, plug_type = %d\n",
...
static irqreturn_t wcd_mbhc_adc_hs_ins_irq(int irq, void *data)
{
	...
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE) {
		...
		if (mbhc->irq_trigger_enable) {
			WCD_MBHC_RSC_UNLOCK(mbhc);
			pr_info("%s: schedule mech_irq_trigger_dwork.\n",
				__func__);
			schedule_delayed_work(&mbhc->mech_irq_trigger_dwork,
...
#ifdef OPLUS_ARCH_EXTENDS
	if (mbhc->irq_trigger_enable) {
		INIT_DELAYED_WORK(&mbhc->mech_irq_trigger_dwork,
				  wcd_mech_irq_dwork);
	}
#endif /* OPLUS_ARCH_EXTENDS */
}
