				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_CHRDET_STATE:
		...
		break;
	case TCP_NOTIFY_BC12_COMPLETE_STATE:
		pinfo->bc12_complete = noti->bc12_complete_state.bc12_complete;
		pr_err("%s bc12 complete = %d\n", __func__, noti->bc12_complete_state.bc12_complete);
...
int oplus_chg_set_pd_config(void)
{
	...
	printk(KERN_ERR "%s: pd_type %d pd9v svooc [%d %d %d]",
		__func__, pinfo->pd_type, chip->limits.vbatt_pdqc_to_9v_thr,
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (pinfo->tcpc && !pinfo->wd0_detect && tcpm_is_floating_ground(pinfo->tcpc)) {
		tcpm_typec_change_role_postpone(pinfo->tcpc, TYPEC_ROLE_SNK, true);
	}
