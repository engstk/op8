...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_WAKELOCK_TIMEOUT_ENABLE:
		...
		wake_lock_timeout(&gf_cmd_wakelock,
				  msecs_to_jiffies(SENDCMD_WAKELOCK_HOLD_TIME));
		break;
...
ssize_t gf_read(struct file *f, char __user *buf, size_t count, loff_t *offset)
{
	...
	if (buf == NULL || f == NULL ||
	    count != sizeof(struct fingerprint_message_t)) {
		return 0;
	}
	...
	if (evdata && evdata->data && val == FB_EARLY_EVENT_BLANK && gf_dev) {
		...
		case FB_BLANK_POWERDOWN:
			if (gf_dev->device_available == 1) {
				...
				sendnlmsg(&msg);
				send_fingerprint_message(E_FP_SENSOR, msg, NULL,
							 0);
#elif defined(GF_FASYNC)
				if (gf_dev->async) {
...
static int gf_opticalfp_irq_handler(struct fp_underscreen_info *tp_info)
{
	...
	if (1 == tp_info->touch_state) {
		...
		send_fingerprint_message(E_FP_TP, tp_info->touch_state, tp_info,
					 sizeof(struct fp_underscreen_info));
	} else {
...
#endif
{
	...
	if (gfspi_ioctl_clk_init(gf_dev))
		goto gfspi_probe_clk_init_failed :
			...
			if (gfspi_ioctl_clk_enable(
				    gf_dev)) goto gfspi_probe_clk_enable_failed;
