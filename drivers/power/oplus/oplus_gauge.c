...
#endif
	int
	oplus_gauge_get_batt_mvolts(void)
{
	if (!g_gauge_chip) {
		return 3800;
	} else {
...
#endif
	int
	oplus_gauge_get_batt_mvolts_2cell_max(void)
{
	if (!g_gauge_chip)
		return 3800;
	else
...
#endif
	int
	oplus_gauge_get_batt_mvolts_2cell_min(void)
{
	if (!g_gauge_chip)
		return 3800;
	else
...
#endif
	int
	oplus_gauge_get_batt_current(void)
{
	if (!g_gauge_chip) {
		return 100;
	} else {
...
#endif
	int
	oplus_gauge_get_sub_current(void)
{
	if (!g_gauge_chip) {
		return 100;
	} else {
...
{
	if (!g_sub_gauge_chip || !g_sub_gauge_chip->gauge_ops ||
	    !g_sub_gauge_chip->gauge_ops->get_prev_battery_mvolts) {
		return 3800;
	} else {
...
#endif
	int
	oplus_gauge_get_prev_batt_mvolts_2cell_max(void)
{
	if (!g_gauge_chip)
		return 3800;
	else {
...
#endif
	int
	oplus_gauge_get_prev_batt_mvolts_2cell_min(void)
{
	if (!g_gauge_chip)
		return 3800;
	else {
...
#endif
	int
	oplus_gauge_get_prev_batt_soc(void)
{
	if (!g_gauge_chip)
		return 50;
	else
...
#endif
	int
	oplus_gauge_get_prev_batt_current(void)
{
	if (!g_gauge_chip)
		return 100;
	else {
...
#endif
	int
	oplus_gauge_get_sub_batt_mvolts(void)
{
...
#endif
	int
	oplus_gauge_get_sub_batt_current(void)
{
...
#endif
	int
	oplus_gauge_get_sub_batt_soc(void)
{
...
#endif
	int
	oplus_gauge_get_sub_batt_temperature(void)
{
	int batt_temp = 0;
	if (!g_sub_gauge_chip || !g_sub_gauge_chip->gauge_ops ||
