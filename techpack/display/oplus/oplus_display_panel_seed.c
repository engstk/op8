...
int __oplus_display_set_seed(int mode)
{
	...
	if (is_support_panel_dc_seed_mode_flag(
...
int dsi_panel_seed_mode_unlock(struct dsi_panel *panel, int mode)
{
	...
	if (oplus_dc_v2_on) {
		...
		int frame_time_us = mult_frac(
			1000, 1000, panel->cur_mode->timing.refresh_rate);
		rc = is_support_panel_dc_exit_backlight_select(panel,
							       frame_time_us);
		if (rc) {
...
int dsi_panel_dc_seed_mode_unlock(struct dsi_panel *panel, int mode)
{
	...
	switch (mode) {
	case OPLUS_SEED_MODE0:
		if ((panel->is_dc_set_color_mode) && (!panel->is_hbm_enabled) &&
		    (oplus_dc_v2_on)) {
		...
		} else if ((panel->is_dc_set_color_mode) &&
			   (!panel->is_hbm_enabled) && (!oplus_dc_v2_on)) {
			panel->is_dc_set_color_mode = false;
			rc = dsi_panel_tx_cmd_set(panel,
...
int dsi_panel_seed_mode(struct dsi_panel *panel, int mode)
{
	...
	if ((!strcmp(panel->oplus_priv.vendor_name, "S6E3HC3") ||
		...
		rc = dsi_panel_loading_effect_mode_unlock(panel, mode);
	} else if (is_support_panel_seed_mode_exceed(
