...
int __attribute__((weak)) request_firmware_select(const struct firmware **firmware_p, const char *name,
		...
		if (!ret) {
			...
			if (chip->vops->fw_check_then_recover) {
				...
				if (ret == FW_CHECK_MODE) {
					chg_debug(
						"update finish, then clean fastchg_dummy , fastchg_started, watch_dog\n");
					chip->fastchg_dummy_started = false;
					chip->fastchg_started = false;
					chip->allow_reading = true;
					del_timer(&chip->watchdog);
...
static int vooc_dump_log_data(char *buffer, int size, void *dev_data)
{
	...
	snprintf(buffer, size, ",%d,%d,%d", oplus_vooc_get_fastchg_started(), oplus_vooc_get_fastchg_dummy_started(),
