			 ...
			 long mask)
{
	...
	case IIO_CHAN_INFO_OFFSET:
		ret = adc_do_conversion(adc, prop, chan, &adc_code_volt,
					&adc_code_cur);
		if (ret)
			break;
		if (*val2 < 0) {
			voltage = (s64)adc_code_volt * adc_vdd_ref_mv * 1000;
			voltage = div64_s64(voltage,
					    adc->data->full_scale_code_volt);
			...
			adc_code_volt = (u16)div64_s64(voltage,
						       (adc_vdd_ref_mv * 1000));
		}
		if ((chan->type == IIO_VOLTAGE) || (chan->type == IIO_TEMP))
			ret = qcom_vadc_hw_scale(
				prop->scale_fn_type,
				&adc_prescale_ratios[prop->prescale], adc->data,
				prop->lut_index, adc_code_volt, val);
		if (ret)
			break;
