...
static struct ini_file_data *ilitek_ini_file_data;
...
static int ilitek_tddi_mp_ini_parser(const char *path)
{
    ...
    ipio_memcpy(tmp, fw->data, fw->size, fw->size);
    /* change all characters to lower case */
    for (i = 0; i < (int)fw->size; i++)
        tmp[i] = tolower(tmp[i]);

    ret = parser_get_ini_phy_data(tmp, (int)fw->size);
    if (ret < 0) {
...
static void run_pixel_test(int index)
{
    ...
    for (y = 0; y < core_mp.ych_len; y++) {
        for (x = 0; x < core_mp.xch_len; x++) {
             ...
             */
            if (y == 0 && x == 0) {
                tmp[0] = abs(centre - p_comb[(shift + 1) + x]);    /* down */
                tmp[1] = abs(centre - p_comb[shift + (x + 1)]);    /* right */
            } else if (y == (core_mp.ych_len - 1) && x == 0) {
                tmp[0] = abs(centre - p_comb[(shift - 1) + x]);    /* up */
                tmp[1] = abs(centre - p_comb[shift + (x + 1)]);    /* right */
            } else if (y == 0 && x == (core_mp.xch_len - 1)) {
                tmp[0] = abs(centre - p_comb[(shift + 1) + x]);    /* down */
                tmp[1] = abs(centre - p_comb[shift + (x - 1)]);    /* left */
                ...
                tmp[0] = abs(centre - p_comb[(shift + 1) + x]);    /* down */
                tmp[1] = abs(centre - p_comb[shift + (x - 1)]);    /* left */
                tmp[2] = abs(centre - p_comb[shift + (x + 1)]);    /* right */
            } else if (y != 0 && x == 0) {
                tmp[0] = abs(centre - p_comb[(shift - 1) + x]);    /* up */
                tmp[1] = abs(centre - p_comb[shift + (x + 1)]);    /* right */
                tmp[2] = abs(centre - p_comb[(shift + 1) + x]);    /* down */
                ...
                tmp[0] = abs(centre - p_comb[(shift - 1) + x]);    /* up */
                tmp[1] = abs(centre - p_comb[(shift + 1) + x]);    /* down */
                tmp[2] = abs(centre - p_comb[shift + (x - 1)]);    /* left */
                tmp[3] = abs(centre - p_comb[shift + (x + 1)]);    /* right */
            }
    ...
    for (y = 0; y < core_mp.ych_len; y++) {
        for (x = 0; x < core_mp.xch_len; x++) {
            ...
            if (y != (core_mp.ych_len - 1))
                core_mp.tx_delta_buf[shift + x] = abs(p_comb[shift + x] - p_comb[(shift + 1) + x]);

            /* Rx Delta */
            if (x != (core_mp.xch_len - 1))
                core_mp.rx_delta_buf[shift + x] = abs(p_comb[shift + x] - p_comb[shift + (x + 1)]);
        }
    }
}
...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line)
{
    ...
    tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len,
                        "This is %s module\n",
                        idev->ts->panel_data.manufacture_info.manufacture);
    ...
    tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len,
                        "Final Result ,%s\n",
                        (core_mp.final_result == MP_PASS) ? "PASS" : "FAIL");
    ...
    if (tItems[index].catalog == TX_RX_DELTA) {
        ...
        if (tItems[index].buf == NULL) {
            ...
            if (ERR_ALLOC_MEM(tItems[index].buf)) {
                ...
                ipio_vfree((void **)&tItems[index].buf);
                return -ENOMEM;
            }
        }
...
static int open_test_sp(int index)
{
    ...
    TPD_DEBUG("open_test_sp: frame_cont %d, AA %d, Border %d, Notch %d, full_open_rate %d\n",
...
{
    int ret, i, x, y, j, csv_len = 0, pass_item_count = 0, line_count = 0, get_frame_cont = 1;
    s32 *max_threshold = NULL, *min_threshold = NULL;
    char *csv = NULL;
    char csv_name[128] = { 0 };
    char tp_module[32] = { 0 };
    ...
    if (core_mp.final_result == MP_FAIL) {
        ...
        if (lcm_on) {
            ret = dev_mkdir(CSV_LCM_ON_NG_PATH, S_IRUGO | S_IWUSR);
            csv_path = CSV_LCM_ON_NG_PATH;
        } else {
            ret = dev_mkdir(CSV_LCM_OFF_NG_PATH, S_IRUGO | S_IWUSR);
            csv_path = CSV_LCM_OFF_NG_PATH;
        }
    } else {
        /* Create the directory for mp_test result */
        if (lcm_on) {
            ret = dev_mkdir(CSV_LCM_ON_OK_PATH, S_IRUGO | S_IWUSR);
            csv_path = CSV_LCM_ON_OK_PATH;
        } else {
            ret = dev_mkdir(CSV_LCM_OFF_OK_PATH, S_IRUGO | S_IWUSR);
            csv_path = CSV_LCM_OFF_OK_PATH;
        }
    }
    if (ret < 0)
...
int ilitek_tddi_mp_test_main(char *apk, struct seq_file *s, char *message, bool lcm_on)
{
    ...
    ret = ilitek_tddi_mp_ini_parser(idev->test_limit_name);
    if (ret < 0) {
