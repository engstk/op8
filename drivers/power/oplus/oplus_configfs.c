...
static struct device_attribute *oplus_ac_attributes[] = { &dev_attr_online, &dev_attr_type, NULL };
...
static ssize_t fast_chg_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_PD_SDP ||
	    (CHARGER_SUBTYPE_PD == type &&
	     (chip->pd_svooc || chip->charger_type == POWER_SUPPLY_TYPE_USB ||
	      chip->charger_type == POWER_SUPPLY_TYPE_USB_CDP || chip->charger_type == POWER_SUPPLY_TYPE_UNKNOWN))) {
...
static struct device_attribute *oplus_usb_attributes[] = {
	&dev_attr_otg_online,		&dev_attr_otg_switch,		&dev_attr_usb_status,
	&dev_attr_typec_cc_orientation, &dev_attr_fast_chg_type,	&dev_attr_usbtemp_volt_l,
	&dev_attr_usbtemp_volt_r,	&dev_attr_water_detect_feature, NULL
};
...
static ssize_t ppschg_ing_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (val == 0 && oplus_quirks_keep_connect_status() == 1 && oplus_voocphy_get_fastchg_start() == 0) {
...
static ssize_t ppschg_power_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (val == OPLUS_PPS_POWER_CLR && oplus_quirks_keep_connect_status() == 1 &&
...
static ssize_t bcc_current_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (is_wls_ocm_available(chip)) {
		pval.intval = val;
		oplus_chg_mod_set_property(chip->wls_ocm, OPLUS_CHG_PROP_BCC_CURRENT, &pval);
	}
...
static ssize_t battery_charging_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t bms_heat_temp_compensation_store(struct device *dev, struct device_attribute *attr, const char *buf,
						size_t count)
{
	int val = 0;
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t battery_log_head_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return battery_log_common_operate(BATTERY_LOG_DUMP_LOG_HEAD, buf, BATTERY_LOG_MAX_SIZE);
}
static DEVICE_ATTR_RO(battery_log_head);

static ssize_t battery_log_content_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct oplus_configfs_device *chip = dev->driver_data;
	...
	return battery_log_common_operate(BATTERY_LOG_DUMP_LOG_CONTENT, buf, BATTERY_LOG_MAX_SIZE);
}
static DEVICE_ATTR_RO(battery_log_content);

static ssize_t pkg_name_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct oplus_chg_chip *chip = NULL;
...
static struct device_attribute *oplus_battery_attributes[] = { &dev_attr_authenticate,
							       &dev_attr_battery_cc,
...
ssize_t __attribute__((weak)) oplus_chg_wls_upgrade_fw_show(struct device *dev, struct device_attribute *attr,
							    char *buf)
{
	return 0;
}
ssize_t __attribute__((weak)) oplus_chg_wls_upgrade_fw_store(struct device *dev, struct device_attribute *attr,
...
static struct device_attribute *oplus_wireless_attributes[] = {
	&dev_attr_tx_voltage_now,   &dev_attr_tx_current_now, &dev_attr_cp_voltage_now,
	&dev_attr_cp_current_now,   &dev_attr_wireless_mode,  &dev_attr_wireless_type,
	&dev_attr_cep_info,	    &dev_attr_real_type,
#ifdef OPLUS_CHG_ADB_ROOT_ENABLE
	&dev_attr_upgrade_firmware,
#endif
	&dev_attr_status_keep,	    &dev_attr_max_w_power,    NULL
};
...
**********************************************************************/
#ifdef OPLUS_CHG_ADB_ROOT_ENABLE
ssize_t __attribute__((weak)) oplus_chg_comm_charge_parameter_show(struct device *dev, struct device_attribute *attr,
								   char *buf)
{
	return 0;
}
ssize_t __attribute__((weak)) oplus_chg_comm_charge_parameter_store(struct device *dev, struct device_attribute *attr,
								    const char *buf, size_t count)
{
	return 0;
}
...
ssize_t __attribute__((weak)) oplus_chg_comm_response_mutual_cmd(struct oplus_chg_mod *comm_ocm, const char *buf,
								 size_t count)
{
	return -EINVAL;
}
...
static ssize_t battlog_push_config_store(struct device *dev, struct device_attribute *attr, const char *buf,
					 size_t count)
{
	char buffer[2] = { 0 };
	int val = 0;
...
static ssize_t bob_status_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int val = 0;
	struct oplus_chg_chip *chip = NULL;
