...
extern void input_report_key_oplus(struct input_dev *dev, unsigned int code, int value);
/*------------------------------------------Fuction Declare----------------------------------------------*/
...
static void int_key(struct synaptics_ts_data *ts)
{
    ...
    switch(button_key&0x03)
    {
        case 0: //up
            ...
            {
                input_report_key_oplus(ts->input_dev, KEY_MENU, 0);
                input_sync(ts->input_dev);
            }
            else if (ts->pre_btn_state == 2)
            {
                input_report_key_oplus(ts->input_dev, KEY_BACK, 0);
                input_sync(ts->input_dev);
            }
            ts->pre_btn_state = 0;
#ifdef LCD_TRIGGER_KEY_FORCE_CAL
        ...
        case 1: //down
            ...
            input_report_key_oplus(ts->input_dev, KEY_MENU, 1);
            input_sync(ts->input_dev);
#ifdef LCD_TRIGGER_KEY_FORCE_CAL
        ...
        case 2: //down
            ...
            input_report_key_oplus(ts->input_dev, KEY_BACK, 1);
            input_sync(ts->input_dev);
#ifdef LCD_TRIGGER_KEY_FORCE_CAL
            key_press_all_the_time = 1;
#endif
...
static const struct file_operations oplus_tp_delta_data_proc_fops =
{
    .read = tp_delta_show,
    .owner = THIS_MODULE,
};

static const struct file_operations oplus_tp_baseline_image_proc_fops =
{
