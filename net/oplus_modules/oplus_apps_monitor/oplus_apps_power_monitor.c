...
enum {
	APPS_POWER_MONITOR_MSG_DL_CTRL = 12,
...
static struct timespec64 last_report_tv;
...
void oplus_app_monitor_send_app_info_report(void)
{
	...
	write_lock_bh(&app_power_monitor_lock);
	...
	write_unlock_bh(&app_power_monitor_lock);
	...
	ktime_get_real_ts64(&last_report_tv);
	return;
}

static void apps_monitor_report_timer_function(struct timer_list *t)
{
...
int app_monitor_dl_ctl_msg_handle(struct  nlattr *nla)
{
	struct app_monitor_dl_ctl_msg_st *dl_ctl_msg = (struct app_monitor_dl_ctl_msg_st *)NLA_DATA(nla);
...
int app_monitor_dl_report_msg_handle(struct nlattr *nla)
{
