...
static struct aw_vmax_map vmax_map[] = {
	{ 800, 0x28, 0x40 }, //6.0V
	{ 900, 0x28, 0x49 },  { 1000, 0x28, 0x51 }, { 1100, 0x28, 0x5A },
	{ 1200, 0x28, 0x62 }, { 1300, 0x28, 0x6B }, { 1400, 0x28, 0x73 },
	{ 1500, 0x28, 0x7C }, { 1600, 0x2A, 0x80 }, //6.142
...
static int parse_vibrate_param_dts(struct device *dev,
				   ...
				   struct device_node *np)
{
	...
	aw_haptic->livetap_support =
		of_property_read_bool(np, "oplus,livetap_support");
	aw_dev_info("oplus,livetap_support = %d\n", aw_haptic->livetap_support);

	if (of_property_read_u32(np, "oplus,aw86927_boost_voltage",
				 &max_boost_voltage))
		AW86927_HAPTIC_HIGH_LEVEL_REG_VAL =
			DEFAULT_BOOST_VOLT; /* boost 8.4V */
	else
		AW86927_HAPTIC_HIGH_LEVEL_REG_VAL = max_boost_voltage;
	aw_dev_info("%s: aw86927 boost_voltage=%d\n", __func__,
		    AW86927_HAPTIC_HIGH_LEVEL_REG_VAL);

	val = of_property_read_u8_array(np, "haptic_hv_vmax", vmax,
					ARRAY_SIZE(vmax));
	if (val != 0) {
	...
	val = of_property_read_u8_array(np, "haptic_hv_gain", gain,
					ARRAY_SIZE(gain));
	if (val != 0) {
...
static int rtp_update(struct aw_haptic *aw_haptic)
{
	...
	return request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				       aw_rtp_name[aw_haptic->rtp_file_num],
				       aw_haptic->dev, GFP_KERNEL, aw_haptic,
				       rtp_loaded);
}
	...
	if (aw_haptic->device_id == DEVICE_ID_0832) {
		...
		if (aw_haptic->vibration_style ==
			...
			return request_firmware_nowait(
				...
				aw_ram_name_150[index], aw_haptic->dev,
				GFP_KERNEL, aw_haptic, ram_load);
		} else if (aw_haptic->vibration_style ==
			   AW_HAPTIC_VIBRATION_SOFT_STYLE) {
			aw_dev_info("%s:150Hz haptic bin name  %s\n", __func__,
				    aw_ram_name_150_soft[index]);
			return request_firmware_nowait(
				THIS_MODULE, FW_ACTION_HOTPLUG,
				aw_ram_name_150_soft[index], aw_haptic->dev,
				GFP_KERNEL, aw_haptic, ram_load);
		} else {
			aw_dev_info("%s:150Hz haptic bin name  %s\n", __func__,
				    aw_ram_name_150[index]);
		...
		if (aw_haptic->vibration_style ==
			...
			aw_dev_info("%s:170Hz soft haptic bin name %s\n",
				    __func__, aw_ram_name_170_soft[index]);
			return request_firmware_nowait(
				THIS_MODULE, FW_ACTION_HOTPLUG,
				aw_ram_name_170_soft[index], aw_haptic->dev,
				GFP_KERNEL, aw_haptic, ram_load);
		} else {
...
{
	struct aw_haptic *aw_haptic =
		container_of(work, struct aw_haptic, ram_work.work);
...
static int f0_cali(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->func->get_f0(aw_haptic)) {
		...
		aw_dev_info(
			"%s: f0_pre = %d, f0_cali_min = %d, f0_cali_max = %d, f0 = %d\n",
			__func__, aw_haptic->info.f0_pre, f0_cali_min,
			f0_cali_max, aw_haptic->f0);
		...
		/* calculate cali step */
		f0_cali_step = 100000 *
			       ((int)f0_limit - (int)aw_haptic->info.f0_pre) /
...
void get_f0_cali_data(struct aw_haptic *aw_haptic)
{
	...
	aw_dev_info(
		"%s: f0_pre = %d, f0_cali_min = %d, f0_cali_max = %d, f0 = %d\n",
		__func__, aw_haptic->info.f0_pre, f0_cali_min, f0_cali_max,
		aw_haptic->f0);
...
static void rtp_trim_lra_cali(struct aw_haptic *aw_haptic)
{
	...
	if (theory_time == real_time) {
		aw_dev_info(
			"%s: theory_time == real_time: %d, no need to calibrate!\n",
			__func__, real_time);
		return;
		...
		if ((real_time - theory_time) <
			...
			aw_dev_info(
				"%s: real_time: %d, theory_time: %d, no need to calibrate!\n",
		...
		real_code = (theory_time - real_time) / (theory_time / 100000) /
			    AW_OSC_CALI_ACCURACY;
		real_code = ((real_code % 10 < 5) ? 0 : 1) + real_code / 10;
		real_code = 32 - real_code;
	}
	if (real_code > 31)
...
static int rtp_osc_cali(struct aw_haptic *aw_haptic)
{
	...
	while (1) {
		if (!aw_haptic->func->rtp_get_fifo_afs(aw_haptic)) {
			...
			aw_dev_info(
				"%s: not almost_full, aw_haptic->rtp_cnt=%d\n",
		...
		if (aw_haptic->func->get_osc_status(aw_haptic)) {
			...
			aw_dev_info(
				"%s: osc trim playback done aw_haptic->rtp_cnt= %d\n",
				__func__, aw_haptic->rtp_cnt);
			break;
		}
		aw_haptic->kend = ktime_get();
		aw_haptic->microsecond = ktime_to_us(
			ktime_sub(aw_haptic->kend, aw_haptic->kstart));
		if (aw_haptic->microsecond > AW_OSC_CALI_MAX_LENGTH) {
			aw_dev_info(
				"%s osc trim time out! aw_haptic->rtp_cnt %d\n",
				__func__, aw_haptic->rtp_cnt);
			break;
		}
	}
	...
	aw_haptic->microsecond =
		ktime_to_us(ktime_sub(aw_haptic->kend, aw_haptic->kstart));
	/*calibration osc */
...
{
	struct aw_haptic *aw_haptic =
		container_of(timer, struct aw_haptic, timer);
...
{
	struct aw_haptic *aw_haptic =
		container_of(work, struct aw_haptic, vibrator_work);
	...
	aw_dev_info(
		"%s enter, aw_haptic->state[%d], aw_haptic->activate_mode[%d], aw_haptic->ram_vbat_comp[%d]\n",
...
static const struct firmware *
rtp_load_file_accord_f0(struct aw_haptic *aw_haptic)
{
	const struct firmware *rtp_file;
	unsigned int f0_file_num = 1024;
	int ret = -1;
	...
	if ((aw_haptic->rtp_file_num >= RINGTONES_START_INDEX &&
	     aw_haptic->rtp_file_num <= RINGTONES_END_INDEX) ||
	    (aw_haptic->rtp_file_num >= NEW_RING_START &&
	     aw_haptic->rtp_file_num <= NEW_RING_END) ||
	    (aw_haptic->rtp_file_num >= OS12_NEW_RING_START &&
	     aw_haptic->rtp_file_num <= OS12_NEW_RING_END) ||
		...
		if (aw_haptic->f0 <= 1650)
			...
			f0_file_num = 3;
		aw_dev_info(
			"%s long sound or old steady test f0_file_num[%d], aw_haptic->rtp_file_num[%d]\n",
			__func__, f0_file_num, aw_haptic->rtp_file_num);

		if (aw_haptic->rtp_file_num == AW_RTP_LONG_SOUND_INDEX) {
			ret = request_firmware(
				&rtp_file, aw_long_sound_rtp_name[f0_file_num],
				aw_haptic->dev);
		} else if (aw_haptic->rtp_file_num ==
			   AW_WAVEFORM_INDEX_OLD_STEADY) {
			ret = request_firmware(
				&rtp_file,
...
static void rtp_work_routine(struct work_struct *work)
{
	...
	const struct firmware *rtp_file;
	struct aw_haptic *aw_haptic =
		container_of(work, struct aw_haptic, rtp_work);

	aw_dev_info("%s: enter device_id = %d, f0 = %d\n", __func__,
		    aw_haptic->device_id, aw_haptic->f0);
	mutex_lock(&aw_haptic->rtp_lock);
	...
	if (!rtp_file) {
		...
		if (aw_haptic->device_id == DEVICE_ID_0815) {
			...
			if (aw_haptic->f0 <= 2280) {
				ret = request_firmware(
					&rtp_file,
					aw_rtp_name_19065_226Hz
						[aw_haptic->rtp_file_num],
					aw_haptic->dev);
			} else if (aw_haptic->f0 <= 2320) {
				ret = request_firmware(
					&rtp_file,
					aw_rtp_name_19065_230Hz
						[aw_haptic->rtp_file_num],
					aw_haptic->dev);
			} else {
				ret = request_firmware(
					&rtp_file,
					aw_rtp_name_19065_234Hz
						[aw_haptic->rtp_file_num],
					aw_haptic->dev);
			}
		} else {
		...
		if (aw_haptic->rtp_init) {
			while ((!aw_haptic->func->rtp_get_fifo_afs(aw_haptic)) &&
				...
				aw_haptic->func->set_rtp_data(
					...
					buf_len);
				aw_haptic->rtp_cnt += buf_len;
				glb_state_val = aw_haptic->func->get_glb_state(
					aw_haptic);
				if ((aw_haptic->rtp_cnt >= aw_rtp->len) ||
				    ((glb_state_val & AW_GLBRD_STATE_MASK) ==
				     AW_STATE_STANDBY)) {
...
static int irq_config(struct device *dev, struct aw_haptic *aw_haptic)
{
	...
	if (gpio_is_valid(aw_haptic->irq_gpio) &&
		...
		ret = devm_request_threaded_irq(
			dev, gpio_to_irq(aw_haptic->irq_gpio), NULL, irq_handle,
			irq_flags, "aw_haptic", aw_haptic);
		if (ret != 0) {
			       ...
			       struct aw_haptic_ctr *haptic_ctr)
{
	...
	p_new = (struct aw_haptic_ctr *)kzalloc(sizeof(struct aw_haptic_ctr),
						GFP_KERNEL);
	if (p_new == NULL) {
...
static void audio_work_routine(struct work_struct *work)
{
	...
	if (aw_haptic->haptic_audio.ctr.cmd == AW_CMD_ENABLE) {
		if (aw_haptic->haptic_audio.ctr.play == AW_PLAY_ENABLE) {
			...
			aw_haptic->func->set_wav_seq(
				aw_haptic, 0x00,
				aw_haptic->haptic_audio.ctr.wavseq);
			aw_haptic->func->set_wav_seq(aw_haptic, 0x01, 0x00);
			aw_haptic->func->set_wav_loop(
				aw_haptic, 0x00,
				aw_haptic->haptic_audio.ctr.loop);
			aw_haptic->func->set_gain(
				aw_haptic, aw_haptic->haptic_audio.ctr.gain);
			aw_haptic->func->play_go(aw_haptic, true);
			mutex_unlock(&aw_haptic->lock);
...
{
	struct aw_haptic *aw_haptic =
		container_of(dev, struct aw_haptic, vib_dev);
			 ...
			 char *buf)
{
	...
	mutex_unlock(&aw_haptic->lock);
	count += snprintf(buf + count, PAGE_SIZE - count, "rtp_loop: 0x%02x\n",
			  aw_haptic->rtp_loop);
	return count;
}
			 ...
			 const char *buf, size_t count)
{
	...
	if (val < (sizeof(aw_rtp_name) / AW_RTP_NAME_MAX)) {
		...
		if (val) {
			if (aw_haptic->device_id == DEVICE_ID_0815 &&
			    ((val >= SG_INPUT_DOWN_HIGH &&
			      val <= SG_INPUT_UP_LOW) ||
			     (val >= INPUT_LOW && val <= INPUT_HIGH))) {
				queue_work(system_unbound_wq,
					   &aw_haptic->rtp_key_work);
			} else {
				queue_work(system_unbound_wq,
					   &aw_haptic->rtp_work);
			}
...
static ssize_t f0_data_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	ssize_t len = 0;
	cdev_t *cdev = dev_get_drvdata(dev);
...
static ssize_t f0_data_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	uint32_t val = 0;
	int rc = 0;
...
static ssize_t osc_data_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	ssize_t len = 0;
	cdev_t *cdev = dev_get_drvdata(dev);
...
static ssize_t osc_data_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	uint32_t val = 0;
	int rc = 0;
...
{
	struct aw_haptic *aw_haptic =
		container_of(work, struct aw_haptic, motor_old_test_work);
	...
	if (aw_haptic->motor_old_test_mode == MOTOR_OLD_TEST_TRANSIENT) {
		...
		if (AW_WAVEFORM_INDEX_OLD_STEADY <
		    (sizeof(aw_rtp_name) / AW_RTP_NAME_MAX)) {
			aw_haptic->rtp_file_num = AW_WAVEFORM_INDEX_OLD_STEADY;
			if (AW_WAVEFORM_INDEX_OLD_STEADY) {
				     ...
				     const char *buf, size_t count)
{
	...
	if (val == 1) {
		...
		for (j = 0; j < retries; j++) {
			...
			aw_haptic->func->set_ram_data(
				aw_haptic, aw_ramtest->data, aw_ramtest->len);
...
static ssize_t rtp_going_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic =
...
static ssize_t gun_mode_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic =
			    ...
			    char *buf)
{
	...
	for (i = 0; i < AW_RTP_NUM; i++) {
		len += snprintf(buf + len, PAGE_SIZE - len,
				"num: %d, serial:%d \n", i,
				aw_haptic->rtp_serial[i]);
	}
	return len;
}

static ssize_t rtp_num_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic =
	...
	if (sscanf(buf, "%x %x %x %x %x %x", &databuf[0], &databuf[1],
		   ...
		   &databuf[5]) == AW_RTP_NUM) {
		for (val = 0; val < AW_RTP_NUM; val++) {
			aw_dev_info("%s: databuf = %d \n", __func__,
				    databuf[val]);
			aw_haptic->rtp_serial[val] =
				(unsigned char)databuf[val];
		}
	} else {
		rc = kstrtouint(buf, 0, &val);
		if (rc < 0)
...
static DEVICE_ATTR(duration, S_IWUSR | S_IWGRP | S_IRUGO, duration_show,
		   duration_store);
static DEVICE_ATTR(activate, S_IWUSR | S_IWGRP | S_IRUGO, activate_show,
		   activate_store);
static DEVICE_ATTR(state, S_IWUSR | S_IRUGO, state_show, state_store);
static DEVICE_ATTR(oplus_duration, S_IWUSR | S_IWGRP | S_IRUGO, duration_show,
		   duration_store);
static DEVICE_ATTR(oplus_activate, S_IWUSR | S_IWGRP | S_IRUGO, activate_show,
		   activate_store);
...
{
	struct aw_haptic *aw_haptic =
		container_of(work, struct aw_haptic, rtp_key_work);
	...
	memcpy(aw_rtp->data, aw_haptic_rtp_key_data,
	       aw_haptic_rtp_key_data_len);
	mutex_unlock(&aw_haptic->rtp_lock);
...
{
	struct aw_haptic *aw_haptic =
		container_of(work, struct aw_haptic, rtp_single_cycle_work);
	const struct firmware *rtp_file;
	...
	while (aw_haptic->rtp_cycle_flag == 1) {
		if (!aw_haptic->func->rtp_get_fifo_afs(aw_haptic)) {
			...
			aw_haptic->func->set_rtp_data(
				aw_haptic, &aw_rtp->data[aw_haptic->rtp_cnt],
				buf_len);
			aw_haptic->rtp_cnt += buf_len;
			...
			while (1) {
				if (aw_haptic->func->rtp_get_fifo_aes(
					    aw_haptic)) {
...
{
	struct aw_haptic *aw_haptic =
		container_of(work, struct aw_haptic, rtp_regroup_work);
	const struct firmware *rtp_file;
	unsigned int buf_len = 0;
	unsigned char reg_val = 0;
	unsigned int cyc_cont = 150;
...
static int rtp_regroup_work(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->rtp_serial[0] > 0) {
		...
		aw_haptic->func->set_rtp_aei(aw_haptic, false);
		aw_haptic->func->irq_clear(aw_haptic);
		if (aw_haptic->rtp_serial[0] <=
		    (sizeof(aw_rtp_name) / AW_RTP_NAME_MAX)) {
			/* if aw_haptic->rtp_loop = 0xff then single cycle ; */
			if (aw_haptic->rtp_loop == 0xFF) {
				aw_haptic->rtp_cycle_flag = 1;
				schedule_work(
					&aw_haptic->rtp_single_cycle_work);
			} else if ((aw_haptic->rtp_loop > 0) &&
				   (aw_haptic->rtp_loop < 0xff)) {
				aw_haptic->rtp_cycle_flag = 1;
...
{
	struct aw_haptic *aw_haptic =
		container_of(work, struct aw_haptic, haptic_rtp_work);
	struct mmap_buf_format *opbuf = aw_haptic->start_buf;
	uint32_t count = 100;
	...
	while (true && count--) {
		if (opbuf->status == MMAP_BUF_DATA_VALID) {
			...
			while (cnt) {
				usleep_range(2000, 2500);
				reg_val = aw_haptic->func->get_glb_state(
					aw_haptic);
				if ((reg_val & AW_GLBRD_STATE_MASK) ==
				    AW_STATE_RTP) {
					cnt = 0;
					rtp_work_flag = true;
					aw_dev_info(
...
static ssize_t aw_file_read(struct file *filp, char *buff, size_t len,
			    loff_t *offset)
{
				   ...
				   unsigned long arg)
{
	...
	case RICHTAP_RTP_MODE:
		...
		aw_haptic->func->play_go(aw_haptic, true);
		usleep_range(2000, 2500);
		aw_haptic->func->set_rtp_data(aw_haptic, &aw_haptic->rtp_ptr[4],
					      tmp);
		break;
...
static int init_vibrator_proc(struct aw_haptic *aw_haptic)
{
	...
	aw_haptic->prEntry_tmp =
		proc_create_data("touch_style", 0664, aw_haptic->prEntry_da,
				 &proc_vibration_style_ops, aw_haptic);
...
{
	aw_haptic->rtp_ptr = kmalloc(
		RICHTAP_MMAP_BUF_SIZE * RICHTAP_MMAP_BUF_SUM, GFP_KERNEL);
	if (aw_haptic->rtp_ptr == NULL) {
		aw_dev_err("%s: malloc rtp memory failed\n", __func__);
		return -ENOMEM;
	}

	aw_haptic->start_buf = (struct mmap_buf_format *)__get_free_pages(
			    ...
			    const struct i2c_device_id *id)
{
	...
	aw_haptic =
		devm_kzalloc(&i2c->dev, sizeof(struct aw_haptic), GFP_KERNEL);
	if (aw_haptic == NULL)
		return -ENOMEM;
	...
	aw_haptic->pinctrl_state =
		pinctrl_lookup_state(aw_haptic->pinctrl, "irq_active");
...
static int awinic_i2c_remove(struct i2c_client *i2c)
{
	...
	free_pages((unsigned long)aw_haptic->start_buf,
...
static const struct i2c_device_id awinic_i2c_id[] = { { AW_I2C_NAME, 0 }, {} };
