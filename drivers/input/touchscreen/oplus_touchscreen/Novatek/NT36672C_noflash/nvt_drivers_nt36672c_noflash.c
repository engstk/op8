...
static uint8_t nvt_wdt_fw_recovery(struct chip_data_nt36672c *chip_info, uint8_t *point_data)
{
...
static void nvt_esd_check_enable(struct chip_data_nt36672c *chip_info, bool enable)
{
...
*******************************************************/
static int32_t nvt_check_fw_reset_state_noflash(struct chip_data_nt36672c *chip_info, RST_COMPLETE_STATE check_reset_state)
{
    uint8_t buf[8] = {0};
    int32_t ret = 0;
    int32_t retry = 0;
...
static int32_t nvt_bin_header_parser(struct chip_data_nt36672c *chip_info, const u8 *fwdata, size_t fwsize)
{
    uint32_t list = 0;
    uint32_t pos = 0x00;
...
return:
        n.a.
*******************************************************/
static void nvt_read_ram_test(struct chip_data_nt36672c *chip_info, uint32_t addr, uint16_t len, char *name)
{
    char file[256] = "";
    uint8_t *fbufp = NULL;
    int32_t ret = 0;
...
*******************************************************/
/*
static int32_t Write_Partition(struct chip_data_nt36672c *chip_info, const u8 *fwdata, size_t fwsize)
{
    uint32_t list = 0;
...
return:
        n.a.
*******************************************************/
static int32_t Download_Firmware_HW_CRC(struct chip_data_nt36672c *chip_info, const struct firmware *fw)
{
    uint8_t retry = 0;
    int32_t ret = 0;
...
static int nvt_enable_edge_limit(struct chip_data_nt36672c *chip_info, int state)
{
    int8_t ret = -1;
    struct touchpanel_data *ts = spi_get_drvdata(chip_info->s_client);
...
static int nvt_enable_charge_mode(struct chip_data_nt36672c *chip_info, bool enable)
{
    int8_t ret = -1;
...
static int nvt_enable_game_mode(struct chip_data_nt36672c *chip_info, bool enable)
{
    int8_t ret = -1;
...
static int nvt_enable_headset_mode(struct chip_data_nt36672c *chip_info, bool enable)
{
    int8_t ret = -1;
...
static void nvt_enable_noise_collect(struct chip_data_nt36672c *chip_info, int32_t frame_num)
{
    int ret = -1;
    uint8_t buf[8] = {0};
...
static int8_t nvt_switch_FreqHopEnDis(struct chip_data_nt36672c *chip_info, uint8_t FreqHopEnDis)
{
    uint8_t buf[8] = {0};
    uint8_t retry = 0;
    int8_t ret = 0;
...
static void nvt_read_mdata(struct chip_data_nt36672c *chip_info, uint32_t xdata_addr, int32_t *xdata, int32_t xdata_len)
{
    int32_t i = 0;
    int32_t j = 0;
    int32_t k = 0;
...
static int32_t nvt_read_fw_noise(struct chip_data_nt36672c *chip_info, int32_t config_Diff_Test_Frame, int32_t *xdata, int32_t *xdata_n, int32_t xdata_len)
{
    uint32_t x = 0;
    uint32_t y = 0;
...
static int32_t nvt_enter_digital_test(struct chip_data_nt36672c *chip_info, uint8_t enter_digital_test)
{
    uint8_t buf[8] = {0};
    int32_t i = 0;
...
static void nvt_enable_doze_noise_collect(struct chip_data_nt36672c *chip_info, int32_t frame_num)
{
    int ret = -1;
    uint8_t buf[8] = {0};
...
static int32_t nvt_read_doze_fw_noise(struct chip_data_nt36672c *chip_info, int32_t config_Doze_Noise_Test_Frame, int32_t doze_X_Channel, int32_t *xdata, int32_t *xdata_n, int32_t xdata_len)
{
    uint8_t buf[128] = {0};
    uint32_t x = 0;
    uint32_t y = 0;
...
static int32_t nvt_read_doze_baseline(struct chip_data_nt36672c *chip_info, int32_t doze_X_Channel, int32_t *xdata, int32_t xdata_len)
{
    uint8_t buf[256] = {0};
    uint32_t x = 0;
    uint32_t y = 0;
...
static int32_t nvt_read_fw_short(struct chip_data_nt36672c *chip_info, int32_t *xdata, int32_t xdata_len)
{
...
static int32_t nvt_read_fw_open(struct chip_data_nt36672c *chip_info, int32_t *xdata, int32_t xdata_len)
{
    uint32_t raw_pipe_addr = 0;
    uint8_t *rawdata_buf = NULL;
    uint32_t x = 0;
...
static void nvt_auto_test(struct seq_file *s, void *chip_data, struct nvt_testdata *nvt_testdata)
{
    ...
    if ((ph->config_Lmt_FW_CC_P != 0) && (ph->config_Lmt_FW_CC_N != 0)) {
        for (j = 0; j < rx_num; j++) {
            for (i = 0; i < tx_num; i++) {
                iArrayIndex = j * tx_num + i;
                if((cc_data[iArrayIndex] > ph->config_Lmt_FW_CC_P) \
                   || (cc_data[iArrayIndex] < ph->config_Lmt_FW_CC_N)) {
                    cc_result = NVT_MP_FAIL;
                    cc_record[iArrayIndex] = 1;
                    TPD_INFO("cc data Test failed at rawdata[%d][%d] = %d [%d,%d]\n", i, j, cc_data[iArrayIndex], ph->config_Lmt_FW_CC_N, ph->config_Lmt_FW_CC_P);
                    if (!err_cnt) {
    ...
    if ((ph->config_Lmt_FW_Diff_P != 0) && (ph->config_Lmt_FW_Diff_N != 0)) {
        for (j = 0; j < rx_num; j++) {
            for (i = 0; i < tx_num; i++) {
                iArrayIndex = j * tx_num + i;
                if((noise_p_data[iArrayIndex] > ph->config_Lmt_FW_Diff_P) \
                   || (noise_p_data[iArrayIndex] < ph->config_Lmt_FW_Diff_N)) {
                    noise_p_result = NVT_MP_FAIL;
                    noise_p_record[iArrayIndex] = 1;
                    TPD_INFO("Noise RawData_Diff_Max Test failed at data[%d][%d] = %d [%d,%d]\n", i, j, noise_p_data[iArrayIndex], ph->config_Lmt_FW_Diff_N, ph->config_Lmt_FW_Diff_P);
                    if (!err_cnt) {
        ...
        for (j = 0; j < rx_num; j++) {
            for (i = 0; i < tx_num; i++) {
                iArrayIndex = j * tx_num + i;
                if((noise_n_data[iArrayIndex] > ph->config_Lmt_FW_Diff_P) \
                   || (noise_n_data[iArrayIndex] < ph->config_Lmt_FW_Diff_N)) {
                    noise_n_result = NVT_MP_FAIL;
                    noise_n_record[iArrayIndex] = 1;
                    TPD_INFO("Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n", i, j, noise_n_data[iArrayIndex], ph->config_Lmt_FW_Diff_N, ph->config_Lmt_FW_Diff_P);
                    if (!err_cnt) {
    ...
    if ((ph->config_Lmt_Doze_Diff_P != 0) && (ph->config_Lmt_Doze_Diff_N != 0)) {
        for (j = 0; j < rx_num; j++) {
            for (i = 0; i < ph->doze_X_Channel; i++) {
                ...
                if((doze_noise_p_data[iArrayIndex] > ph->config_Lmt_Doze_Diff_P) \
                   || (doze_noise_p_data[iArrayIndex] < ph->config_Lmt_Doze_Diff_N)) {
                    doze_noise_p_result = NVT_MP_FAIL;
                    doze_noise_p_record[iArrayIndex] = 1;
                    TPD_INFO("Doze Noise RawData_Diff_Max Test failed at data[%d][%d] = %d [%d,%d]\n", i, j, doze_noise_p_data[iArrayIndex], ph->config_Lmt_Doze_Diff_N, ph->config_Lmt_Doze_Diff_P);
                    if (!err_cnt) {
        ...
        for (j = 0; j < rx_num; j++) {
            for (i = 0; i < ph->doze_X_Channel; i++) {
                iArrayIndex = j * ph->doze_X_Channel + i;
                if((doze_noise_n_data[iArrayIndex] > ph->config_Lmt_Doze_Diff_P) \
                   || (doze_noise_n_data[iArrayIndex] < ph->config_Lmt_Doze_Diff_N)) {
                    doze_noise_n_result = NVT_MP_FAIL;
                    doze_noise_n_record[iArrayIndex] = 1;
                    TPD_INFO("Doze Noise RawData_Diff_Min Test failed at data[%d][%d] = %d [%d,%d]\n", i, j, doze_noise_n_data[iArrayIndex], ph->config_Lmt_Doze_Diff_N, ph->config_Lmt_Doze_Diff_P);
                    if (!err_cnt) {
    ...
    if ((ph->config_Lmt_Doze_Rawdata_P != 0) && (ph->config_Lmt_Doze_Rawdata_N != 0)) {
        for (j = 0; j < rx_num; j++) {
            for (i = 0; i < ph->doze_X_Channel; i++) {
                iArrayIndex = j * ph->doze_X_Channel + i;
                if((doze_raw_data[iArrayIndex] > ph->config_Lmt_Doze_Rawdata_P) \
                   || (doze_raw_data[iArrayIndex] < ph->config_Lmt_Doze_Rawdata_N)) {
                    doze_rawdata_result = NVT_MP_FAIL;
                    doze_raw_record[iArrayIndex] = 1;
                    TPD_INFO("Doze FW Rawdata Test failed at data[%d][%d] = %d [%d,%d]\n", i, j, doze_raw_data[iArrayIndex], ph->config_Lmt_Doze_Rawdata_N, ph->config_Lmt_Doze_Rawdata_P);
                    if (!err_cnt) {
    ...
    if ((ph->config_Lmt_Short_Rawdata_P != 0) && (ph->config_Lmt_Short_Rawdata_N != 0)) {
        for (j = 0; j < rx_num; j++) {
            for (i = 0; i < tx_num; i++) {
                iArrayIndex = j * tx_num + i;
                if((short_data[iArrayIndex] > ph->config_Lmt_Short_Rawdata_P) \
                   || (short_data[iArrayIndex] < ph->config_Lmt_Short_Rawdata_N)) {
                    short_result = NVT_MP_FAIL;
                    short_record[iArrayIndex] = 1;
                    TPD_INFO("Short Test failed at data[%d][%d] = %d [%d,%d]\n", i, j, short_data[iArrayIndex], ph->config_Lmt_Short_Rawdata_N, ph->config_Lmt_Short_Rawdata_P);
                    if (!err_cnt) {
...
TEST_END:
    ...
    /* Test Item */
    /* 1. rawdata        */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "rawData",
                                 rawdata_result, raw_record, tx_num, rx_num);
    /* 2. cc             */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "ccData",
                                 cc_result, cc_record, tx_num, rx_num);
    /* 3. noise max      */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "RawData_Diff_Max",
                                 noise_p_result, noise_p_record, tx_num, rx_num);
    /* 4. noise min      */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "RawData_Diff_Min",
                                 noise_n_result, noise_n_record, tx_num, rx_num);
    /* 5. doze noise max */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "Doze RawData_Diff_Max",
                                 doze_noise_p_result, doze_noise_p_record, ph->doze_X_Channel, rx_num);
    /* 6. doze noise min */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "Doze RawData_Diff_Min",
                                 doze_noise_n_result, doze_noise_n_record, ph->doze_X_Channel, rx_num);
    /* 7. doze rawdata   */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "Doze FW Rawdata",
                                 doze_rawdata_result, doze_raw_record, ph->doze_X_Channel, rx_num);
#ifdef CONFIG_TOUCHPANEL_NT_DIGITALNOISE_TEST
    /* 8. noise max      */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "Digital_RawData_Diff_Max",
                                 digital_noise_p_result, digital_noise_p_record, tx_num, rx_num);
    /* 9. noise min      */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "Digital_RawData_Diff_Min",
                                 digital_noise_n_result, digital_noise_n_record, tx_num, rx_num);
#endif
    /*10. short          */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "RawData_Short",
                                 short_result, short_record, tx_num, rx_num);
    /*11. open           */
    nvt_store_testrecord_to_file(nvt_testdata->fd, "RawData_Open",
                                 open_result, open_record, tx_num, rx_num);

    /* Test data */
    /* 1. rawdata        */
    nvt_store_testdata_to_file(nvt_testdata->fd, "rawData",
                               rawdata_result, raw_data, tx_num, rx_num);
    /* 2. cc             */
    nvt_store_testdata_to_file(nvt_testdata->fd, "ccData",
                               cc_result, cc_data, tx_num, rx_num);
    /* 3. noise max      */
    ...
    nvt_store_testdata_to_file(nvt_testdata->fd, "RawData_Diff_Min",
                               noise_n_result, noise_n_data, tx_num, rx_num);
    /* 5. doze noise max */
    nvt_store_testdata_to_file(nvt_testdata->fd, "Doze RawData_Diff_Max",
                               doze_noise_p_result, doze_noise_p_data, ph->doze_X_Channel, rx_num);
    /* 6. doze noise min */
    nvt_store_testdata_to_file(nvt_testdata->fd, "Doze RawData_Diff_Min",
                               doze_noise_n_result, doze_noise_n_data, ph->doze_X_Channel, rx_num);
    /* 7. doze rawdata   */
    nvt_store_testdata_to_file(nvt_testdata->fd, "Doze FW Rawdata",
                               doze_rawdata_result, doze_raw_data, ph->doze_X_Channel, rx_num);
#ifdef CONFIG_TOUCHPANEL_NT_DIGITALNOISE_TEST
    /* 8. noise max      */
    nvt_store_testdata_to_file(nvt_testdata->fd, "Digital RawData_Diff_Max",
                               digital_noise_p_result, digital_noise_p_data, tx_num, rx_num);
    /* 9. noise min      */
    nvt_store_testdata_to_file(nvt_testdata->fd, "Digital RawData_Diff_Min",
                               digital_noise_n_result, digital_noise_n_data, tx_num, rx_num);
#endif
    /*10. short          */
    nvt_store_testdata_to_file(nvt_testdata->fd, "RawData_Short",
                               short_result, short_data, tx_num, rx_num);
    /*11. open           */
    nvt_store_testdata_to_file(nvt_testdata->fd, "RawData_Open",
                               open_result, open_data, tx_num, rx_num);
