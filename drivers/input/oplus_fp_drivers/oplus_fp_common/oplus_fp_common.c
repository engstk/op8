...
static char fp_manu[FP_ID_MAX_LENGTH] =
	CHIP_UNKNOWN; /* the length of this string should be less than FP_ID_MAX_LENGTH */
static char lcd_manu[FP_ID_MAX_LENGTH] =
	CHIP_UNKNOWN; /* the length of this string should be less than FP_ID_MAX_LENGTH */

static struct fp_data *fp_data_ptr = NULL;
char g_engineermode_menu_config[ENGINEER_MENU_SELECT_MAXLENTH] =
...
static int get_manufacture_id_value(struct fp_data *fp_data)
{
	...
	if (IS_ERR_OR_NULL(fp_id_pull_up)) {
		dev_err(fp_data->dev,
			"falied to find pinctrl fp_id_pull_up!\n");
		goto exit;
	}
	...
	if (IS_ERR_OR_NULL(fp_id_pull_down)) {
		dev_err(fp_data->dev,
			"falied to find pinctrl fp_id_pull_down!\n");
		goto exit;
	}

	fp_id_gpio =
		of_get_named_gpio(fp_data->dev->of_node, "oplus,fp_gpio_0", 0);
	if (fp_id_gpio < 0) {
	...
	if (ret) {
		dev_err(fp_data->dev,
			"falied to select pinctrl fp_id_pull_up, ret = %d!\n",
			ret);
		goto exit;
	}
...
static int fp_gpio_parse_parent_dts(struct fp_data *fp_data)
{
	...
	ret = of_property_read_u32(np, FP_GPIO_NUM_NODE,
	...
	ret = of_property_read_u32(np, "oplus,one_gpio_for_three_ic",
				   &one_for_three);
	if (ret) {
		dev_err(fp_data->dev,
			"oplus,one_gpio_for_three_ic is not define\n");
		ret = FP_OK;
	}
	if (one_for_three == 1) {
		...
		if (ret) {
			dev_err(fp_data->dev,
				"get_manufacture_id_value failed\n");
		} else {
			dev_info(fp_data->dev,
				 "get_manufacture_id_value success fp_id: %d\n",
				 fp_data->fp_id[0]);
		}
		goto exit;
	} else {
		for (fp_id_index = 0; fp_id_index < fp_data->fp_id_amount;
		     fp_id_index++) {
			char fp_gpio_current_node[FP_ID_MAX_LENGTH] = { 0 };
			snprintf(fp_gpio_current_node, FP_ID_MAX_LENGTH - 1,
				 "%s%d", FP_GPIO_PREFIX_NODE, fp_id_index);
			dev_info(fp_data->dev, "fp_gpio_current_node: %s\n",
				 fp_gpio_current_node);
			fp_data->gpio_index[fp_id_index] =
				of_get_named_gpio(np, fp_gpio_current_node, 0);
			if (fp_data->gpio_index[fp_id_index] < 0) {
				dev_err(fp_data->dev,
					"the param %s is not found !\n",
					fp_gpio_current_node);
				ret = -FP_ERROR_GENERAL;
				goto exit;
			}
			fp_data->fp_id[fp_id_index] = gpio_get_value(
				fp_data->gpio_index[fp_id_index]);
			dev_info(fp_data->dev, "gpio_index: %d,fp_id: %d\n",
				 fp_data->gpio_index[fp_id_index],
				 fp_data->fp_id[fp_id_index]);
		}
	}
exit:
	return ret;
				...
				size_t count, loff_t *pos)
{
	...
	local_count = (FP_ID_MAX_LENGTH - 1) < count ? (FP_ID_MAX_LENGTH - 1) :
						       count;
...
static int fp_gpio_parse_child_dts(struct fp_data *fp_data)
{
	...
	for_each_available_child_of_node(np, child) {
		child_amount = of_property_count_elems_of_size(
			child, FP_ID_VALUE_NODE, sizeof(u32));
		...
		for (child_node_index = 0; child_node_index < child_amount;
		     child_node_index++) {
			ret = of_property_read_u32_index(
				child, FP_ID_VALUE_NODE, child_node_index,
				&(child_fp_id[child_node_index]));
			if (ret) {
				dev_err(fp_data->dev,
					"the param %s is not found !\n",
					FP_ID_VALUE_NODE);
			...
			if (fp_data->fp_id[child_node_index] !=
			    child_fp_id[child_node_index]) {
				break;
			}
		...
		if (found_matched_sensor) {
			ret = of_property_read_u32(child, FP_VENDOR_CHIP_NODE,
						   &fpsensor_type);
			if (ret) {
				dev_err(fp_data->dev,
					"the param %s is not found !\n",
			...
			ret = of_property_read_string(child, FP_CHIP_NAME_NODE,
						      &chip_name);
			if (ret) {
				dev_err(fp_data->dev,
					"the param %s is not found !\n",
			...
			if (strlen(chip_name) <= 0 ||
			    strlen(chip_name) >= FP_ID_MAX_LENGTH) {
				dev_err(fp_data->dev,
					"the strlen of param %s is illegal !\n",
					FP_CHIP_NAME_NODE);
				ret = -FP_ERROR_GENERAL;
				goto exit;
			}

			ret = of_property_read_string(child, FP_ENG_MENU_NODE,
						      &eng_menu);
			if (ret) {
				dev_err(fp_data->dev,
					"the param %s is not found !\n",
			...
			if (strlen(eng_menu) <= 0 ||
			    strlen(eng_menu) >= ENGINEER_MENU_SELECT_MAXLENTH) {
				dev_err(fp_data->dev,
					"the strlen of param %s is illegal !\n",
					FP_ENG_MENU_NODE);
				ret = -FP_ERROR_GENERAL;
			...
			strncpy(g_engineermode_menu_config, eng_menu,
				ENGINEER_MENU_SELECT_MAXLENTH - 1);
			dev_info(
				dev,
				"fpsensor_type: %d, chip_name: %s, eng_menu: %s\n",
...
static ssize_t lcd_type_node_read(struct file *file, char __user *buf,
				  size_t count, loff_t *pos)
{
	char page[FP_ID_MAX_LENGTH] = { 0 };
	char *p = page;
...
static ssize_t fp_tee_node_write(struct file *file, const char __user *buf,
				 size_t count, loff_t *pos)
{
	char fp_state[32] = { '\0' };
	size_t local_count;
	...
	local_count = (sizeof(fp_state) - 1) < count ? (sizeof(fp_state) - 1) :
						       count;
