...
bool oplus_hall_update_threshold(unsigned int id, int position, short lowthd,
				 short highthd)
{
	if (!g_the_chip)
		return -EINVAL;
...
static int reupdata_threshold(struct extcon_dev_data *chip)
{
	...
	case MID_STATE:
		if (chip->dhall_data0 < 0 || chip->dhall_data1 < 0) {
			res = oplus_hall_update_threshold(
				DHALL_1, MID_STATE, chip->dhall_data1 - tolen,
				chip->dhall_data1 + tolen);
			if (res < 0) {
				TRI_KEY_LOG("updata_threshold fail:%d\n", res);
				goto fail;
			...
			TRI_KEY_LOG(
				"tri_key:updata_threshold up:low:%d,high:%d\n",
				chip->dhall_data1 - tolen,
				chip->dhall_data1 + tolen);
		} else {
			res = oplus_hall_update_threshold(
		...
		if (chip->dhall_data0 < 0 || chip->dhall_data1 < 0) {
			res = oplus_hall_update_threshold(
				DHALL_0, MID_STATE, chip->dhall_data0 - tolen,
				chip->dhall_data0 + tolen);
			if (res < 0) {
				TRI_KEY_LOG("updata_threshold fail:%d\n", res);
				goto fail;
			}
			TRI_KEY_LOG(
				"tri_key:updata_threshold down:low:%d,high:%d\n",
				chip->dhall_data0 - tolen,
				chip->dhall_data0 + tolen);
		} else {
			res = oplus_hall_update_threshold(
	...
	case DOWN_STATE:
		...
		TRI_KEY_LOG("tri_key:updata_threshold down:low:%d,high:%d\n",
			    chip->dhall_data0 - tolen,
			    chip->dhall_data0 + tolen);
		res = oplus_hall_update_threshold(DHALL_1, DOWN_STATE, -500,
						  500);
		if (res < 0) {
			TRI_KEY_LOG("updata_threshold fail:%d\n", res);
			goto fail;
...
{
	struct extcon_dev_data *chip =
		container_of(work, struct extcon_dev_data, dwork);
	int res = 0;
	int position = -1;
	int diff0 = 0;
	int diff1 = 0;
		       ...
		       short calib_upHall_MdV, short calib_upHall_DnV)
{
	...
	up_mid_tol = (short)(abs(calib_UpValueMin - calib_MdValueMin) * 4 / 10);
	up_tolerance =
		(short)(abs(calib_UpValueMin - calib_MdValueMin) * 11 / 10);
	mid_down_tol =
		(short)(abs(calib_MdValueMin - calib_DnValueMin) * 4 / 10);
	down_tolerance =
		(short)(abs(calib_MdValueMin - calib_DnValueMin) * 11 / 10);
	up_mid_distance =
		(short)(abs(calib_UpValueMin - calib_MdValueMin) * 2 / 10);
	mid_down_distance =
		(short)(abs(calib_MdValueMin - calib_DnValueMin) * 2 / 10);
	TRI_KEY_LOG("Upmin:%d, Mdmin:%d, Dnmin:%d\n", calib_UpValueMin,
		    calib_MdValueMin, calib_DnValueMin);
...
static ssize_t proc_hall_data_calib_read(struct file *file,
...
static ssize_t proc_hall_data_calib_write(struct file *file,
...
static ssize_t proc_hall_debug_info_write(struct file *file,
					  const char __user *buffer,
					  size_t count, loff_t *ppos)
{
	int tmp = 0;
...
static ssize_t proc_hall_debug_info_read(struct file *file,
					 char __user *user_buf, size_t count,
					 loff_t *ppos)
{
	int ret = -1;
...
static int init_trikey_proc(struct extcon_dev_data *hall_dev)
{
	...
	prEntry_tmp = proc_create("hall_data", 0644, prEntry_trikey,
				  &proc_hall_data_ops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
	...
	prEntry_tmp = proc_create("tri_state", 0644, prEntry_trikey,
				  &proc_tri_state_ops);
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TRI_KEY_ERR("%s: Couldn't create proc entry, %d\n", __func__,
...
{
	struct extcon_dev_data *chip =
		container_of(work, struct extcon_dev_data, register_work);
	...
	report_key_value(chip);
	last_position = chip->position;
	err = oplus_hall_set_detection_mode(DHALL_0, DETECTION_MODE_INTERRUPT);
	...
	err = oplus_hall_set_detection_mode(DHALL_1, DETECTION_MODE_INTERRUPT);
			...
			struct extcon_dev_data *hall_dev_t)
{
	...
	if (!g_the_chip) {
		struct extcon_dev_data *chip =
			kzalloc(sizeof(struct extcon_dev_data), GFP_KERNEL);
		if (!chip) {
			TRI_KEY_ERR("kzalloc err\n");
			return -ENOMEM;
		}
	...
	if (hall_count > 1) {
		INIT_WORK(&g_the_chip->register_work,
			  register_tri_key_dev_work);
