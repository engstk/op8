	...
	*use_size = 0;
	...
	if (situation != typec_port_info->situation) {
		*use_size += snprintf(buf + *use_size, len - *use_size,
				      ...
				      typec_port_info->name, typec_port_info->case_num, typec_port_info->situation,
				      situation);
		pass = false;
	}
	...
	if ((g_charger_chip != NULL) && (g_charger_chip->chg_ops != NULL) &&
		...
		if (val != typec_port_info->status) {
			*use_size += snprintf(buf + *use_size, len - *use_size,
					      ...
					      typec_port_info->name, typec_port_info->case_num, typec_port_info->status,
					      val);
			pass = false;
		}
		return pass;
...
static void oplus_chg_det_typec_testkit_work(struct work_struct *work)
{
	...
	if (g_det_typec_count < DET_TESTKIT_MAX_TIMES && ret != DET_SUCCESS) {
		g_det_typec_count++;
...
static void oplus_chg_det_gpio_testkit_work(struct work_struct *work)
{
	...
	if (g_det_gpio_count < DETECT_TESTKIT_MAX_TIMES && ret != DETECT_SUCCESS) {
		g_det_gpio_count++;
...
static ssize_t chg_cycle_write(struct file *file, const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if ((strncmp(proc_chg_cycle_data, "en808", 5) == 0) || (strncmp(proc_chg_cycle_data, "user_enable", 11) == 0)) {
...
{
	if (!proc_create("charger_cycle", 0662, NULL, &chg_cycle_proc_fops)) {
...
static ssize_t proc_chg_ctl(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
	...
	len = sprintf(page, "asic_current=%d\nmain_cp_ichg=%d\nslave_cp_ichg=%d\nslave_b_cp_ichg=%d\n", asic_current,
		      cp_ibus, slave_cp_ibus, slave_b_cp_ibus);
...
static ssize_t proc_ui_soc_decimal_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
	...
	if (chip->vooc_show_ui_soc_decimal) {
		svooc_is_control_by_vooc = oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC;
		control_by_wireless =
		...
		if (svooc_is_control_by_vooc != true && chip->calculate_decimal_time == 0 &&
			...
			if ((oplus_is_vooc_project() == DUAL_BATT_150W || oplus_is_vooc_project() == DUAL_BATT_240W) &&
			    ...
			    !oplus_pps_get_last_charging_status() && !oplus_ufcs_get_last_charging_status()) {
...
void oplus_chg_test_gpio_info_init(struct oplus_chg_chip *chip)
{
	...
	if (gpio_is_valid(switch2_gpio)) {
		...
		g_chg_switch2_gpio_info[SWITCH2_GPIO_INFO_INDEX].num = switch2_gpio - gpio_chip->base;
	...
	if (gpio_is_valid(uart_tx) && gpio_is_valid(uart_rx)) {
		...
		g_chg_uart_gpio_info[UART_TX_GPIO_INFO_INDEX].num = uart_tx - gpio_chip->base;
		gpio_chip = gpio_to_chip(uart_rx);
		g_chg_uart_gpio_info[UART_RX_GPIO_INFO_INDEX].chip = gpio_chip;
		g_chg_uart_gpio_info[UART_RX_GPIO_INFO_INDEX].num = uart_rx - gpio_chip->base;
...
void oplus_chg_usbtemp_v2_parse_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,usbtemp_cool_down_recoup_high", &chip->usbtemp_cool_down_recoup_high);
	if (rc) {
		chip->usbtemp_cool_down_recoup_high = OPCHG_USBTEMP_COOL_DOWN_RECOUP_HIGH;
	}

	rc = of_property_read_u32(node, "qcom,usbtemp_cool_down_recoup_low", &chip->usbtemp_cool_down_recoup_low);
	if (rc) {
...
static int oplus_chg_endurance_mode_dt(struct oplus_chg_chip *chip)
{
	...
	chip->support_super_endurance_mode = of_property_read_bool(node, "oplus,super_endurance_mode_support");

	rc = of_property_read_u32(node, "oplus,super_endurance_mode_volt_low", &chip->super_endurance_mode_volt_low);
	if (rc)
	...
	charger_xlog_printk(CHG_LOG_CRTI,
			    "support_super_endurance_mode = %d, "
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "oplus,subboard_ntc_abnormal_high_temp",
				  &chip->subboard_ntc_abnormal_high_temp);
	if (rc) {
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if (chip->support_integrated_pmic && oplus_vooc_get_fastchg_started() == false &&
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if (ui_soc_cp_flag == 0) {
		...
		if (chip->chg_ops->get_rtc_soc() > 100 || (chip->chg_ops->get_rtc_soc() <= 0)) {
			...
			if (chip->shutdown_uisoc >= 0 && chip->shutdown_uisoc <= OPLUS_FULL_SOC &&
				...
				if (chip->rsd.smooth_switch_v2 && chip->rsd.is_soc_jump_range)
					soc_load = (chip->soc > (chip->smooth_soc - 1)) ? chip->soc :
											  (chip->smooth_soc - 1);
				else
		...
		if ((soc_load != 0) && ((abs(soc_load - chip->soc)) <= 20)) {
			...
			if (chip->rsd.smooth_switch_v2 && chip->rsd.is_soc_jump_range)
				chip->ui_soc =
					(chip->soc > (chip->smooth_soc - 1)) ? chip->soc : (chip->smooth_soc - 1);
			else
			...
			if (chip->ui_soc == 0) {
				if (chip->support_super_endurance_mode && chip->super_endurance_mode_status) {
					if ((chip->super_endurance_mode_count <
					     chip->super_endurance_mode_volt_low_count) &&
					    chip->vbatt_power_off != chip->super_endurance_mode_volt_low)
						chip->vbatt_power_off = chip->super_endurance_mode_volt_low;
					else if ((chip->super_endurance_mode_count >=
						  chip->super_endurance_mode_volt_low_count) &&
...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
	...
	if ((chip->charger_exist || oplus_wpc_get_status()) && chip->batt_exist && chip->batt_full && chip->mmi_chg &&
		...
		if (chip->ui_soc == 100 &&
		    (chip->rsd.smooth_switch_v2 || (chip->fastchg_to_ffc == false && !oplus_chg_is_wls_ffc())) &&
		    ((oplus_vooc_get_fastchg_started() == false) ||
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		...
		if (!oplus_vooc_get_fastchg_started() && !oplus_vooc_get_fastchg_dummy_started() &&
			...
			if (g_charger_chip->voocphy_notify_ap_suspend &&
			    (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY ||
			     oplus_chg_get_voocphy_support() == AP_DUAL_CP_VOOCPHY) &&
			    reset_mcu_delay != RESET_MCU_DELAY_15S && reset_mcu_delay < RESET_MCU_DELAY_30S) {
				oplus_vooc_switch_mode(NORMAL_CHARGER_MODE);
				oplus_chg_suspend_charger();
				msleep(1500);
				oplus_chg_unsuspend_charger();
...
static void oplus_chg_check_status_full(struct oplus_chg_chip *chip)
{
	...
	if (oplus_vooc_get_allow_reading() == false) {
		...
		if (((oplus_vooc_get_fastchg_to_normal() == true) || (oplus_vooc_get_fastchg_to_warm() == true) ||
			...
			if ((oplus_pps_get_ffc_started() == true || oplus_ufcs_get_ffc_started() == true ||
			     chip->full_pre_ffc_judge) &&
			    (chip->fastchg_to_ffc == false)) {
				...
				if ((fastchg_present_wait_count == FULL_DELAY_COUNTS) &&
				    ...
				    ((!chip->full_pre_ffc_judge) ||
				     (chip->full_pre_ffc_judge && chip->chg_ops->get_charging_enable() == false))) {
					oplus_chg_ffc_variable_reset(chip);
					chip->charging_state = CHARGING_STATUS_FULL;
				}
			}
			...
			if (fastchg_present_wait_count == FULL_DELAY_COUNTS &&
				...
				if (chip->ffc_support && chip->ffc_temp_status != FFC_TEMP_STATUS__HIGH &&
				    ...
				    (oplus_voocphy_get_btb_temp_over() != true &&
				     oplus_vooc_get_btb_temp_over() != true && oplus_pps_get_btb_temp_over() != true &&
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	if (chip->vbatt_num == 1) {
		charger_xlog_printk(CHG_LOG_CRTI,
				    ...
				    chip->boot_mode, chip->charger_subtype, chip->batt_exist, chip->batt_full,
				    chip->chging_on, chip->in_rechging, chip->charging_state, chip->total_time,
				    chip->temperature, chip->tbatt_temp, chip->batt_volt, chip->batt_volt_min,
				    ...
				    oplus_gauge_get_batt_qs(), chip->vbatt_over, chip->chging_over_time,
				    chip->vchg_status, chip->tbatt_status, chip->stop_voter, chip->notify_code,
				    chip->usb_status, chip->otg_online, chip->otg_switch, chip->mmi_chg,
				    ...
				    oplus_vooc_get_fastchg_started(), oplus_vooc_get_fastchg_ing(),
				    oplus_vooc_get_fastchg_dummy_started(), oplus_vooc_get_fastchg_to_normal(),
				    oplus_vooc_get_fastchg_to_warm(), oplus_vooc_get_fast_chg_type(),
		...
		charger_xlog_printk(CHG_LOG_CRTI,
				    ...
				    chip->stop_voter, chip->notify_code, chip->usb_status, chip->otg_online,
				    chip->otg_switch, chip->mmi_chg, chip->boot_reason, chip->boot_mode,
				    chip->chargerid_volt, chip->chargerid_volt_got, chip->shell_temp,
				    chip->subboard_temp, oplus_is_vooc_project(), oplus_vooc_get_fastchg_started(),
				    oplus_vooc_get_fastchg_ing(), oplus_vooc_get_fastchg_dummy_started(),
				    oplus_vooc_get_fastchg_to_normal(), oplus_vooc_get_fastchg_to_warm(),
				    oplus_vooc_get_fast_chg_type(), oplus_voocphy_get_fastchg_commu_ing());
	}
...
static int comm_info_dump_log_data(char *buffer, int size, void *dev_data)
{
	...
	snprintf(buffer, size,
		 ...
		 chip->input_current_limit, chip->charging_current, chip->soc, chip->ui_soc, chip->charger_exist,
		 chip->charger_type, chip->ibus, chip->charger_volt, chip->smooth_soc, chip->led_on, chip->notify_code,
		 chip->stop_chg, chip->mmi_chg, chip->dischg_flag, chip->smart_charge_user, chip->limits.vfloat_sw_set,
		 chip->otg_switch, chip->cool_down, chip->bcc_cool_down, chip->normal_cool_down, chip->charger_subtype,
		 chip->sw_full, chip->hw_full_by_sw, chip->subboard_temp, chip->in_rechging, chip->prop_status,
		 chip->charging_state, chip->pd_svooc, chip->ac_online);
...
static void oplus_chg_print_bcc_log(struct oplus_chg_chip *chip)
{
	...
	if (oplus_vooc_get_fastchg_started() && (oplus_chg_get_voocphy_support() == ADSP_VOOCPHY ||
						 (oplus_chg_get_voocphy_support() == AP_SINGLE_CP_VOOCPHY &&
						  oplus_pps_get_support_type() == PPS_SUPPORT_3CP)))
		oplus_pps_read_ibus();
}
...
static bool oplus_chg_extra_check_qchv_condition(struct oplus_chg_chip *chip)
{
	...
	chg_debug("[%d, %d, %d, %d, %d, %d],tbatt_pdqc_to_5v_thr:%d\n", !chip->calling_on, !chip->camera_on,
		  chip->charger_volt, chip->soc, chip->temperature, !chip->cool_down_force_5v,
		  chip->limits.tbatt_pdqc_to_5v_thr);
...
static void oplus_chg_update_work(struct work_struct *work)
{
	...
	if (timer_pending(&chip->update_work.timer) && !delayed_work_pending(&chip->update_work)) {
		mod_delayed_work(system_wq, &chip->update_work, OPLUS_CHG_UPDATE_INTERVAL(oplus_chg_update_slow(chip)));
	} else {
...
int oplus_chg_show_vooc_logo_ornot(void)
{
	...
	if (oplus_vooc_get_fastchg_started()) {
		...
		if ((g_charger_chip->vooc_project == 1 || g_charger_chip->vooc_project == 5 ||
		     g_charger_chip->vooc_project == 13 || g_charger_chip->vooc_project == 15 ||
		     g_charger_chip->vooc_project == DUAL_BATT_65W || g_charger_chip->vooc_project == DUAL_BATT_80W ||
		...
		if (g_charger_chip->pd_svooc == false &&
		    g_charger_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
...
{
	if (g_charger_chip && !g_charger_chip->use_audio_switch && g_charger_chip->chg_ops->set_chargerid_switch_val) {
...
{
	if (g_charger_chip && !g_charger_chip->use_audio_switch && g_charger_chip->chg_ops->get_chargerid_switch_val) {
...
int oplus_chg_get_cool_down_status(void)
{
	...
	if (!chip->skip_usbtemp_cool_down && (chip->smart_charge_user != SMART_CHARGE_USER_OTHER)) {
		if (chip->smart_charging_screenoff) {
			if (oplus_is_ufcs_charging()) {
				if (chip->smart_charge_user == SMART_CHARGE_USER_USBTEMP3)
					chip->usbtemp_cool_down = oplus_convert_pps_current_to_level(
						chip, USBTEMP_CHARGING_CURRENT_LIMIT3);
				else if (chip->smart_charge_user == SMART_CHARGE_USER_USBTEMP2)
					chip->usbtemp_cool_down = oplus_convert_pps_current_to_level(
						chip, USBTEMP_CHARGING_CURRENT_LIMIT2);
				else if (chip->smart_charge_user == SMART_CHARGE_USER_USBTEMP1)
					chip->usbtemp_cool_down = oplus_convert_pps_current_to_level(
						chip, USBTEMP_CHARGING_CURRENT_LIMIT1);
				else
					chip->usbtemp_cool_down = oplus_convert_pps_current_to_level(
						chip, USBTEMP_CHARGING_CURRENT_LIMIT);
				...
				chip->usbtemp_cool_down = oplus_convert_pps_current_to_level(
					chip, (USBTEMP_CHARGING_CURRENT_LIMIT / oplus_pps_get_icurr_ratio()));
			} else {
	...
	if (get_subboard_ntc_abnormal_status() == SUBBOARD_NTC_ABNORMAL && chip->subboard_ntc_abnormal_current != 0) {
		if (oplus_is_ufcs_charging() || oplus_is_pps_charging())
			chip->subboard_ntc_abnormal_cool_down =
				oplus_convert_pps_current_to_level(chip, chip->subboard_ntc_abnormal_current);
		else
			chip->subboard_ntc_abnormal_cool_down =
				oplus_convert_current_to_level(chip, chip->subboard_ntc_abnormal_current);
		if (ret == 0)
			ret = chip->subboard_ntc_abnormal_cool_down;
		else
			ret = (chip->subboard_ntc_abnormal_cool_down < ret) ? chip->subboard_ntc_abnormal_cool_down :
									      ret;
	}

	charger_xlog_printk(CHG_LOG_CRTI,
			    "ret = 0x%x, cool_down=%d,usbtemp_cool_down=%d,"
...
struct current_level SVOOC_1_0[] = { { 1, 2000 },   { 2, 2000 },   { 3, 2000 },	  { 4, 3000 },	{ 5, 4000 },
...
#endif
	int
	oplus_chg_match_temp_for_chging(void)
{
	int batt_temp = 0;
	int subboard_temp = 0;
	int sub_batt_temp = 0;
	...
	if (g_charger_chip->bms_heat_temp_compensation != 0 && chging_temp < OPLUS_BMS_HEAT_THRE)
...
static int parallel_chg_check_balance_bat_status(void)
{
	...
	if (status != PARALLEL_NOT_NEED_BALANCE_BAT__START_CHARGE || status != chip->balancing_bat_status)
...
static void oplus_chg_check_abnormal_adapter(int vbus_rising)
{
	...
	if (oplus_chg_fg_package_read_support()) {
		...
		if (chip->mmi_chg && pre_is_abnormal_adapter &&
...
static void quick_mode_check(void)
{
	...
	if (oplus_pps_get_support_type() != PPS_SUPPORT_NOT) {
		...
		chg_err("%s, batt_curv=%d, current_now=%d, normal_cool_down=%d \n", __func__, batt_curve_current,
			current_cool_down, current_normal_cool_down);
	}
...
int oplus_chg_adspvoocphy_get_abnormal_adapter_disconnect_cnt(void)
{
	...
	if (oplus_chg_get_voocphy_support() != ADSP_VOOCPHY || !chip->chg_ops->get_abnormal_adapter_disconnect_cnt)
		return 0;
