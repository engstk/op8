...
static int charge_pump_set_work_mode(int work_mode)
{
	...
	if (divider_ic->fixed_mode_set_by_dev_file == true && work_mode == CHARGE_PUMP_WORK_MODE_AUTO) {
	...
	case HWID_DA9313:
		if (work_mode != 0) {
			rc = charge_pump_config_interface(DA9313_WORK_MODE_REG, DA9313_WORK_MODE_AUTO,
							  DA9313_WORK_MODE_MASK);
		} else {
			rc = charge_pump_config_interface(DA9313_WORK_MODE_REG, DA9313_WORK_MODE_FIXED,
							  DA9313_WORK_MODE_MASK);
		}
		break;
	case HWID_MAX77932:
		if (work_mode != 0) {
			rc = charge_pump_config_interface(MAX77932_WORK_MODE_REG, MAX77932_WORK_MODE_AUTO,
							  MAX77932_WORK_MODE_MASK);
		} else {
			rc = charge_pump_config_interface(MAX77932_WORK_MODE_REG, MAX77932_WORK_MODE_FIXED,
							  MAX77932_WORK_MODE_MASK);
		}
		break;
	case HWID_MAX77938:
		if (work_mode != 0) {
			rc = charge_pump_config_interface(MAX77938_WORK_MODE_REG, MAX77938_WORK_MODE_AUTO,
							  MAX77938_WORK_MODE_MASK);
		} else {
			rc = charge_pump_config_interface(MAX77938_WORK_MODE_REG, MAX77938_WORK_MODE_FIXED,
							  MAX77938_WORK_MODE_MASK);
		}
		break;
	case HWID_SD77313:
		if (work_mode != 0) {
			rc = charge_pump_config_interface(SD77313_WORK_MODE_REG, SD77313_WORK_MODE_AUTO,
							  SD77313_WORK_MODE_MASK);
		} else {
			rc = charge_pump_config_interface(SD77313_WORK_MODE_REG, SD77313_WORK_MODE_FIXED,
							  SD77313_WORK_MODE_MASK);
		}
		break;
	default:
...
static int charge_pump_get_hwid(struct chip_charge_pump *chip)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->charge_pump_hwid_active) ||
	    IS_ERR_OR_NULL(chip->charge_pump_hwid_sleep) || IS_ERR_OR_NULL(chip->charge_pump_hwid_default)) {
