...
int dsi_panel_spr_mode(struct dsi_panel *panel, int mode)
{
	...
	default:
		rc = dsi_panel_tx_cmd_set(
			panel,
			DSI_CMD_SPR_MODE0); /*gaos edit default is spr mode 0*/
		if (rc) {
			pr_err("[%s] failed to send DSI_CMD_SPR_MODE0 cmds, rc=%d\n",
			       panel->name, rc);
		}
...
int oplus_display_panel_dump_info(void *data)
{
	...
	timing_info->clk_rate_hz_l32 =
		(uint32_t)(temp_display->modes->timing.clk_rate_hz &
			   0x00000000FFFFFFFF);
	timing_info->clk_rate_hz_h32 =
		(uint32_t)(temp_display->modes->timing.clk_rate_hz >> 32);
				...
				size_t rlen)
{
	int rc = 0;
	struct dsi_panel_cmd_set cmdset;
	struct dsi_cmd_desc dsi_cmd = { { 0, MIPI_DSI_DCS_READ,
					  MIPI_DSI_MSG_REQ_ACK, 0, 0, 1, &cmd,
					  rlen, rbuf },
					1,
					0 };
...
int oplus_big_endian_copy(void *dest, void *src, int count)
{
	...
	while (count > 0) {
		u_dest[index] = ((u_src[knum] << 24) | (u_src[knum + 1] << 16) |
				 (u_src[knum + 2] << 8) | u_src[knum + 3]);
		index += 1;
		knum += 4;
		count = count - 1;
	}
...
int oplus_display_panel_get_reg(void *data)
{
	...
	u32_bytes = oplus_rx_len % u32_bytes ? (oplus_rx_len / u32_bytes + 1) :
					       oplus_rx_len / u32_bytes;
...
int oplus_display_panel_set_reg(void *data)
{
	...
	if (reg_rw->rw_flags == REG_WRITE) {
		...
		if (get_oplus_display_power_status() ==
	...
	if (osc_clock_rate) {
		if (display->panel->oplus_priv.is_osc_rewrite_support &&
		    (display->panel->cur_mode->timing.clk_rate_hz ==
		     display->panel->oplus_priv.osc_rewrite_clk_rate)) {
			if (osc_clock_rate ==
			    display->panel->oplus_priv.osc_clk_mode0_rate) {
				rc = dsi_panel_tx_cmd_set(
					display->panel, DSI_CMD_OSC_CLK_MODEO2);
			} else if (osc_clock_rate ==
				   display->panel->oplus_priv
					   .osc_clk_mode1_rate) {
				rc = dsi_panel_tx_cmd_set(
					display->panel, DSI_CMD_OSC_CLK_MODEO3);
			} else {
			...
			if (osc_clock_rate ==
			    display->panel->oplus_priv.osc_clk_mode0_rate) {
				rc = dsi_panel_tx_cmd_set(
					display->panel, DSI_CMD_OSC_CLK_MODEO0);
			} else if (osc_clock_rate ==
				   display->panel->oplus_priv
					   .osc_clk_mode1_rate) {
				rc = dsi_panel_tx_cmd_set(
					display->panel, DSI_CMD_OSC_CLK_MODEO1);
			} else {
				pr_err("[%s]not support osc clk rate=%d\n",
				       __func__, osc_clock_rate);
			}
		...
		if (rc) {
			...
			dynamic_osc_clock =
				display->panel->oplus_priv.osc_clk_rate_lastest;
		} else {
			display->panel->oplus_priv.osc_clk_rate_lastest =
				osc_clock_rate;
		}
...
int oplus_display_get_softiris_color_status(void *data)
{
	...
	color_vivid_status =
		utils->read_bool(utils->data, "oplus,color_vivid_status");
	DSI_INFO("oplus,color_vivid_status: %s",
		 color_vivid_status ? "true" : "false");

	color_srgb_status =
		utils->read_bool(utils->data, "oplus,color_srgb_status");
	DSI_INFO("oplus,color_srgb_status: %s",
		 color_srgb_status ? "true" : "false");

	color_softiris_status =
		utils->read_bool(utils->data, "oplus,color_softiris_status");
	DSI_INFO("oplus,color_softiris_status: %s",
	...
	iris_color_status->color_softiris_status =
...
int oplus_display_panel_get_id2(void)
{
	...
	if (get_oplus_display_power_status() == OPLUS_DISPLAY_POWER_ON) {
		...
		if (!strcmp(display->panel->oplus_priv.vendor_name,
			    "S6E3HC3")) {
			ret = dsi_display_read_panel_reg(display, 0xDB, read,
							 1);
			if (ret < 0) {
				pr_err("failed to read DB ret=%d\n", ret);
				return -EINVAL;
