...
#define LOCK_BUFFER(buffer) mutex_lock(&buffer.buf_mutex)

#define UNLOCK_BUFFER(buffer) mutex_unlock(&buffer.buf_mutex)
...
struct device_hcd {
	...
	int (*read_message)(struct syna_tcm_data *tcm_info,
				       ...
				       unsigned int size)
{
	...
	if (size > buffer->buf_size) {
		...
		retval = secure_memcpy(buffer->buf, size, temp,
...
int syna_tcm_rmi_read(struct syna_tcm_data *tcm_info, unsigned short addr,
		      unsigned char *data, unsigned int length);

int syna_tcm_rmi_write(struct syna_tcm_data *tcm_info, unsigned short addr,
		       unsigned char *data, unsigned int length);
