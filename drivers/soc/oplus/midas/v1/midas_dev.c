...
static int rasm_resume(struct device *dev) {
	...
	dev_info = dev->driver_data;
	if (!dev_info) {
		pr_err("driver_data is empty");
		return 0;
	}

	data = dev_info->rasm_data;
	if (!data) {
...
static int midas_pdev_probe(struct platform_device *pdev)
{
	...
	rasm_data = kzalloc(sizeof(struct rasm_data), GFP_KERNEL);
	if (IS_ERR_OR_NULL(rasm_data)) {
		pr_err("Fail to alloc rasm_data\n");
		ret = -ENOMEM;
		goto err_rasm_alloc;
	}
	dev_info->rasm_data = rasm_data;
...
static int __init midas_dev_init(void)
{
	...
	ret = platform_driver_register(&midas_pdev_driver);
	if (ret < 0) {
