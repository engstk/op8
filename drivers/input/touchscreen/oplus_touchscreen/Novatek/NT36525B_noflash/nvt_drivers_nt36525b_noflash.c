...
static void nova_write_log_buf(struct chip_data_nt36525b *chip_info, u8 main_id, u8 sec_id)
{
...
static void nova_debug_info(struct chip_data_nt36525b *chip_info, u8 *buf)
{
...
static void nvt_read_debug_gesture_coordinate_buffer(struct chip_data_nt36525b *chip_info,
...
static void nvt_dbg_gesture_record_coor_read(struct chip_data_nt36525b *chip_info, u8 pointdata)
{
    u8 *xdata = NULL;
    uint32_t buf_len = 0;
...
static int nvt_enable_headset_mode(struct chip_data_nt36525b *chip_info, bool enable)
{
    int8_t ret = -1;
...
static int32_t nvt_clear_fw_status(struct chip_data_nt36525b *chip_info)
{
    uint8_t buf[8] = {0};
    int32_t i = 0;
...
static void nvt_enable_noise_collect(struct chip_data_nt36525b *chip_info, int32_t frame_num)
{
    int ret = -1;
    uint8_t buf[8] = {0};
...
static int8_t nvt_switch_FreqHopEnDis(struct chip_data_nt36525b *chip_info, uint8_t FreqHopEnDis)
{
    uint8_t buf[8] = {0};
    uint8_t retry = 0;
    int8_t ret = 0;
...
static void nvt_change_mode(struct chip_data_nt36525b *chip_info, uint8_t mode)
{
    uint8_t buf[8] = {0};
...
static uint8_t nvt_get_fw_pipe_noflash(struct chip_data_nt36525b *chip_info)
{
    int ret = -1;
    uint8_t buf[8] = {0};
...
static void nvt_read_mdata(struct chip_data_nt36525b *chip_info, uint32_t xdata_addr, int32_t *xdata, int32_t xdata_len)
{
    int32_t i = 0;
    int32_t j = 0;
    int32_t k = 0;
...
#ifdef CONFIG_OPLUS_TP_APK
static void nvt_read_debug_mdata(struct chip_data_nt36525b *chip_info,
...
static int32_t nvt_polling_hand_shake_status(struct chip_data_nt36525b *chip_info)
{
    uint8_t buf[8] = {0};
    int32_t i = 0;
...
static int32_t nvt_check_fw_status(struct chip_data_nt36525b *chip_info)
{
    uint8_t buf[8] = {0};
    int32_t i = 0;
    const int32_t retry = 50;
...
static uint8_t nvt_get_fw_pipe(struct chip_data_nt36525b *chip_info)
{
    int ret = -1;
    uint8_t buf[8] = {0};
...
static int32_t nvt_read_fw_noise(struct chip_data_nt36525b *chip_info, int32_t config_Diff_Test_Frame, int32_t *xdata, int32_t *xdata_n, int32_t xdata_len)
{
    uint32_t x = 0;
    uint32_t y = 0;
...
#ifdef CONFIG_OPLUS_TP_APK
static void nvt_debug_data_read(struct seq_file *s, struct chip_data_nt36525b *chip_info, DEBUG_READ_TYPE read_type)
{
    int ret = -1;
    int i, j;
    //uint8_t pipe;
...
static int32_t nvt_read_fw_short(struct chip_data_nt36525b *chip_info, int32_t *xdata, int32_t xdata_len)
{
...
static void nvt_enable_open_test(struct chip_data_nt36525b *chip_info)
{
    uint8_t buf[8] = {0};
...
static int32_t nvt_read_fw_open(struct chip_data_nt36525b *chip_info, int32_t *xdata, int32_t xdata_len)
{
    uint32_t raw_pipe_addr = 0;
    uint8_t *rawdata_buf = NULL;
    uint32_t x = 0;
