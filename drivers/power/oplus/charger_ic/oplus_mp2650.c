...
static int mp2650_chg_track_upload_icl_err_info(struct chip_mp2650 *chip, int err_type);
static int mp2650_track_upload_i2c_err_info(struct chip_mp2650 *chip, int err_type, u8 reg);
...
static ssize_t mp2650_reg_access_show(struct device *dev, struct device_attribute *attr, char *buf)
{
#if 0
#ifdef TIMED_OUTPUT
...
static ssize_t mp2650_reg_access_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
#if 0
#ifdef TIMED_OUTPUT
    struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
static ssize_t mp2650_reg_set_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	size_t count = 0;
	int reg_val = 0;
...
static ssize_t mp2650_reg_set_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t mp2650_regs_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	int i = 0;
	int reg_val = 0;
...
static ssize_t mp2650_regs_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}
...
static struct attribute *mp2650_attributes[] = { &dev_attr_reg_access.attr, &dev_attr_reg_set.attr,
						 &dev_attr_read_regs.attr, NULL };

static struct attribute_group mp2650_attribute_group = { .attrs = mp2650_attributes };
...
int mp2650_set_vindpm_vol(int vol) //default 4.5V
{
	...
	rc = mp2650_config_interface(REG01_MP2650_ADDRESS, tmp << REG01_MP2650_VINDPM_THRESHOLD_SHIFT,
...
int mp2650_get_charger_vol(void)
{
	...
	if (oplus_vooc_get_allow_reading() == true &&
	    (oplus_pps_get_support_type() != PPS_SUPPORT_2CP && oplus_pps_get_support_type() != PPS_SUPPORT_3CP)) {
...
int mp2650_input_current_limit_ctrl_by_vooc_write(int current_ma)
{
	...
	for (count = (tmp / 500); count > 2; count--) {
		...
		rc = mp2650_config_interface(REG00_MP2650_ADDRESS, tmp << REG00_MP2650_1ST_CURRENT_LIMIT_SHIFT,
					     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
		rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, tmp << REG0F_MP2650_2ND_CURRENT_LIMIT_SHIFT,
...
int mp2650_input_current_limit_write(int current_ma)
{
	...
	if ((oplus_chip->pdqc_9v_voltage_adaptive) && (oplus_vooc_get_fastchg_started() == false) &&
	    ((subtype == CHARGER_SUBTYPE_PD) || (subtype == CHARGER_SUBTYPE_QC)) &&
	...
	chg_debug("hw_sw_vol[%d %d] chg_vol [%d] pdqc_9v_voltage_adaptive %s\n", tmp_aicl_hw_point, sw_aicl_point,
		  chg_vol, oplus_chip->pdqc_9v_voltage_adaptive == true ? "true" : "false");
	...
	if (pre_icl_index < 2) {
		...
		for (i = pre_icl_index - 1; i > 0; i--) {
			...
			case INPUT_CURRENT_LIMIT_INDEX_1:
				rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_800MA,
							     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
				rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_800MA,
							     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
				break;
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_500MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_1000MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_1000MA,
				     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	msleep(50);
	chg_vol = mp2650_get_charger_vol();
	if (chg_vol < sw_aicl_point) {
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_1200MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_1200MA,
				     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	msleep(50);
	chg_vol = mp2650_get_charger_vol();
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_1500MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_1500MA,
				     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	msleep(50);
	chg_vol = mp2650_get_charger_vol();
	if (chg_vol < sw_aicl_point) {
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_1750MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_1750MA,
				     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	msleep(50);
	chg_vol = mp2650_get_charger_vol();
	if (chg_vol < sw_aicl_point) {
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_2000MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_2000MA,
				     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	msleep(50);
	chg_vol = mp2650_get_charger_vol();
	if (chg_vol < sw_aicl_point) {
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_2200MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_2200MA,
				     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	msleep(50);
	chg_vol = mp2650_get_charger_vol();
	if (chg_vol < sw_aicl_point) {
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_2500MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_2500MA,
				     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	msleep(50);
	chg_vol = mp2650_get_charger_vol();
	if (chg_vol < sw_aicl_point) {
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_2750MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_2750MA,
				     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	msleep(50);
	chg_vol = mp2650_get_charger_vol();
	if (chg_vol < sw_aicl_point) {
	...
	rc = mp2650_config_interface(REG00_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_3000MA,
				     REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
	rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, REG00_MP2650_1ST_CURRENT_LIMIT_3000MA,
				     REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	msleep(50);
	chg_vol = mp2650_get_charger_vol();
	if (chg_vol < sw_aicl_point) {
...
aicl_pre_step:
	chg_debug("usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_pre_step\n", chg_vol, i,
		  mp2650_usbin_input_current_limit[i], sw_aicl_point);
	goto aicl_rerun;
aicl_end:
...
aicl_rerun:
	...
	if (chip->track_init_done &&
	    ((oplus_chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP &&
	      current_ma >= OPLUS_CHG_TRACK_ICL_MONITOR_THD_MA && aicl_result < OPLUS_CHG_TRACK_ICL_MONITOR_THD_MA) ||
	     chip->debug_force_icl_err != 0))
		mp2650_chg_track_upload_icl_err_info(chip, TRACK_PMIC_ERR_ICL_VBUS_LOW_POINT);
...
int mp2650_charging_current_write_fast(int chg_cur)
{
	...
	rc = mp2650_config_interface(REG02_MP2650_ADDRESS, tmp << REG02_MP2650_CHARGE_CURRENT_SETTING_SHIFT,
...
void mp2650_set_aicl_point(int vbatt)
{
	...
	if ((oplus_chip->pdqc_9v_voltage_adaptive) && (oplus_vooc_get_fastchg_started() == false) &&
		...
		chg_debug("pdqc_hw_volpont_vol [%d] chg_vol %d hw_sw_vbat [%d %d %d]\n", tmp_pdqc_hw_vol, chg_vol,
			  chip->hw_aicl_point, chip->sw_aicl_point, vbatt);
	} else {
		if (chip->hw_aicl_point == MP2762_HW_AICL_POINT_5V_PHASE1 && vbatt > MP2762_AICL_POINT_VOL_PHASE2) {
		...
		chg_debug("chg_vol %d hw_sw_vbat [%d %d %d]\n", chg_vol, chip->hw_aicl_point, chip->sw_aicl_point,
			  vbatt);
	}
}
...
int mp2650_set_complete_charge_timeout(int val)
{
	...
	rc = mp2650_config_interface(REG09_MP2650_ADDRESS, val,
				     REG09_MP2650_FAST_CHARGE_TIMER_MASK | REG09_MP2650_CHARGE_TIMER_EN_MASK);
	if (rc < 0) {
...
int mp2650_float_voltage_write(int vfloat_mv)
{
	...
	rc = mp2650_config_interface(REG04_MP2650_ADDRESS, tmp << REG04_MP2650_CHARGE_FULL_VOL_SHIFT,
...
int mp2650_set_prechg_voltage_threshold(void)
{
	...
	rc = mp2650_config_interface(REG07_MP2650_ADDRESS, REG07_MP2650_PRECHARGE_THRESHOLD_7200MV,
...
int mp2650_set_prechg_current(int ipre_mA)
{
	...
	rc = mp2650_config_interface(REG03_MP2650_ADDRESS, (tmp + 1) << REG03_MP2650_PRECHARGE_CURRENT_LIMIT_SHIFT,
...
int mp2650_set_termchg_current(int term_curr)
{
	...
	rc = mp2650_config_interface(REG03_MP2650_ADDRESS, tmp << REG03_MP2650_TERMINATION_CURRENT_LIMIT_SHIFT,
...
int mp2650_set_rechg_voltage(int recharge_mv)
{
	...
	rc = mp2650_config_interface(REG04_MP2650_ADDRESS, tmp << REG04_MP2650_BAT_RECHARGE_THRESHOLD_SHIFT,
...
int mp2650_set_chging_term_disable(void)
{
	...
	rc = mp2650_config_interface(REG09_MP2650_ADDRESS, REG09_MP2650_CHARGE_TERMINATION_EN_DISABLE,
...
int mp2650_set_chging_term_enable(void)
{
	...
	rc = mp2650_config_interface(REG09_MP2650_ADDRESS, REG09_MP2650_CCHARGE_TERMINATION_EN_ENABLE,
				     REG09_MP2650_CHARGE_TERMINATION_EN_MASK);
	if (rc) {
...
int mp2650_registers_read_full(void)
{
	...
	reg_full = ((reg_full & REG13_MP2650_CHARGING_STATUS_MASK) == REG13_MP2650_CHARGING_STATUS_CHARGE_TERMINATION) ?
			   1 :
			   0;
	if (reg_full) {
...
int mp2650_reset_charger_for_wired_charge(void)
{
	...
	rc = mp2650_config_interface(REG08_MP2650_ADDRESS, REG08_MP2650_REG_RST_RESET, REG08_MP2650_REG_RST_MASK);
	if (rc < 0) {
...
int mp2650_reset_charger(void)
{
	...
	if (chip->probe_flag) {
		rc = mp2650_config_interface(REG08_MP2650_ADDRESS, REG08_MP2650_REG_RST_RESET,
...
int mp2650_otg_ilim_set(int ilim)
{
	...
	rc = mp2650_config_interface(REG07_MP2650_ADDRESS, ilim, REG07_MP2650_OTG_CURRENT_LIMIT_MASK);
	if (rc < 0) {
		chg_err("Couldn't mp2650_otg_ilim_set  rc = %d\n", rc);
	}
...
int mp2650_set_prochot_psys_cfg(void)
{
	...
	rc = mp2650_config_interface(REG0B_MP2650_ADDRESS, REG0B_MP2650_PROCHOT_PSYS_CFG_DISABLE,
...
int mp2650_set_mps_otg_current(void)
{
	...
	rc = mp2650_config_interface(REG07_MP2650_ADDRESS, REG07_MP2650_OTG_CURRENT_LIMIT_1500MA,
...
int mp2650_set_mps_otg_voltage(bool is_9v)
{
	...
	if (is_9v) {
		rc = mp2650_config_interface(REG06_MP2650_ADDRESS, REG06_MP2650_OTG_VOL_OPTION_8750MV,
					     REG06_MP2650_OTG_VOL_OPTION_MASK);
	} else {
		rc = mp2650_config_interface(REG06_MP2650_ADDRESS, REG06_MP2650_OTG_VOL_OPTION_4750MV,
					     REG06_MP2650_OTG_VOL_OPTION_MASK);
	}
	return 0;
}
...
int mp2650_set_mps_second_otg_voltage(bool is_750mv)
{
	...
	if (is_750mv) {
		rc = mp2650_config_interface(REG06_MP2650_ADDRESS, REG06_MP2650_2ND_OTG_VOL_SETTING_400MV,
					     REG06_MP2650_2ND_OTG_VOL_SETTING_MASK);
	} else {
		rc = mp2650_config_interface(REG06_MP2650_ADDRESS, REG06_MP2650_2ND_OTG_VOL_SETTING_250MV,
					     REG06_MP2650_2ND_OTG_VOL_SETTING_MASK);
	}
	return 0;
}
...
int mp2650_enable_buck_switch(void)
{
	...
	rc = mp2650_config_interface(REG12_MP2650_ADDRESS, REG12_MP2650_BUCK_SWITCH_ENABLE,
...
int mp2650_disable_buck_switch(void)
{
	...
	rc = mp2650_config_interface(REG12_MP2650_ADDRESS, REG12_MP2650_BUCK_SWITCH_DISABLE,
				     REG12_MP2650_BUCK_SWITCH_MASK);
	if (rc < 0) {
...
int mp2650_enable_async_mode(void)
{
	...
	rc = mp2650_config_interface(REG36_MP2650_ADDRESS, REG36_MP2650_ASYNC_MODE_ENABLE,
...
int mp2650_disable_async_mode(void)
{
	...
	rc = mp2650_config_interface(REG36_MP2650_ADDRESS, REG36_MP2650_ASYNC_MODE_DISABLE,
				     REG36_MP2650_ASYNC_MODE_MASK);
	if (rc < 0) {
...
void mp2650_dump_registers(void)
{
	...
	if (!chip || !oplus_chip) {
		chg_err("chip || oplus_chip is null");
		return;
	}
	...
	if (!oplus_chip->charger_exist)
		dump_count++;

	if (oplus_chip->charger_exist || (dump_count == DUMP_REG_LOG_CNT_30S)) {
		dump_count = 0;
		...
		printk(KERN_ERR "mp2650_dump_reg: [0x%02x, 0x%02x, 0x%02x, 0x%02x], [0x%02x, 0x%02x, 0x%02x, 0x%02x], "
		       ...
		       val_buf[8], val_buf[9], val_buf[10], val_buf[11], val_buf[12], val_buf[13], val_buf[14],
		       val_buf[15], val_buf[16], val_buf[17], val_buf[18], val_buf[19], val_buf[20], val_buf[21],
		       val_buf[22], val_buf[23], val_buf[24], val_buf[25], val_buf[26], val_buf[27], val_buf[28],
		       val_buf[29], val_buf[30], val_buf[31], val_buf[32], val_buf[33], val_buf[34], val_buf[35],
		...
		snprintf(buck_ic_reg_info, BATTERY_LOG_REG_MAX_SIZE,
			 ...
			 val_buf[0], val_buf[1], val_buf[2], val_buf[4], val_buf[8], val_buf[9], val_buf[19],
			 val_buf[20], val_buf[35]);
	}
}
...
int mp2650_hardware_init(void)
{
	...
	/* Wait for driver to unsuspend to initialize MP2650 hardware properly */
	rc = mp2650_wait_resume(chip);
	if (rc < 0) {
		chg_err("wait resume error:%d, can't init MP2650 hardware\n", rc);
		return rc;
	}
	...
	if (gpio_otg_value == MP2650_GPIO_OTG_EN && chip->probe_flag == false) {
...
void mp2650_wireless_set_mps_otg_en_val(int value)
{
	...
	if (IS_ERR_OR_NULL(chip->pinctrl) || IS_ERR_OR_NULL(chip->mps_otg_en_active) ||
	    IS_ERR_OR_NULL(chip->mps_otg_en_sleep) || IS_ERR_OR_NULL(chip->mps_otg_en_default)) {
...
static void mp2650_track_i2c_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct chip_mp2650 *chip = container_of(dwork, struct chip_mp2650, i2c_err_load_trigger_work);
...
static int mp2650_track_upload_i2c_err_info(struct chip_mp2650 *chip, int err_type, u8 reg)
{
	...
	if (curr_time - pre_upload_time < TRACK_I2CERROR_SAMEADDR_MIN_INTERVAL && pre_reg == reg) {
...
static void mp2650_chg_track_icl_err_load_trigger_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct chip_mp2650 *chip = container_of(dwork, struct chip_mp2650, icl_err_load_trigger_work);
...
static int mp2650_chg_track_upload_icl_err_info(struct chip_mp2650 *chip, int err_type)
{
	...
	if (err_type != TRACK_PMIC_ERR_ICL_VBUS_COLLAPSE && err_type != TRACK_PMIC_ERR_ICL_VBUS_LOW_POINT) {
		mutex_unlock(&chip->track_upload_lock);
		return -EINVAL;
	}
	...
	chip->icl_err_load_trigger->type_reason = TRACK_NOTIFY_TYPE_DEVICE_ABNORMAL;
	chip->icl_err_load_trigger->flag_reason = TRACK_NOTIFY_FLAG_EXTERN_PMIC_ABNORMAL;
	...
	memset(chip->err_reason, 0, sizeof(chip->err_reason));
	index += snprintf(&(chip->icl_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$device_id@@%s", "mp2762");
	index += snprintf(&(chip->icl_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_scene@@%s", OPLUS_CHG_TRACK_SCENE_PMIC_ICL_ERR);

	oplus_chg_track_get_pmic_err_reason(err_type, chip->err_reason, sizeof(chip->err_reason));
	index += snprintf(&(chip->icl_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			  "$$err_reason@@%s", chip->err_reason);

	memset(chip->track_temp, 0, sizeof(chip->track_temp));
	oplus_chg_track_obtain_power_info(chip->track_temp, sizeof(chip->track_temp));
	index += snprintf(&(chip->icl_err_load_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "%s",
			  chip->track_temp);
...
static int mp2650_chg_track_debugfs_init(struct chip_mp2650 *chip)
{
	...
	debugfs_create_u32("debug_force_icl_err", 0644, debugfs_mp2650, &(chip->debug_force_icl_err));

	debugfs_create_u32("debug_force_i2c_err", 0644, debugfs_mp2650, &(chip->debug_force_i2c_err));
...
static int mp2650_chg_track_init(struct chip_mp2650 *chip)
{
	...
	INIT_DELAYED_WORK(&chip->icl_err_load_trigger_work, mp2650_chg_track_icl_err_load_trigger_work);

	INIT_DELAYED_WORK(&chip->i2c_err_load_trigger_work, mp2650_track_i2c_err_load_trigger_work);
...
static int mp2650_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chg_ic = devm_kzalloc(&client->dev, sizeof(struct chip_mp2650), GFP_KERNEL);
	if (!chg_ic) {
...
static int mp2650_pm_resume(struct device *dev)
{
	...
	if (abs(ui_soc - real_soc) > URGENT_CHECK_THRESHOLD || sleep_time > NORMAL_CHECK_UPDATE_INTERVAL)
