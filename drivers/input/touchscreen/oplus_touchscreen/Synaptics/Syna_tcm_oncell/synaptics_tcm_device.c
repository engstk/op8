...
static void device_capture_touch_report(unsigned int count)
{
	...
	case REPORT_TOUCH:
		...
		retval = syna_tcm_alloc_mem(&g_device_hcd->report,
					    remaining_size);
		if (retval < 0) {
...
static int device_capture_touch_report_config(unsigned int count)
{
	...
	retval = secure_memcpy(tcm_info->config.buf, tcm_info->config.buf_size,
			       data, size, size);
	if (retval < 0) {
			   ...
			   loff_t *f_pos)
{
	...
	if (g_device_hcd->raw_mode) {
		...
		retval = g_device_hcd->read_message(
			tcm_info, g_device_hcd->resp.buf, count);
		if (retval < 0) {
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	retval = syna_tcm_alloc_mem(&g_device_hcd->out,
	...
	if (g_device_hcd->raw_mode) {
		retval = g_device_hcd->write_message(
			tcm_info, g_device_hcd->out.buf[0],
			&g_device_hcd->out.buf[1],
			count == 1 ? count : count - 1, NULL, NULL, NULL, 0);
	} else {
		mutex_lock(&tcm_info->reset_mutex);
		retval = g_device_hcd->write_message(
			tcm_info, g_device_hcd->out.buf[0],
			&g_device_hcd->out.buf[1],
			count == 1 ? count : count - 1, &g_device_hcd->resp.buf,
			&g_device_hcd->resp.buf_size,
			&g_device_hcd->resp.data_length, 0);
