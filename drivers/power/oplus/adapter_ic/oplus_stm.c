...
static bool vooc_uart_read_addr_line_and_check(unsigned int addr)
{
	...
	if (!(addr % 0x20)) {
		if (addr == ((adapter_stm8s_firmware_data[fw_line * 34 + 1] << 8) |
		...
		if ((addr - 16) == ((adapter_stm8s_firmware_data[fw_line * 34 + 1] << 8) |
				    (adapter_stm8s_firmware_data[fw_line * 34]))) {
			for (i = 0; i < 16; i++) {
				if (fw_check_buf[i + 2] !=
				    adapter_stm8s_firmware_data[fw_line * 34 + 2 + HALF_ONE_LINE + i]) {
					goto read_addr_line_err;
				}
			}
		}
...
read_addr_line_err:
	...
	if (!check_result) {
		chg_err(" fw_check err, addr:0x%x, check_buf[%d]:0x%x != fw_data[%d]:0x%x\n", addr, i + 2,
			fw_check_buf[i + 2], (fw_line * 34 + 2 + i), adapter_stm8s_firmware_data[fw_line * 34 + 2 + i]);
	}
	return check_result;
}
...
static bool vooc_adapter_update_handle(unsigned long tx_pin, unsigned long rx_pin)
{
	...
	rc = vooc_uart_write_some_addr(&adapter_stm8s_firmware_data[0], (sizeof(adapter_stm8s_firmware_data) - 34));
	if (rc) {
