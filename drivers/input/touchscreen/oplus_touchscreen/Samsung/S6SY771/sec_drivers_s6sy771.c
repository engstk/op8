...
static int sec_get_verify_result(struct chip_data_s6sy771 *chip_info);
static int sec_read_mutual(struct chip_data_s6sy771 *chip_info, u8 type,
			   char *data, int len);
static bool check_calibration(struct chip_data_s6sy771 *chip_info);
...
static struct chip_data_s6sy771 *g_chip_info;
/**************************** end of global variable delcare*****************************************/

/****** Start of other functions that work for oplus_touchpanel_operations callbacks***********/
static int sec_enable_black_gesture(struct chip_data_s6sy771 *chip_info,
				    bool enable)
{
	int ret = 0;
	int i = 0;
	...
	if (enable) {
		...
		for (i = 0; i < 20; i++) {
			...
			sec_mdelay(10);
			ret = touch_i2c_read_byte(chip_info->client,
		...
		if (chip_info->debug_gesture_sta) {
			...
			touch_i2c_write_byte(chip_info->client,
					     SEC_CMD_GESTURE_DEBUG, 0x01);
		}
#endif // end of CONFIG_OPLUS_TP_APK
	} else {
		touch_i2c_write_word(chip_info->client,
				     SEC_CMD_WAKEUP_GESTURE_MODE, 0x0000);
		if (chip_info->fingerprint_status != FINGERPRINT_DOWN_DETECT ||
		    *chip_info->in_suspend) {
			for (i = 0; i < 20; i++) {
				touch_i2c_write_byte(chip_info->client,
						     SEC_CMD_SET_POWER_MODE,
						     0x00);
				sec_mdelay(10);
				ret = touch_i2c_read_byte(
					chip_info->client,
					SEC_CMD_SET_POWER_MODE);
				if (0x00 == ret)
					break;
	...
	if (i >= 20) {
		ret = -1;
		TPD_INFO("%s: %s black gesture failed\n", __func__,
			 enable ? "enter" : "exit");
	} else {
		TPD_INFO("%s: %d times %s black gesture success\n", __func__, i,
			 enable ? "enter" : "exit");
	}
	return ret;
}
...
{
	struct chip_data_s6sy771 *chip_info =
		(struct chip_data_s6sy771 *)chip_data;
	int ret = -1;
	int i = 0;
	bool single_tap_support_b = 0;
	struct touchpanel_data *ts = i2c_get_clientdata(chip_info->client);
	...
	if (enable) {
		for (i = 0; i < 20; i++) {
			if (single_tap_support_b) {
				touch_i2c_write_word(
					chip_info->client,
					SEC_CMD_WAKEUP_GESTURE_MODE, 0xFFFF);
			} else {
				touch_i2c_write_word(
					chip_info->client,
					SEC_CMD_WAKEUP_GESTURE_MODE, 0xFF1F);
			}
			touch_i2c_write_byte(chip_info->client,
					     SEC_CMD_SET_POWER_MODE, 0x01);
			sec_mdelay(10);
...
static int sec_send_charge_state(struct chip_data_s6sy771 *chip_info)
{
	int ret = -1;
	u8 data = 1;
	if (chip_info->sec_charger_sta) {
		data = chip_info->sec_charger_sta << 1;
	}
	ret = touch_i2c_write_byte(chip_info->client, SET_CMD_SET_CHARGER_MODE,
				   data);
...
static int sec_enable_charge_mode(struct chip_data_s6sy771 *chip_info,
				  bool enable)
{
	int ret = -1;

	if (enable) {
		SET_BIT(chip_info->sec_charger_sta, NORMAL_CHARGER);
	} else {
		CLR_BIT(chip_info->sec_charger_sta, NORMAL_CHARGER);
	}

	ret = sec_send_charge_state(chip_info);

	return ret;
}

static int sec_enable_wireless_charge_mode(struct chip_data_s6sy771 *chip_info,
					   bool enable)
{
	int ret = -1;

	if (enable) {
		SET_BIT(chip_info->sec_charger_sta, WIRELESS_CHARGER);

	} else {
		CLR_BIT(chip_info->sec_charger_sta, WIRELESS_CHARGER);
	}
...
static int sec_enable_earsense_mode(struct chip_data_s6sy771 *chip_info,
				    bool enable)
{
	int ret = -1;
...
static int sec_enable_palm_reject(struct chip_data_s6sy771 *chip_info,
				  bool enable)
{
	int ret = -1;

	if (enable) {
		ret = touch_i2c_write_word(chip_info->client,
					   SEC_CMD_PALM_SWITCH, 0x0061);
	} else {
		ret = touch_i2c_write_word(chip_info->client,
					   SEC_CMD_PALM_SWITCH, 0x0041);
	}
...
static int sec_enable_game_mode(struct chip_data_s6sy771 *chip_info,
				bool enable)
{
	int ret = -1;
	char buf[4] = { 0x00, 0x00, 0x00, 0x00 };

	buf[3] = enable ? 0x05 : chip_info->default_lock_point_level;
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_SENSETIVE_CTRL,
				    sizeof(buf), buf);
	ret |= touch_i2c_write_byte(chip_info->client, SEC_CMD_GAME_MODE,
				    enable ? 1 : 0);
	TPD_INFO("%s: state: %d %s!\n", __func__, enable,
		 ret < 0 ? "failed" : "success");
	return ret;
...
static int sec_enable_headset_mode(struct chip_data_s6sy771 *chip_info,
				   bool enable)
{
	int ret = -1;

	ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_HEADSET_MODE,
				   enable ? 1 : 0);
	TPD_INFO("%s: state: %d %s!\n", __func__, enable,
		 ret < 0 ? "failed" : "success");
	return ret;
...
static int sec_smooth_lv_set(void *chip_data, int level)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_SMOOTH_LEVEL,
				   (u8)level);
	TPD_INFO("%s: state: %d %s!\n", __func__, level,
		 ret < 0 ? "failed" : "success");
	return ret;
...
int sec_wait_for_ready(struct chip_data_s6sy771 *chip_info, unsigned int ack)
{
	int rc = -1;
	int retry = 0, retry_cnt = 200;
	int8_t status = -1;
	u8 tBuff[SEC_EVENT_BUFF_SIZE] = {
		0,
	};

	while (touch_i2c_read_block(chip_info->client, SEC_READ_ONE_EVENT,
				    SEC_EVENT_BUFF_SIZE, tBuff) > 0) {
		...
		if (retry++ > retry_cnt) {
			TPD_INFO(
				...
				__func__, tBuff[0], tBuff[1], tBuff[2],
				tBuff[3], tBuff[4], tBuff[5], tBuff[6],
				tBuff[7]);
			break;
		}
		sec_mdelay(20);
...
static int sec_enter_fw_mode(struct chip_data_s6sy771 *chip_info)
{
	int ret = -1;
	u8 device_id[3] = { 0 };
	u8 fw_update_mode_passwd[] = { 0x55, 0xAC };

	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_ENTER_FW_MODE,
	...
	ret = touch_i2c_read_byte(
		chip_info->client,
		SEC_READ_BOOT_STATUS); //after reset, check bootloader again
	if (ret < 0) {
	...
	ret = touch_i2c_read_block(chip_info->client, SEC_READ_ID, 3,
				   device_id);
	if (ret < 0) {
	...
	if ((device_id[1] == 0x37) && (device_id[2] == 0x71))
...
static int sec_flash_page_erase(struct chip_data_s6sy771 *chip_info,
				u32 page_idx, u32 page_num)
{
	...
	ret = touch_i2c_write_block(chip_info->client, tCmd[0], 5, &tCmd[1]);

	return ret;
}

static int sec_flash_page_write(struct chip_data_s6sy771 *chip_info,
				u32 page_idx, u8 *page_data)
{
	...
	ret = touch_i2c_write_block(chip_info->client, tCmd[0],
				    1 + 2 + flash_page_size, &tCmd[1]);
	return ret;
}

static bool sec_limited_flash_page_write(struct chip_data_s6sy771 *chip_info,
					 u32 page_idx, u8 *page_data)
{
	int ret = -1;
	u8 *tCmd = NULL;
	...
	copy_data[2 + flash_page_size] =
		sec_checksum(copy_data, 0, 2 + flash_page_size); /* CS */
...
static int sec_flash_write(struct chip_data_s6sy771 *chip_info, u32 mem_addr,
			   u8 *mem_data, u32 mem_size)
{
	...
	for (page_idx = page_num - 1;; page_idx--) {
		...
		if (chip_info->boot_ver[0] == 0xB2) {
			ret = sec_flash_page_write(chip_info,
						   (page_idx + page_idx_start),
						   page_buf);
			if (ret < 0) {
				sec_mdelay(50);
				ret = sec_flash_page_write(
					chip_info, (page_idx + page_idx_start),
					page_buf);
				if (ret < 0) {
					TPD_INFO(
			...
			ret = sec_limited_flash_page_write(
				chip_info, (page_idx + page_idx_start),
				page_buf);
			if (ret < 0) {
				sec_mdelay(50);
		...
		if (page_idx ==
		    0) /* end condition (page_idx >= 0)   page_idx type unsinged int */
			break;
	}
...
static int sec_block_read(struct chip_data_s6sy771 *chip_info, u32 mem_addr,
			  int mem_size, u8 *buf)
{
	int ret;
	u8 cmd[5];
	...
	ret = touch_i2c_write_block(chip_info->client, cmd[0], 4, &cmd[1]);
	if (ret < 0) {
	...
	ret = touch_i2c_write_block(chip_info->client, cmd[0], 2, &cmd[1]);
	if (ret < 0) {
		TPD_INFO("%s: send command failed, %02X\n", __func__, cmd[0]);
		return -EIO;
	}
	...
	ret = touch_i2c_read_block(chip_info->client, cmd[0], mem_size, data);
	if (ret < 0) {
...
static int sec_memory_read(struct chip_data_s6sy771 *chip_info, u32 mem_addr,
			   u8 *mem_data, u32 mem_size)
{
	int ret;
	int retry = 3;
	...
	while (read_left > 0) {
		...
		retry = 3;
		do {
			ret = sec_block_read(chip_info, mem_addr, unit_size,
					     tmp_data);
			if (retry-- == 0) {
				TPD_INFO(
...
static int sec_chunk_update(struct chip_data_s6sy771 *chip_info, u32 addr,
			    u32 size, u8 *data)
{
	int ii = 0, ret = 0;
	u8 *mem_rb = NULL;
...
static int sec_read_calibration_report(struct chip_data_s6sy771 *chip_info)
{
	int ret;
	u8 buf[5] = { 0 };
	...
	ret = touch_i2c_read_block(chip_info->client, buf[0], 4, &buf[1]);
	if (ret < 0) {
...
static int sec_execute_force_calibration(struct chip_data_s6sy771 *chip_info)
{
	int rc = -1;

	TPD_INFO("start do force calibration\n");
	if (touch_i2c_write_block(chip_info->client,
				  SEC_CMD_FACTORY_PANELCALIBRATION, 0,
				  NULL) < 0) {
...
{
	int i = 0;
	struct Coordinate left_most = point[0], right_most = point[0],
			  top_most = point[0], down_most = point[0];
...
void sec_refresh_switch(int fps)
{
	int ret = -1;
	...
	g_chip_info->display_refresh_rate = fps;

	if (!g_chip_info->is_power_down) {
		ret = touch_i2c_write_byte(g_chip_info->client,
					   SEC_CMD_REFRESH_RATE_SWITCH, fps);
...
EXPORT_SYMBOL(sec_refresh_switch);
...
static int sec_reset(void *chip_data)
{
	...
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_SENSE_ON, 0,
				    NULL);
...
static int sec_get_chip_info(void *chip_data)
{
	...
	ret = touch_i2c_read_block(chip_info->client, SEC_CMD_SENSETIVE_CTRL, 4,
				   buf);
	if (ret < 0) {
		TPD_INFO("%s: failed to read default lock point level\n",
			 __func__);
	} else {
		chip_info->default_lock_point_level = buf[3];
		TPD_INFO("%s: get default lock point level : 0x%2x\n", __func__,
			 chip_info->default_lock_point_level);
	}
...
static int sec_power_control(void *chip_data, bool enable)
{
	...
	if (true == enable) {
		...
		sec_wait_for_ready(chip_info, SEC_ACK_BOOT_COMPLETE);
		ret = touch_i2c_write_block(chip_info->client, SEC_CMD_SENSE_ON,
					    0, NULL);
		TPD_INFO("%s: write sense on %s\n", __func__,
			 (ret < 0) ? "failed" : "success");
				   ...
				   struct panel_info *panel_data)
{
	...
	ret = touch_i2c_read_byte(
		chip_info->client,
		SEC_READ_FIRMWARE_INTEGRITY); //judge whether fw is right
	if (ret < 0) {
	...
	ret = touch_i2c_read_block(chip_info->client, SEC_READ_TS_STATUS, 4,
				   &data[1]);
	if (ret < 0) {
	...
	panel_data->TP_FW = (data[0] << 24) | (data[1] << 16) | (data[2] << 8) |
			    data[3];
	if (panel_data->manufacture_info.version) {
		if (panel_data->vid_len == 0) {
			...
			snprintf(dev_version, MAX_DEVICE_VERSION_LENGTH, "%X",
				 panel_data->TP_FW);
			strlcpy(&panel_data->manufacture_info.version[ver_len],
				dev_version,
				MAX_DEVICE_VERSION_LENGTH - ver_len);
		}
	}
				     ...
				     bool force)
{
	...
	fw_version_in_bin = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) |
			    buf[3];
	memset(buf, 0, 4);
	touch_i2c_read_block(chip_info->client, SEC_READ_IMG_VERSION, 4, buf);
	fw_version_in_ic = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) |
			   buf[3];
	...
	config_version_in_bin = (buf[0] << 24) | (buf[1] << 16) |
				(buf[2] << 8) | buf[3];
	memset(buf, 0, 4);
	touch_i2c_read_block(chip_info->client, SEC_READ_CONFIG_VERSION, 4,
			     buf);
	config_version_in_ic = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) |
			       buf[3];
	TPD_INFO(
	...
	sec_reset(chip_info);
	cal_status = sec_read_calibration_report(
		chip_info); //read out calibration result
	if ((cal_status == 0) || (cal_status == 0xFF) ||
	    (config_version_in_ic != config_version_in_bin)) {
		TPD_INFO(
			"start calibration because of cal_status or config version changed\n");
		sec_execute_force_calibration(chip_info);
		ret = touch_i2c_write_byte(
			chip_info->client, SEC_VENDOR_INFO_CALIBRATION_STATUS,
			0); // clear calibration status after change config
		if (ret < 0) {
			TPD_INFO(
				"%s : clear calibration status after config change failed\n",
				__func__);
		}
	}
...
CAL_CHECK:
	if (check_calibration(
		    chip_info)) { //check whether we need do calibration
		TPD_INFO(
			"touchpanel have something wrong, need do calibration\n");
		chip_info->cal_needed =
			true; //set flag to show touchpanel need to be calibrated
		...
		if (check_calibration(chip_info)) {
			...
			chip_info->cal_needed =
				false; //set false when still check failed after calibration
		}
	}
	TPD_INFO("update firmware state: %s\n",
		 (update_state == FW_UPDATE_SUCCESS) ? "update success!" :
		 (update_state == FW_UPDATE_ERROR)   ? "update failed!" :
						       "no need update!");
	return update_state;
}

static u32 sec_trigger_reason(void *chip_data, int gesture_enable,
			      int is_suspended)
{
	int ret = 0;
	int event_id = 0;
	...
	ret = touch_i2c_read_block(chip_info->client, SEC_READ_ONE_EVENT,
	...
	if (event_id == SEC_STATUS_EVENT) {
		...
		if ((p_event_status->stype == TYPE_STATUS_EVENT_VENDOR_INFO) &&
		    (p_event_status->status_id ==
		     SEC_VENDOR_INFO_NOISE_LEVEL_CHANGED)) {
		...
		if ((p_event_status->stype == TYPE_STATUS_EVENT_VENDOR_INFO) &&
			...
			if (p_event_status->status_data_1 == 0x01) {
				chip_info->fingerprint_status =
					FINGERPRINT_DOWN_DETECT;
			} else if (p_event_status->status_data_1 == 0x00) {
				chip_info->fingerprint_status =
					FINGERPRINT_UP_DETECT;
			}
		...
		if ((p_event_status->stype == TYPE_STATUS_EVENT_ERR) &&
		    (p_event_status->status_id == SEC_ERR_EVENT_QUEUE_FULL)) {
		...
		if ((p_event_status->stype == TYPE_STATUS_EVENT_VENDOR_INFO) &&
		    ((p_event_status->status_id == SEC_STATUS_PAGE_ERASE) ||
		     (p_event_status->status_id == SEC_STATUS_SECTOR_ERASE) ||
		     (p_event_status->status_id == SEC_STATUS_WORD_WRITE) ||
		     (p_event_status->status_id == SEC_STATUS_PAGE_WRITE) ||
		     (p_event_status->status_id == SEC_STATUS_UPDATE_ADDR) ||
		     (p_event_status->status_id == SEC_STATUS_UPDATE_SIZE))) {
			TPD_INFO(
				"%s: Flash change detected. 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
				__func__, chip_info->first_event[0],
				chip_info->first_event[1],
				chip_info->first_event[2],
				chip_info->first_event[3],
				chip_info->first_event[4],
				chip_info->first_event[5],
				chip_info->first_event[6],
				chip_info->first_event[7]);
			return IRQ_IGNORE;
		}
...
bool corner_point_filtered(struct chip_data_s6sy771 *chip_info, uint16_t x,
			   uint16_t y)
{
	/*
				...
				int max_num)
{
	...
	if ((t_id < max_num) &&
		...
		points[t_id].x = (p_event_coord->x_11_4 << 4) |
				 (p_event_coord->x_3_0);
		points[t_id].y = (p_event_coord->y_11_4 << 4) |
				 (p_event_coord->y_3_0);
		...
		if (!corner_point_filtered(chip_info, points[t_id].x,
					   points[t_id].y))
			obj_attention = obj_attention |
					(1 << t_id); //set touch bit
#ifdef CONFIG_OPLUS_TP_APK
		if (chip_info->debug_gesture_sta && chip_info->in_gesture) {
			if (chip_info->geture_points_count < 1000 && t_id < 3) {
				chip_info->gesture_buf
					[chip_info->geture_points_count * 4] =
					t_id;
				chip_info->gesture_buf
					[chip_info->geture_points_count * 4 + 1] =
					chip_info->first_event[1];
				chip_info->gesture_buf
					[chip_info->geture_points_count * 4 + 2] =
					chip_info->first_event[2];
				chip_info->gesture_buf
					[chip_info->geture_points_count * 4 + 3] =
					chip_info->first_event[3];
...
#ifdef CONFIG_OPLUS_TP_APK
			if (chip_info->debug_gesture_sta &&
			    chip_info->in_gesture) {
				if (chip_info->geture_points_count < 1000 &&
				    t_id < 3) {
					chip_info->gesture_buf
						[chip_info->geture_points_count *
						 4] = t_id;
...
static int sec_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	ret = touch_i2c_read_block(chip_info->client, SEC_READ_GESTURE_EVENT,
	...
	case GESTURE_RIGHT:
		...
		gesture->Point_start.x = (coord[0] << 4) |
					 ((coord[2] >> 4) & 0x0F);
		gesture->Point_start.y = (coord[1] << 4) |
					 ((coord[2] >> 0) & 0x0F);
		gesture->Point_end.x = (coord[3] << 4) |
				       ((coord[5] >> 4) & 0x0F);
		gesture->Point_end.y = (coord[4] << 4) |
				       ((coord[5] >> 0) & 0x0F);
		break;
	...
	case GESTURE_UP_V:
		...
		gesture->Point_start.y = (coord[1] << 4) |
					 ((coord[2] >> 0) & 0x0F);
		gesture->Point_end.x = (coord[6] << 4) |
				       ((coord[8] >> 4) & 0x0F);
		gesture->Point_end.y = (coord[7] << 4) |
				       ((coord[8] >> 0) & 0x0F);
		gesture->Point_1st.x = (coord[3] << 4) |
				       ((coord[5] >> 4) & 0x0F);
		gesture->Point_1st.y = (coord[4] << 4) |
				       ((coord[5] >> 0) & 0x0F);
		break;
	...
	case GESTURE_O:
		...
		gesture->Point_start.y = (coord[1] << 4) |
					 ((coord[2] >> 0) & 0x0F);
		limitPoint[0].x = (coord[3] << 4) |
				  ((coord[5] >> 4) & 0x0F); //ymin
		limitPoint[0].y = (coord[4] << 4) | ((coord[5] >> 0) & 0x0F);
		limitPoint[1].x = (coord[6] << 4) |
				  ((coord[8] >> 4) & 0x0F); //xmin
		limitPoint[1].y = (coord[7] << 4) | ((coord[8] >> 0) & 0x0F);
		limitPoint[2].x = (coord[9] << 4) |
				  ((coord[11] >> 4) & 0x0F); //ymax
		limitPoint[2].y = (coord[10] << 4) | ((coord[11] >> 0) & 0x0F);
		limitPoint[3].x = (coord[12] << 4) |
				  ((coord[14] >> 4) & 0x0F); //xmax
		limitPoint[3].y = (coord[13] << 4) | ((coord[14] >> 0) & 0x0F);
		gesture->Point_end.x = (coord[15] << 4) |
				       ((coord[17] >> 4) & 0x0F);
		gesture->Point_end.y = (coord[16] << 4) |
				       ((coord[17] >> 0) & 0x0F);
	...
	case GESTURE_DOUBLE_LINE:
		...
		gesture->Point_end.y = (coord[4] << 4) |
				       ((coord[5] >> 0) & 0x0F);
		gesture->Point_1st.x = (coord[6] << 4) |
				       ((coord[8] >> 4) & 0x0F);
		gesture->Point_1st.y = (coord[7] << 4) |
				       ((coord[8] >> 0) & 0x0F);
		gesture->Point_2nd.x = (coord[9] << 4) |
				       ((coord[11] >> 4) & 0x0F);
		gesture->Point_2nd.y = (coord[10] << 4) |
				       ((coord[11] >> 0) & 0x0F);
		break;
	...
	case GESTURE_M:
		...
		gesture->Point_1st.y = (coord[4] << 4) |
				       ((coord[5] >> 0) & 0x0F);
		gesture->Point_2nd.x = (coord[6] << 4) |
				       ((coord[8] >> 4) & 0x0F);
		gesture->Point_2nd.y = (coord[7] << 4) |
				       ((coord[8] >> 0) & 0x0F);
		gesture->Point_3rd.x = (coord[9] << 4) |
				       ((coord[11] >> 4) & 0x0F);
		gesture->Point_3rd.y = (coord[10] << 4) |
				       ((coord[11] >> 0) & 0x0F);
		gesture->Point_end.x = (coord[12] << 4) |
				       ((coord[14] >> 4) & 0x0F);
		gesture->Point_end.y = (coord[13] << 4) |
				       ((coord[14] >> 0) & 0x0F);
		break;
	...
	case GESTURE_S:
		...
		gesture->Point_2nd.y = (coord[7] << 4) |
				       ((coord[8] >> 0) & 0x0F);
		gesture->Point_end.x = (coord[9] << 4) |
				       ((coord[11] >> 4) & 0x0F);
		gesture->Point_end.y = (coord[10] << 4) |
				       ((coord[11] >> 0) & 0x0F);
		break;
	...
	TPD_INFO(
		...
		gesture->clockwise, gesture->Point_start.x,
		gesture->Point_start.y, gesture->Point_end.x,
		gesture->Point_end.y, gesture->Point_1st.x,
		gesture->Point_1st.y, gesture->Point_2nd.x,
		gesture->Point_2nd.y, gesture->Point_3rd.x,
		gesture->Point_3rd.y, gesture->Point_4th.x,
		gesture->Point_4th.y);
...
static void sec_enable_fingerprint_mode(void *chip_data, uint32_t enable)
{
	...
	if (enable == 1) {
		...
		chip_info->fingerprint_status = 0;
	}
...
static void sec_get_fingerprint_info(void *chip_data,
				     struct fp_underscreen_info *fp_tpinfo)
{
	...
	fp_tpinfo->x = p_event_status->status_data_2 |
		       (p_event_status->status_data_3 << 4 & 0x0F00);
	fp_tpinfo->y = (p_event_status->status_data_3 << 8 & 0x0F00) |
		       p_event_status->status_data_4;

	fp_tpinfo->touch_state = chip_info->fingerprint_status;

	if (chip_info->fingerprint_status == FINGERPRINT_UP_DETECT &&
	    !*chip_info->in_suspend) {
		for (i = 0; i < 20; i++) {
			ret = touch_i2c_read_byte(chip_info->client,
						  SEC_CMD_SET_POWER_MODE);
...
static int sec_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_WIRELESS_CHARGE:
		...
		if (ret < 0) {
			TPD_INFO(
				"%s: enable wireless charge mode : %d failed\n",
				__func__, flag);
		}
		break;
...
//static int sec_get_wireless_state(void)
//{
//    return 0;
//}
//#else
...
static struct oplus_touchpanel_operations sec_ops = {
	...
	//    .get_wireless_state         = sec_get_wireless_state,
...
static int sec_fix_tmode(struct chip_data_s6sy771 *chip_info, u8 mode, u8 state)
{
	int ret = -1;
	u8 tBuff[2] = { mode, state };

	ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_STATEMANAGE_ON,
				   STATE_MANAGE_OFF);
	sec_mdelay(20);
	ret |= touch_i2c_write_block(chip_info->client, SEC_CMD_CHG_SYSMODE,
				     sizeof(tBuff), tBuff);
	sec_mdelay(20);
...
static int sec_release_tmode(struct chip_data_s6sy771 *chip_info)
{
	int ret = -1;
...
static int sec_read_self(struct chip_data_s6sy771 *chip_info, u8 type,
			 char *data, int len)
{
	int ret = 0;
	...
	ret = sec_fix_tmode(chip_info, TOUCH_SYSTEM_MODE_TOUCH,
	...
	ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_SELF_RAW_TYPE,
				   type);
	if (ret < 0) {
	...
	ret = touch_i2c_read_block(chip_info->client,
				   SEC_READ_TOUCH_SELF_RAWDATA, data_len, data);
	if (ret < 0) {
			   ...
			   char *data, int len)
{
	...
	ret = touch_i2c_write_byte(chip_info->client, SEC_CMD_MUTU_RAW_TYPE,
				   type);
	if (ret < 0) {
	...
	ret = touch_i2c_read_block(chip_info->client, SEC_READ_TOUCH_RAWDATA,
				   len, data);
	if (ret < 0) {
...
static void sec_self_delta_read(struct seq_file *s, void *chip_data)
{
	...
	for (x = 0; x < chip_info->hw_res->RX_NUM; x++) {
		temp_delta = (data[x * 2 + rx_offset] << 8) |
			     data[x * 2 + 1 + rx_offset];
		seq_printf(s, "%4d, ", temp_delta);
	}
	seq_printf(s, "\n");
...
static void sec_main_register_read(struct seq_file *s, void *chip_data)
{
	...
	state = touch_i2c_read_byte(chip_info->client,
				    SEC_VENDOR_INFO_CALIBRATION_STATUS);
	seq_printf(s, "oplus customized calibration status : 0x%02x\n", state);

	state = touch_i2c_read_byte(chip_info->client,
				    SET_CMD_SET_CHARGER_MODE);
	...
	seq_printf(
		s,
		"touch function: 0x%02x(proximity/wetmode/palm/stylus/glove/cover/hover/touch)\n",
		state);
	...
	state = touch_i2c_read_word(chip_info->client,
				    SEC_CMD_WAKEUP_GESTURE_MODE);
	...
	seq_printf(
		s,
		"power mode: 0x%02x[normal-0x02/lpwg-0x05], 0x%02x[indle-0x00/active-0x02]\n",
		buf[1], buf[3]);

	memset(grip_buf, 0, 5);
	touch_i2c_read_block(chip_info->client, SEC_CMD_GRIP_ENABLE, 1,
			     grip_buf);
	seq_printf(s, "grip enable bit : 0x%02x\n", grip_buf[0]);

	memset(grip_buf, 0, 5);
	touch_i2c_read_block(chip_info->client, SEC_CMD_GRIP_DIRECTION, 1,
			     grip_buf);
	seq_printf(
		s,
		"touch direction(0x00 (portrait mode), 0x01 (landscape mode), 0x02 (reverse landscape mode), 0x03 (reverse portrait mode)) : 0x%02x\n",
		grip_buf[0]);

	memset(grip_buf, 0, 5);
	touch_i2c_write_byte(chip_info->client, SEC_CMD_GRIP_AREA,
			     DEAD_ZONE_TYPE_1);
	...
	seq_printf(
		s,
		"corner triangle dead zone X: 0x%02x 0x%02x, Y: 0x%02x 0x%02x\n",
		grip_buf[1], grip_buf[2], grip_buf[3], grip_buf[4]);
	...
	seq_printf(
		s,
		"conditon long press zone X: 0x%02x 0x%02x, Y: 0x%02x 0x%02x\n",
		grip_buf[1], grip_buf[2], grip_buf[3], grip_buf[4]);

	memset(grip_buf, 0, 5);
	touch_i2c_write_byte(chip_info->client, SEC_CMD_GRIP_PARA,
			     LONG_PRESS_ZONE);
	touch_i2c_read_block(chip_info->client, SEC_CMD_GRIP_PARA, 3, grip_buf);
	seq_printf(
		s,
		"conditon long press zone delay time: 0x%02x , drag distance: 0x%02x\n",
		grip_buf[1], grip_buf[2]);

	memset(grip_buf, 0, 5);
	touch_i2c_write_byte(chip_info->client, SEC_CMD_GRIP_AREA,
			     LARGE_TOUCH_REJECTION);
	touch_i2c_read_block(chip_info->client, SEC_CMD_GRIP_AREA, 5, grip_buf);
	...
	touch_i2c_read_block(chip_info->client, SEC_CMD_GRIP_PARA, 3, grip_buf);
	seq_printf(s,
		   "large touch zone delay time: 0x%02x , touch size: 0x%02x\n",
		   grip_buf[1], grip_buf[2]);

	memset(grip_buf, 0, 5);
	touch_i2c_write_byte(chip_info->client, SEC_CMD_GRIP_AREA,
			     CORNER_LONG_PRESS_ZONE);
	touch_i2c_read_block(chip_info->client, SEC_CMD_GRIP_AREA, 5, grip_buf);
	seq_printf(
		s,
		"corner long press zone X: 0x%02x 0x%02x, Y: 0x%02x 0x%02x\n",
		grip_buf[1], grip_buf[2], grip_buf[3], grip_buf[4]);
	...
	memset(grip_buf, 0, 5);
	touch_i2c_read_block(chip_info->client, SEC_CMD_EDGE_SCREEN, 4,
			     grip_buf);
	seq_printf(
		s,
		"grip edage screen zone: 0x%02x, 0x%02x, 0x%02x direction: 0x%02x\n",
...
static void sec_rtdp_start(struct chip_data_s6sy771 *chip_info)
{
	int ret = -1;
	u8 tpara[4] = {
		0x0B, 0x06, 0x64, 0x06
	}; //scan type, mutual rawdata type, trigger threshold, self rawdata type

	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_RTDP_ERASE, 0,
				    NULL);
	if (ret < 0) {
	...
	sec_mdelay(1000);
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_RTDP_START, 4,
				    tpara);
	if (ret < 0) {
...
static int sec_rtdp_read_event(struct chip_data_s6sy771 *chip_info)
{
	int ret = -1;
...
static void sec_rtdp_dump(struct seq_file *s,
			  struct chip_data_s6sy771 *chip_info)
{
	uint8_t temp[2] = { 0 };
	uint8_t *pRead = NULL;
	...
	ret = touch_i2c_write_block(chip_info->client, SEC_CMD_RTDP_INIT_PTR, 0,
				    NULL);
	if (ret < 0) {
		TPD_INFO("%s: init rtdp read pointer failed\n", __func__);
	}

	ret = touch_i2c_read_block(chip_info->client, SEC_READ_RTDP_FRAMENUM, 2,
				   temp);
	if (ret < 0) {
	...
	for (fnum = 0; fnum < framenum; fnum++) {
		...
		for (i = 0; i < tx_num + 1; i++) {
			...
			for (j = 0; j < rx_num; j++) {
				...
				nodeData = (pRead[iArrayIndex * 2] << 8) |
					   pRead[iArrayIndex * 2 + 1];
		...
		for (i = 0; i < tx_num + 1; i++) {
			nodeData = (pRead[(iArrayIndex + i) * 2] << 8) |
				   pRead[(iArrayIndex + i) * 2 + 1];
			seq_printf(s, "%4d, ", nodeData);
		}
		seq_printf(s, "\n");
		iArrayIndex = (tx_num + 1) * rx_num + tx_num + 1;
		for (j = 0; j < rx_num; j++) {
			nodeData = (pRead[(iArrayIndex + j) * 2] << 8) |
				   pRead[(iArrayIndex + j) * 2 + 1];
			seq_printf(s, "%4d, ", nodeData);
		}
	}
ERR_OUT:
...
static void sec_reserve_read(struct seq_file *s, void *chip_data)
{
	...
	if (0x01 == state) { //enabled, but not ready for reading
		...
		return;
		   ...
		   (0x0A == state)) { //abnormal, need start rtdp
...
static void sec_earsese_rawdata_read(void *chip_data, char *rawdata,
				     int read_len)
{
	int ret = 0;
	u8 buf[2] = { 0 };
	...
	touch_i2c_write_byte(chip_info->client, SEC_CMD_MUTU_RAW_TYPE,
			     TYPE_DATA_RAWDATA);
	touch_i2c_write_block(chip_info->client, SEC_CMD_TOUCH_RAWDATA_SETLEN,
			      2, buf);
	ret = touch_i2c_read_block(chip_info->client,
				   SEC_CMD_TOUCH_RAWDATA_READ, read_len,
				   rawdata);
	if (ret < 0) {
	...
	for (i = 0; i < len_y; i++) {
		for (j = 0; j < len_x / 2; j++) {
			...
			rawdata[2 * (len_x * i + j)] =
				rawdata[2 * (len_x * i + len_x - 1 - j) + 1];
			rawdata[2 * (len_x * i + j) + 1] =
				rawdata[2 * (len_x * i + len_x - 1 - j)];
...
static void sec_earsese_delta_read(void *chip_data, char *earsense_delta,
				   int read_len)
{
	...
	buf[0] = (u8)((read_len >> 8) & 0xFF);
	buf[1] = (u8)(read_len & 0xFF);
	hover_status = touch_i2c_read_byte(
		chip_info->client, SEC_CMD_HOVER_DETECT); //read hover state
	data_type = touch_i2c_read_byte(
		chip_info->client,
		SEC_CMD_MUTU_RAW_TYPE); //read current data type
	...
	touch_i2c_write_block(chip_info->client, SEC_CMD_TOUCH_RAWDATA_SETLEN,
			      2, buf);
	ret = touch_i2c_read_block(chip_info->client, SEC_CMD_TOUCH_DELTA_READ,
				   read_len, earsense_delta);
	if (ret < 0) {
	...
	for (i = 0; i < len_y; i++) {
		for (j = 0; j < len_x / 2; j++) {
			...
			earsense_delta[2 * (len_x * i + j)] =
				earsense_delta[2 * (len_x * i + len_x - 1 - j) +
					       1];
			earsense_delta[2 * (len_x * i + j) + 1] =
				earsense_delta[2 * (len_x * i + len_x - 1 - j)];
			earsense_delta[2 * (len_x * i + len_x - 1 - j)] =
				tmp_byte[1];
			earsense_delta[2 * (len_x * i + len_x - 1 - j) + 1] =
				tmp_byte[0];
		}
	}
	if (len_x % 2) {
...
static void sec_earsese_selfdata_read(void *chip_data, char *self_data,
				      int read_len)
{
	int i = 0, ret = 0;
	int8_t tmp = 0;
	...
	ret = touch_i2c_read_block(chip_info->client,
				   SEC_CMD_TOUCH_SELFDATA_READ, read_len,
				   self_data);
	if (ret < 0) {
	...
	for (i = 0; i < chip_info->hw_res->TX_NUM + chip_info->hw_res->RX_NUM;
	     i++) {
		tmp = self_data[2 * i];
...
static int sec_execute_selftest(struct seq_file *s, int fd,
				struct chip_data_s6sy771 *chip_info,
				struct sec_testdata *sec_testdata)
{
	...
	u8 *rBuff = NULL;
	int i;
	int result_size = SEC_SELFTEST_REPORT_SIZE +
	...
	//send self test cmd
	rc = touch_i2c_write_block(chip_info->client, SEC_CMD_SELFTEST, 2,
				   tpara);
	if (rc < 0) {
	...
	rc = touch_i2c_read_block(chip_info->client, SEC_READ_SELFTEST_RESULT,
				  result_size, rBuff);
	if (rc < 0) {
	...
	for (i = 0; i < 80; i += 4) {
		if (i / 4 == 0)
			store_to_file(fd, "SIG: ");
		else if (i / 4 == 1)
			store_to_file(fd, "VER: ");
		else if (i / 4 == 2)
			store_to_file(fd, "SIZ: ");
		else if (i / 4 == 3)
			store_to_file(fd, "CRC: ");
		else if (i / 4 == 4)
			store_to_file(fd, "RES: ");
		else if (i / 4 == 5)
			store_to_file(fd, "COU: ");
		else if (i / 4 == 6)
			store_to_file(fd, "PAS: ");
		else if (i / 4 == 7)
			store_to_file(fd, "FAI: ");
		else if (i / 4 == 8)
			store_to_file(fd, "CHA: ");
		else if (i / 4 == 9)
			store_to_file(fd, "AMB: ");
		else if (i / 4 == 10)
			store_to_file(fd, "RXS: ");
		else if (i / 4 == 11)
			store_to_file(fd, "TXS: ");
		else if (i / 4 == 12)
			store_to_file(fd, "RXO: ");
		else if (i / 4 == 13)
			store_to_file(fd, "TXO: ");
		else if (i / 4 == 14)
			store_to_file(fd, "RXG: ");
		else if (i / 4 == 15)
			store_to_file(fd, "TXG: ");
		else if (i / 4 == 16)
			store_to_file(fd, "RXR: ");
		else if (i / 4 == 17)
			store_to_file(fd, "TXT: ");
		else if (i / 4 == 18)
			store_to_file(fd, "RXT: ");
		else if (i / 4 == 19)
			store_to_file(fd, "TXR: ");

		store_to_file(fd, "0x%02X, 0x%02X, 0x%02X, 0x%02X \n", rBuff[i],
			      rBuff[i + 1], rBuff[i + 2], rBuff[i + 3]);
		...
		if (i / 4 == 4) {
			...
			/* TX, RX GND(VDD) short check. */
				...
				seq_printf(
					s,
					"TX, RX GND(VDD) short check failed\n");
			}
			/* RX-RX, TX-TX short check. */
			else if ((rBuff[i + 2] & 0x03) != 0) {
				rc = 0;
				seq_printf(s,
					   "RX-RX, TX-TX short check failed\n");
			}
			/* TX-RX short check. */
...
static int sec_execute_p2ptest(struct seq_file *s,
			       struct chip_data_s6sy771 *chip_info,
			       struct sec_testdata *sec_testdata)
{
	int rc;
	...
	rc = touch_i2c_write_block(chip_info->client, SEC_CMD_SET_P2PTEST_MODE,
				   2, tpara);
	if (rc < 0) {
	...
	tpara[0] = 0x00;
	tpara[1] = 0x64;
	rc = touch_i2c_write_block(chip_info->client, SEC_CMD_P2PTEST, 2,
				   tpara);
	if (rc < 0) {
...
static uint32_t search_for_item(const struct firmware *fw, int item_cnt,
				uint8_t item_index)
{
	int i = 0;
	...
	for (i = 0; i < item_cnt; i++) {
		item_header = (struct sec_test_item_header *)(fw->data +
							      p_item_offset[i]);
		if (item_header->item_bit ==
		    item_index) { //check the matched item offset
...
static void sec_auto_test(struct seq_file *s, void *chip_data,
			  struct sec_testdata *sec_testdata)
{
	...
	int i = 0, j = 0, item_cnt = 0, iArrayIndex = 0, iArrayIndex_2 = 0,
	    ret = -1;
	struct chip_data_s6sy771 *chip_info =
		(struct chip_data_s6sy771 *)chip_data;
	...
	const uint32_t readselfbytes =
		(sec_testdata->TX_NUM + sec_testdata->RX_NUM) * 2;
	struct sec_test_item_header *item_header = NULL;
	int32_t *p_mutual_p = NULL, *p_mutual_n = NULL, *p_mutualGap_p = NULL,
		*p_mutualGap_n = NULL;
	int32_t *p_tx_offset_p = NULL, *p_tx_offset_n = NULL,
		*p_rx_offset_p = NULL, *p_rx_offset_n = NULL;
	int32_t *p_p2p_p = NULL, *p_p2p_n = NULL;

	//interrupt pin short test
	read_gpio_num = 10;
	touch_i2c_write_byte(chip_info->client, SEC_CMD_INTERRUPT_SWITCH,
			     1); //disable interrupt
	touch_i2c_write_block(chip_info->client, SEC_CMD_CLEAR_EVENT_STACK, 0,
			      NULL); //clear event buffer
	while (read_gpio_num--) {
		sec_mdelay(5);
	...
	ret = touch_i2c_read_block(chip_info->client,
				   SEC_VENDOR_INFO_CALIBRATION_STATUS, 1,
				   &cal_status);
	if (ret < 0) {
		TPD_INFO("%s : read calibration status failed\n", __func__);
	}

	TPD_INFO("%s: oplus customized calibration status : 0x%02x\n", __func__,
		 cal_status);

	if ((cal_status == 0 && ret >= 0) ||
	    (cal_status == 0xFF && chip_info->auto_test_need_cal_support)) {
		err_cnt++;
		TPD_INFO(
			"error : Not calibration! Please do the calibration first.\n");
		seq_printf(
			s,
			"Not calibration! Please do the calibration first.\n");
		goto ERR_OUT;
	}

	/* execute selftest */
	ret = sec_execute_selftest(s, sec_testdata->fd, chip_info,
				   sec_testdata);
	if (ret <= 0) {
		err_cnt++;
	...
	if (sec_testdata->test_item & (1 << TYPE_MUTUAL_RAW_OFFSET_DATA_SDC)) {
		...
		item_offset = search_for_item(sec_testdata->fw, item_cnt,
					      TYPE_MUTUAL_RAW_OFFSET_DATA_SDC);
		if (item_offset == 0) {
			err_cnt++;
		...
		item_header =
			(struct sec_test_item_header *)(sec_testdata->fw->data +
							item_offset);
		if (item_header->item_magic != Limit_ItemMagic &&
		    item_header->item_magic != Limit_ItemMagic_V2) {
			err_cnt++;
			TPD_INFO("test item: %d magic number(%4x) is wrong\n",
				 TYPE_MUTUAL_RAW_OFFSET_DATA_SDC,
				 item_header->item_magic);
			seq_printf(s,
				   "test item: %d magic number(%4x) is wrong\n",
				   TYPE_MUTUAL_RAW_OFFSET_DATA_SDC,
				   item_header->item_magic);
			goto ERR_OUT;
		}
		if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
			p_mutual_p = (int32_t *)(sec_testdata->fw->data +
						 item_header->top_limit_offset);
			p_mutualGap_p =
				(int32_t *)(sec_testdata->fw->data +
					    item_header->top_limit_offset +
					    4 * sec_testdata->TX_NUM *
						    sec_testdata->RX_NUM);
			p_mutual_n =
				(int32_t *)(sec_testdata->fw->data +
		...
		/* read reference data */
		ret = touch_i2c_read_block(chip_info->client,
					   SEC_READ_TOUCH_RAWDATA, readbytes,
					   pRead);
		if (ret < 0) {
			err_cnt++;
		...
		for (i = 0; i < sec_testdata->TX_NUM; i++) {
			for (j = 0; j < sec_testdata->RX_NUM; j++) {
				...
				if (sec_testdata->fd >= 0) {
					store_to_file(sec_testdata->fd, "%d, ",
						      nodeData);
				}
				if ((nodeData < p_mutual_n[iArrayIndex]) ||
				    (nodeData > p_mutual_p[iArrayIndex])) {
					TPD_INFO(
						" mutual offset_data failed at data[%d][%d] = %d [%d,%d]\n",
						i, j, nodeData,
						p_mutual_n[iArrayIndex],
						p_mutual_p[iArrayIndex]);
					if (!err_cnt) {
						seq_printf(
							s,
							"mutual offset_data failed at data[%d][%d] = %d [%d,%d]\n",
		...
		for (i = 0; i < sec_testdata->TX_NUM - 1; i++) {
			for (j = 0; j < sec_testdata->RX_NUM; j++) {
				iArrayIndex = i * sec_testdata->RX_NUM + j;
				iArrayIndex_2 =
					(i + 1) * sec_testdata->RX_NUM + j;
				Buff16 = (pRead[iArrayIndex * 2] << 8) |
					 pRead[iArrayIndex * 2 + 1];
				Buff16_2 = (pRead[iArrayIndex_2 * 2] << 8) |
					   pRead[iArrayIndex_2 * 2 + 1];
				if (Buff16 > Buff16_2) {
					nodeGap =
						100 - (Buff16_2 * 100 / Buff16);
				} else {
					nodeGap =
						100 - (Buff16 * 100 / Buff16_2);
				}
				if (sec_testdata->fd >= 0) {
					store_to_file(sec_testdata->fd, "%d, ",
						      nodeGap);
				}
				if ((nodeGap > p_mutualGap_p[iArrayIndex]) ||
				    (nodeGap < p_mutualGap_n[iArrayIndex])) {
					TPD_INFO(
						"mutual node[%d, %d]=%d and node[%d, %d]=%d gap beyond [%d, %d]\n",
						i, j, Buff16, i + 1, j,
						Buff16_2,
						p_mutualGap_n[iArrayIndex],
						p_mutualGap_p[iArrayIndex]);
					if (!err_cnt) {
						seq_printf(
							s,
							"mutual node[%d, %d]=%d and node[%d, %d]=%d gap beyond [%d, %d]\n",
	...
	if (sec_testdata->test_item & (1 << TYPE_SELF_RAW_OFFSET_DATA_SDC)) {
		...
		if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
			...
			p_rx_offset_p =
				(int32_t *)(sec_testdata->fw->data +
					    item_header->top_limit_offset +
					    2 * 4 * sec_testdata->TX_NUM);
			p_rx_offset_n =
				(int32_t *)(sec_testdata->fw->data +
					    item_header->top_limit_offset +
					    2 * 4 * sec_testdata->TX_NUM +
					    4 * sec_testdata->RX_NUM);
		} else {
			err_cnt++;
			TPD_INFO("item: %d has invalid limit type(%d)\n",
				 TYPE_SELF_RAW_OFFSET_DATA_SDC,
		...
		for (i = 0; i < sec_testdata->TX_NUM; i++) {
			...
			if ((nodeData < p_tx_offset_n[i]) ||
			    (nodeData > p_tx_offset_p[i])) {
				TPD_INFO(
					"self_offset_tx_data failed at data[%d] = %d [%d,%d]\n",
					i, nodeData, p_tx_offset_n[i],
					p_tx_offset_p[i]);
				if (!err_cnt) {
					seq_printf(
						s,
		...
		for (i = 0; i < sec_testdata->RX_NUM; i++) {
			nodeData =
				(pRead[sec_testdata->TX_NUM * 2 + i * 2] << 8) |
				pRead[sec_testdata->TX_NUM * 2 + i * 2 + 1];
			if (sec_testdata->fd >= 0) {
				store_to_file(sec_testdata->fd, "%d, ",
					      nodeData);
			}
			if ((nodeData < p_rx_offset_n[i]) ||
			    (nodeData > p_rx_offset_p[i])) {
				TPD_INFO(
					"self_offset_rx_data failed at data[%d] = %d [%d,%d]\n",
					i, nodeData, p_rx_offset_n[i],
					p_rx_offset_p[i]);
				if (!err_cnt) {
					seq_printf(
						s,
	...
	if (sec_testdata->test_item & (1 << TYPE_MUTU_RAW_NOI_P2P)) {
		...
		if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
			p_p2p_p = (int32_t *)(sec_testdata->fw->data +
					      item_header->top_limit_offset);
			p_p2p_n = (int32_t *)(sec_testdata->fw->data +
					      item_header->floor_limit_offset);
		} else {
			err_cnt++;
			TPD_INFO("item: %d has invalid limit type(%d)\n",
		...
		for (i = 0; i < sec_testdata->TX_NUM; i++) {
			for (j = 0; j < sec_testdata->RX_NUM; j++) {
				...
				if ((nodeData < p_p2p_n[iArrayIndex])) {
					TPD_INFO(
						"p2p_min_data failed at data[%d][%d] = %d [%d]\n",
						i, j, nodeData,
						p_p2p_n[iArrayIndex]);
					if (!err_cnt) {
						seq_printf(
		...
		for (i = 0; i < sec_testdata->TX_NUM; i++) {
			for (j = 0; j < sec_testdata->RX_NUM; j++) {
				...
				if ((nodeData > p_p2p_p[iArrayIndex])) {
					TPD_INFO(
						"p2p_max_data failed at data[%d][%d] = %d [%d]\n",
						i, j, nodeData,
						p_p2p_p[iArrayIndex]);
					if (!err_cnt) {
						seq_printf(
...
static int sec_get_verify_result(struct chip_data_s6sy771 *chip_info)
{
	...
	sec_fix_tmode(chip_info, TOUCH_SYSTEM_MODE_TOUCH,
		      TOUCH_MODE_STATE_TOUCH);
	touch_i2c_write_block(chip_info->client, 0xA7, 0,
			      NULL); //set to verify calibration
	sec_mdelay(100);
...
static void sec_write_calibration_status(void *chip_data)
{
	...
	if (cal_status == 0xFF) { //default value is 0xFF
		cal_status = 0;
	}

	if (cal_status <= 0xFA) { //max valus is 0xFA(250)
		cal_status++;
	}
	...
	ret = touch_i2c_write_byte(chip_info->client,
				   ...
				   cal_status);
	if (ret < 0) {
		TPD_INFO("%s : write calibration status failed\n", __func__);
	}
...
static void sec_verify_calibration(struct seq_file *s, void *chip_data)
{
	...
	if (ret != 0) {
		seq_printf(
			s,
			"1 error, verify calibration result failed(0x%02x)\n",
			ret);
	} else {
...
{
	struct chip_data_s6sy771 *chip_info;
	chip_info = (struct chip_data_s6sy771 *)chip_data;
	...
	for (i = 0; i < 3; i++) {
		...
		for (j = 0; j < num; j++) {
			if (chip_info->gesture_buf[j * 4] == i) {
				...
				x = (chip_info->gesture_buf[j * 4 + 1] << 4) |
				    (chip_info->gesture_buf[j * 4 + 3] >> 4);

				y = (chip_info->gesture_buf[j * 4 + 2] << 4) |
				    (chip_info->gesture_buf[j * 4 + 3] & 0x0F);
...
static int sec_apk_tp_info_get(void *chip_data, char *buf, int len)
{
	...
	ret = snprintf(buf, len, "IC:S6SY%03X\nFW_VER:0x%02X\nCH:%dX%d\n",
		       0x771, fw_ver, chip_info->hw_res->TX_NUM,
...
{
	struct chip_data_s6sy771 *chip_info = NULL;
	...
	/* 1. alloc chip info */
	chip_info = kzalloc(sizeof(struct chip_data_s6sy771), GFP_KERNEL);
	if (chip_info == NULL) {
	...
	chip_info->display_refresh_rate = 90;
	...
	chip_info->auto_test_need_cal_support = of_property_read_bool(
		ts->dev->of_node, "auto_test_need_cal_support");
...
static int sec_i2c_suspend(struct device *dev)
{
	...
	TPD_INFO("%s: is called, irq status : %d .\n", __func__,
