...
#ifdef ANC_CONFIG_PM_WAKELOCKS
	__pm_wakeup_event(&g_anc_data->fp_wakelock,
			  msecs_to_jiffies(ANC_WAKELOCK_HOLD_TIME));
#else
	wake_lock_timeout(&g_anc_data->fp_wakelock,
			  msecs_to_jiffies(ANC_WAKELOCK_HOLD_TIME));
#endif
	netlink_msg.area_rate = tp_info->area_rate;
	netlink_msg.x = tp_info->x;
	netlink_msg.y = tp_info->y;
	pr_info("[anc] Netlink touch info area:%d,x:%d,y:%d",
		netlink_msg.area_rate, netlink_msg.x, netlink_msg.y);

	if (1 == tp_info->touch_state) {
		netlink_msg.event = (char)ANC_NETLINK_EVENT_TOUCH_DOWN;
		pr_info("[anc] Netlink touch down!");
		netlink_send_message_to_user((const char *)(&netlink_msg),
					     sizeof(netlink_msg));
				     ...
				     unsigned long val, void *data)
{
	...
	if (val == MSM_DRM_ONSCREENFINGERPRINT_EVENT) {
		...
		case ANC_UI_READY:
			...
			netlink_send_message_to_user(&netlink_msg,
						     sizeof(netlink_msg));
			break;
		default:
...
static int select_pin_ctl(struct anc_data *data, const char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(data->pinctrl_state); i++) {
		...
		if (!strncmp(n, name, strlen(n))) {
			rc = pinctrl_select_state(data->fingerprint_pinctrl,
						  data->pinctrl_state[i]);
			if (rc)
 ...
 */
static ssize_t device_power_set(struct device *dev,
 ...
 */
static ssize_t sensor_id_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct anc_data *data = dev_get_drvdata(dev);
 ...
 */
static ssize_t irq_control_set(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	ssize_t rc = count;
...
static irqreturn_t anc_irq_handler(int irq, void *handle)
{
	...
	pr_info("irq handler\n");
#ifdef ANC_CONFIG_PM_WAKELOCKS
	__pm_wakeup_event(&data->fp_wakelock,
			  msecs_to_jiffies(ANC_WAKELOCK_HOLD_TIME));
#else
	wake_lock_timeout(&data->fp_wakelock,
			  msecs_to_jiffies(ANC_WAKELOCK_HOLD_TIME));
#endif
...
static int anc_request_named_gpio(struct anc_data *data, const char *label,
				  int *gpio)
{
	struct device *dev = data->dev;
...
static int anc_gpio_init(struct device *dev, struct anc_data *data)
{
	...
	if (anc_gpio_pwr_flag == 1) {
		rc = anc_request_named_gpio(data, "anc,gpio_pwr",
					    &data->pwr_gpio);
		if (rc)
			goto exit;
	...
	for (i = 0; i < ARRAY_SIZE(data->pinctrl_state); i++) {
		...
		struct pinctrl_state *state =
			pinctrl_lookup_state(data->fingerprint_pinctrl, n);
		if (IS_ERR(state)) {
...
{
	int rc = 0;
	int irqf = IRQF_TRIGGER_FALLING |
		   IRQF_ONESHOT; // IRQF_TRIGGER_FALLING or IRQF_TRIGGER_RISING
	...
	rc = devm_request_threaded_irq(dev, data->irq, NULL, anc_irq_handler,
				       irqf, dev_name(dev), data);
	if (rc) {
...
#ifdef CONFIG_COMPAT
static long anc_compat_ioctl(struct file *filp, unsigned int cmd,
			     unsigned long arg)
{
...
static ssize_t anc_read(struct file *filp, char __user *buf, size_t count,
			loff_t *f_pos)
{
	ssize_t status = 0;
	// struct anc_data *dev_data = filp->private_data;
...
static ssize_t anc_write(struct file *filp, const char __user *buf,
...
static uint32_t anc_read_sensor_id(struct anc_data *data)
{
	...
	do {
		...
		sensor_chip_id = (uint32_t)((spi_buffer[3] & 0x00FF) |
					    ((spi_buffer[2] << 8) & 0xFF00));
...
static int anc_probe(anc_device_t *pdev)
{
	...
	if (anc_major_num) {
		...
		rc = register_chrdev_region(dev_data->dev_num, 1,
					    ANC_DEVICE_NAME);
	} else {
		rc = alloc_chrdev_region(&dev_data->dev_num, 0, 1,
					 ANC_DEVICE_NAME);
		if (rc < 0) {
			dev_err(dev,
	...
	device_ptr = device_create(dev_data->dev_class, NULL, dev_data->dev_num,
				   dev_data, ANC_DEVICE_NAME);
...
#else
	wake_lock_init(&dev_data->fp_wakelock, WAKE_LOCK_SUSPEND,
		       "anc_fp_wakelock");
#endif
...
static int __init ancfp_init(void)
{
	...
	if ((FP_JIIOV_0302 != get_fpsensor_type()) &&
	    (FP_JIIOV_0301 != get_fpsensor_type())) {
