...
static int mdss_mipi_dsi_command(void __user *values)
{
	...
	if (cmd.length < SZ_4K && cmd.payload) { //CID799200 CID89803
	...
	if (cmd.flags & MSMFB_MIPI_DSI_COMMAND_TO_PANEL) {
		if (iris5_abypass_mode_get(pcfg->panel) == PASS_THROUGH_MODE)
			iris5_panel_cmd_passthrough(pcfg->panel, &cmdset);
		else
			iris5_dsi_cmds_send(pcfg->panel, cmdset.cmds, cmdset.count, cmdset.state);
...
int iris_configure(u32 display, u32 type, u32 value)
{
	...
	int i;
	bool no_need_lock = false;
	static u32 no_need_lock_types[] = {
		IRIS_DEMO_MODE,
		IRIS_HDR_MAXCLL,
		IRIS_DEBUG_CAP,
		IRIS_VIDEO_FRAME_RATE_SET,
		IRIS_OUT_FRAME_RATE_SET,
		IRIS_OSD_AUTOREFRESH,
		IRIS_DBG_KERNEL_LOG_LEVEL,
		IRIS_FRC_LOW_LATENCY,
		IRIS_N2M_ENABLE,
		IRIS_WAIT_VSYNC,
		IRIS_MEMC_LEVEL,
		IRIS_MEMC_OSD,
	...
	IRIS_LOGI("%s type=0x%04x, value=%d, mode = %d", __func__, type, value, pcfg->abypss_ctrl.abypass_mode);
	...
	*/
	...
	if (type != IRIS_DBG_KERNEL_LOG_LEVEL && pcfg->chip_ver == IRIS3_CHIP_VERSION)
		return -1;

	for(i=0; i<ARRAY_SIZE(no_need_lock_types); i++) {
		if (type == no_need_lock_types[i]) {
			no_need_lock = true;
			IRIS_LOGI("no need lock for type: %d", type);
			break;
		}
	}
	if (!no_need_lock) {
		if (!mutex_trylock(&pcfg1->lock_send_pkt)) {
			IRIS_LOGE("%s:%d lock_send_pkt is locked!", __func__, __LINE__);
			mutex_lock(&pcfg1->lock_send_pkt);
		}
	}
	...
	case IRIS_OSD_ENABLE:
		...
		if ((value == 1) || (value == 0)) {
			...
			iris_second_channel_power(false);
		} else if (value == 0x40) {
			iris_dom_set(0);
		} else if (value == 0x41) {
			iris_dom_set(2);
	...
	case IRIS_N2M_ENABLE:
		...
		break;
	case IRIS_WAIT_VSYNC:
		return iris_wait_vsync(); //CID89779
	case IRIS_MEMC_LEVEL:
		if (value <= 3) { //CID89734
	...
	if (!no_need_lock)
		mutex_unlock(&pcfg1->lock_send_pkt);
	return 0;
	...
	switch (type) {
		case IRIS_LUX_VALUE:
			...
			if (iris_ambient.lut_lut2_payload != NULL) {
				...
				{
					LutPos = 0;
					iris_ambient_lut_update(AMBINET_SDR2HDR_LUT,1);
				}
				else if(LutPos == 1)
					LutPos = 0;
				else if(!LutPos)
					LutPos = 1;
				iris_ambient_lut_update(AMBINET_SDR2HDR_LUT,LutPos);
			}
		...
		case IRIS_HDR_MAXCLL:
			...
			if(!LutPos)
				...
				LutPos = 0;
			iris_maxcll_lut_update(AMBINET_HDR_GAIN,LutPos);
		...
		case IRIS_HUE_SAT_ADJ:
			...
			if (values[5] == 1) {
				for(i=0;i<=4;i++) {
					if (pcfg->iris5_i2c_write) {
						if (pcfg->iris5_i2c_write(CM_CNTL[10] +i*4, values[i]) < 0) {
		...
		case IRIS_CONTRAST_DIMMING:
			IRIS_LOGI("dpp csc value: csc0 = 0x%x, csc1 = 0x%x, csc2 = 0x%x, csc3 = 0x%x, csc4 = 0x%x", values[0], values[1], values[2], values[3], values[4]);
		...
		case IRIS_COLOR_TEMP_VALUE:
			...
			if (is_phone) {
				...
				if (pcfg->iris5_i2c_write) {
					if (pcfg->iris5_i2c_write(CM_CNTL[4], TempValue) < 0) {
		...
		case IRIS_DBG_TARGET_REGADDR_VALUE_SET:
			if (0 == adb_type) {
				...
				if (iris_i2c_ver == 0) {
					if (pcfg->iris5_i2c_write) {
						if (pcfg->iris5_i2c_write(values[0], values[1]) < 0) {
		...
		case IRIS_CM_6AXES:
			...
			// game station
			if (pcfg->iris5_i2c_write) {
				if (pcfg->iris5_i2c_write(CM_CNTL[0], values[1] ? CM_CNTL[3] : CM_CNTL[1]) < 0) {
					IRIS_LOGE("i2c set reg fails, reg=0x%x", CM_CNTL[0]);
				} else if (pcfg->iris5_i2c_write(CM_CNTL[12], CM_CNTL[13]) < 0) {
		...
		case IRIS_CM_COLOR_TEMP_MODE:
			...
			// game station
			if (pcfg->iris5_i2c_write) {
				if (pcfg->iris5_i2c_write(CM_CNTL[6], values[1] ? CM_CNTL[9] : CM_CNTL[7]) < 0) {
...
int iris_configure_get(u32 display, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_CHIP_VERSION:
		if (pcfg->chip_ver == IRIS5_CHIP_VERSION)
			*values = IRIS5_VER;
		else
			*values = IRIS3LITE_VER;
		break;
	...
	case IRIS_DBG_TARGET_REGADDR_VALUE_GET:
		...
		if ((ANALOG_BYPASS_MODE == pcfg->abypss_ctrl.abypass_mode) && (0 == adb_type)) {
			return -ENOTCONN;
		}
		if (0 == adb_type) {
			if (!mutex_trylock(&pcfg1->lock_send_pkt)) {
			...
			*values = iris_ocp_read(*values, DSI_CMD_SET_STATE_HS);
			...
			reg_addr = *values;
			if (iris_i2c_ver == 0) {
				if (pcfg->iris5_i2c_read) {
					if (pcfg->iris5_i2c_read(reg_addr, &reg_val) < 0) {
	...
	case IRIS_OSD_AUTOREFRESH:
		...
		*values = pcfg->panel->reset_config.iris_osd_autorefresh ? 1 : 0;
		break;
	case IRIS_OSD_OVERFLOW_ST:
		// Always use secondary display.
