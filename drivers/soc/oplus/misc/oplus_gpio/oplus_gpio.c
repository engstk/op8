...
#define OPLUS_GPIO_ERR(a, arg...)                                          \
	do {                                                               \
		printk(KERN_NOTICE log_fmt(a), __LINE__, GPIO_DEVICE_NAME, \
		       __func__, ##arg);                                   \
	} while (0)

#define OPLUS_GPIO_MSG(a, arg...)                                        \
	do {                                                             \
		printk(KERN_INFO log_fmt(a), __LINE__, GPIO_DEVICE_NAME, \
		       __func__, ##arg);                                 \
	} while (0)
	...
	if (oplus_gpio_info_table[GPIO_TYPE_ESIM].gpio >= 0) {
		...
		if (esim_status == 1) {
			...
			gpio_direction_output(
				oplus_gpio_info_table[GPIO_TYPE_ESIM].gpio, 0);
...
static void dual_sim_det_uim2_to_esim(struct work_struct *work)
{
	if (oplus_gpio_info_table[GPIO_TYPE_ESIM].gpio >= 0) {
		...
		gpio_direction_output(
			oplus_gpio_info_table[GPIO_TYPE_ESIM].gpio, 1);
...
static int dual_sim_det_show(struct seq_file *m, void *v)
{
	...
	if (det_info) {
		...
		if (esim_status == 1) {
			...
			INIT_DELAYED_WORK(&recover_work,
					  dual_sim_det_uim2_to_esim);
			schedule_delayed_work(&recover_work,
					      msecs_to_jiffies(4100));
		}
...
static int dual_sim_det_init(struct platform_device *pdev, void *gpio_info_ptr)
{
	...
	struct oplus_gpio_info *gpio_info =
		(struct oplus_gpio_info *)gpio_info_ptr;
	...
	det_info->pull_high_state =
		pinctrl_lookup_state(det_info->pinctrl, "sim2_det_pull_high");
	...
	det_info->pull_low_state =
		pinctrl_lookup_state(det_info->pinctrl, "sim2_det_pull_low");
	...
	det_info->no_pull_state =
		pinctrl_lookup_state(det_info->pinctrl, "sim2_det_no_pull");
	...
	det_info->reset_pin =
		of_get_named_gpio(pdev->dev.of_node, "oplus,uim-reset-pin", 0);
			     ...
			     unsigned long data)
{
	...
	OPLUS_GPIO_MSG("dev = %s cmd = %d\n",
		       gpio_info->dev_node_desc ? gpio_info->dev_node_desc :
						  "NULL",
			       ...
			       loff_t *ppos)
{
	...
	len = snprintf(buff, len,
		       ...
		       gpio_info->gpio, gpio_info->gpio_mode,
...
static int oplus_gpio_probe(struct platform_device *pdev)
{
	...
	status = alloc_chrdev_region(&gpio_data->devt, 0, MAX_GPIOS,
				     GPIO_DEVICE_NAME);
	...
	gpio_data->oplus_gpio_class =
		class_create(THIS_MODULE, GPIO_DEVICE_NAME);
	...
	for (i = 0; i < MAX_GPIOS; i++) {
		...
		if (gpio_data->gpio_info[i].dev_node_desc) {
			if (gpio_data->gpio_info[i].is_proc &&
			    gpio_data->gpio_info[i].proc_init) {
				if (0 !=
				    gpio_data->gpio_info[i].proc_init(
					    pdev, &(gpio_data->gpio_info[i]))) {
			...
			oplus_gpio_device = device_create(
				gpio_data->oplus_gpio_class, NULL, devt, NULL,
				gpio_data->gpio_info[i].dev_node_desc);
		...
		if (gpio_data->gpio_info[i].gpio_mode == 1) {
			...
			gpio_data->gpio_info[i].gpio_status =
				gpio_get_value(gpio_data->gpio_info[i].gpio);
		}
	}
