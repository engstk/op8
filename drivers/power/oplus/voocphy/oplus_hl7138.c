...
static int hl7138_update_bits(struct i2c_client *client, u8 reg, u8 mask, u8 data)
{
	int ret;
	u8 tmp;
...
static void hl7138_update_data(struct oplus_voocphy_manager *chip)
{
	 ...
	 */
	ret = i2c_smbus_read_i2c_block_data(chip->client, HL7138_REG_42, HL7138_REG_NUM_12,
					    data_block); /* JL:first Reg is 13,need to change to 42; */
	...
	chip->cp_vbus = ((data_block[HL7138_REG_ADC_H_BIT_VBUS] << HL7138_REG_ADC_BIT_OFFSET_4) |
			 data_block[HL7138_REG_ADC_L_BIT_VBUS]) *
			HL7138_FACTOR_400_100; /* vbus_lsb=4mV; */
	chip->cp_vsys = ((data_block[HL7138_REG_ADC_H_BIT_VSYS] << HL7138_REG_ADC_BIT_OFFSET_4) |
			 data_block[HL7138_REG_ADC_L_BIT_VSYS]) *
			HL7138_FACTOR_125_100; /* vout_lsb=1.25mV; */
	chip->cp_vbat = ((data_block[HL7138_REG_ADC_H_BIT_VBAT] << HL7138_REG_ADC_BIT_OFFSET_4) |
			 data_block[HL7138_REG_ADC_L_BIT_VBAT]) *
			HL7138_FACTOR_125_100; /* vout_lsb=1.25mV; */
	chip->cp_vac = chip->cp_vbus;
	chg_debug("cp_ichg = %d cp_vbus = %d, cp_vsys = %d, cp_vbat = %d, int_flag = %d", chip->cp_ichg, chip->cp_vbus,
	...
	if (enable) {
		if (oplus_chg_check_pd_svooc_adapater()) {
			hl7138_write_byte(chip->client, HL7138_REG_13,
					  HL7138_IBUS_UCP_DEB_100ms
						  << HL7138_IBUS_UCP_DEB_SHIFT); /* UCP debounce time 100ms */
			return hl7138_write_byte(chip->client, HL7138_REG_12,
						 HL7138_CHG_EN | HL7138_IBUS_UCP_EN |
							 HL7138_IBUS_UCP_DEFAULT); /* is pdsvooc adapter: disable ucp */
		} else {
			return hl7138_write_byte(
				chip->client, HL7138_REG_12,
				HL7138_CHG_EN | HL7138_IBUS_UCP_EN |
...
static int hl7138_get_cp_ichg(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->adapter_type == ADAPTER_SVOOC) {
		chip->cp_ichg = ((data_block[0] << HL7138_REG_ADC_BIT_OFFSET_4) | data_block[1]) *
				HL7138_SVOOC_IBUS_FACTOR; /* Iin_lbs=1.10mA@CP; */
	} else {
		chip->cp_ichg = ((data_block[0] << HL7138_REG_ADC_BIT_OFFSET_4) | data_block[1]) *
				HL7138_VOOC_IBUS_FACTOR; /* Iin_lbs=2.15mA@BP; */
...
static void hl7138_set_pd_svooc_config(struct oplus_voocphy_manager *chip, bool enable)
{
	...
	if (enable)
		...
		hl7138_write_byte(chip->client, HL7138_REG_13, HL7138_IBUS_UCP_DEB_10ms << HL7138_IBUS_UCP_DEB_SHIFT);
...
static int hl7138_set_adc_enable(struct oplus_voocphy_manager *chip, bool enable)
{
	...
	if (enable)
		return hl7138_update_bits(chip->client, HL7138_REG_40, HL7138_ADC_EN_MASK, HL7138_ADC_ENABLE);
	else
		return hl7138_update_bits(chip->client, HL7138_REG_40, HL7138_ADC_EN_MASK, HL7138_ADC_DISABLE);
}
...
static int hl7138_set_adc_forcedly_enable(struct oplus_voocphy_manager *chip, int mode)
{
	...
	switch (mode) {
	case ADC_AUTO_MODE:
		ret = hl7138_update_bits(chip->client, HL7138_REG_40, HL7138_ADC_FORCEDLY_EN_MASK,
					 HL7138_ADC_AUTO_MODE);
		break;
	case ADC_FORCEDLY_ENABLED:
		ret = hl7138_update_bits(chip->client, HL7138_REG_40, HL7138_ADC_FORCEDLY_EN_MASK,
					 HL7138_ADC_FORCEDLY_ENABLED);
		break;
	case ADC_FORCEDLY_DISABLED_10:
		ret = hl7138_update_bits(chip->client, HL7138_REG_40, HL7138_ADC_FORCEDLY_EN_MASK,
					 HL7138_ADC_FORCEDLY_DISABLED_10);
		break;
	case ADC_FORCEDLY_DISABLED_11:
		ret = hl7138_update_bits(chip->client, HL7138_REG_40, HL7138_ADC_FORCEDLY_EN_MASK,
					 HL7138_ADC_FORCEDLY_DISABLED_11);
		break;
	default:
...
static int hl7138_irq_register(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->irq) {
		ret = request_threaded_irq(chip->irq, NULL, hl7138_charger_interrupt,
					   IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "hl7138_charger_irq", chip);
		if (ret < 0) {
...
static ssize_t hl7138_show_registers(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t hl7138_store_register(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct oplus_voocphy_manager *chip = dev_get_drvdata(dev);
	int ret;
	unsigned int reg;
...
static void hl7138_set_switch_fast_charger(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->switch1_gpio <= 0) {
		chg_err("switch switch2 not exist, return\n");
		return;
	}
...
static int hl7138_get_chip_id(void)
{
...
static int hl7138_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
