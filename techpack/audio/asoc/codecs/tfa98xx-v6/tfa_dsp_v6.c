...
int tfa_get_tap_pattern(struct tfa_device *tfa)
{
	...
	int bitshift;
	uint8_t field_len =
		1 + (TFA9912_BF_CFTAPPAT & 0x0f); /* length of bitfield */
...
int tfa_irq_ena(struct tfa_device *tfa, enum tfa9912_irq bit, int state)
{
	...
	if (bit == tfa9912_irq_all) {
		...
		for (reg = TFA98XX_INTERRUPT_ENABLE_REG1;
		     reg <=
		     TFA98XX_INTERRUPT_ENABLE_REG1 + tfa9912_irq_max / 16;
		     reg++) {
			tfa_reg_write(tfa, (unsigned char)reg,
				      state ? 0xffff : 0); /* all bits */
			tfa->interrupt_enable[reg -
					      TFA98XX_INTERRUPT_ENABLE_REG1] =
				state ? 0xffff : 0; /* all bits */
		}
	} else if (bit < tfa9912_irq_max) {
		/* only this bit */
...
void tfa_set_query_info(struct tfa_device *tfa)
{
	...
	tfa->ext_dsp =
		-1; /* respond to external DSP: -1:none, 0:no_dsp, 1:cold, 2:warm */
	tfa->bus = 0;
	...
	case 0: /* tfanone : non-i2c external DSP device */
		...
		tfa->daimap = 0;
		tfanone_ops(
			&tfa->dev_ops); /* register device operations via tfa hal*/
		tfa->bus = 1;
		break;
	case 0x72:
		/* tfa9872 */
 ...
 */
enum Tfa98xx_DMEM tfa98xx_filter_mem_v6(struct tfa_device *tfa,
					int filter_index,
 ...
 */
enum Tfa98xx_Error tfa98xx_supported_saam_v6(struct tfa_device *tfa,
					     enum Tfa98xx_saam *saam)
{
	...
	if (tfa->support_saam == supportNotSet) {
		...
		if (error != Tfa98xx_Error_Ok)
			return error;
		tfa->support_saam = (features & 0x8000) ?
					    supportYes :
					    supportNo; /* SAAM is bit15 */
	}
	*saam = tfa->support_saam == supportYes ? Tfa98xx_saam :
						  Tfa98xx_saam_none;
 ...
 */
enum Tfa98xx_Error tfa98xx_compare_features_v6(struct tfa_device *tfa,
					       ...
					       int features_from_cnt[3])
{
	...
	tfa98xx_convert_bytes2data_v6(sizeof(bytes), bytes,
				      &features_from_MTP[1]);
...
enum Tfa98xx_Error tfa98xx_dsp_system_stable_v6(struct tfa_device *tfa,
						int *ready)
{
...
enum Tfa98xx_Error tfa98xx_dsp_write_tables_v6(struct tfa_device *tfa,
					       int sample_rate)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error tfa98xx_dsp_get_memory_v6(struct tfa_device *tfa,
...
enum Tfa98xx_Error tfa98xx_dsp_set_memory_v6(struct tfa_device *tfa,
					     int memoryType, int offset,
...
enum Tfa98xx_Error tfa98xx_set_mtp_v6(struct tfa_device *tfa, uint16_t value,
				      uint16_t mask)
{
...
int tfa_calibrate_v6(struct tfa_device *tfa)
{
	...
	/* clear mtpex */
	error = tfa98xx_set_mtp_v6(tfa, 0,
...
#ifdef OPLUS_ARCH_EXTENDS
enum Tfa98xx_Error tfa98xx_set_ana_volume_v6(struct tfa_device *tfa,
					     unsigned int vol)
{
	if (tfa->in_use == 0)
		return Tfa98xx_Error_NotOpen;
...
enum Tfa98xx_Error tfa98xx_get_ana_volume_v6(struct tfa_device *tfa,
					     unsigned int *vol)
{
	if (tfa->in_use == 0)
		return Tfa98xx_Error_NotOpen;
...
static enum Tfa98xx_Error tfa98xx_set_mute_v6_tfa2(struct tfa_device *tfa,
...
static enum Tfa98xx_Error tfa98xx_set_mute_v6_tfa1(struct tfa_device *tfa,
						   enum Tfa98xx_Mute mute)
{
	enum Tfa98xx_Error error;
...
/****************** patching **********************************************************/
static enum Tfa98xx_Error tfa98xx_process_patch_file(struct tfa_device *tfa,
						     int length,
...
enum Tfa98xx_Error tfa_dsp_patch_v6(struct tfa_device *tfa, int patchLength,
...
TFA_INTERNAL enum Tfa98xx_Error tfa98xx_wait_result_v6(struct tfa_device *tfa,
...
enum Tfa98xx_Error tfa_dsp_msg_v6_write_v6(struct tfa_device *tfa, int length,
					   const char *buffer)
{
	int offset = 0;
	...
	TFA_SET_BF_VALUE(tfa, DMEM, (uint16_t)Tfa98xx_DMEM_XMEM,
			 &cfctl); /* set cf ctl to DMEM  */
	...
	while ((error == Tfa98xx_Error_Ok) && (remaining_bytes > 0)) {
		...
		error = tfa98xx_write_data_v6(
			tfa, FAM_TFA98XX_CF_MEM, chunk_size,
...
enum Tfa98xx_Error tfa_dsp_msg_v6_write_v6_id_v6(struct tfa_device *tfa,
						 ...
						 uint8_t cmdid[3])
{
	...
	/* write cmd-id */
	error = tfa98xx_write_data_v6(tfa, FAM_TFA98XX_CF_MEM, 3,
...
*/
enum Tfa98xx_Error tfa_dsp_msg_v6_status(struct tfa_device *tfa,
					 int *pRpcStatus)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error tfa_dsp_msg_v6_read_v6(struct tfa_device *tfa, int length,
					  unsigned char *bytes)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	...
	while (num_bytes > 0) {
		...
		error = tfa98xx_read_data_v6(tfa, FAM_TFA98XX_CF_MEM,
			   ...
			   const char *buf24)
{
	...
	if (tfa->convert_dsp32) {
		...
		for (i = 0; i < length24; i += 3) {
			int tmp = (buf24[i] << 16) + (buf24[i + 1] << 8) +
				  buf24[i + 2];
		...
		if (error == Tfa98xx_Error_Buffer_too_small) {
			...
			if (tfa->has_msg == 0) /* via i2c */ {
				...
				error = (tfa->dev_ops.dsp_msg)(
					tfa, len, (const char *)blob);
			} else { /* via msg hal */
				error = tfa98xx_write_dsp(tfa, len,
							  (const char *)blob);
			}
			kfree(blob);
	...
	if (error != Tfa98xx_Error_Ok)
		error = (enum Tfa98xx_Error)(
			error +
			Tfa98xx_Error_RpcBase); /* Get actual error code from softDSP */
...
enum Tfa98xx_Error dsp_msg_read(struct tfa_device *tfa, int length24,
				unsigned char *bytes24)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	int i;
	int length = length24;
...
enum Tfa98xx_Error tfa_dsp_msg_v6(struct tfa_device *tfa, int length,
				  const char *buf)
{
	enum Tfa98xx_Error error;
	...
	if (rpc_status != Tfa98xx_I2C_Req_Done) {
		...
		error = (enum Tfa98xx_Error)(rpc_status +
					     Tfa98xx_Error_RpcBase);
		pr_debug("DSP msg status: %d (%s)\n", rpc_status,
			 tfa98xx_get_i2c_status_id_string_v6(rpc_status));
	}
	return error;
}
 ...
 */
enum Tfa98xx_Error tfa_dsp_msg_v6_id_v6(struct tfa_device *tfa, int length,
...
tfa98xx_check_rpc_status_v6(struct tfa_device *tfa, int *pRpcStatus)
{
	...
	/* 1) write DMEM=XMEM to the DSP XMEM */
	{
		...
		error = tfa98xx_write_data_v6(tfa, FAM_TFA98XX_CF_CONTROLS,
...
enum Tfa98xx_Error tfa_cont_write_filterbank_v6(struct tfa_device *tfa,
						...
						int biquad_index)
{
	...
	tfa98xx_convert_data2bytes_v6(BIQUAD_COEFF_SIZE, coeff_buffer,
				      &bytes[nr]);
...
enum Tfa98xx_Error tfa_dsp_cmd_id_MBDrc_dynamics_v6(
	struct tfa_device *tfa, unsigned char module_id, unsigned char param_id,
	int index_subband, int num_bytes, unsigned char data[])
{
	enum Tfa98xx_Error error;
	unsigned char buffer[2 * 3];
	int nr = 0;
 ...
 */
enum Tfa98xx_Error tfa98xx_dsp_get_hw_feature_bits_v6(struct tfa_device *tfa,
						      int *features)
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	uint32_t value;
		...
		*features = tfa->hw_feature_bits;
		...
		if (tfa->tfa_family == 1) {
			...
			if (!status) {
				...
				/* skip reading MTP: */
				return (*features == -1) ? Tfa98xx_Error_Fail :
							   Tfa98xx_Error_Ok;
			}
			mtpbf = 0x850f; /* MTP5 for tfa1,16 bits */
		} else
...
enum Tfa98xx_Error tfa98xx_dsp_get_sw_feature_bits_v6(struct tfa_device *tfa,
						      int features[2])
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
...
enum Tfa98xx_Error tfa98xx_dsp_get_state_info_v6(struct tfa_device *tfa,
						 unsigned char bytes[],
						 unsigned int *statesize)
{
	...
	if (err == Tfa98xx_Error_Ok) {
		if (bSupportFramework) {
			err = tfa_dsp_cmd_id_write_read_v6(
			...
			stateSize = 8;
			err = tfa_dsp_cmd_id_write_read_v6(
				tfa, MODULE_SPEAKERBOOST, SB_PARAM_GET_STATE,
				3 * stateSize, bytes);
		}
	}
	...
	*pbSupportDrc = 0;
	...
	if (tfa->supportDrc != supportNotSet) {
		*pbSupportDrc = (tfa->supportDrc == supportYes);
		...
		if (error == Tfa98xx_Error_Ok) {
			tfa->supportDrc = *pbSupportDrc ? supportYes :
							  supportNo;
		}
	}
	return error;
...
enum Tfa98xx_Error tfa98xx_dsp_support_framework_v6(struct tfa_device *tfa,
...
enum Tfa98xx_Error
tfa98xx_dsp_write_speaker_parameters_v6(struct tfa_device *tfa, int length,
...
enum Tfa98xx_Error tfa98xx_dsp_write_drc_v6(struct tfa_device *tfa, int length,
...
enum Tfa98xx_Error tfa98xx_select_mode_v6(struct tfa_device *tfa,
...
int tfa_set_bf_v6(struct tfa_device *tfa, const uint16_t bf,
...
int tfa_set_bf_v6_volatile(struct tfa_device *tfa, const uint16_t bf,
			   const uint16_t value)
{
	enum Tfa98xx_Error err;
...
int tfa_set_bf_v6_value(const uint16_t bf, const uint16_t bf_value,
...
int tfa_write_reg_v6(struct tfa_device *tfa, const uint16_t bf,
				      ...
				      unsigned char *pFirmwareVersion)
{
	...
	if (tfa == NULL)
		return Tfa98xx_Error_Bad_Parameter;
	if (!tfa->is_probus_device) {
		err = mem_read(tfa, FW_VAR_API_VERSION, 1,
			       (int *)pFirmwareVersion);
		if (err) {
			pr_debug(
				"%s Error: Unable to get API Version from DSP \n",
				__FUNCTION__);
			return err;
		}
	} else {
 ...
 *
 */
enum Tfa98xx_Error tfaRunSpeakerBoost_v6(struct tfa_device *tfa, int force,
					 int profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	int value;
	...
	if (value) {
		...
		if (!tfa->is_probus_device) {
			tfaRunSpeakerCalibration_result_v6(tfa,
							   &calibrate_done);
		}
#endif /* OPLUS_ARCH_EXTENDS */
	}
...
enum Tfa98xx_Error tfaRunSpeakerStartup_v6(struct tfa_device *tfa, int force,
					   int profile)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
...
#ifdef OPLUS_ARCH_EXTENDS
enum Tfa98xx_Error tfaRunSpeakerCalibration_result_v6(struct tfa_device *tfa,
						      int *result)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	int calibrateDone;
	...
	if (err == Tfa98xx_Error_Ok) {
		...
		if (spkr_count == 1) {
			...
			pr_info("%s:Prim:%d mOhms, Sec:%d mOhms\n", __func__,
				tfa->mohm[0], tfa->mohm[1]);
		}

		pr_info("%s: valid mohms[%d, %d]\n", __func__, tfa->min_mohms,
			tfa->max_mohms);
		if ((tfa->mohm[0] < tfa->min_mohms) ||
		    (tfa->mohm[0] > tfa->max_mohms)) {
...
static void individual_calibration_results(struct tfa_device *tfa)
{
	...
	if (value_P != 1 && value_S != 1)
		...
		TFA_SET_BF_VOLATILE(
			tfa, SSLEFTE,
			0); /* Disable the sound for the left speaker */
	} else if (value_S != 1) {
		pr_debug("Calibration failed on Secondary (Right) channel! \n");
		TFA_SET_BF_VOLATILE(
			tfa, SSRIGHTE,
			0); /* Disable the sound for the right speaker */
	}
 ...
 */
enum Tfa98xx_Error tfaRunWaitCalibration_v6(struct tfa_device *tfa,
					    int *calibrateDone)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	...
	*calibrateDone = 0;
	...
	/* poll xmem for calibrate always
		...
		*/
	while ((*calibrateDone != 1) &&
	       (tries < TFA98XX_API_WAITRESULT_NTRIES)) {
		err = mem_read(tfa, TFA_FW_XMEM_CALIBRATION_DONE, 1,
			       calibrateDone);
		if (*calibrateDone == -1)
			break;
		tries++;
...
/* Go to the Operating state */
	...
	/* If the profile contains the .standby suffix go to powerdown
		 ...
		 */
	if (strstr(tfaContProfileName_v6(tfa->cnt, tfa->dev_idx, next_profile),
		   ".standby") != NULL) {
	...
	if ((TFA_GET_BF(tfa, CFE) != 0) && (vstep != tfa->vstep) &&
	    (vstep != -1)) {
...
int tfa_reset_v6(struct tfa_device *tfa)
{
	...
	if (tfa->tfa_family == 1) {
		...
		if (err)
			return err;
		err = tfa98xx_powerdown_v6(tfa, 0);
		if (err)
			return err;
		err = tfa_cf_powerup_v6(tfa);
		if (err)
			return err;
		err = tfaRunColdboot_v6(tfa, 1);
		if (err)
			return err;
		err = TFA_SET_BF(tfa, I2CR, 1);
	} else {
	...
	if (tfa_dev_mtp_get(tfa, TFA_MTP_OTC) && !aging_flag) {
		...
		if ((tfa->rev & 0xFF) == 0x88) {
			for (i = 0; i < spkr_count; i++) {
				if (i == 0)
					tfa->mohm[i] = tfa_dev_mtp_get(
						tfa, TFA_MTP_RE25_PRIM);
				else
					tfa->mohm[i] = tfa_dev_mtp_get(
						tfa, TFA_MTP_RE25_SEC);
			}
		} else {
		...
		error = tfa_dsp_cmd_id_write_read_v6(tfa, MODULE_SPEAKERBOOST,
						     SB_PARAM_GET_RE25C,
						     nr_bytes, bytes);
		if (error == Tfa98xx_Error_Ok) {
...
static const int eq_biquads[] = { 10, 10, 2, 2, 2, 2 };
...
enum tfa_error tfa_dev_set_state(struct tfa_device *tfa, enum tfa_state state)
{
	...
	case TFA_STATE_INIT_CF: /* coolflux HW access possible (~initcf) */
		...
		do {
			err = (enum tfa_error)tfa98xx_dsp_system_stable_v6(
				tfa, &ready);
			if (err != tfa_error_ok)
				return err;
			if (ready)
		...
		if (!tfa->is_probus_device) {
#ifdef OPLUS_ARCH_EXTENDS
			err = (enum tfa_error)tfa98xx_set_mtp_v6(
				tfa, 1, TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_MSK);
			if (err != tfa_error_ok) {
				return err;
			}
#endif /* OPLUS_ARCH_EXTENDS */
	...
	case TFA_STATE_OPERATING: /* Amp and Algo running */
		...
		if (!tfa->is_probus_device)
			...
			TFA_SET_BF(
				tfa, AMPE,
				1); /* No SBSL for probus device, we set AMPE to 1  */
...
#endif /* OPLUS_ARCH_EXTENDS */
			count = MTPEX_WAIT_NTRIES *
				4; /* Calibration takes a lot of time */
		...
		if (tfa->sync_iv_delay) {
			...
			count = 10;
			while ((TFA_GET_BF(tfa, ACS) == 1) && (count-- > 0)) {
...
enum tfa_state tfa_dev_get_state(struct tfa_device *tfa)
{
	...
	if (tfa->tfa_family == 1) {
		...
		case 8: /* if dsp reset if off assume framework is running */
			tfa->state = TFA_GET_BF(tfa, RST) ? TFA_STATE_INIT_CF :
							    TFA_STATE_INIT_FW;
			break;
		case 9:
			tfa->state = TFA_STATE_OPERATING;
...
int tfa_dev_mtp_get(struct tfa_device *tfa, enum tfa_mtp item)
{
	...
	case TFA_MTP_RE25_SEC:
		if ((tfa->rev & 0xFF) == 0x88) {
			...
			pr_debug(
				"Error: Current device has no secondary Re25 channel \n");
		}
		break;
	case TFA_MTP_LOCK:
		break;
		...
		if (tfa->tfa_family == 2) {
			...
			if ((tfa->rev & 0xFF) == 0x88)
				...
				if (tfa->is_probus_device == 1)
					tfa2_manual_mtp_cpy(tfa, 0xF4, value,
							    2);
...
void tfa_adapt_noisemode(struct tfa_device *tfa)
{
	int i, avbatt;
	long total_bats = 0;
	if ((tfa_get_bf_v6(tfa, 0x5900) == 0) ||
	    (tfa_get_bf_v6(tfa, TFA9873_BF_LP1) == 1)) {
		if (tfa->verbose)
			pr_debug(
...
int tfa_plop_noise_interrupt(struct tfa_device *tfa, int profile, int vstep)
{
	...
	if (tfa_irq_get(tfa, tfa9912_irq_stnoclk)) {
		...
		if (no_clk == 1) {
			...
			*/
			if (strstr(tfaContProfileName_v6(tfa->cnt, tfa->dev_idx,
							 profile),
				   ".saam")) {
...
{
	const int irq_stclp0 =
		36; /* FIXME: this 72 interrupt does not excist for 9912 */
	int lp0, lp1;
