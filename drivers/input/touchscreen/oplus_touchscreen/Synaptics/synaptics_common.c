...
int synaptics_get_limit_data(char *type, const unsigned char *fw_image)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (strncmp(limit_block->name, type, MAX_LIMIT_NAME_SIZE) ==
		    0) {
			break;
		}

		offset +=
			(sizeof(struct limit_block) - 4 +
			 2 * limit_block->size); /*minus 4, because byte align*/
	}
			      ...
			      const unsigned char *fw_image)
{
	...
	for (i = 0; i < number_of_areas; i++) {
		...
		if (0 == strncmp((char *)descriptor->id_string, BOOT_CONFIG_ID,
			...
			pr_info("Boot config size = %d, address = 0x%08x\n",
				length, flash_addr);
					...
					APP_CODE_ID, strlen(APP_CODE_ID))) {
			if (checksum != (crc32(~0, content, length) ^ ~0)) {
...
void synaptics_print_limit_v3(struct seq_file *s, struct touchpanel_data *ts,
			      const struct firmware *fw)
{
	...
	for (m = 0; m < item_cnt; m++) {
		item_head = (struct syna_test_item_header *)(fw->data +
							     p_item_offset[m]);
		if (item_head->item_magic != Limit_ItemMagic &&
		    item_head->item_magic != Limit_ItemMagic_V2) {
		...
		seq_printf(s,
			   ...
			   item_head->item_bit, item_head->item_size,
			   item_head->item_limit_type, item_head->para_num);
		if (item_head->item_limit_type == LIMIT_TYPE_NO_DATA) {
			seq_printf(s, "no limit data\n");
		} else if (item_head->item_limit_type ==
			   LIMIT_TYPE_CERTAIN_DATA) {
			p_data32 = (int32_t *)(fw->data +
			...
			seq_printf(s, "floor limit data: %d\n", *p_data32);
			   ...
			   LIMIT_TYPE_EACH_NODE_DATA) {
			if ((item_head->item_bit == TYPE_FULLRAW_CAP) ||
			    (item_head->item_bit == TYPE_DELTA_NOISE) ||
			    (item_head->item_bit == TYPE_RAW_CAP)) {
				p_data32 =
					(int32_t *)(fw->data +
						    item_head->top_limit_offset);
				...
				seq_printf(s, "top data: \n");
				for (i = 0;
				     i < ts->hw_res.TX_NUM + ts->hw_res.RX_NUM;
				     i++) {
		...
		p_data32 = (int32_t *)(fw->data + p_item_offset[m] +
				       sizeof(struct syna_test_item_header));
...
static int tp_auto_test_read_func(struct seq_file *s, void *v)
{
	...
	sprintf(data_buf,
		"/sdcard/TpTestReport/tp_testlimit_%02d%02d%02d-%02d%02d%02d-utc.csv",
	...
	syna_testdata.fingerprint_underscreen_support =
			  ...
			  struct synaptics_proc_operations *syna_ops)
{
	...
	if (ts->face_detect_support) {
		prEntry_tmp = proc_create_data("touch_filter", 0666,
					       ts->prEntry_tp,
