...
int lcd_bl_enable(bool enable)
{
	if (enable) {
		usleep_range(30000,
			     30001); /* keep mipi on-bl on 40ms,now 42ms */
		lcd_bl_write_byte(
			0x02,
			0xD3); /* BL_CFG1 OVP=34V Exponential dimming PWM Enable */
		lcd_bl_write_byte(0x03,
				  0xCD); /* diming 256ms PWM_HYST 10 LSBs*/
		lcd_bl_write_byte(
			0x11, 0xB7); /* BL_OPTION2 10uH BL_CURRENT_LIMIT 2.5A*/
		lcd_bl_write_byte(
			0x15,
			0xC1); /* Backlight Full-scale LED Current 24.4mA/ PWM transent time 4ms*/
		lcd_bl_write_byte(
			0x08,
			0x5F); /* BL enabled and Current sink 1/2/3/4 /5 enabled£»*/
	} else {
...
int lcd_bl_set_led_brightness(int value) //for set bringhtness
{
	...
	if (value > 0) {
		if (dis_set_first_level) {
			lcd_bl_enable(
				true); /* BL enabled and Current sink 1/2/3/4/5 enabled */
			dis_set_first_level = 0;
			pr_info("%d %s dis_set_first_level=%d\n", __LINE__,
				__func__, dis_set_first_level);
		}
	...
	if (value == 0) {
		...
		lcd_bl_enable(
			false); /* BL disabled and Current sink 1/2/3/4/5 disabled */
	}
	return 0;
}
...
static int ktz8866_pwr_parse_gpios(struct device *dev)
{
	...
	for (index = 0; index < LCD_PWR_GPIO_MAX; index++) {
		gpio_arrays[index].gpio_num =
			of_get_named_gpio(np, gpio_arrays[index].gpio_name, 0);
		if (!gpio_is_valid(gpio_arrays[index].gpio_num)) {
			rc = gpio_arrays[index].gpio_num;
			pr_err("[%s] failed get gpio from dts, rc=%d\n",
			       __func__, rc);
		} else {
			rc = gpio_request(gpio_arrays[index].gpio_num,
					  gpio_arrays[index].gpio_name);
			if (rc) {
				pr_err("request %s gpio %d failed,rc = %d\n",
				       gpio_arrays[index].gpio_name,
				       gpio_arrays[index].gpio_num, rc);
			} else {
...
int turn_on_ktz8866_hw_en(bool on)
{
	...
	if (gpio_is_valid(gpio_arrays[KTZ8866_HW_EN].gpio_num)) {
		ret = gpio_direction_output(gpio_arrays[KTZ8866_HW_EN].gpio_num,
					    value);
		if (ret) {
			pr_err("failed to set %s gpio %d, ret = %d\n",
			       gpio_arrays[KTZ8866_HW_EN].gpio_name, value,
			       ret);
			return ret;
		}
	} else {
