...
static void ufshid_remove_sysfs(struct ufshid_dev *hid);
...
static void ufshid_block_enter_suspend(struct ufshid_dev *hid)
{
	...
	HID_DEBUG(hid,
		  ...
		  "hba->hibern8_on_idle.active_reqs %d",
					  ...
					  const char *buf, size_t count)
{
	...
	INFO_MSG("HID_trigger(input) %lu", val);
						...
						const char *buf, size_t count)
{
	...
	if (hid->block_suspend ^ val) {
		if (val)
			ufshid_block_enter_suspend(hid);
		else
			ufshid_allow_enter_suspend(hid);

		INFO_MSG("[Before] block_suspend %d", hid->block_suspend);
		hid->block_suspend = val;
		INFO_MSG("[After] block_suspend %d", hid->block_suspend);
	} else {
		INFO_MSG("block_suspend %d. so it does not changed",
			 hid->block_suspend);
	}
	...
	if (!err) {
		for (entry = hid->sysfs_entries; entry->attr.name != NULL;
			...
			INFO_MSG("ufshid sysfs attr creates: %s",
				 entry->attr.name);
			if (sysfs_create_file(&hid->kobj, &entry->attr))
				break;
		}
		INFO_MSG("ufshid sysfs adds uevent");
...
{
	kobject_uevent(&hid->kobj, KOBJ_REMOVE);
	INFO_MSG("ufshid_dev removes sysfs %p", &hid->kobj);
