...
void mtk_charger_int_handler(void)
{
	...
	if (is_vooc_project() == false) {
		...
		//charger_dev_set_input_current(g_oplus_chip->chgic_mtk.oplus_info->chg1_dev, 500000);
		if (g_oplus_chip) {
			g_oplus_chip->chg_ops->input_current_write(500);
...
static void get_chargeric_temp(struct charger_data *pdata)
{
	...
	if (pinfo && pinfo->support_ntc_01c_precision) {
		...
		step_vol = ((con_volt_19165[i] - con_volt_19165[i + 1]) * 1000)/ 10;
		temp_offset = ((con_volt_19165[i] - pdata->chargeric_temp_volt) * 1000) / step_vol;
		pdata->chargeric_temp -= temp_offset;
	}
	chg_err("chargeric_temp:%d con_volt_19165[%d] = %d con_temp_19165[%d] = %d chargeric_temp_volt = %d step_vol = %d temp_offset = %d\n", pdata->chargeric_temp, i, con_volt_19165[i], i, con_temp_19165[i], pdata->chargeric_temp_volt, step_vol, temp_offset);
	return;
}
#endif
...
int oplus_mt6360_pd_setup(void)
{
	...
	}else if (pinfo->data.dual_charger_support) {
		cap.nr = 0;
		cap.pdp = 0;
		for (i = 0; i < ADAPTER_CAP_MAX_NR; i++) {
			cap.max_mv[i] = 0;
...
bool oplus_chg_check_vbus_change(const unsigned int checkpoint, const bool isVbusBoost)
{
	...
	for(reCheckCount = 0; reCheckCount < checkCountMax; reCheckCount++){
		...
		if( isVbusBoost ){
			chg_vol = chip->chg_ops->get_charger_volt();
			if(chg_vol < checkpoint)
				continue;
			else
				break;
		}
	...
	pr_err("%s, isVbusBoost=%d reCheckCount=%d chg_vol=%d checkCountMax=%d\n", __func__, reCheckCount, chg_vol, checkCountMax);
...
int oplus_chg_set_qc_config_forsvooc(void)
{
	...
	if (!chip->calling_on && chip->charger_volt < 6500 && chip->soc < 90
		...
		msleep(300);
		oplus_chg_unsuspend_charger();
		if(oplus_chg_check_vbus_change(8600, true)) {
			ret = 0;
		}
...
static void mtk_charger_shutdown(struct platform_device *dev)
{
	...
	if (g_oplus_chip->charger_exist && is_vooc_project()) {
		  g_oplus_chip->chg_ops->input_current_write(500);
		  pr_debug("%s: setting input 500mA\n", __func__);
	}
#endif	
}
