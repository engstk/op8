...
MODULE_PARM_DESC(pcm_sample_format,
		 "PCM sample format: 0=S16_LE, 1=S24_LE, 2=S32_LE\n");
...
static const struct snd_kcontrol_new ftm_spk_rev_controls[] = {
	SOC_ENUM_EXT("SPK_Pa Revision", ftm_spk_rev_enum, ftm_spk_rev_get,
		     ftm_spk_rev_put),
};
#endif /* OPLUS_ARCH_EXTENDS */
...
static int tfa98xx_ana_volume_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static int tfa98xx_ana_volume_set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
{
	uint8_t cmd[9] = {
		0x00, 0x81, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	};
...
static int tfadsp_volume_get(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int tfadsp_volume_set(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
...
static inline char *tfa_cont_profile_name(struct tfa98xx *tfa98xx, int prof_idx)
{
	if (tfa98xx->tfa->cnt == NULL)
		return "NONE";
	return tfaContProfileName_v6(tfa98xx->tfa->cnt, tfa98xx->tfa->dev_idx,
				     prof_idx);
}
	...
	if (tfa98xx->tfa->dev_idx == 0) {
		...
		list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
			...
			if (tfa98xx_device_count == 4 &&
			    tfa98xx->tfa->dev_idx == 2) {
				pr_info("tfa98xx_ins2 is init\n");
				tfa98xx_ins2 = tfa98xx;
			}
#endif /*OPLUS_ARCH_EXTENDS*/
#ifndef OPLUS_ARCH_EXTENDS
			err = (enum Tfa98xx_Error)tfa_dev_mtp_set(
				tfa98xx->tfa, TFA_MTP_OTC, 0);
			if (err) {
		...
		if (tfa98xx_ins2 != NULL) {
			...
			err = tfaContWriteProfile_v6(tfa98xx_ins2->tfa, profile,
						     0);
			if (err) {
				pr_err("profile write failed\n");
				return Tfa98xx_Error_Bad_Parameter;
		...
		err = tfa_dsp_cmd_id_write_read_v6(tfa98xx->tfa,
						   MODULE_FRAMEWORK,
		...
		if (tfa98xx_ins2 != NULL)
			err = tfa_dsp_cmd_id_write_read_v6(
				tfa98xx_ins2->tfa, MODULE_FRAMEWORK,
				FW_PAR_ID_GET_STATUS_CHANGE, 6,
				(unsigned char *)buffer);
#endif /*OPLUS_ARCH_EXTENDS*/
		/* We need to send zero's to trigger calibration */
		memset(bytes, 0, 6);
		err = tfa_dsp_cmd_id_write_v6(tfa98xx->tfa, MODULE_SPEAKERBOOST,
					      SB_PARAM_SET_RE25C, sizeof(bytes),
					      bytes);
#ifdef OPLUS_ARCH_EXTENDS
		/*modified for 4 pa solution*/
		if (tfa98xx_ins2 != NULL)
			err = tfa_dsp_cmd_id_write_v6(tfa98xx_ins2->tfa,
						      MODULE_SPEAKERBOOST,
						      SB_PARAM_SET_RE25C,
						      sizeof(bytes), bytes);
#endif /*OPLUS_ARCH_EXTENDS*/
/* Wait a maximum of 5 seconds to get the calibration results */
		...
		/*Add for 4 PA solution*/
		if (tfa98xx_ins2 != NULL)
			err = tfa_dsp_get_calibration_impedance_v6(
				tfa98xx_ins2->tfa);

		if ((tfa98xx_device_count == 2) ||
		    (tfa98xx_device_count == 4)) {
#endif
			/*make sure:left and right impedance update to the correct device*/
			impLR[0] =
				(tfa_get_calibration_info_v6(tfa98xx->tfa, 0));
			impLR[1] =
				(tfa_get_calibration_info_v6(tfa98xx->tfa, 1));
#ifdef OPLUS_ARCH_EXTENDS
			/*Add for 4 PA solution*/
			if (tfa98xx_ins2 != NULL) {
				impLR[2] = (tfa_get_calibration_info_v6(
					tfa98xx_ins2->tfa, 0));
				impLR[3] = (tfa_get_calibration_info_v6(
					tfa98xx_ins2->tfa, 1));
...
/*if channel not initialized, set as defalut, cnt-id 0 is left, cnt-id 1 is right*/
		...
		*speakerImpedance =
		...
		list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
			...
			tfa_dev_set_swprof(tfa98xx->tfa,
...
#else
static ssize_t tfa98xx_dbgfs_otc_get(struct file *file, char __user *user_buf,
...
#else
static ssize_t tfa98xx_dbgfs_mtpex_get(struct file *file, char __user *user_buf,
				       size_t count, loff_t *ppos)
{
#ifdef CONFIG_DEBUG_FS
	struct i2c_client *i2c = file->private_data;
...
#else
static ssize_t tfa98xx_dbgfs_temp_get(struct file *file, char __user *user_buf,
				      size_t count, loff_t *ppos)
{
#ifdef CONFIG_DEBUG_FS
	struct i2c_client *i2c = file->private_data;
...
#ifdef OPLUS_ARCH_EXTENDS
static int tfa98xx_speaker_recalibration_v6(struct tfa_device *tfa,
					    int *speakerImpedance)
{
	...
	error = tfaRunSpeakerBoost_v6(
		tfa, 1, profile); /* No force coldstart (with profile 0) */
	if (error) {
		...
		*speakerImpedance = tfa->mohm[0];
		if (TFA_GET_BF(tfa, PWDN) != 0) {
			err = tfa98xx_powerdown_v6(tfa,
						   0); //leave power off state
		}
				    ...
				    size_t count, loff_t *ppos)
{
...
#ifdef OPLUS_ARCH_EXTENDS
		ret = tfaRunSpeakerCalibration_result_v6(tfa98xx->tfa,
							 &calibrate_done);
#else /* OPLUS_ARCH_EXTENDS */
...
#ifdef OPLUS_ARCH_EXTENDS
		if (1 == calibrate_done) {
			tfa98xx_speaker_recalibration_v6(tfa98xx->tfa,
							 &speakerImpedance);
		}
#endif /* OPLUS_ARCH_EXTENDS */
	}
	...
	ret = snprintf(str, PAGE_SIZE, " Prim:%d mOhms, Sec:%d mOhms\n",
		       speakerImpedance, tfa98xx->tfa->mohm[1]);
#else /* OPLUS_ARCH_EXTENDS */
...
static void tfa98xx_debug_init(struct tfa98xx *tfa98xx, struct i2c_client *i2c)
{
...
#ifdef OPLUS_ARCH_EXTENDS
	debugfs_create_file("range", S_IRUGO, tfa98xx->dbg_dir, i2c,
			    &tfa98xx_dbgfs_range_fops);
	debugfs_create_file("r_aging", S_IRUGO, tfa98xx->dbg_dir, i2c,
			    &tfa98xx_dbgfs_r_aging_fops);
	debugfs_create_file("r_impedance", S_IRUGO, tfa98xx->dbg_dir, i2c,
			    &tfa98xx_dbgfs_r_impedance_fops);
#endif /* OPLUS_ARCH_EXTENDS */
	...
	proc_create_data("R", S_IRUGO, tfa98xx->dbg_dir, &tfa98xx_dbgfs_r_fops,
			 i2c);
#ifdef OPLUS_ARCH_EXTENDS
	...
	if (tfa98xx->flags & TFA98XX_FLAG_SAAM_AVAILABLE) {
		...
		proc_create_data("pga_gain", S_IRUGO, tfa98xx->dbg_dir,
				 &tfa98xx_dbgfs_pga_gain_fops, tfa98xx->i2c);
	}
#endif /*CONFIG_DEBUG_FS*/
}
			      ...
			      struct snd_ctl_elem_info *uinfo)
{
	...
	uinfo->value.integer.max =
		max(0, tfacont_get_max_vstep_v6(tfa98xx->tfa, profile) - 1);
	pr_info("vsteps count: %d [prof=%d]\n",
		tfacont_get_max_vstep_v6(tfa98xx->tfa, profile), profile);
	return 0;
}
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		if (selector == CHIP_SELECTOR_4PA_LEFT_UP) {
			...
			if ((tfa98xx->i2c->addr == CHIP_LEFT_UP_ADDR) ||
			    (tfa98xx->i2c->addr == CHIP_LEFT_DOWN_ADDR))
				tfa98xx->flags |= TFA98XX_FLAG_CHIP_SELECTED;
			else
				tfa98xx->flags &= ~TFA98XX_FLAG_CHIP_SELECTED;
		} else if (selector == CHIP_SELECTOR_4PA_RIGHT_BOTH) {
			if ((tfa98xx->i2c->addr == CHIP_RIGHT_UP_ADDR) ||
			    (tfa98xx->i2c->addr == CHIP_RIGHT_DOWN_ADDR))
				tfa98xx->flags |= TFA98XX_FLAG_CHIP_SELECTED;
			else
				tfa98xx->flags &= ~TFA98XX_FLAG_CHIP_SELECTED;
		} else {
...
#define REG_BITS 16
#define TFA9874_STATUS_NORMAL_VALUE \
	((0x850F << REG_BITS) +     \
	 0x16) /*reg 0x13 high 16 bits and 0x10 low 16 bits*/
#define TFA9874_STATUS_CHECK_MASK \
	((0x300 << REG_BITS) +    \
	 0x9C) /*reg 0x10 mask bit2~4, bit7, reg 0x13 mask bit8 , bit9 */
#define TFA9873_STATUS_NORMAL_VALUE \
	((0x850F << REG_BITS) +     \
	 0x56) /*reg 0x13 high 16 bits and 0x10 low 16 bits*/
#define TFA9873_STATUS_CHECK_MASK \
	((0x300 << REG_BITS) +    \
	 0x15C) /*reg 0x10 mask bit2~4, bit6, bit8, reg 0x13 mask bit8 , bit9*/

static ktime_t last_fb = 0;
static bool g_chk_err = false;
static char const *tfa98xx_check_feedback_text[] = { "Off", "On" };
static const struct soc_enum tfa98xx_check_feedback_enum = SOC_ENUM_SINGLE_EXT(
	ARRAY_SIZE(tfa98xx_check_feedback_text), tfa98xx_check_feedback_text);

enum { PA_TFA9874 = 0, PA_TFA9873, PA_MAX };
...
const unsigned char fb_regs[] = { 0x00, 0x01, 0x02, 0x04, 0x05,
				  0x11, 0x14, 0x15, 0x16 };
...
static int tfa98xx_check_status_reg(void)
{
	...
	if ((last_fb != 0) &&
	    ktime_before(ktime_get(),
			 ktime_add_ms(last_fb, MM_FB_KEY_RATELIMIT_5MIN))) {
		return 0;
	}
	mutex_lock(&tfa98xx_mutex);
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if (Tfa98xx_Error_Ok == err) {
			err = tfa98xx_read_register16_v6(tfa98xx->tfa, 0x13,
							 &reg13);
		}
		pr_info("%s: read SPK%d status regs ret=%d, reg[0x10]=0x%x, reg[0x13]=0x%x",
			__func__, num, err, reg10, reg13);

		if (Tfa98xx_Error_Ok == err) {
			reg_val = (reg13 << REG_BITS) + reg10;
			flag = 0;
			if ((g_pa_type == PA_TFA9874) &&
			    ((TFA9874_STATUS_NORMAL_VALUE &
			      TFA9874_STATUS_CHECK_MASK) !=
			     (reg_val & TFA9874_STATUS_CHECK_MASK))) {
				offset = strlen(info);
				scnprintf(
					info + offset,
					sizeof(info) - offset - 1,
					"TFA9874 SPK%d:reg[0x10]=0x%x,reg[0x13]=0x%x,",
					num, reg10, reg13);
				for (i = 0; i < ARRAY_SIZE(check_err_tfa9874);
				     i++) {
					if (check_err_tfa9874[i].err_val ==
					    (1 & (reg_val >>
						  check_err_tfa9874[i].bit))) {
						offset = strlen(info);
						scnprintf(info + offset,
							  sizeof(info) -
								  offset - 1,
							  "%s,",
							  check_err_tfa9874[i]
				...
				flag = 1;
				   ...
				   ((TFA9873_STATUS_NORMAL_VALUE &
				     TFA9873_STATUS_CHECK_MASK) !=
				    (reg_val & TFA9873_STATUS_CHECK_MASK))) {
				offset = strlen(info);
				scnprintf(
					info + offset,
					sizeof(info) - offset - 1,
					"TFA9873 SPK%d:reg[0x10]=0x%x,reg[0x13]=0x%x,",
					num, reg10, reg13);
				for (i = 0; i < ARRAY_SIZE(check_err_tfa9873);
				     i++) {
					if (check_err_tfa9873[i].err_val ==
					    (1 & (reg_val >>
						  check_err_tfa9873[i].bit))) {
						offset = strlen(info);
						scnprintf(info + offset,
							  sizeof(info) -
								  offset - 1,
			...
			if (flag == 1) {
				...
				for (i = 0; i < sizeof(fb_regs); i++) {
					err = tfa98xx_read_register16_v6(
						tfa98xx->tfa, fb_regs[i],
						&reg_tmp);
					if (Tfa98xx_Error_Ok == err) {
						offset = strlen(info);
						scnprintf(info + offset,
			...
			offset = strlen(info);
			scnprintf(
				info + offset, sizeof(info) - offset - 1,
				"%s SPK%d: failed to read regs 0x10 and 0x13, error=%d,",
				(g_pa_type == PA_TFA9873) ? "TFA9873" :
							    "TFA9874",
				num, err);
			last_fb = ktime_get();
		}
...
static int tfa98xx_check_speaker_status(struct tfa98xx *tfa98xx)
{
	...
	if (err == Tfa98xx_Error_Ok) {
		if (buffer[2] & 0x6) {
			...
			if (buffer[2] & 0x2) {
				...
				scnprintf(fd_buf + strlen(fd_buf),
					  sizeof(fd_buf) - strlen(fd_buf),
					  " SPK1 damaged or hole blocked");
			}
		...
		scnprintf(fd_buf, sizeof(fd_buf) - 1,
			  "payload@@spk protection algorithm error, ret=%d,",
			  err);
		mm_fb_audio_kevent_named(OPLUS_AUDIO_EVENTID_SMARTPA_ERR,
					 MM_FB_KEY_RATELIMIT_5MIN, fd_buf);
		last_fb = ktime_get();
...
enum Tfa98xx_Error tfa98xx_write_raw_v6(struct tfa_device *tfa, int len,
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (!tfa98xx->tfa->is_probus_device) {
		...
		if (!value) {
			tfa98xx->profile =
				tfaContGetCalProfile_v6(tfa98xx->tfa);
...
static void rcv_tfa_on(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx_selector == CHIP_SELECTOR_STEREO) {
		...
		if (pre_fadein_flag) {
			...
			if ((spk_on_jiffies > rcv_off_jiffies) &&
			    ((spk_on_jiffies - rcv_off_jiffies) <
			     usecs_to_jiffies(dev_switch_us))) {
				do_fadein_flag = true;
			}
		}
	}
	...
	if (do_fadein_flag) {
		...
		queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->fadein_work,
				   0);
	}
}
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values(void)
{
	...
	if (tfa98xx_device_count == bytes[0]) {
		...
		if (tfa98xx_device_count == 4) {
			...
			bytes_id2[2] = 0x81;
			bytes_id2[3] = 0x05;
			ret = send_tfa_cal_in_band(&bytes_id2[1],
						   sizeof(bytes_id2) - 1);
			if (ret) {
...
{
	uint8_t cmd[9] = {
		0x00, 0x81, 0x04, 0x00, 0x00, 0xff, 0x00, 0x00, 0xff
	};
...
static int tfa98xx_probe(struct snd_soc_component *component)
{
...
#ifdef OPLUS_ARCH_EXTENDS
	snd_soc_add_component_controls(tfa98xx->component, tfa98xx_snd_controls,
				       ARRAY_SIZE(tfa98xx_snd_controls));
#endif /* OPLUS_ARCH_EXTENDS */

#ifdef OPLUS_ARCH_EXTENDS
	snd_soc_add_component_controls(tfa98xx->component, ftm_spk_rev_controls,
				       ARRAY_SIZE(ftm_spk_rev_controls));
#endif /* OPLUS_ARCH_EXTENDS */

#ifdef OPLUS_FEATURE_SPEAKER_MUTE
	snd_soc_add_component_controls(
		tfa98xx->component, tfa98xx_snd_control_spk_mute,
		ARRAY_SIZE(tfa98xx_snd_control_spk_mute));
#endif /* OPLUS_FEATURE_SPEAKER_MUTE */

#ifdef OPLUS_FEATURE_FADE_IN
	snd_soc_add_component_controls(tfa98xx->component,
				       tfadsp_volume_controls,
				       ARRAY_SIZE(tfadsp_volume_controls));
#endif /* OPLUS_FEATURE_FADE_IN */
	...
	dev_info(component->dev, "%s: codec registered (%s) ret=%d", __func__,
		 tfa98xx->fw.name, ret);
			     ...
			     const struct i2c_device_id *id)
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		for (i = 0; i < retries; i++) {
			...
			if ((ret < 0) || !is_tfa98xx_series(reg & 0xff)) {
				dev_err(&i2c->dev,
					"Failed to read Revision register: reg 0x%x, ret %d\n",
					reg, ret);
				msleep(2); //2ms
			} else {
				break;
	...
	ret = of_property_read_u32(i2c->dev.of_node, "tfa_min_range",
	...
	ret = of_property_read_u32(i2c->dev.of_node, "tfa_max_range",
	...
	ret = of_property_read_u32(i2c->dev.of_node, "tfa_channel",
				   &tfa98xx->tfa->channel);
	if (ret) {
	...
	dev_err(&i2c->dev,
		"channel=%d   (0-left/top, 1-right/bottom, 0xff-default, not initialized)\n",
		tfa98xx->tfa->channel);
#endif /* OPLUS_ARCH_EXTENDS */

#ifdef OPLUS_FEATURE_FADE_IN
	ret = of_property_read_u32(i2c->dev.of_node, "tfa_fadein_feature",
				   &fade_in_config);
	if (ret) {
	...
	ret = snd_soc_register_component(&i2c->dev, &soc_component_dev_tfa98xx,
