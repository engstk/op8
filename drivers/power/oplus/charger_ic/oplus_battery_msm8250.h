...
enum qc2_non_comp_voltage { QC2_COMPLIANT, QC2_NON_COMPLIANT_9V, QC2_NON_COMPLIANT_12V };
...
enum chg_term_config_src { ITERM_SRC_UNSPECIFIED, ITERM_SRC_ADC, ITERM_SRC_ANALOG };
...
struct smb_chg_param {
	...
	int max_u;
	int step_u;
	int (*get_proc)(struct smb_chg_param *param, u8 val_raw);
	int (*set_proc)(struct smb_chg_param *param, int val_u, u8 *val_raw);
};
...
struct smb_charger {
	...
	int wireless_conn_gpio;
	int wireless_conn_irq;
	struct pinctrl *wireless_conn_pinctrl;
	struct pinctrl_state *wireless_conn_active;
	struct pinctrl_state *wireless_conn_sleep;
...
enum skip_reason { REASON_OTG_ENABLED = BIT(0), REASON_FLASH_ENABLED = BIT(1) };
...
int smblib_get_prop_usb_suspend(struct smb_charger *chg, union power_supply_propval *val);
...
int smblib_get_usb_prop_typec_mode(struct smb_charger *chg, union power_supply_propval *val);
