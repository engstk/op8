...
int oplus_mcu_bcc_stop_curr_dt(struct oplus_vooc_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,svooc_0_to_50_little_cold_stop_cur",
				  &chip->svooc_0_to_50_little_cold_stop_cur);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,svooc_0_to_50_little_cool_stop_cur",
				  &chip->svooc_0_to_50_little_cool_stop_cur);
	if (rc) {
		chip->svooc_0_to_50_little_cool_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_0_to_50_normal_low_stop_cur",
				  &chip->svooc_0_to_50_normal_low_stop_cur);
	if (rc) {
		chip->svooc_0_to_50_normal_low_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_0_to_50_normal_high_stop_cur",
				  &chip->svooc_0_to_50_normal_high_stop_cur);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,svooc_51_to_75_little_cold_stop_cur",
				  &chip->svooc_51_to_75_little_cold_stop_cur);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,svooc_51_to_75_little_cool_stop_cur",
				  &chip->svooc_51_to_75_little_cool_stop_cur);
	if (rc) {
		chip->svooc_51_to_75_little_cool_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_51_to_75_normal_low_stop_cur",
				  &chip->svooc_51_to_75_normal_low_stop_cur);
	if (rc) {
		chip->svooc_51_to_75_normal_low_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_51_to_75_normal_high_stop_cur",
				  &chip->svooc_51_to_75_normal_high_stop_cur);
	if (rc) {
		chip->svooc_51_to_75_normal_high_stop_cur = 0;
	}

	/* add for bcc get stop current soc is 76-85 */
	rc = of_property_read_u32(node, "qcom,svooc_76_to_85_little_cold_stop_cur",
				  &chip->svooc_76_to_85_little_cold_stop_cur);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,svooc_76_to_85_little_cool_stop_cur",
				  &chip->svooc_76_to_85_little_cool_stop_cur);
	if (rc) {
		chip->svooc_76_to_85_little_cool_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_76_to_85_normal_low_stop_cur",
				  &chip->svooc_76_to_85_normal_low_stop_cur);
	if (rc) {
		chip->svooc_76_to_85_normal_low_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_76_to_85_normal_high_stop_cur",
				  &chip->svooc_76_to_85_normal_high_stop_cur);
	if (rc) {
		chip->svooc_76_to_85_normal_high_stop_cur = 0;
	}

	/* add for bcc get stop current soc is 86-90 */
	rc = of_property_read_u32(node, "qcom,svooc_86_to_90_little_cold_stop_cur",
				  &chip->svooc_86_to_90_little_cold_stop_cur);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,svooc_86_to_90_little_cool_stop_cur",
				  &chip->svooc_86_to_90_little_cool_stop_cur);
	if (rc) {
		chip->svooc_86_to_90_little_cool_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_86_to_90_normal_low_stop_cur",
				  &chip->svooc_86_to_90_normal_low_stop_cur);
	if (rc) {
		chip->svooc_86_to_90_normal_low_stop_cur = 0;
	}

	rc = of_property_read_u32(node, "qcom,svooc_86_to_90_normal_high_stop_cur",
				  &chip->svooc_86_to_90_normal_high_stop_cur);
	if (rc) {
...
static int opchg_bq27541_gpio_pinctrl_init(struct oplus_vooc_chip *chip)
{
	...
	/* set clock is active*/
	chip->vooc_gpio.gpio_clock_active = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "clock_active");
	if (IS_ERR_OR_NULL(chip->vooc_gpio.gpio_clock_active)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set clock is sleep*/
	chip->vooc_gpio.gpio_clock_sleep = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "clock_sleep");
	if (IS_ERR_OR_NULL(chip->vooc_gpio.gpio_clock_sleep)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set clock is active*/
	chip->vooc_gpio.gpio_data_active = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "data_active");
	if (IS_ERR_OR_NULL(chip->vooc_gpio.gpio_data_active)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set clock is sleep*/
	chip->vooc_gpio.gpio_data_sleep = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "data_sleep");
	if (IS_ERR_OR_NULL(chip->vooc_gpio.gpio_data_sleep)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set reset is atcive*/
	chip->vooc_gpio.gpio_reset_active = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "reset_active");
	if (IS_ERR_OR_NULL(chip->vooc_gpio.gpio_reset_active)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}
	/* set reset is sleep*/
	chip->vooc_gpio.gpio_reset_sleep = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "reset_sleep");
	if (IS_ERR_OR_NULL(chip->vooc_gpio.gpio_reset_sleep)) {
		chg_err(": %d Failed to get the state 3 pinctrl handle\n", __LINE__);
		return -EINVAL;
	}

	chip->vooc_gpio.gpio_mcu_ctrl_cp_active = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "mcu_ctrl_active");
	...
	chip->vooc_gpio.gpio_mcu_ctrl_cp_sleep = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "mcu_ctrl_sleep");
...
void oplus_vooc_fw_type_dt(struct oplus_vooc_chip *chip)
{
	...
	chip->vooc_current_lvl_cnt =
		of_property_count_elems_of_size(node, "qcom,vooc_current_lvl", sizeof(*chip->vooc_current_lvl));
	if (chip->vooc_current_lvl_cnt > 0) {
		...
		rc = of_property_read_u32_array(node, "qcom,vooc_current_lvl", chip->vooc_current_lvl,
						chip->vooc_current_lvl_cnt);
		if (rc) {
	...
	chip->support_vooc_by_normal_charger_path =
		of_property_read_bool(node, "qcom,support_vooc_by_normal_charger_path");
	...
	rc = of_property_read_u32(node, "qcom,vooc-version", &chip->vooc_version);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc-little-cool-temp", &chip->vooc_little_cool_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc-little-cold-temp", &chip->vooc_little_cold_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc-normal-low-temp", &chip->vooc_normal_low_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc_little_cool_bat_suspend_volt",
	...
	chip->vooc_multistep_adjust_current_support =
		of_property_read_bool(node, "qcom,vooc_multistep_adjust_current_support");
	chg_debug("qcom,vooc_multistep_adjust_current_supportis %d\n", chip->vooc_multistep_adjust_current_support);

	rc = of_property_read_u32(node, "qcom,vooc_reply_mcu_bits", &chip->vooc_reply_mcu_bits);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,support_fake_vooc_check", &chip->support_fake_vooc_check);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc_over_high_or_low_current", &chip->vooc_over_high_or_low_current);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,vooc_break_frequence", &chip->vooc_break_frequence);
	if (!rc) {
...
int oplus_vooc_asic_hwid_check(struct oplus_vooc_chip *chip)
{
	...
	chip->vooc_gpio.vooc_asic_id_gpio = of_get_named_gpio(node, "qcom,vooc_asic_id-gpio", 0);
	...
	chip->vooc_gpio.gpio_vooc_asic_id_sleep = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "vooc_asic_id_sleep");
	...
	chip->vooc_gpio.gpio_vooc_asic_id_active = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "vooc_asic_id_active");
	...
	pinctrl_select_state(chip->vooc_gpio.pinctrl, chip->vooc_gpio.gpio_vooc_asic_id_sleep);
	...
	pinctrl_select_state(chip->vooc_gpio.pinctrl, chip->vooc_gpio.gpio_vooc_asic_id_active);
...
/*This is only for P60 P(17197 P)*/
	...
	chip->vooc_gpio.gpio_vooc_mcu_id_default = pinctrl_lookup_state(chip->vooc_gpio.pinctrl, "vooc_mcu_id_default");
	...
	pinctrl_select_state(chip->vooc_gpio.pinctrl, chip->vooc_gpio.gpio_vooc_mcu_id_default);
...
static void delay_reset_mcu_work_func(struct work_struct *work)
{
	...
	struct oplus_vooc_chip *chip = container_of(dwork, struct oplus_vooc_chip, delay_reset_mcu_work);
...
bool is_allow_fast_chg_real(struct oplus_vooc_chip *chip)
{
	...
	if (chip->vooc_normal_high_temp != -EINVAL && (temp > chip->vooc_normal_high_temp) &&
	    (!(soc < chip->vooc_warm_allow_soc && (oplus_chg_get_batt_volt() < chip->vooc_warm_allow_vol)))) {
		return false;
	}

	mcu_hwid_type = get_vooc_mcu_type(chip);
	chg_err("temp is %d, volt is %d, suspend_charger is %d, mcu_hwid_type is %d\n", temp, volt,
		chip->suspend_charger, mcu_hwid_type);
...
void switch_fast_chg(struct oplus_vooc_chip *chip)
{
	...
	if (chip->dpdm_switch_mode == VOOC_CHARGER_MODE && gpio_get_value(chip->vooc_gpio.switch1_gpio) == 1) {
...
void opchg_reply_mcu_data_4bits(struct oplus_vooc_chip *chip, int ret_info, int device_type)
{
	int i = 0;
	for (i = 0; i < 4; i++) {
...
void oplus_vooc_eint_register(struct oplus_vooc_chip *chip)
{
	...
	if (node) {
		...
		ret = request_irq(chip->vooc_gpio.data_irq, (irq_handler_t)irq_rx_handler, IRQF_TRIGGER_RISING,
				  "VOOC_AP_DATA-eint", chip);
		if (ret < 0) {
...
int oplus_vooc_choose_bcc_fastchg_curve(struct oplus_vooc_chip *chip)
{
	...
	for (idx = 0; idx < svooc_curves_target_curve[0].bcc_curv_num; idx++) {
		...
		chg_err("bcc para idx:%d, vbat:%d, max_ibus:%d, min_ibus:%d, exit:%d", idx, chip->bcc_target_vbat,
			chip->bcc_curve_max_current, chip->bcc_curve_min_current, chip->bcc_exit_curve);
	}

	chip->svooc_batt_curve[0].bcc_curv_num = svooc_curves_target_curve[0].bcc_curv_num;
	memcpy((&(chip->svooc_batt_curve[0]))->batt_bcc_curve, (&(svooc_curves_target_curve[0]))->batt_bcc_curve,
	...
	for (idx = 0; idx < chip->svooc_batt_curve[0].bcc_curv_num; idx++) {
		chg_err("chip svooc bcc para idx:%d vbat:%d, max_ibus:%d, min_ibus:%d, exit:%d curve num:%d\n", idx,
			...
			chip->svooc_batt_curve[0].batt_bcc_curve[idx].exit, chip->svooc_batt_curve[0].bcc_curv_num);
	}
