...
static uint32_t LUTUVY_fw[LUT_LEN + LUT_LEN + LUT_LEN + LUT_LEN + LUT_LEN +
			  LUT_LEN] = {};
...
static int _iris_change_lut_type_addr(struct iris_ip_opt *dest,
				      struct iris_ip_opt *src)
{
	int rc = -EINVAL;
...
static int _iris_change_gamma_type_addr(void)
{
	...
	if (!popt) {
		IRIS_LOGE(
			"%s(), cann't find valid option, input ip: %#x, opt: %#x.",
			__func__, ip, opt_id);
		return rc;
	}
	...
	for (i = 0; i < gamma_lut_opt_cnt; i++) {
		...
		if (!lut_popt) {
			IRIS_LOGE(
				"%s(), cann't find valid lut option, input ip: %#x, opt: %#x.",
...
static u8 _iris_parse_calibrate_status(const struct firmware *fw)
{
	...
	IRIS_LOGI("%s(), panel calibrate status: 0x%02x.", __func__,
		  calibrate_status);
...
int iris_parse_lut_cmds(u8 flag)
{
	...
	if (!ret) {
		...
		data[0].buf = fw->data;
		data[0].size = fw->size;
		IRIS_LOGI("%s(%d), request name: %s, size: %u.", __func__,
			  __LINE__, IRIS_FIRMWARE_NAME, data[0].size);
	} else {
		IRIS_LOGE("%s(), failed to request_iris: %s", __func__,
			  IRIS_FIRMWARE_NAME);
	}
	...
	if (!ret) {
		...
		if ((ccf1_fw->size - ccf1_tail_size) % ccf1_per_pkg_size == 0 &&
			...
			cm_lut_opt_cnt = (ccf1_fw->size - ccf1_tail_size) /
					 ccf1_per_pkg_size;
			IRIS_LOGI(
				"%s(%d), request name: %s, size: %u, option count: %u.",
	...
	if (!ret) {
		...
		if ((ccf2_fw->size - ccf2_tail_size) % ccf2_per_pkg_size == 0 &&
			...
			gamma_lut_opt_cnt = (ccf2_fw->size - ccf2_tail_size) /
					    ccf2_per_pkg_size;
			IRIS_LOGI(
				"%s(%d), request name: %s, size: %u, option count: %u.",
	...
	firmware_loaded = (firmware_state == 0x07 ? FIRMWARE_LOAD_SUCCESS :
						    FIRMWARE_LOAD_FAIL);
	...
	if (firmware_state != 0) {
		pcfg = iris_get_cfg();
		ret = iris_attach_cmd_to_ipidx(
			data, (sizeof(data)) / (sizeof(data[0])), pip_index);
		if (ret)
...
static void _iris_prepare_lut_cmds(struct iris_ip_opt *popt)
{
	...
	IRIS_LOGD("%s(), %p %p len: %d", __func__, &pdesc[pos], popt,
		  popt->cmd_cnt);
...
static void _iris_fomat_lut_cmds(u8 lut_type, u8 opt_id)
{
	...
	if (!popt) {
		IRIS_LOGW("%s(%d), invalid opt id: %#x.", __func__, __LINE__,
			  opt_id);
		return;
	}
...
int iris_send_lut(u8 lut_type, u8 lut_table_index, u32 lut_abtable_index)
{
	...
	case DBC_LUT:
		if (!((lut_table_index <=
		       DBC_HIGH /*&& lut_table_index >= DBC_INIT*/) ||
		      (lut_table_index <= CABC_DLV_HIGH &&
		       lut_table_index >= CABC_DLV_OFF)))
			break;
		...
		if (lut_table_index < CABC_DLV_OFF) {
			...
			lut_opt_id = (lut_table_index & 0x3f) |
				     (lut_abtable_index << 7);
			/*even and odd*/
	...
	case CM_LUT:
		if (lut_table_index >= cm_lut_opt_cnt) {
			IRIS_LOGW(
				"%s(%d), invalid table index %d of type: %#x.",
		...
		_iris_fomat_lut_cmds(lut_type, lut_opt_id);
		IRIS_LOGI("%s(): call CM_LUT, index: %#x.", __func__,
			  lut_table_index);
		break;

	case SDR2HDR_LUT:
		if (lut_table_index >
		    (SDR2HDR_LEVEL5 | (SDR2HDR_INV_UV1 << 4))) {
			IRIS_LOGW(
				"%s(%d), invalid table index %d of type: %#x.",
				__func__, __LINE__, lut_table_index, lut_type);
			break;
	...
	case AMBINET_HDR_GAIN:
		...
		memcpy(&pdesc[pcfg->iris_cmds.cmds_index],
		       ...
		       sizeof(struct dsi_cmd_desc) *
			       iris_lut_param.hdr_lutuvy_pkt_cnt);
	...
	case GAMMA_LUT:
		...
		lut_opt_id = lut_table_index & 0xff;
		_iris_fomat_lut_cmds(lut_type, lut_opt_id);
		IRIS_LOGI("%s(), call GAMMA_LUT, index: %d.", __func__,
			  lut_table_index);
		break;
	...
	case FRC_PHASE_LUT:
		...
		lut_opt_id = lut_table_index & 0xff;
		_iris_fomat_lut_cmds(lut_type, lut_opt_id);
		IRIS_LOGI("%s(), call FRC_PHASE_LUT, index: %d.", __func__,
			  lut_table_index);
		break;
	...
	case APP_CODE_LUT:
		...
		lut_opt_id = lut_table_index & 0xff;
		_iris_fomat_lut_cmds(lut_type, lut_opt_id);
		IRIS_LOGI("%s(), call APP_CODE_LUT, index: %d.", __func__,
			  lut_table_index);
		break;
	...
	case DPP_DITHER_LUT:
		...
		lut_opt_id = lut_table_index & 0xff;
		_iris_fomat_lut_cmds(lut_type, lut_opt_id);
		IRIS_LOGI("%s(), call DPP_DITHER_LUT, index: %d.", __func__,
			  lut_table_index);
		break;
	...
	case DTG_PHASE_LUT:
		...
		lut_opt_id = lut_table_index & 0xff;
		_iris_fomat_lut_cmds(lut_type, lut_opt_id);
		IRIS_LOGI("%s(), call DTG_PHASE_LUT, index: %d.", __func__,
			  lut_table_index);
		break;
	...
	IRIS_LOGD(
		"%s(), lut type: %#x, lut index: %#x, ab index: %#x, cmd count: %d, max count: %d",
		__func__, lut_type, lut_table_index, lut_abtable_index,
...
void iris_update_ambient_lut(enum LUT_TYPE lutType, u32 lutpos)
{
	...
	if (lutpos == 0xFFE00000)
		...
		dbus_addr_start = SDR2HDR_LUT2_ADDRESS +
				  lutpos * SDR2HDR_LUT_BLOCK_SIZE / 2;
	...
	for (index_i = 0; index_i < LUT_LEN; index_i++) {
		if (lutpos == 0xFFE00000) {
			...
			if (index_i % 2 == 0) {
				...
				LUT2_fw[lut_fw_index] = lut_lut2[index_i];
				LUT2_fw[lut_fw_index + LUT_LEN / 2] =
					lut_lut2[index_i];
				LUT2_fw[lut_fw_index + LUT_LEN / 2 +
					LUT_LEN / 2] = lut_lut2[index_i];
			}
		}
	}

	if (dynamic_lut_send_cmd == NULL) {
		len = sizeof(struct dsi_cmd_desc) * hdr_pkt_size *
	...
	for (lut_block_index = 0; lut_block_index < lut_block_cnt;
		...
		for (lut_pkt_cmd_index = 0;
		     ...
		     lut_pkt_cmd_index++) {
			iris_lut_buf_index = lut_block_index * hdr_pkt_size *
						     hdr_block_pkt_cnt +
...
void iris_update_maxcll_lut(enum LUT_TYPE lutType, u32 lutpos)
{
	...
	for (index_i = 0; index_i < LUT_LEN; index_i++) {
		if (lutpos == 0xFFFF0000) {
			...
			LUTUVY_fw[lut_fw_index + 2 * LUT_LEN] =
				lut_lutuv[index_i];
			LUTUVY_fw[lut_fw_index + 3 * LUT_LEN] =
				lut_lutuv[index_i];
			LUTUVY_fw[lut_fw_index + 4 * LUT_LEN] =
				lut_luty[index_i];
			LUTUVY_fw[lut_fw_index + 5 * LUT_LEN] =
				lut_luty[index_i];
		} else {
			if (index_i % 2 == 0) {
				lut_fw_index = index_i / 4;
				if (index_i % 4 != 0) // ODD
					lut_fw_index += LUT_LEN / 4;
				LUTUVY_fw[lut_fw_index] = lut_lutuv[index_i];
				LUTUVY_fw[lut_fw_index + LUT_LEN / 2] =
					lut_lutuv[index_i];
				LUTUVY_fw[lut_fw_index + LUT_LEN] =
					lut_lutuv[index_i];
				LUTUVY_fw[lut_fw_index + 3 * LUT_LEN / 2] =
					lut_lutuv[index_i];
				LUTUVY_fw[lut_fw_index + 2 * LUT_LEN] =
					lut_luty[index_i];
				LUTUVY_fw[lut_fw_index + 5 * LUT_LEN / 2] =
					lut_luty[index_i];
			}
		}
	}
	...
	if (dynamic_lutuvy_send_cmd == NULL) {
		dynamic_lutuvy_send_cmd = vzalloc(
			sizeof(struct dsi_cmd_desc) * hdr_pkt_size *
			hdr_block_pkt_cnt * SDR2HDR_LUTUVY_BLOCK_NUMBER);
		if (dynamic_lutuvy_send_cmd == NULL) {
			IRIS_LOGE("%s: failed to alloc mem", __func__);
			return;
		}
	...
	if (iris_maxcll_lut_buf)
		memset(iris_maxcll_lut_buf, 0,
		       hdr_pkt_size * iris_lut_param.hdr_lutuvy_pkt_cnt);

	if (!iris_maxcll_lut_buf)
		iris_maxcll_lut_buf = vzalloc(
			hdr_pkt_size * iris_lut_param.hdr_lutuvy_pkt_cnt);
	...
	for (lut_block_index = 0; lut_block_index < lut_block_cnt;
		...
		for (lut_pkt_cmd_index = 0;
			...
			dynamic_lutuvy_send_cmd[temp_index].msg.tx_len =
				cmd_payload_len + DIRECT_BUS_HEADER_SIZE;
			dynamic_lutuvy_send_cmd[temp_index].post_wait_ms = 0;
			dynamic_lutuvy_send_cmd[temp_index].msg.tx_buf =
				iris_maxcll_lut_buf + iris_lut_buf_index;
			...
			memcpy(&iris_maxcll_lut_buf[iris_lut_buf_index],
			       &LUTUVY_fw[lut_fw_index], cmd_payload_len);
			lut_fw_index += cmd_payload_len / 4;
			ocp_dbus_header.address += cmd_payload_len;
		}
	}
