...
static DEFINE_MUTEX(sec_mtx_lock);
...
static int sec_read_mutual(struct chip_data_s6sy771 *chip_info, u8 type, char *data, int len);
...
static int sec_enable_black_gesture(struct chip_data_s6sy771 *chip_info, bool enable)
{
    int ret = 0;
    int i = 0;
    ...
    if (enable) {
        ...
        if (chip_info->fingerprint_status != FINGERPRINT_DOWN_DETECT || *chip_info->in_suspend) {
            for (i = 0; i < 20; i++) {
...
static int sec_send_charge_state(struct chip_data_s6sy771 *chip_info)
{
    ...
    ret = touch_i2c_write_byte(chip_info->client, SET_CMD_SET_CHARGER_MODE, data);
    TPD_INFO("%s: state: %d %s!\n", __func__, data, ret < 0 ? "failed" : "success");
    return ret;
}

static int sec_enable_charge_mode(struct chip_data_s6sy771 *chip_info, bool enable)
{
    int ret = -1;
...
static int sec_enable_wireless_charge_mode(struct chip_data_s6sy771 *chip_info, bool enable)
{
    int ret = -1;
...
static int sec_enable_earsense_mode(struct chip_data_s6sy771 *chip_info, bool enable)
{
    int ret = -1;
...
static int sec_enable_palm_reject(struct chip_data_s6sy771 *chip_info, bool enable)
{
    int ret = -1;
...
static int sec_enable_game_mode(struct chip_data_s6sy771 *chip_info, bool enable)
{
    int ret = -1;
...
static int sec_enable_headset_mode(struct chip_data_s6sy771 *chip_info, bool enable)
{
    int ret = -1;
...
static int sec_smooth_lv_set(void *chip_data, int level)
{
    ...
    ret = touch_i2c_write_byte(chip_info->client,SEC_CMD_SMOOTH_LEVEL, (u8)level);
...
static int sec_flash_page_erase(struct chip_data_s6sy771 *chip_info, u32 page_idx, u32 page_num)
{
    int ret = -1;
    u8 tCmd[6] = {0};
...
static int sec_flash_page_write(struct chip_data_s6sy771 *chip_info, u32 page_idx, u8 *page_data)
{
    int ret;
    ...
    ret = touch_i2c_write_block(chip_info->client, tCmd[0], 1 + 2 + flash_page_size, &tCmd[1]);
    return ret;
}

static bool sec_limited_flash_page_write(struct chip_data_s6sy771 *chip_info, u32 page_idx, u8 *page_data)
{
    int ret = -1;
    u8 *tCmd = NULL;
    ...
    while (copy_left > 0) {
        ...
        ret = touch_i2c_write_block(chip_info->client, tCmd[0], copy_cur, &tCmd[1]);
        if (ret < 0) {
            ret = touch_i2c_write_block(chip_info->client, tCmd[0], copy_cur, &tCmd[1]);
            if (ret < 0) {
...
static int sec_flash_write(struct chip_data_s6sy771 *chip_info, u32 mem_addr, u8 *mem_data, u32 mem_size)
{
    int ret = -1;
...
static int sec_block_read(struct chip_data_s6sy771 *chip_info, u32 mem_addr, int mem_size, u8 *buf)
{
    int ret;
    u8 cmd[5];
    u8 *data;
...
static int sec_memory_read(struct chip_data_s6sy771 *chip_info, u32 mem_addr, u8 *mem_data, u32 mem_size)
{
    int ret;
    int retry = 3;
    int read_size = 0;
...
static int sec_chunk_update(struct chip_data_s6sy771 *chip_info, u32 addr, u32 size, u8 *data)
{
    int ii = 0, ret = 0;
    u8 *mem_rb = NULL;
    u32 write_size = 0;
...
void sec_refresh_switch(int fps)
{
	...
	if(g_chip_info->sec_refresh_rate_delay) {
		mutex_lock(&sec_mtx_lock);
		g_chip_info->display_refresh_rate = fps;

	    if (!g_chip_info->is_power_down) {
			ret = touch_i2c_write_byte(g_chip_info->client, SEC_CMD_REFRESH_RATE_SWITCH, fps);
			TPD_INFO("%s: refresh_switch: %d HZ %s! 100ms\n", __func__, fps, ret < 0 ? "failed" : "success");
			sec_mdelay(100); /* Adding delay when switching 60hz to 90hz and vice versa, to set refresh rate properly in some race condition situations*/
		}
		mutex_unlock(&sec_mtx_lock);
	} else {
		g_chip_info->display_refresh_rate = fps;
...
static fw_update_state sec_fw_update(void *chip_data, const struct firmware *fw, bool force)
{
	...
	if(chip_info->sec_refresh_rate_delay) {
		sec_reset(chip_info); /*This condition only for reno4pro gaudi model, to fix I2C communication failed issue */
	}
    ...
    if ((cal_status == 0) || (cal_status == 0xFF) || (config_version_in_ic != config_version_in_bin)) {
        ...
        ret = touch_i2c_write_byte(chip_info->client, SEC_VENDOR_INFO_CALIBRATION_STATUS, 0);  // clear calibration status after change config
        if (ret < 0){
            TPD_INFO("%s : clear calibration status after config change failed\n", __func__);
        }
    }
    ...
    if (event_id == SEC_STATUS_EVENT) {
        ...
        if ((p_event_status->stype == TYPE_STATUS_EVENT_VENDOR_INFO) && (p_event_status->status_id == SEC_STATUS_TOUCHHOLD)) {
            if (p_event_status->status_data_1 == 0x01) {
                chip_info->fingerprint_status = FINGERPRINT_DOWN_DETECT;
            } else if (p_event_status->status_data_1 == 0x00) {
                chip_info->fingerprint_status = FINGERPRINT_UP_DETECT;
            }
        ...
        if ((p_event_status->stype == TYPE_STATUS_EVENT_VENDOR_INFO) && ((p_event_status->status_id == SEC_STATUS_PAGE_ERASE) ||
            (p_event_status->status_id == SEC_STATUS_SECTOR_ERASE) || (p_event_status->status_id == SEC_STATUS_WORD_WRITE) ||
            (p_event_status->status_id == SEC_STATUS_PAGE_WRITE) || (p_event_status->status_id == SEC_STATUS_UPDATE_ADDR) ||
            (p_event_status->status_id == SEC_STATUS_UPDATE_SIZE))) {
            TPD_INFO("%s: Flash change detected. 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n", __func__,
                chip_info->first_event[0], chip_info->first_event[1], chip_info->first_event[2], chip_info->first_event[3],
                chip_info->first_event[4], chip_info->first_event[5], chip_info->first_event[6], chip_info->first_event[7]);
            return IRQ_IGNORE;
        }
...
bool corner_point_filtered(struct chip_data_s6sy771 *chip_info, uint16_t x, uint16_t y)
{
    /*
...
static void sec_get_fingerprint_info(void *chip_data, struct fp_underscreen_info *fp_tpinfo)
{
    ...
    fp_tpinfo->x = p_event_status->status_data_2 | (p_event_status->status_data_3 << 4 & 0x0F00);
    fp_tpinfo->y = (p_event_status->status_data_3 << 8 & 0x0F00) | p_event_status->status_data_4;

    fp_tpinfo->touch_state = chip_info->fingerprint_status;

    if (chip_info->fingerprint_status == FINGERPRINT_UP_DETECT && !*chip_info->in_suspend) {
        for (i = 0; i < 20; i++) {
            ret = touch_i2c_read_byte(chip_info->client, SEC_CMD_SET_POWER_MODE);
            if (0x00 == ret) {
                break;
...
static int sec_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_WIRELESS_CHARGE:
	    ...
	    if (ret < 0) {
	        TPD_INFO("%s: enable wireless charge mode : %d failed\n", __func__, flag);
	    }
	    break;
...
static int sec_read_self(struct chip_data_s6sy771 *chip_info, u8 type, char *data, int len)
{
    int ret = 0;
...
static void sec_main_register_read(struct seq_file *s, void *chip_data)
{
    ...
    state = touch_i2c_read_byte(chip_info->client, SEC_VENDOR_INFO_CALIBRATION_STATUS);
...
static void sec_rtdp_dump(struct seq_file *s, struct chip_data_s6sy771 *chip_info)
{
    uint8_t temp[2] = {0};
    uint8_t *pRead = NULL;
    int16_t nodeData = 0;
...
static int sec_execute_selftest(struct seq_file *s, int fd, struct chip_data_s6sy771 *chip_info, struct sec_testdata *sec_testdata)
{
    int rc = -1;
...
static int sec_execute_p2ptest(struct seq_file *s, struct chip_data_s6sy771 *chip_info, struct sec_testdata *sec_testdata)
{
    int rc;
...
static void sec_auto_test(struct seq_file *s, void *chip_data, struct sec_testdata *sec_testdata)
{
    ...
    ret = touch_i2c_read_block(chip_info->client, SEC_VENDOR_INFO_CALIBRATION_STATUS, 1, &cal_status);
    if (ret < 0) {
    ...
    if ((cal_status == 0 && ret >= 0) || (cal_status == 0xFF && chip_info->auto_test_need_cal_support)) {
        err_cnt++;
...
static void sec_write_calibration_status(void *chip_data)
{
    ...
    ret = touch_i2c_write_byte(chip_info->client, SEC_VENDOR_INFO_CALIBRATION_STATUS, cal_status);
    if (ret < 0){
...
static int sec_tp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip_info->sec_refresh_rate_delay = of_property_read_bool(ts->dev->of_node, "sec_refresh_rate_delay");
...
static int sec_i2c_suspend(struct device *dev)
{
    ...
    TPD_INFO("%s: is called, irq status : %d .\n", __func__, gpio_get_value(ts->hw_res.irq_gpio));
    tp_i2c_suspend(ts);
