...
extern struct device_hcd *
syna_remote_device_init(struct syna_tcm_data *tcm_info);
				...
				unsigned int *data)
{
	...
	if (bits == 0 || bits > 32) {
		...
		retval = secure_memcpy((unsigned char *)data, bits / 8,
				       &touch_report[byte_offset], bits / 8,
				       bits / 8);
		if (retval < 0) {
...
static int syna_parse_report(struct syna_tcm_data *tcm_info)
{
	...
	while (idx < config_size) {
		...
		case TOUCH_OBJECT_N_CLASSIFICATION:
			...
			if (obj >= touch_hcd->max_objects) {
				TPD_INFO(
					"obj> max_obj!! obj[%d]Report Data[%d]:",
					obj, report_size);
				if (tp_debug != 0) {
					for (i = 0; i < report_size; i++) {
						TPD_INFO("syna data:[0x%2x]",
							 tcm_info->report.buffer
								 .buf[i]);
					}
				}
				return -1;
		...
		case TOUCH_REPORT_CUSTOMER_GRIP_INFO:
			bits = config_data[idx++];
			retval = syna_get_report_data(
				tcm_info, offset, bits,
...
static int syna_set_default_report_config(struct syna_tcm_data *tcm_info)
{
	...
	if (tcm_info->default_config.buf) {
		...
		memcpy(tcm_info->config.buf, tcm_info->default_config.buf,
		       length);
...
static int syna_get_default_report_config(struct syna_tcm_data *tcm_info)
{
	...
	retval = syna_tcm_write_message(tcm_info, CMD_GET_TOUCH_REPORT_CONFIG,
					NULL, 0, &tcm_info->default_config.buf,
...
static int syna_set_normal_report_config(struct syna_tcm_data *tcm_info)
{
	...
	retval = syna_tcm_write_message(tcm_info, CMD_SET_TOUCH_REPORT_CONFIG,
					touch_hcd->out.buf, length,
					&touch_hcd->resp.buf,
					&touch_hcd->resp.buf_size,
					&touch_hcd->resp.data_length, 0);
	if (retval < 0) {
		TPD_INFO("Failed to write command %s\n",
	...
	if (tcm_info->report.id == REPORT_TOUCH) {
		...
		if (*tcm_info->in_suspend) {
			if ((touch_data->lpwg_gesture == TOUCH_HOLD_UP) ||
				...
				syna_set_trigger_reason(tcm_info,
							IRQ_FINGERPRINT);
				goto exit;
			}
		...
		if (tcm_info->cb.async_work &&
		    tcm_info->id_info.mode == MODE_APPLICATION)
...
static void syna_tcm_dispatch_message(struct syna_tcm_data *tcm_info)
{
	...
	if (tcm_info->report_code == REPORT_IDENTIFY) {
		...
		retval = secure_memcpy(
			(unsigned char *)&tcm_info->id_info,
		...
		if (0x0b == tcm_info->id_info.mode) {
			...
			if (!tcm_info->upload_flag &&
			    tcm_info->firmware_mode_count >=
				    FIRMWARE_MODE_BL_MAX) {
				tp_healthinfo_report(tcm_info->monitor_data_v2,
						     HEALTH_REPORT,
						     "firmware mode = 0x0b");
		...
		if (atomic_read(&tcm_info->command_status) == CMD_BUSY) {
			...
			default:
				...
				atomic_set(&tcm_info->command_status,
					   CMD_ERROR);
				complete(&response_complete);
				break;
			}
...
static int syna_tcm_continued_read(struct syna_tcm_data *tcm_info)
{
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		if (retval < 0) {
			TPD_INFO(
				"Failed to allocate memory for tcm_info->temp.buf\n");
			UNLOCK_BUFFER(tcm_info->temp);
			UNLOCK_BUFFER(tcm_info->in);
			return retval;
		}

		retval = touch_i2c_continue_read(
			tcm_info->client, xfer_length + 2, tcm_info->temp.buf);
		if (retval < 0) {
		...
		retval = secure_memcpy(&tcm_info->in.buf[offset],
				       total_length - offset,
				       &tcm_info->temp.buf[2], xfer_length,
				       xfer_length);
		if (retval < 0) {
			TPD_INFO("Failed to copy payload\n");
			UNLOCK_BUFFER(tcm_info->temp);
			      ...
			      unsigned int length)
{
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		if (retval < 0) {
			TPD_INFO(
				"Failed to allocate memory for tcm_info->out.buf\n");
		...
		retval = syna_tcm_write(tcm_info, tcm_info->out.buf,
					xfer_length + 1);
		if (retval < 0) {
				 ...
				 unsigned char *in_buf, unsigned int length)
{
	...
	retval = touch_i2c_continue_read(
		tcm_info->client, tcm_info->read_length, tcm_info->in.buf);
	if (retval < 0) {
		TPD_INFO("Failed to read from device\n");
		UNLOCK_BUFFER(tcm_info->in);
		goto exit;
...
exit:
	if ((retval < 0) &&
	    (atomic_read(&tcm_info->command_status) == CMD_BUSY)) {
		atomic_set(&tcm_info->command_status, CMD_ERROR);
		complete(&response_complete);
	}
...
get_app_info:
	...
	retval = secure_memcpy((unsigned char *)&tcm_info->app_info,
			       sizeof(tcm_info->app_info), resp_buf,
			       resp_buf_size,
...
static int syna_tcm_get_boot_info(struct syna_tcm_data *tcm_info)
{
	...
	retval = syna_tcm_write_message(tcm_info, CMD_GET_BOOT_INFO, NULL, 0,
	...
	retval = secure_memcpy((unsigned char *)&tcm_info->boot_info,
			       sizeof(tcm_info->boot_info), resp_buf,
			       resp_buf_size,
...
static int syna_tcm_identify(struct syna_tcm_data *tcm_info, bool id)
{
	...
	retval = secure_memcpy((unsigned char *)&tcm_info->id_info,
			       sizeof(tcm_info->id_info), resp_buf,
			       resp_buf_size,
...
static int syna_tcm_run_bootloader_firmware(struct syna_tcm_data *tcm_info)
{
	...
	retval = syna_tcm_write_message(tcm_info, CMD_RUN_BOOTLOADER_FIRMWARE,
					NULL, 0, &resp_buf, &resp_buf_size,
					&resp_length, 0);
	if (retval < 0) {
		TPD_INFO("Failed to write command %s\n",
				       ...
				       unsigned short *value)
{
	...
	retval = syna_tcm_write_message(tcm_info, CMD_GET_DYNAMIC_CONFIG,
				       ...
				       unsigned short value)
{
	...
	retval = syna_tcm_write_message(tcm_info, CMD_SET_DYNAMIC_CONFIG,
					out_buf, sizeof(out_buf), &resp_buf,
					&resp_buf_size, &resp_length,
					RESPONSE_TIMEOUT_MS_SHORT);
	if (retval < 0) {
...
static int syna_tcm_sleep(struct syna_tcm_data *tcm_info, bool en)
{
	...
	retval = syna_tcm_write_message(tcm_info, command, NULL, 0, &resp_buf,
					&resp_buf_size, &resp_length, 0);
	if (retval < 0) {
		TPD_INFO("Failed to write command %s\n",
			 en ? STR(CMD_ENTER_DEEP_SLEEP) :
	...
	if (!*tcm_info->in_suspend && !tcm_info->game_mode) {
		for (i = 0; i < tcm_info->fps_report_rate_num; i = i + 2) {
			if (fps == tcm_info->fps_report_rate_array[i]) {
				send_value =
					tcm_info->fps_report_rate_array[i + 1];
			}
		}
		retval = syna_tcm_set_dynamic_config(
			tcm_info, DC_SET_REPORT_FRE, send_value);
		if (retval < 0) {
			TPD_INFO(
...
dispatch_reset:
	TPD_INFO(
		"Firmware mode = 0x%02x, boot status 0x%02x, app status 0x%02x\n",
		tcm_info->id_info.mode, tcm_info->boot_info.status,
		tcm_info->app_status);
				 ...
				 int max_num)
{
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		points[idx].touch_major =
			max(object_data[idx].x_width, object_data[idx].y_width);
		points[idx].width_major =
			min(object_data[idx].x_width, object_data[idx].y_width);
				      ...
				      struct resolution_info *resolution_info)
{
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
		...
		if (max_x_inchip == max_x) {
			...
			points[idx].x =
				(object_data[idx].x_pos * max_x) / max_x_inchip;
		}
		if (max_y_inchip == max_y) {
			points[idx].y = object_data[idx].y_pos;
		} else {
			points[idx].y =
				(object_data[idx].y_pos * max_y) / max_y_inchip;
		}
		points[idx].touch_major =
			max(object_data[idx].x_width, object_data[idx].y_width);
		points[idx].width_major =
				     ...
				     bool enable)
{
	...
	if (enable) {
		...
		retval = syna_tcm_set_dynamic_config(
			tcm_info, DC_IN_WAKEUP_GESTURE_MODE, true);
		if (retval < 0) {
			TPD_INFO("%s: Failed to set dynamic gesture config\n",
				 __func__);
			return retval;
		}
		retval = syna_tcm_set_dynamic_config(tcm_info, DC_GESTURE_MASK,
						     config);
		if (retval < 0) {
			TPD_INFO(
				"%s: Failed to set dynamic gesture mask config\n",
				__func__);
			return retval;
		}
	} else {
...
static int syna_tcm_set_game_mode(struct syna_tcm_data *tcm_info, bool enable)
{
	...
	if (enable) {
		retval = syna_tcm_set_dynamic_config(
			tcm_info, DC_ERROR_PRIORITY, regval | 0x01);
		if (retval < 0) {
			TPD_INFO(
				"Failed to set dynamic error priority config\n");
			return retval;
		}
		...
		retval = syna_tcm_set_dynamic_config(
			tcm_info, DC_SET_REPORT_FRE, tcm_info->game_rate);
		if (retval < 0) {
			TPD_INFO(
				"Failed to set dynamic report frequence config\n");
			return retval;
		...
		retval = syna_tcm_set_dynamic_config(
			tcm_info, DC_ERROR_PRIORITY, regval & 0xF0);
		if (retval < 0) {
			TPD_INFO(
				"Failed to set dynamic error priority config\n");
			return retval;
...
static int syna_tcm_normal_mode(struct syna_tcm_data *tcm_info)
{
	...
	retval = syna_tcm_set_dynamic_config(tcm_info,
					     DC_IN_WAKEUP_GESTURE_MODE, false);
	if (retval < 0) {
...
static int syna_corner_limit_handle(struct syna_tcm_data *tcm_info)
{
	...
	if (LANDSCAPE_SCREEN_90 == tcm_info->touch_direction) {
		ret = syna_tcm_set_dynamic_config(
			tcm_info, DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL, 0x01);
		if (ret < 0) {
			TPD_INFO(
				"%s:failed to set DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL\n",
				__func__);
			return ret;
		}
		ret = syna_tcm_set_dynamic_config(tcm_info,
						  DC_GRIP_ABS_DARK_SEL, 0x0F);
		if (ret < 0) {
			TPD_INFO("%s:failed to set DC_GRIP_ABS_DARK_SEL\n",
				 __func__);
			return ret;
		}
		ret = syna_tcm_set_dynamic_config(tcm_info, DC_GRIP_ABS_DARK_X,
						  0x0A);
		if (ret < 0) {
		...
		ret = syna_tcm_set_dynamic_config(tcm_info, DC_GRIP_ABS_DARK_Y,
						  0x0A);
		if (ret < 0) {
		...
		ret = syna_tcm_set_dynamic_config(tcm_info, DC_DARK_ZONE_ENABLE,
						  0x03);
		if (ret < 0) {
		...
		if (tcm_info->fingerprint_and_grip_param_equal_19805) {
			ret = syna_tcm_set_dynamic_config(
				tcm_info, DC_GRIP_DARK_ZONE_X, 0x88);
			if (ret < 0) {
				TPD_INFO(
					"%s:failed to set DC_GRIP_DARK_ZONE_X\n",
					__func__);
				return ret;
			}
		} else {
			ret = syna_tcm_set_dynamic_config(
				tcm_info, DC_GRIP_DARK_ZONE_X, 0xFF);
			if (ret < 0) {
				TPD_INFO(
					"%s:failed to set DC_GRIP_DARK_ZONE_X\n",
					__func__);
		...
		ret = syna_tcm_set_dynamic_config(tcm_info, DC_GRIP_DARK_ZONE_Y,
						  0x44);
		if (ret < 0) {
		...
		ret = syna_tcm_set_dynamic_config(
			tcm_info, DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL, 0x00);
		if (ret < 0) {
			TPD_INFO(
				"%s:failed to set DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL\n",
				__func__);
		...
		ret = syna_tcm_set_dynamic_config(tcm_info, DC_GRIP_DARK_ZONE_X,
						  0x24);
		if (ret < 0) {
...
static int syna_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_NORMAL:
		...
		if (ret < 0) {
			...
			if (tcm_info->error_state_count >= ERROR_STATE_MAX) {
				syna_tcm_reset(
					tcm_info); /*ic state err, need to reset the IC*/
				tp_healthinfo_report(tcm_info->monitor_data_v2,
						     HEALTH_REPORT,
						     "ic state err rest");
			}
	...
	case MODE_CHARGE:
		ret = syna_tcm_set_dynamic_config(
			tcm_info, DC_CHARGER_CONNECTED, flag ? 1 : 0);
		if (ret < 0) {
...
static void syna_tcm_helper_work(struct work_struct *work)
{
	...
	struct syna_tcm_data *tcm_info =
		container_of(work, struct syna_tcm_data, helper_work);
				  ...
				  enum report_type report_type, bool enable)
{
	...
	retval = syna_tcm_write_message(
		tcm_info, enable ? CMD_ENABLE_REPORT : CMD_DISABLE_REPORT, out,
		1, &resp_buf, &resp_buf_size, &resp_length, 0);
	if (retval < 0) {
		TPD_INFO("Failed to write message %s\n",
			 enable ? STR(CMD_ENABLE_REPORT) :
...
static void syna_tcm_enable_fingerprint(void *chip_data, uint32_t enable)
{
	...
	if (enable) {
		if (tcm_info->fingerprint_and_grip_param_equal_19805) {
			retval = syna_tcm_set_dynamic_config(
				tcm_info, DC_TOUCH_HOLD, 0x07);
			if (retval < 0) {
				TPD_INFO(
					"Failed to set dynamic touch and hold config\n");
				return;
			}
		} else {
			retval = syna_tcm_set_dynamic_config(
				tcm_info, DC_TOUCH_HOLD,
			...
			retval = syna_tcm_enable_report(
				tcm_info, REPORT_TOUCH_HOLD,
				*tcm_info->in_suspend ? false : true);
			if (retval < 0) {
				TPD_INFO(
					"Failed to set enable touch and hold report\n");
				return;
			}
		}
	} else {
		...
		if (!tcm_info->fingerprint_and_grip_param_equal_19805) {
			retval = syna_tcm_enable_report(
				tcm_info, REPORT_TOUCH_HOLD, false);
			if (retval < 0) {
				TPD_INFO(
					"Failed to set disable touch and hold report\n");
				return;
			}
		}
	}
...
static void
syna_tcm_fingerprint_info_auto(void *chip_data,
			       struct fp_underscreen_info *fp_tpinfo,
			       struct resolution_info *resolution_info)
{
	struct syna_tcm_data *tcm_info = (struct syna_tcm_data *)chip_data;
				     ...
				     struct monitor_data *mon_data)
{
	...
	struct health_info *health_info =
		(struct health_info *)tcm_info->report.buffer.buf;
				...
				unsigned int page_count)
{
	...
	ret = syna_tcm_write_message(tcm_info, CMD_ERASE_FLASH, out_buf,
				     cmd_length, &resp_buf, &resp_buf_size,
				     &resp_length, ERASE_FLASH_DELAY_MS);
	if (ret < 0) {
		TPD_INFO("Failed to write command %s\n", STR(CMD_ERASE_FLASH));
	}
				...
				unsigned int datalen)
{
	...
	while (remaining_len) {
		...
		retval = secure_memcpy(&out.buf[2], xfer_len, &data[offset],
				       datalen - offset, xfer_len);
		if (retval < 0) {
		...
		if (retval < 0) {
			TPD_INFO(
				"Failed to write message %s, Addr 0x%08x, Len 0x%d\n",
					  ...
					  const struct firmware *fw, bool force)
{
	...
	if (tcm_info->fwupdate_bootloader) {
		...
		if (tcm_info->g_fw_sta) {
			ret = synaptics_parse_header_v2(&image_info,
							tcm_info->g_fw_buf);
			if (ret < 0) {
				tp_healthinfo_report(
					tcm_info->monitor_data_v2,
					HEALTH_FW_UPDATE,
					"synaptics_parse_header_v2 fail");
	...
	if (image_info.app_firmware.size == 0) {
		tp_healthinfo_report(tcm_info->monitor_data_v2,
				     HEALTH_FW_UPDATE,
				     "no application firmware in image");
	...
	page_count =
		ceil_div(image_info.app_firmware.size, reflash_hcd.page_size);
	ret = syna_tcm_erase_flash(tcm_info, page_start, page_count);
	if (ret < 0) {
		tp_healthinfo_report(tcm_info->monitor_data_v2,
				     HEALTH_FW_UPDATE,
				     "Failed to erase firmware");
	...
	ret = syna_tcm_write_flash(tcm_info, &reflash_hcd, flash_addr, data,
				   size);
	if (ret < 0) {
		tp_healthinfo_report(tcm_info->monitor_data_v2,
				     HEALTH_FW_UPDATE, "Failed to write flash");
	...
	page_count =
		ceil_div(image_info.app_config.size, reflash_hcd.page_size);

	ret = syna_tcm_erase_flash(tcm_info, page_start, page_count);
	if (ret < 0) {
		tp_healthinfo_report(tcm_info->monitor_data_v2,
				     HEALTH_FW_UPDATE,
				     "Failed to erase config");
...
static int syna_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	if (gesture->gesture_type != UnkownGesture) {
		gesture->Point_start.x = (touch_data->data_point[0] |
					  (touch_data->data_point[1] << 8));
		gesture->Point_start.y = (touch_data->data_point[2] |
					  (touch_data->data_point[3] << 8));
	...
	TPD_INFO(
		"lpwg:0x%x, type:%d, clockwise: %d, points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n",
				      ...
				      struct resolution_info *resolution_info)
{
	...
	if (max_x_inchip != max_x) {
		gesture->Point_start.x =
			(gesture->Point_start.x * max_x) / max_x_inchip;
		gesture->Point_end.x =
			(gesture->Point_end.x * max_x) / max_x_inchip;
		gesture->Point_1st.x =
			(gesture->Point_1st.x * max_x) / max_x_inchip;
		gesture->Point_2nd.x =
			(gesture->Point_2nd.x * max_x) / max_x_inchip;
		gesture->Point_3rd.x =
			(gesture->Point_3rd.x * max_x) / max_x_inchip;
		gesture->Point_4th.x =
			(gesture->Point_4th.x * max_x) / max_x_inchip;
	}

	if (max_y_inchip != max_y) {
		gesture->Point_start.y =
			(gesture->Point_start.y * max_y) / max_y_inchip;
		gesture->Point_end.y =
			(gesture->Point_end.y * max_y) / max_y_inchip;
		gesture->Point_1st.y =
			(gesture->Point_1st.y * max_y) / max_y_inchip;
		gesture->Point_2nd.y =
			(gesture->Point_2nd.y * max_y) / max_y_inchip;
		gesture->Point_3rd.y =
			(gesture->Point_3rd.y * max_y) / max_y_inchip;
		gesture->Point_4th.y =
			(gesture->Point_4th.y * max_y) / max_y_inchip;
	}

	TPD_INFO(
		"changed points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n",
...
int syna_tcm_get_frame_size_words(struct syna_tcm_data *tcm_info,
				  bool image_only)
{
	unsigned int rows;
	unsigned int cols;
				      ...
				      enum test_item_bit test_code)
{
	...
	if (retval < 0) {
		TPD_INFO(
			"Failed to allocate memory for test_hcd->test_out.buf\n");
	...
	retval = syna_tcm_write_message(
		tcm_info, CMD_PRODUCTION_TEST, test_hcd->test_out.buf, 1,
		&test_hcd->test_resp.buf, &test_hcd->test_resp.buf_size,
		&test_hcd->test_resp.data_length, RESPONSE_TIMEOUT_MS_LONG);
	if (retval < 0) {
		TPD_INFO("Failed to write command %s\n",
				...
				uint8_t item_index)
{
	...
	uint32_t *p_item_offset =
		(uint32_t *)(fw->data + sizeof(struct test_header_new));

	for (i = 0; i < item_cnt; i++) {
		item_header =
			(struct syna_test_item_header *)(fw->data +
							 p_item_offset[i]);
...
static int syna_int_pin_test(struct seq_file *s, struct syna_tcm_data *tcm_info,
			     ...
			     int item_offset, int error_count)
{
...
static int syna_trx_short_test(struct seq_file *s,
			       struct syna_tcm_data *tcm_info,
			       struct syna_testdata *syna_testdata,
			       int item_offset, int error_count)
{
	...
	item_header = (struct syna_test_item_header *)(syna_testdata->fw->data +
						       item_offset);
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(
				s,
				"trx short test magic number(%4x) is wrong.\n",
				item_header->item_magic);
		error_count++;
		return error_count;
	}
		...
		for (j = 0; j < 8; j++) {
			if (1 == (u_data8 & (1 << j))) {
				...
				if (!error_count)
					seq_printf(
						s,
						"trx short test failed at %d bits.\n",
						checked_bits + 1);
				error_count++;
			}
			checked_bits++;
...
static int syna_trx_open_test(struct seq_file *s,
			      struct syna_tcm_data *tcm_info,
			      struct syna_testdata *syna_testdata,
			      int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(
				s,
				"trx open test magic number(%4x) is wrong.\n",
				item_header->item_magic);
		error_count++;
		return error_count;
	}
		...
		for (j = 0; j < 8; j++) {
			if (0 == (u_data8 & (1 << j))) {
				...
				if (!error_count)
					seq_printf(
						s,
						"trx open test failed at %d bits.\n",
						checked_bits + 1);
				error_count++;
			}
			checked_bits++;
...
static int syna_trx_gndshort_test(struct seq_file *s,
				  struct syna_tcm_data *tcm_info,
				  struct syna_testdata *syna_testdata,
				  int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(
				s,
				"trx gndshort test magic number(%4x) is wrong.\n",
				item_header->item_magic);
		error_count++;
		return error_count;
	}
	...
	for (i = 0; i < test_hcd->test_resp.data_length;) {
		...
		for (j = 0; j < 8; j++) {
			if (0 == (u_data8 & (1 << j))) {
				TPD_INFO(
					"trx gndshort test failed at %d bits.\n",
					checked_bits + 1);
				if (!error_count)
					seq_printf(
						s,
...
static int syna_full_rawcap_test(struct seq_file *s,
				 struct syna_tcm_data *tcm_info,
				 struct syna_testdata *syna_testdata,
				 int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(
				s,
				"full rawcap test magic number(%4x) is wrong.\n",
				item_header->item_magic);
		error_count++;
		return error_count;
	}
	if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
		...
		if (!error_count)
			seq_printf(
				s,
				"full rawcap test limit type(%2x) is wrong.\n",
				item_header->item_limit_type);
		error_count++;
		return error_count;
	}
	...
	for (i = 0; i < test_hcd->test_resp.data_length;) {
		...
		if ((u_data16 < p_mutual_n[index]) ||
		    (u_data16 > p_mutual_p[index])) {
			TPD_INFO(
				"full rawcap test failed at node[%d]=%d [%d %d].\n",
				index, u_data16, p_mutual_n[index],
				p_mutual_p[index]);
			if (!error_count)
				seq_printf(
					s,
...
static int syna_delta_noise_test(struct seq_file *s,
				 struct syna_tcm_data *tcm_info,
				 struct syna_testdata *syna_testdata,
				 int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(
				s,
				"delta noise test magic number(%4x) is wrong.\n",
				item_header->item_magic);
		error_count++;
		return error_count;
	}
	if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
		...
		if (!error_count)
			seq_printf(
				s,
				"delta noise test limit type(%2x) is wrong.\n",
				item_header->item_limit_type);
		error_count++;
		return error_count;
	}
	...
	for (i = 0; i < test_hcd->test_resp.data_length;) {
		...
		if ((data16 < p_mutual_n[index]) ||
		    (data16 > p_mutual_p[index])) {
			TPD_INFO(
				"delta noise test failed at node[%d]=%d [%d %d].\n",
				index, data16, p_mutual_n[index],
				p_mutual_p[index]);
			if (!error_count)
				seq_printf(
					s,
...
static int syna_hybrid_rawcap_test(struct seq_file *s,
				   struct syna_tcm_data *tcm_info,
				   struct syna_testdata *syna_testdata,
				   int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(
				s,
				"hybrid_rawcap test magic number(%4x) is wrong.\n",
				item_header->item_magic);
		error_count++;
		return error_count;
	}
	if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
		...
		p_hybridcap_n = (int32_t *)(syna_testdata->fw->data +
					    item_header->top_limit_offset +
					    4 * (syna_testdata->TX_NUM +
						 syna_testdata->RX_NUM));
	} else {
		...
		if (!error_count)
			seq_printf(
				s,
				"hybrid_rawcap test limit type(%2x) is wrong.\n",
				item_header->item_limit_type);
		error_count++;
		return error_count;
	}
	...
	for (i = 0; i < test_hcd->test_resp.data_length;) {
		index = i / byte_cnt;
		data32 = (buf[i] | (buf[i + 1] << 8) | (buf[i + 2] << 16) |
			  (buf[i + 3] << 24));
		store_to_file(syna_testdata->fd, "%08d, ", data32);
		if ((data32 < p_hybridcap_n[index]) ||
		    (data32 > p_hybridcap_p[index])) {
			TPD_INFO(
				"hybrid rawcap test failed at node[%d]=%d [%d %d].\n",
				index, data32, p_hybridcap_n[index],
				p_hybridcap_p[index]);
			if (!error_count)
				seq_printf(
					s,
...
static int syna_hybrid_rawcap_test_ad(struct seq_file *s,
				      struct syna_tcm_data *tcm_info,
				      struct syna_testdata *syna_testdata,
				      int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
	    item_header->item_magic != Limit_ItemMagic_V2) {
		TPD_INFO(
			"hybrid_rawcap_whth_ad test magic number(%4x) is wrong.\n",
			item_header->item_magic);
		if (!error_count)
			seq_printf(
				s,
	...
	if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
		...
		TPD_INFO(
			"hybrid_rawcap_whth_ad test limit type(%2x) is wrong.\n",
			item_header->item_limit_type);
		if (!error_count)
			seq_printf(
				s,
	...
	ret = testing_run_prod_test_item(tcm_info,
					 TYPE_HYBRIDRAW_CAP_WITH_AD + 24);
	if (ret < 0) {
		TPD_INFO("run hybrid rawcap whth_ad test failed.\n");
		if (!error_count)
			seq_printf(s,
				   "run hybrid rawcap whth_ad test failed.\n");
		error_count++;
		return error_count;
	}
	...
	for (i = 0; i < test_hcd->test_resp.data_length;) {
		...
		if ((data32 < p_hybridcap_n[index]) ||
		    (data32 > p_hybridcap_p[index])) {
			TPD_INFO(
				"hybrid rawcap whth_ad test failed at node[%d]=%d [%d %d].\n",
				index, data32, p_hybridcap_n[index],
				p_hybridcap_p[index]);
			if (!error_count)
				seq_printf(
			    ...
			    int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(s,
				   "raw cap test magic number(%4x) is wrong.\n",
				   item_header->item_magic);
		error_count++;
		return error_count;
	}
	if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
		...
		if (!error_count)
			seq_printf(s,
				   "raw cap test limit type(%2x) is wrong.\n",
				   item_header->item_limit_type);
		error_count++;
		return error_count;
	}
	...
	for (i = 0; i < test_hcd->test_resp.data_length;) {
		...
		if ((data16 < p_mutual_n[index]) ||
			...
			if (!error_count)
				seq_printf(
					s,
					"rawcap test failed at node[%d]=%d [%d %d].\n",
					index, data16, p_mutual_n[index],
					p_mutual_p[index]);
			error_count++;
		}
...
static int syna_trex_shortcustom_test(struct seq_file *s,
				      struct syna_tcm_data *tcm_info,
				      struct syna_testdata *syna_testdata,
				      int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(
				s,
				"trex short custom test magic number(%4x) is wrong.\n",
				item_header->item_magic);
		error_count++;
		return error_count;
	}
	if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
		p_tx_p = (int32_t *)(syna_testdata->fw->data +
				     item_header->top_limit_offset);
		p_tx_n = (int32_t *)(syna_testdata->fw->data +
				     item_header->top_limit_offset +
				     4 * syna_testdata->RX_NUM);
	} else {
		...
		if (!error_count)
			seq_printf(
				s,
				"trex short custom test limit type(%2x) is wrong.\n",
				item_header->item_limit_type);
		error_count++;
		return error_count;
	}
	...
	for (i = 0; i < test_hcd->test_resp.data_length;) {
		...
		if ((u_data16 < p_tx_n[index]) || (u_data16 > p_tx_p[index])) {
			TPD_INFO(
				"trex_shorcustom test failed at node[%d]=%d [%d %d].\n",
				index, u_data16, p_tx_n[index], p_tx_p[index]);
			if (!error_count)
				seq_printf(
					s,
					"trex_shorcustom test failed at node[%d]=%d [%d %d].\n",
					index, u_data16, p_tx_n[index],
					p_tx_p[index]);
			error_count++;
		}
...
static int syna_hybrid_diffcbc_test(struct seq_file *s,
				    struct syna_tcm_data *tcm_info,
				    struct syna_testdata *syna_testdata,
				    int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(
				s,
				"hybrid diffcbc test magic number(%4x) is wrong.\n",
				item_header->item_magic);
		error_count++;
		return error_count;
	}
	if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
		...
		if (!error_count)
			seq_printf(
				s,
				"hybrid diffcbc test limit type(%2x) is wrong.\n",
				item_header->item_limit_type);
		error_count++;
		return error_count;
	}
	...
	for (i = 0; i < test_hcd->test_resp.data_length;) {
		...
		u_data16 = (buf[i] | (buf[i + 1] << 8));
		store_to_file(syna_testdata->fd, "%04d, ", u_data16);
		if ((u_data16 < p_selfdata_n[index]) ||
		    (u_data16 > p_selfdata_p[index])) {
			TPD_INFO(
				"hybrid diffcbc test failed at node[%d]=%d [%d %d].\n",
				index, u_data16, p_selfdata_n[index],
				p_selfdata_p[index]);
			if (!error_count)
				seq_printf(
					s,
...
static int syna_hybrid_absnoise_test(struct seq_file *s,
				     struct syna_tcm_data *tcm_info,
				     struct syna_testdata *syna_testdata,
				     int item_offset, int error_count)
{
	...
	if (item_header->item_magic != Limit_ItemMagic &&
		...
		if (!error_count)
			seq_printf(
				s,
				"hybrid abs noise test magic number(%4x) is wrong.\n",
				item_header->item_magic);
		error_count++;
		return error_count;
	}
	if (item_header->item_limit_type == LIMIT_TYPE_EACH_NODE_DATA) {
		...
		if (!error_count)
			seq_printf(
				s,
				"hybrid abs noise test limit type(%2x) is wrong.\n",
				item_header->item_limit_type);
		error_count++;
		return error_count;
	}
	...
	for (i = 0; i < test_hcd->test_resp.data_length;) {
		...
		data16 = (buf[i] | (buf[i + 1] << 8));
		store_to_file(syna_testdata->fd, "%04d, ", data16);
		if ((data16 < p_selfdata_n[index]) ||
		    (data16 > p_selfdata_p[index])) {
			TPD_INFO(
				"hybrid abs noise test failed at node[%d]=%d [%d %d].\n",
				index, data16, p_selfdata_n[index],
				p_selfdata_p[index]);
			if (!error_count)
				seq_printf(
					s,
...
static void syna_auto_test(struct seq_file *s, void *chip_data,
			   struct syna_testdata *syna_testdata)
{
	...
	if (syna_testdata->test_item & (1 << TYPE_TRX_SHORT)) {
		item_offset = search_for_item(syna_testdata->fw, item_cnt,
					      TYPE_TRX_SHORT);
		if (!item_offset) {
			error_count++;
			TPD_INFO("search for item(%d) limit offset failed.\n",
				 TYPE_TRX_SHORT);
			seq_printf(s,
				   "search for item(%d) limit offset failed.\n",
				   TYPE_TRX_SHORT);
		} else {
			error_count =
				syna_trx_short_test(s, tcm_info, syna_testdata,
						    item_offset, error_count);
		}
	}
	...
	if (syna_testdata->test_item & (1 << TYPE_TRX_OPEN)) {
		...
		if (!item_offset) {
			...
			error_count =
				syna_trx_open_test(s, tcm_info, syna_testdata,
						   item_offset, error_count);
		}
	}
	...
	if (syna_testdata->test_item & (1 << TYPE_TRXGND_SHORT)) {
		...
		if (!item_offset) {
			...
			error_count = syna_trx_gndshort_test(s, tcm_info,
							     syna_testdata,
							     item_offset,
							     error_count);
		}
	...
	if (syna_testdata->test_item & (1 << TYPE_FULLRAW_CAP)) {
		...
		if (!item_offset) {
			...
			error_count = syna_full_rawcap_test(s, tcm_info,
							    syna_testdata,
							    item_offset,
							    error_count);
		}
	...
	if (syna_testdata->test_item & (1 << TYPE_DELTA_NOISE)) {
		...
		if (!item_offset) {
			...
			error_count = syna_delta_noise_test(s, tcm_info,
							    syna_testdata,
							    item_offset,
							    error_count);
		}
	...
	if (syna_testdata->test_item & (1 << TYPE_HYBRIDRAW_CAP)) {
		...
		if (!item_offset) {
			...
			error_count = syna_hybrid_rawcap_test(s, tcm_info,
							      syna_testdata,
							      item_offset,
							      error_count);
		}
	...
	if (syna_testdata->test_item &
	    (uint64_t)((uint64_t)1 << TYPE_HYBRIDRAW_CAP_WITH_AD)) {
		...
		if (!item_offset) {
			...
			error_count = syna_hybrid_rawcap_test_ad(s, tcm_info,
								 syna_testdata,
								 item_offset,
								 error_count);
		}
	...
	if (syna_testdata->test_item & (1 << TYPE_RAW_CAP)) {
		...
		if (!item_offset) {
			...
			error_count =
				syna_rawcap_test(s, tcm_info, syna_testdata,
						 item_offset, error_count);
		}
	}
	...
	if (syna_testdata->test_item & (1 << TYPE_TREXSHORT_CUSTOM)) {
		...
		if (!item_offset) {
			...
			error_count = syna_trex_shortcustom_test(s, tcm_info,
								 syna_testdata,
								 item_offset,
								 error_count);
		}
	...
	if (syna_testdata->test_item & (1 << TYPE_HYBRIDABS_DIFF_CBC)) {
		...
		if (!item_offset) {
			...
			error_count = syna_hybrid_diffcbc_test(s, tcm_info,
							       syna_testdata,
							       item_offset,
							       error_count);
		}
	...
	if (syna_testdata->test_item & (1 << TYPE_HYBRIDABS_NOSIE)) {
		...
		if (!item_offset) {
			...
			error_count = syna_hybrid_absnoise_test(s, tcm_info,
								syna_testdata,
								item_offset,
								error_count);
		}
	...
	tp_healthinfo_report(tcm_info->monitor_data_v2, HEALTH_TEST_AUTO,
			     &error_count);
				    ...
				    unsigned int num_of_reports)
{
	...
	retval = syna_tcm_write_message(tcm_info, CMD_ENABLE_REPORT, out, 1,
					&resp_buf, &resp_buf_size, &resp_length,
					0);
	if (retval < 0) {
		TPD_INFO("Failed to write message %s\n",
	...
	retval = syna_tcm_write_message(tcm_info, CMD_DISABLE_REPORT, out, 1,
					&resp_buf, &resp_buf_size, &resp_length,
					0);
	if (retval < 0) {
		TPD_INFO("Failed to write message %s\n",
				  ...
				  struct syna_tcm_data *tcm_info, char *buffer)
{
	...
	if (test_hcd->report.data_length ==
	    rows * cols * 2 + (rows + cols) * 2) {
...
static void syna_main_register(struct seq_file *s, void *chip_data)
{
	...
	retval = syna_tcm_get_dynamic_config(
		tcm_info, DC_IN_WAKEUP_GESTURE_MODE, &config);
	if (retval < 0) {
	...
	TPD_INFO("report frequence (0x01:120HZ,0x02:180HZ,0x03:240HZ) : %d\n",
		 config);
	seq_printf(s,
		   "report frequence (0x01:120HZ,0x02:180HZ,0x03:240HZ) : %d\n",
		   config);
	...
	retval =
		syna_tcm_get_dynamic_config(tcm_info, DC_GRIP_ENABLED, &config);
	if (retval < 0) {
		TPD_INFO("Failed to get dynamic config\n");
		seq_printf(s, "Failed to get dynamic config\n");
		return;
	...
	retval = syna_tcm_get_dynamic_config(
		tcm_info, DC_GRIP_ROATE_TO_HORIZONTAL_LEVEL, &config);
	if (retval < 0) {
		TPD_INFO("Failed to get dynamic config\n");
		seq_printf(s, "Failed to get dynamic config\n");
		return;
	...
	retval = syna_tcm_get_dynamic_config(tcm_info, DC_LOW_TEMP_ENABLE,
					     &config);
	if (retval < 0) {
		TPD_INFO("Failed to get dynamic config\n");
		seq_printf(s, "Failed to get dynamic config\n");
	...
	seq_printf(s, "APP info : status:%d\n",
		   le2_to_uint(tcm_info->app_info.status));
	seq_printf(
		s, "APP info : max_touch_report_config_size:%d\n",
		le2_to_uint(tcm_info->app_info.max_touch_report_config_size));
	seq_printf(
		s, "APP info : max_touch_report_payload_size:%d\n",
		le2_to_uint(tcm_info->app_info.max_touch_report_payload_size));
...
static void syna_freq_hop_trigger(void *chip_data)
{
	...
	case 0:
		...
		retval = syna_tcm_set_dynamic_config(
			tcm_info, DC_FREQUENCE_HOPPING, freq_point);
		if (retval < 0) {
...
static int syna_tcm_smooth_lv_set(void *chip_data, int level)
{
	...
	TPD_INFO("OK synaptics smooth lv to %d, now reg_val:0x%x", level,
		 regval);
	return 0;
}
static int syna_tcm_sensitive_lv_set(void *chip_data, int level)
{
	...
	retval =
		syna_tcm_get_dynamic_config(tcm_info, DC_NOISE_LENGTH, &regval);
	if (retval < 0) {
		TPD_INFO("Failed to get sensitive config\n");
		return 0;
	}
	TPD_INFO("OK synaptics sensitive lv to %d, now reg_val:%d", level,
		 regval);
...
static int syna_tcm_set_high_frame_rate(void *chip_data, int level, int time)
{
	...
	if (level > 0) {
		retval = syna_tcm_set_dynamic_config(
			tcm_info, DC_ERROR_PRIORITY, regval | 0x0c02);
		if (retval < 0) {
		...
		retval = syna_tcm_set_dynamic_config(
			tcm_info, DC_ERROR_PRIORITY, regval & 0xfffd);
		if (retval < 0) {
...
static int syna_tcm_send_temperature(void *chip_data, int temp, bool status)
{
	...
	if (tcm_info->syna_low_temp_enable > 1 ||
	    tcm_info->syna_low_temp_disable > 1) {
		TPD_DEBUG(
			"enable or disable low temp mode is more than one time\n");
		return 0;
	}
	retval = syna_tcm_get_dynamic_config(tcm_info, DC_LOW_TEMP_ENABLE,
					     &regval);
	...
	if (1 == temp_mode) {
		tp_healthinfo_report(tcm_info->monitor_data_v2, HEALTH_REPORT,
				     "syna_low_temp_enable");
	} else {
		tp_healthinfo_report(tcm_info->monitor_data_v2, HEALTH_REPORT,
				     "syna_low_temp_disable");
	}
	...
	retval = syna_tcm_set_dynamic_config(tcm_info, DC_LOW_TEMP_ENABLE,
					     temp_mode);
	if (retval < 0) {
	...
	TPD_INFO("OK synaptics temperature to %d, now reg_val:%d", temp_mode,
		 regval);
...
static void init_chip_dts(struct device *dev, void *chip_data)
{
	...
	if (tcm_info->fps_report_rate_num > 0 &&
		...
		rc = of_property_read_u32_array(chip_np, "fps_report_rate",
						temp_array,
	...
	rc = of_property_read_u32(
		chip_np, "fingerprint_and_grip_param_equal_19805",
			  ...
			  const struct i2c_device_id *id)
{
	...
	tcm_info->touch_hcd = (struct touch_hcd *)kzalloc(
	...
	tcm_info->test_hcd = (struct syna_tcm_test *)kzalloc(
	...
	if (ts->health_monitor_v2_support) {
		tp_healthinfo_report(&ts->monitor_data_v2, HEALTH_PROBE,
				     &time_counter);
	}
...
static const struct i2c_device_id syna_tmc_id[] = { { TPD_DEVICE, 0 }, {} };
...
static int __init syna_tcm_module_init(void)
{
	...
	if (!tp_judge_ic_match("synaptics-s3908"))
		return -1;
