...
#define FW_UPDATE_INFO(a, arg...) \
	pr_err("[FW_UPDATE]" FW_UPDATE_DEVICE ": " a, ##arg)

struct device *fw_update_dev = NULL;

static ssize_t debug_level(struct device *dev, struct device_attribute *attr,
...
static int __init kernel_fw_update_init(void)
{
	...
	if (fw_update_dev) {
		ret = sysfs_create_group(&fw_update_dev->kobj,
					 &fw_update_attr_group);
		if (ret < 0) {
			FW_UPDATE_INFO(
				"kernel_fw_update:sysfs_create_group fail\n");
			return ret;
		}
	} else {
